    ...
    if ZipCandidates =:= [] -> ok;        
        true ->
            ...
            case zip:zip(GoodZipFileName, ZipCandidates) of
                {error, Reason} -> ?Log("~p old snapshot backup to ~p failed reason : ~p~n", [?MODULE, GoodZipFileName, Reason]);
                _ -> ?Log("~p old snapshots are backed up to ~p~n", [?MODULE, GoodZipFileName])
            end
    end;
backup_snap(MatchPattern) ->
    {_, SnapDir} = application:get_env(imem, imem_snapshot_dir),
    SnapFiles = filelib:wildcard(filename:join([SnapDir, MatchPattern])),
    backup_snap({files, SnapFiles}).
...
snap_format({bkp, [ {dbtables, DbTables}
