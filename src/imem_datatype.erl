...
%% ' is ignored in integer strings or expressions

-define(BinaryMaxLen, 250).

%% more represented by "..." suffix
...
-export([is_binterm/1, term_to_binterm/1, binterm_to_term/1]).
-export(
    [
        add_squotes/1,
        add_dquotes/1,
        ...
        %% strip double quotes
        strip_dquotes/1,
        %% strip both quotes in any order
        strip_quotes/1,
        binary_to_hex/1,
        %% single quotes added if necessary
        atom_to_quoted_list/1,
        %% single quotes added if necessary
        atom_to_quoted_binary/1
    ]
).
-export(
    [
        %% Shift datetime value by
        offset_datetime/3,
        %% %% local time difference in (fraction of) days
        datetime_diff/2,
        offset_timestamp/3,
        %% UTC time difference in microseconds towards ?TIMESTAMP
        musec_diff/1,
        %% UTC time difference in microseconds
        musec_diff/2,
        %% UTC time difference in milliseconds towards ?TIMESTAMP
        msec_diff/1,
        %% UTC time difference in milliseconds
        msec_diff/2,
        %% UTC time difference in milliseconds towards ?TIMESTAMP
        sec_diff/1,
        %% UTC time difference in milliseconds
...
-export([field_value/6, field_value_type/6]).
-export([item1/1, item2/1, item3/1, item4/1, item5/1, item6/1, item7/1, item8/1, item9/1]).
-export(
    [
        %% return rows in raw erlang db format
        select_rowfun_raw/1,
...
bind_arg_types() -> [atom_to_binary(T, utf8) || T <- ?CLM_TYPES].

select_rowfun_raw(ColMap) -> fun (Recs) -> select_rowfun_raw(Recs, ColMap, []) end.

select_rowfun_raw(_Recs, [], Acc) -> lists:reverse(Acc);
...
select_rowfun_str(ColMap, DateFmt, NumFmt, StrFmt) ->
    fun (Recs) -> select_rowfun_str(Recs, ColMap, DateFmt, NumFmt, StrFmt, []) end.

select_rowfun_str(_Recs, [], _DateFmt, _NumFmt, _StrFmt, Acc) -> lists:reverse(Acc);
select_rowfun_str(
    Recs,
    [#bind{type = T, prec = P, tind = 0, cind = 0, func = F} | ColMap],
    DateFmt,
    NumFmt,
    StrFmt,
    Acc
...
select_rowfun_str(
    Recs,
    [#bind{type = T, prec = P, tind = Ti, cind = Ci, func = F, default = D} | ColMap],
    DateFmt,
    NumFmt,
    StrFmt,
    Acc
...
is_datatype(Types) when is_list(Types) -> (not lists:member(false, [is_datatype(T) || T <- Types]));
...
type_check({{Y, M, D}, {Hh, Mm, Ss}}, datetime, _)
when is_integer(Y), is_integer(M), is_integer(D), is_integer(Hh), is_integer(Mm), is_integer(Ss) ->
    ok;
...
type_check({A, B, C, D}, ipaddr, _) when is_integer(A), is_integer(B), is_integer(C), is_integer(D) -> ok;
type_check({A, B, C, D, E, F, G, H}, ipaddr, _)
when is_integer(A),
     is_integer(B),
     is_integer(C),
     is_integer(D),
...
type_check({Meg, Sec, Mic, Cnt}, timestamp, _) when is_integer(Meg), is_integer(Sec), is_integer(Mic), is_integer(Cnt) ->
    ok;
type_check({Meg, Sec, Mic}, timestamp, _) when is_integer(Meg), is_integer(Sec), is_integer(Mic) -> ok;
type_check(V, tuple, _) when is_tuple(V) -> ok;
type_check(V, userid, _) when is_integer(V) -> ok;
type_check(V, Type, Def) -> {error, {"Wrong data type for value, expecting type or default", {V, Type, Def}}}.
...
binterm_to_term(B) when is_binary(B) -> sext:decode(B);
binterm_to_term(L) when is_list(L) -> [binterm_to_term(I) || I <- L];
binterm_to_term(T) when is_tuple(T) -> list_to_tuple([binterm_to_term(I) || I <- tuple_to_list(T)]).
...
is_unicode_binary(_) -> false.

is_term_or_fun_text(F) when is_function(F) -> false;
is_term_or_fun_text(B) when is_binary(B) -> is_term_or_fun_text(binary_to_list(B));
is_term_or_fun_text([$f, $u, $n | _] = Str) ->
    try
        case re:run(Str, "fun\\((.*)\\)[ ]*->(.*)end.", [global, {capture, [1, 2], list}]) of
            {match, [[_Params, _Body]]} -> is_function((catch io_to_fun(Str, undefined)));
            nomatch -> true
        end
    catch
        throw : {'ClientError', Reason} -> ?ClientError(Reason);
        _:_ ->
            %% some term, not a string
            true
    end;
is_term_or_fun_text(_) -> true.

to_term_or_fun(B) when is_binary(B) -> to_term_or_fun(B, binary_to_list(B));
to_term_or_fun(L) when is_list(L) -> to_term_or_fun(L, L);
to_term_or_fun(T) -> T.
...
to_term_or_fun(T, [$f, $u, $n | _] = Str) ->
    try
        ...
        throw : {'ClientError', Reason} -> ?ClientError(Reason);
        _:_ ->
            %% not a valid string representation of a fun
            T
    end;
    ...
    try
        {DefAsStr, OldAsStr} =
            case Type of
                binterm ->
                    {
                        io_to_binstr(io_lib:format("~10000p", [Def])),
                        io_to_binstr(io_lib:format("~10000p", [term_to_binterm(Old)]))
                    };
                _ -> {io_to_binstr(io_lib:format("~10000p", [Def])), io_to_binstr(io_lib:format("~10000p", [Old]))}
            end,
        ...
        _ : {'UnimplementedException', _} ->
            ?ClientErrorNoLogging({"Unimplemented data type conversion", {Item, {Type, Val}}});
        _ : {'ClientError', {Text, Reason}} -> ?ClientErrorNoLogging({Text, {Item, Reason}});
        _:_ -> ?ClientErrorNoLogging({"Data conversion format error", {Item, {Type, Val}}})
    end.
...
strip_dquotes(B) when is_binary(B) ->
    ...
    L = binary:last(B),
    if
        (F == $") andalso (L == $") -> binary:part(B, 1, size(B) - 2);
        true -> B
    end;
strip_dquotes([]) -> [];
strip_dquotes([H]) -> [H];
...
strip_quotes(B) when is_binary(B) ->
    ...
    L = binary:last(B),
    if
        (F == $') andalso (L == $') -> strip_dquotes(binary:part(B, 1, size(B) - 2));
        (F == $") andalso (L == $") -> strip_squotes(binary:part(B, 1, size(B) - 2));
        true -> B
    end;
strip_quotes([]) -> [];
strip_quotes([H]) -> [H];
strip_quotes([H | T] = Str) ->
    L = lists:last(Str),
    if
        H == $' andalso L == $' -> strip_dquotes(lists:sublist(T, length(T) - 1));
        H == $" andalso L == $" -> strip_squotes(lists:sublist(T, length(T) - 1));
        true -> Str
    end.

io_to_atom(Val) when is_binary(Val) -> binary_to_atom(strip_quotes(Val), utf8);
io_to_atom(Val) when is_list(Val) -> list_to_atom(strip_quotes(Val)).

io_to_pid(Val) when is_binary(Val) -> list_to_pid(binary_to_list(Val));
io_to_pid(Val) when is_list(Val) -> list_to_pid(Val).

io_to_integer(Val) -> io_to_integer(Val, undefined, 0).

io_to_integer(Val, Len) -> io_to_integer(Val, Len, 0).

io_to_integer(Val, 0, Prec) -> io_to_integer(Val, undefined, Prec);
io_to_integer(Val, Len, undefined) -> io_to_integer(Val, Len, 0);
io_to_integer(Val, Len, Prec) when is_binary(Val) -> io_to_integer(binary_to_list(Val), Len, Prec);
io_to_integer(Val, Len, Prec) ->
    ValStr = string:join(string:tokens(Val, ?THOUSAND_SEP), []),
    Value = try list_to_integer(ValStr) catch _:_ -> try erlang:round(list_to_float(Val)) catch
                    _:_ ->
    ...
    if
        ...
        RLen > Len -> ?ClientErrorNoLogging({"Data conversion format error", {integer, Len, Prec, Val}});
        true -> Result
    end.

io_to_float(Val, Prec) when is_binary(Val) -> io_to_float(binary_to_list(Val), Prec);
io_to_float(Val, Prec) ->
    Value = try list_to_float(Val) catch
            _:_ ->
...
io_to_binary(Val, Len) when is_binary(Val) ->
    ...
    L = binary:last(Val),
    if
        (F < $0) orelse (F > $F) -> ?ClientErrorNoLogging({"Invalid hex string starts with", {binary, [F]}});
        (F > $9) andalso (F < $A) -> ?ClientErrorNoLogging({"Invalid hex string starts with", {binary, [F]}});
        (L < $0) orelse (L > $F) -> ?ClientErrorNoLogging({"Invalid hex string starts with", {binary, [L]}});
        (L > $9) andalso (L < $A) -> ?ClientErrorNoLogging({"Invalid hex string starts with", {binary, [L]}});
        (Len /= undefined) andalso (S > Len + Len) -> ?ClientErrorNoLogging({"Binary data is too long", {binary, Len}});
        (S rem 2) == 1 -> ?ClientErrorNoLogging({"Hex string must have even number of characters", {binary, S}});
        true -> hexstr_to_bin(Val)
    end;
io_to_binary(Val, Len) when is_list(Val) ->
    ...
    if
        (FirstOK == false) -> ?ClientErrorNoLogging({"Invalid hex string starts with", {binary, [hd(Val)]}});
        (LastOK == false) -> ?ClientErrorNoLogging({"Invalid hex string ends with", {binary, [lists:last(Val)]}});
        (Len /= undefined) andalso (L > Len + Len) -> ?ClientErrorNoLogging({"Binary data is too long", {binary, Len}});
        (L rem 2) == 1 -> ?ClientErrorNoLogging({"Hex string must have even number of characters", {binary, L}});
        true -> hexstr_to_bin(Val)
    end.

hexstr_to_bin(B) when is_binary(B) -> hexstr_to_bin(binary_to_list(B));
hexstr_to_bin(S) when is_list(S) -> hexstr_to_bin(S, []).
...
io_to_userid(Id) when is_binary(Id) -> try list_to_integer(binary_to_list(Id)) catch _:_ -> io_to_atom(Id) end;
io_to_userid(Id) when is_list(Id) -> io_to_userid(list_to_binary(Id)).

-spec io_to_timestamp(ddIo()) -> ddTimeUID().
io_to_timestamp(TS) -> io_to_timestamp(TS, undefined).

-spec io_to_timestamp(ddIo(), undefined | integer()) -> ddTimeUID().
io_to_timestamp(TS, undefined) -> io_to_timestamp(TS, 6);
io_to_timestamp(B, Prec) when is_binary(B) -> io_to_timestamp(binary_to_list(B), Prec);
io_to_timestamp("systime", Prec) -> io_to_timestamp("now", Prec);
io_to_timestamp("sysdate", Prec) -> io_to_timestamp("now", Prec);
io_to_timestamp("now", Prec) ->
    {Secs, Micros, Node, Cnt} = ?TIME_UID,
    {
        Secs,
        erlang:round(erlang:round(math:pow(10, Prec - 6) * Micros) * erlang:round(math:pow(10, 6 - Prec))),
        Node,
        Cnt
    };
io_to_timestamp([${ | _] = Val, _Prec) ->
    case io_to_tuple(Val, undefined) of
        ...
        {Meg, Sec, Mic} when is_integer(Meg), is_integer(Sec), is_integer(Mic) ->
            {1000000 * Meg + Sec, Mic, node(), ?INTEGER_UID};
...
io_to_timestamp(Val0, 6) ->
    ...
    try
            ...
            case re:run(lists:sublist(Val, 5), "[/.-]+", [{capture, all, list}]) of
                    ...
                    case string:tokens(Val, " ") of
                        [D, T] ->
                            case re:split(T, "[.]", [{return, list}]) of
                                [Hms, M] -> {parse_date_us(D), parse_time(Hms), parse_micro(M)};
                                [Hms] -> {parse_date_us(D), parse_time(Hms), 0.0}
                            end;
                ...
                {match, ["."]} ->
                    case string:tokens(Val, " ") of
                        [D, T] ->
                            case re:split(T, "[.]", [{return, list}]) of
                                [Hms, M] -> {parse_date_eu(D), parse_time(Hms), parse_micro(M)};
                                [Hms] -> {parse_date_eu(D), parse_time(Hms), 0.0}
                            end;
                ...
                _ ->
                    case string:tokens(Val, " ") of
                        ...
                        [DT] when length(DT) > 14 ->
                            {D, T} = lists:split(8, DT),
        ...
        _ : {'ClientError', Reason} ->
            ?ClientErrorNoLogging({"Data conversion format error", {timestamp, Val, Reason}});
...
io_to_timestamp(Val, Prec) when Prec == 0 ->
    ...
    if
        (Micros >= 500000) and (Secs == 999999) -> {Secs + 1, 0, Node, Cnt};
        true -> {Secs, 0, Node, Cnt}
    end;
io_to_timestamp(Val, Prec) when Prec > 0 ->
    {Secs, Micros, Node, Cnt} = io_to_timestamp(Val, 6),
    {
...
io_to_datetime(B) when is_binary(B) -> io_to_datetime(binary_to_list(B));
io_to_datetime("today") ->
    {Date, _} = io_to_datetime("localtime"),
    {Date, {0, 0, 0}};
io_to_datetime("systime") -> io_to_datetime("localtime");
io_to_datetime("sysdate") -> io_to_datetime("localtime");
io_to_datetime("now") -> io_to_datetime("localtime");
io_to_datetime("localtime") -> erlang:localtime();
...
io_to_datetime(Val0) ->
    Val = re:replace(re:replace(Val0, "T", " ", [{return, list}]), "(\\.[0-9]*Z|Z)$", "", [{return, list}]),
    try
        case re:run(lists:sublist(Val, 5), "[/.-]+", [{capture, all, list}]) of
            {match, ["."]} ->
                case string:tokens(Val, " ") of
...
parse_date_raw(Val) ->
    case length(Val) of
        8 ->
            validate_date(
                {
                    parse_year(lists:sublist(Val, 1, 4)),
                    parse_month(lists:sublist(Val, 5, 2)),
                    parse_day(lists:sublist(Val, 7, 2))
                }
            );
        6 ->
            validate_date(
                {
                    parse_year(lists:sublist(Val, 1, 2)),
                    parse_month(lists:sublist(Val, 3, 2)),
                    parse_day(lists:sublist(Val, 5, 2))
                }
            );
...
parse_year(Val) ->
    case length(Val) of
        ...
        2 ->
            Year2 = list_to_integer(Val),
            if
                Year2 < 50 -> 2000 + Year2;
                true -> 1900 + Year2
            end;
        _ -> ?ClientErrorNoLogging({"parse_year", Val})
    end.
...
parse_month(Val) ->
    case length(Val) of
        ...
        _ -> ?ClientErrorNoLogging({"parse_month", Val})
    end.
...
parse_day(Val) ->
    case length(Val) of
        1 -> list_to_integer(Val);
        2 -> list_to_integer(Val);
        _ -> ?ClientErrorNoLogging({"parse_day", Val})
    end.
    ...
    case string:tokens(Val, ":") of
        ...
        _ ->
            case length(Val) of
                6 ->
                    {
                        parse_hour(lists:sublist(Val, 1, 2)),
                        parse_minute(lists:sublist(Val, 3, 2)),
                        parse_minute(lists:sublist(Val, 5, 2))
                    };
                4 -> {parse_hour(lists:sublist(Val, 1, 2)), parse_minute(lists:sublist(Val, 3, 2)), 0};
                2 -> {parse_hour(lists:sublist(Val, 1, 2)), 0, 0};
                0 -> {0, 0, 0};
                _ -> ?ClientErrorNoLogging({"parse_time", Val})
            end
    end.
...
parse_hour(Val) ->
    ...
    if
        H >= 0 andalso H < 25 -> H;
        true -> ?ClientErrorNoLogging({"parse_hour", Val})
    end.
...
parse_minute(Val) ->
    ...
    if
        M >= 0 andalso M < 60 -> M;
        true -> ?ClientErrorNoLogging({"parse_minute", Val})
    end.
...
parse_second(Val) ->
    ...
    if
        S >= 0 andalso S < 60 -> S;
        true -> ?ClientErrorNoLogging({"parse_second", Val})
    end.

parse_micro(Val) -> list_to_float("0." ++ Val).
...
local_datetime_to_utc1900_seconds({Date, Time}) ->
    ...
    case calendar:local_time_to_universal_time_dst({Date, Time}) of
        [DateTimeUTC] -> calendar:datetime_to_gregorian_seconds(DateTimeUTC) - 59958230400;
        [DstDateTimeUTC, _] -> calendar:datetime_to_gregorian_seconds(DstDateTimeUTC) - 59958230400
    end.
...
local_datetime_to_utc1970_seconds({Date, Time}) ->
    ...
    %%  calendar:datetime_to_gregorian_seconds({Date, Time}) - DateTime1900.
    case calendar:local_time_to_universal_time_dst({Date, Time}) of
        [DateTimeUTC] -> calendar:datetime_to_gregorian_seconds(DateTimeUTC) - 62167219200;
        [DstDateTimeUTC, _] -> calendar:datetime_to_gregorian_seconds(DstDateTimeUTC) - 62167219200
    end.
...
timestamp_to_local_datetime({Secs, Micros}) ->
    timestamp_to_local_datetime({Secs div 1000000, Secs rem 1000000, Micros});
...
seconds_since_epoch({Date, Time}) when is_tuple(Date), is_tuple(Time) ->
    local_datetime_to_utc1970_seconds({Date, Time}).

validate_date(Date) ->
    case calendar:valid_date(Date) of
        true -> Date;
        false -> ?ClientErrorNoLogging({"validate_date", Date})
    end.

io_to_ipaddr(Val) -> io_to_ipaddr(Val, undefined).

io_to_ipaddr(Val, 0) -> io_to_ipaddr(Val, undefined);
io_to_ipaddr(Val, Len) when is_binary(Val) -> io_to_ipaddr(binary_to_list(Val), Len);
    ...
    case io_to_term(Val) of
        {A, B, C, D} when is_integer(A), is_integer(B), is_integer(C), is_integer(D) ->
            if
                ...
                Len == 4 -> {A, B, C, D};
                true -> ?ClientErrorNoLogging({"Data conversion format error", {ipaddr, Len, Val}})
            end;
        {A, B, C, D, E, F, G, H}
        when is_integer(A),
             is_integer(B),
        ...
        _ -> ?ClientErrorNoLogging({"Data conversion format error", {ipaddr, Len, Val}})
    end;
io_to_ipaddr(Val, Len) ->
    Result =
        try
            ...
            _:_ -> ?ClientErrorNoLogging({"Data conversion format error", {ipaddr, Len, Val}})
        end,
    RLen = size(Result),
    if
...
io_to_decimal(Val, Len, Prec) when is_binary(Val) -> io_to_decimal(binary_to_list(Val), Len, Prec);
io_to_decimal(Val, Len, undefined) -> io_to_decimal(Val, Len, 0);
io_to_decimal(Val, Len, 0) ->
    %% use fixed point arithmetic with implicit scaling factor
    ...
    case string:to_integer(DotRemoved) of
        {Valid, []} when Valid >= 0 ->
            ...
            if
                Len == undefined -> Result;
                ResultLength > Len ->
                    ?ClientErrorNoLogging({"Data conversion format error", {decimal, Len, Prec, Val}});
                true -> Result
            end;
        _NotInteger -> ?ClientErrorNoLogging({"Data conversion format error", {decimal, Len, Prec, Val}})
    end.

io_to_decimal(DotRemoved, _NewLength, Sign, 0) -> {0, Sign ++ DotRemoved};
...
io_to_decimal(_DotRemoved, NewLength, _Sign, _ZerosToAdd) when NewLength < 0 -> {0, []};
...
io_to_binstr(Val) -> io_to_binstr(Val, undefined).
...
io_to_binstr(Val, Len) ->
    ...
    if
        ...
        Len == 0 -> Bin;
        size(Bin) =< Len -> Bin;
        true -> ?ClientErrorNoLogging({"String is too long", {Val, Len}})
    end.

io_to_string(Bin) -> io_to_string(Bin, undefined).

io_to_string(Bin, Len) when is_binary(Bin) ->
    case strip_dquotes(Bin) of
        Bin -> ?ClientErrorNoLogging({"Missing double quotes for list string format", {Bin, Len}});
        B ->
            List =
                case unicode:characters_to_list(B, utf8) of
                    L when is_list(L) ->
                        %% Bin was utf8 encoded
                        lists:flatten(io_lib:format("~s", [un_escape_io(L)]));
                    _ ->
                        %% Bin is bytewise encoded
                        lists:flatten(io_lib:format("~s", [un_escape_io(B)]))
                end,
            if
                Len == undefined -> List;
                Len == 0 -> List;
                length(List) =< Len -> List;
                true -> ?ClientErrorNoLogging({"String is too long", {B, Len}})
            end
    end.
    ...
    case io_to_term(Val) of
        V when is_list(V) ->
            if
                ...
                Len == 0 -> V;
                length(V) == Len -> V;
                true -> ?ClientErrorNoLogging({"Data conversion format error", {list, Len, Val}})
            end;
        _ -> ?ClientErrorNoLogging({"Data conversion format error", {list, Len, Val}})
    end.

io_to_map(Val) ->
    case io_to_term(Val) of
        V when is_map(V) -> V;
        _ -> ?ClientErrorNoLogging({"Data conversion format error", {map, Val}})
    end.
    ...
    case io_to_term(Val) of
        V when is_tuple(V) ->
            if
                ...
                Len == 0 -> V;
                size(V) == Len -> V;
                true -> ?ClientErrorNoLogging({"Data conversion format error", {tuple, Len, Val}})
            end;
        _ -> ?ClientErrorNoLogging({"Data conversion format error", {tuple, Len, Val}})
    end.

io_to_boolean(Val) ->
    case io_to_term(Val) of
        V when is_boolean(V) -> V;
        _ -> ?ClientErrorNoLogging({"Data conversion format error", {boolean, Val}})
    end.

io_to_term(Val) ->
    {Encrypt, Value} =
        case re:replace(Val, "^enc\\((.*)\\)$", "\\1", [dotall, global]) of
            Val -> {false, Val};
            [[Val1]] -> {true, Val1}
        end,
    try
        ...
        if
            Encrypt -> imem_config:encrypt(Value1);
            true -> Value1
        end
    catch
        _:_ ->
...
io_to_binterm(Val) -> try sext:encode(imem_compiler:compile(Val)) catch _:_ -> ?ClientErrorNoLogging({}) end.

io_to_fun(Str) ->
    Fun = imem_compiler:compile(Str),
    if
        is_function(Fun) -> Fun;
        true -> ?ClientErrorNoLogging({"Data conversion format error", {'fun', Str}})
    end.
...
io_to_fun(Str, Len) ->
    ...
    if
        ...
        true -> ?ClientErrorNoLogging({"Data conversion format error", {'fun', Len, Str}})
    end.
...
io_to_fun(Str, Len, Bindings) ->
    ...
    if
        Len == undefined -> Fun;
        is_function(Fun, Len) -> Fun;
        true -> ?ClientErrorNoLogging({"Data conversion format error", {'fun', Len, Str, Bindings}})
    end.
...
atom_to_io(Val) -> list_to_binary(io_lib:format("~tp", [Val])).

datetime_to_io(Datetime) -> datetime_to_io(Datetime, eu).

datetime_to_io({{Year, Month, Day}, {Hour, Min, Sec}}, eu) ->
    list_to_binary(
        io_lib:format("~2.10.0B.~2.10.0B.~4.10.0B ~2.10.0B:~2.10.0B:~2.10.0B", [Day, Month, Year, Hour, Min, Sec])
    );
datetime_to_io(Datetime, erlang) -> term_to_io(Datetime);
...
datetime_to_io({{Year, Month, Day}, {Hour, Min, Sec}}, us) ->
    list_to_binary(
        io_lib:format("~2.10.0B/~2.10.0B/~4.10.0B ~2.10.0B:~2.10.0B:~2.10.0B", [Month, Day, Year, Hour, Min, Sec])
    );
datetime_to_io(Datetime, Fmt) -> ?ClientErrorNoLogging({"Data conversion format error", {datetime, Fmt, Datetime}}).

-spec timestamp_to_io(ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}) -> ddBinStr().
timestamp_to_io(TS) -> timestamp_to_io(TS, 6, eu).

-spec timestamp_to_io(ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}, undefined | integer()) ->
    ddBinStr().
timestamp_to_io(TS, undefined) -> timestamp_to_io(TS, 6, eu);
timestamp_to_io(TS, Prec) -> timestamp_to_io(TS, Prec, eu).

-spec timestamp_to_io(ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}, undefined | integer(), atom()) ->
    ddBinStr().
timestamp_to_io(TS, undefined, Fmt) -> timestamp_to_io(TS, 6, Fmt);
timestamp_to_io({Secs, Micros, Node, Cnt}, _Prec, raw) ->
    list_to_binary(io_lib:format("~12.12.0w~6.6.0w~p~p", [Secs, Micros, Node, Cnt]));
timestamp_to_io({Secs, Micros}, _Prec, raw) -> list_to_binary(io_lib:format("~12.12.0w~6.6.0w", [Secs, Micros]));
timestamp_to_io(TS, _Prec, erlang) -> term_to_io(TS);
timestamp_to_io({Megas, Secs, Micros}, Prec, Fmt) -> timestamp_to_io({1000000 * Megas + Secs, Micros}, Prec, Fmt);
...
timestamp_to_io({Secs, Micros}, Prec, Fmt) when Prec > 0 ->
    ...
    MStr1 =
        case list_to_integer(lists:sublist(MStr0, Prec + 1, 6 - Prec)) of
...
timestamp_to_io({Secs, Micros}, _, Fmt) -> timestamp_to_io({Secs, Micros}, 6, Fmt).

decimal_to_io(Val, undefined) -> decimal_to_io(Val, 0);
decimal_to_io(Val, 0) -> list_to_binary(io_lib:format("~p", [Val]));
decimal_to_io(Val, Prec) when Val < 0 -> list_to_binary(io_lib:format("-~s", [decimal_to_io(- Val, Prec)]));
...
decimal_to_io(Val, Prec) -> list_to_binary(io_lib:format("~s~s", [integer_to_list(Val), lists:duplicate(- Prec, $0)])).

binstr_to_io(BinStr) -> BinStr.

binary_to_io(Val) ->
    if
        byte_size(Val) =< ?BinaryMaxLen -> list_to_binary(io_lib:format("~s", [binary_to_hex(Val)]));
        true -> list_to_binary(io_lib:format("~s...", [binary_to_hex(binary:part(Val, 0, ?BinaryMaxLen))]))
    end.

userid_to_io(A) when is_atom(A) -> list_to_binary(atom_to_list(A));
userid_to_io(Val) -> integer_to_binary(Val).

offset_datetime('-', DT, Offset) -> offset_datetime('+', DT, - Offset);
offset_datetime('+', {{Y, M, D}, {HH, MI, SS}}, Offset) ->
    %% for local time we should use calendar:local_time_to_universal_time_dst(DT)
    GregSecs = calendar:datetime_to_gregorian_seconds({{Y, M, D}, {HH, MI, SS}}),
    %% calendar:universal_time_to_local_time(
    calendar:gregorian_seconds_to_datetime(GregSecs + round(Offset * 8.64e4));
offset_datetime(OP, DT, Offset) -> ?ClientErrorNoLogging({"Illegal datetime offset operation", {OP, DT, Offset}}).

-spec datetime_diff(ddDatetime(), ddDatetime()) -> float().
datetime_diff(TS1, TS2) ->
    (calendar:datetime_to_gregorian_seconds(TS2) - calendar:datetime_to_gregorian_seconds(TS1)) / 8.64e4.

-spec offset_timestamp(atom(), ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}, number()) ->
    ddTimeUID().
offset_timestamp('+', TS, Offset) when Offset < 0.0 -> offset_timestamp('-', TS, - Offset);
offset_timestamp('-', TS, Offset) when Offset < 0.0 -> offset_timestamp('+', TS, - Offset);
offset_timestamp(_, TS, Offset) when Offset < 5.787e-12 -> TS;
offset_timestamp('+', {Sec, Micro}, Offset) -> offset_timestamp('+', {Sec, Micro, node(), 0}, Offset);
offset_timestamp('+', {Mega, Sec, Micro}, Offset) ->
    %% to be removed
    %% to be removed
    offset_timestamp('+', {1000000 * Mega + Sec, Micro, node(), 0}, Offset);
...
offset_timestamp(OP, TS, Offset) -> ?ClientErrorNoLogging({"Illegal timestamp offset operation", {OP, TS, Offset}}).

-spec musec_diff(ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}) -> integer().
musec_diff(TS1) -> musec_diff(TS1, ?TIMESTAMP).

-spec musec_diff(
    ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()},
    ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}
) ->
    integer().
musec_diff({Sec, Micro, _, _}, TS2) -> musec_diff({Sec, Micro}, TS2);
musec_diff({Mega, Sec, Micro}, TS2) ->
    %% to be removed
    musec_diff({1000000 * Mega + Sec, Micro}, TS2);
...
msec_diff({Mega, Sec, Micro}, TS2) ->
    %% to be removed
    msec_diff({1000000 * Mega + Sec, Micro}, TS2);
msec_diff(TS1, {Sec, Micro, _, _}) -> msec_diff(TS1, {Sec, Micro});
msec_diff(TS1, {Mega, Sec, Micro}) ->
    %% to be removed
    msec_diff(TS1, {1000000 * Mega + Sec, Micro});
...
sec_diff({Mega, Sec, Micro}, TS2) ->
    %% to be removed
    sec_diff({1000000 * Mega + Sec, Micro}, TS2);
sec_diff(TS1, {Sec, Micro, _, _}) -> sec_diff(TS1, {Sec, Micro});
sec_diff(TS1, {Mega, Sec, Micro}) ->
    %% to be removed
    sec_diff(TS1, {1000000 * Mega + Sec, Micro});
sec_diff({Sec1, _Micro1}, {Sec2, _Micro2}) -> Sec2 - Sec1.

ipaddr_to_io(IpAddr) -> list_to_binary(inet_parse:ntoa(IpAddr)).

float_to_io(Val, _Prec, _NumFmt) -> float_to_binary(Val, [{decimals, 15}, compact]).

%% ToDo: implement rounding to db precision

boolean_to_io(T) -> list_to_binary(io_lib:format("~p", [T])).

term_to_io(T) -> list_to_binary(io_lib:format("~100000p", [T])).

%list_to_binary(t2s(T)).
binterm_to_io(B) when is_binary(B) -> list_to_binary(io_lib:format("~100000p", [sext:decode(B)])).

%list_to_binary(t2s(sext:decode(B))).
un_escape_io(Str) when is_list(Str) -> re:replace(Str, "(\\\\\")", "\"", [global, {return, list}]);
un_escape_io(Bin) when is_binary(Bin) -> re:replace(Bin, "(\\\\\")", "\"", [global, {return, binary}]).

string_to_io(Val) when is_list(Val) ->
    case io_lib:printable_unicode_list(Val) of
        true -> unicode:characters_to_binary(io_lib:format("~p", [Val]));
        false -> list_to_binary(lists:flatten(io_lib:format("\"~tp\"", [Val])))
    end;
string_to_io(Val) -> list_to_binary(lists:flatten(io_lib:format("~tp", [Val]))).

%% "\"~tp\""

-ifdef(USE_THOUSAND_SEP).

integer_to_io(Val) -> list_to_binary(lists:foldl(fun integer_filter/2, [], lists:reverse(integer_to_list(Val)))).
...
integer_to_io(Val) -> list_to_binary(integer_to_list(Val)).
...
-spec field_value_type(any(), atom(), undefined | integer(), undefined | integer(), any(), list() | binary()) ->
    {any(), any(), atom(), integer()}.
...
item(I, T) when is_tuple(T) ->
    if
        size(T) >= I -> term_to_io(element(I, T));
        true ->
            %% ?ClientErrorNoLogging({"Tuple too short",{T,I}})
            ?emptyIo
    end;
item(I, L) when is_list(L) ->
    if
        length(L) >= I -> term_to_io(lists:nth(I, L));
        true ->
            %% ?ClientErrorNoLogging({"List too short",{L,I}})
            ?emptyIo
    end;
...
%% ?ClientErrorNoLogging({"Tuple or list expected",T}).
...
binary_to_hex(B) when is_binary(B) -> binary_to_hex(B, <<>>).

binary_to_hex(<<>>, Acc) -> Acc;
binary_to_hex(Bin, Acc) when byte_size(Bin) band 7 =:= 0 -> binary_to_hex_(Bin, Acc);
binary_to_hex(<<X : 8, Rest/binary>>, Acc) -> binary_to_hex(Rest, <<Acc/binary, ?H(X)>>).

binary_to_hex_(<<>>, Acc) -> Acc;
binary_to_hex_(<<A : 8, B : 8, C : 8, D : 8, E : 8, F : 8, G : 8, H : 8, Rest/binary>>, Acc) ->
    binary_to_hex_(Rest, <<Acc/binary, ?H(A), ?H(B), ?H(C), ?H(D), ?H(E), ?H(F), ?H(G), ?H(H)>>).
...
db_test_() -> {setup, fun () -> ok end, fun (_) -> ok end, {with, [fun data_types/1]}}.
...
data_types(_) ->
    try
        ...
        ?assertException(
            throw,
            {ClEr, {"Cannot handle dates before 1970"}},
            local_datetime_to_utc1970_seconds({{1900, 1, 1}, {0, 0, 0}})
        ),
        ...
        case local_datetime_to_utc1970_seconds({{1970, 1, 1}, {1, 0, 0}}) of
            0 ->
                %% DLS offset CH
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.123456">>, timestamp_to_io({0, 0, 123456}, 0)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.123456">>, timestamp_to_io({0, 123456}, 0)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.123456">>, timestamp_to_io({0, 123456, n, 321}, 0)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.000000">>, timestamp_to_io({0, 0}, 0)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.000000">>, timestamp_to_io({0, 0, n, 321}, 0)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.123">>, timestamp_to_io({0, 123000}, 3)),
                %% with DLS offset CH
                ?assertEqual(<<"01.01.1970 01:00:00.123">>, timestamp_to_io({0, 123000, n, 321}, 3)),
                %% with DLS offset CH
                ?assertEqual(<<"12.01.1970 14:46:42.123456">>, timestamp_to_io({1000002, 123456}, 6)),
                %% with DLS offset CH
                ?assertEqual(<<"12.01.1970 14:46:42.123456">>, timestamp_to_io({1000002, 123456, n, 321}, 6));
            _ -> ok
        end,
        ...
        case local_datetime_to_utc1970_seconds({{1970, 1, 1}, {1, 0, 0}}) of
            0 ->
                %% DLS offset CH
                %% with DLS offset CH
                ?assertMatch({0, 0, _, _}, io_to_timestamp(<<"01.01.1970 01:00:00.000000">>, 0)),
                %% with DLS offset CH
                ?assertMatch({- 3600, 0, _, _}, io_to_timestamp(<<"01.01.1970">>, 0)),
                %% with DLS offset CH
                ?assertMatch({- 3600, 0, _, _}, io_to_timestamp(<<"1970-01-01">>)),
                %% with DLS offset CH
                ?assertMatch({162946800, 0, _, _}, io_to_timestamp(<<"1975-03-02">>)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123456, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, undefined)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123456, _, _}, io_to_timestamp(<<"12.01.70 14:46:42.123456">>, undefined)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123456, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, 6)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123000, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, 3)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123456, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, undefined)),
                %% with DLS offset CH
                ?assertMatch({1000002, 0, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.654321">>, 0)),
                %% with DLS offset CH
                ?assertMatch({1000002, 123000, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, 3)),
                %% with DLS offset CH
                ?assertMatch({1000002, 100000, _, _}, io_to_timestamp(<<"12.01.1970 14:46:42.123456">>, 1)),
                %% with DLS offset CH
                ?assertMatch({587863439, 585000, _, _}, io_to_timestamp(<<"1988-8-18T01:23:59.585Z">>));
            _ -> ok
        end,
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {datetime, "8/18/1888 1"}}},
            io_to_datetime(<<"8/18/1888 1">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {datetime, "8/18/1888 1"}}},
        ...
        ?assertEqual(
            io_to_string(<<"\"NöVal\"">>),
            io_to_db(Item, OldString, string, 6, Prec, Def, RW, <<"\"NöVal\"">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"String is too long", {0, {<<"NewVal">>, 3}}}},
            io_to_db(Item, OldString, string, Len, Prec, Def, RW, <<"\"NewVal\"">>)
        ),
        ...
        ?assertEqual(
            <<"OldäString"/utf8>>,
            io_to_db(Item, <<"OldäString"/utf8>>, binstr, 11, Prec, Def, RW, <<"OldäString"/utf8>>)
        ),
        ?assertException(
            throw,
            {ClEr, {"String is too long", {0, {<<"OldäString"/utf8>>, 3}}}},
            io_to_db(Item, <<"OldäString"/utf8>>, binstr, Len, Prec, Def, RW, <<"OldäString"/utf8>>)
        ),
        ?assertException(
            throw,
            {ClEr, {"String is too long", {0, {<<"NewVal">>, 3}}}},
            io_to_db(Item, <<"OldäString"/utf8>>, binstr, Len, Prec, Def, RW, <<"NewVal">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {integer, 3, 1, "1234"}}}},
            io_to_db(Item, OldInteger, integer, Len, Prec, Def, RW, <<"1234">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {integer, <<"-">>}}}},
            io_to_db(Item, OldInteger, integer, Len, Prec, Def, RW, <<"-">>)
        ),
        ?assertEqual(default, io_to_db(Item, OldInteger, integer, Len, Prec, Def, RW, ?emptyIo)),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {integer, 3, 1, "-100"}}}},
            io_to_db(Item, OldInteger, integer, Len, Prec, Def, RW, <<"-100">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {integer, 3, 1, "9999"}}}},
            io_to_db(Item, OldInteger, integer, Len, Prec, Def, RW, <<"9999">>)
        ),
        %?LogDebug("io_to_db success 5~n", []),
        OldFloat = - 1.2,
        ...
        %% rounding not supported any more for floats
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 5, 5, "1.123"}}}},
            io_to_db(Item, OldDecimal, decimal, 5, 5, Def, RW, <<"1.123">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 6, 5, "-1.123"}}}},
            io_to_db(Item, OldDecimal, decimal, 6, 5, Def, RW, <<"-1.123">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 5, Prec, "1234567.89"}}}},
            io_to_db(Item, OldDecimal, decimal, 5, Prec, Def, RW, <<"1234567.89">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 5, 2, "12345"}}}},
            io_to_db(Item, OldDecimal, decimal, 5, 2, Def, RW, <<"12345">>)
        ),
        ?assertEqual(
            300000000000000000000000000000000000000,
            io_to_db(Item, OldDecimal, decimal, undefined, 38, Def, RW, <<"3">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 5, - 2, "123bb"}}}},
            io_to_db(Item, OldDecimal, decimal, 5, - 2, Def, RW, <<"123bb">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {decimal, 5, - 1, "--123"}}}},
            io_to_db(Item, OldDecimal, decimal, 5, - 1, Def, RW, <<"--123">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {term, <<"[a|]">>}}}},
            io_to_db(Item, OldTerm, term, Len, Prec, Def, RW, <<"[a|]">>)
        ),
        ...
        ?assertEqual(
            term_to_binterm("default"),
            io_to_db(Item, OldBinTerm, binterm, Len, Prec, Def, RW, <<"\"default\"">>)
        ),
        ?assertEqual(
            term_to_binterm("'default'"),
            io_to_db(Item, OldBinTerm, binterm, Len, Prec, Def, RW, <<"\"'default'\"">>)
        ...
        ?assertEqual(
            term_to_binterm([a, b]),
            io_to_db(Item, OldBinTerm, binterm, undefined, undefined, Def, RW, <<"[a,b]">>)
        ),
        ?assertEqual(term_to_binterm([a, b]), io_to_db(Item, OldBinTerm, binterm, Len, Prec, Def, RW, <<"[a,b]">>)),
        ?assertEqual(
            term_to_binterm(- 1.1234567),
            io_to_db(Item, OldBinTerm, binterm, Len, Prec, Def, RW, <<"-1.1234567">>)
        ),
        ?assertEqual(
            term_to_binterm("'-1.1234567'"),
            io_to_db(Item, OldBinTerm, binterm, Len, Prec, Def, RW, <<"\"'-1.1234567'\"">>)
        ...
        ?assertEqual(
            term_to_binterm({[1, 2, 3]}),
            io_to_db(Item, OldBinTerm, binterm, undefined, Prec, Def, RW, <<"{[1,2,3]}">>)
        ),
        ?assertEqual(
            term_to_binterm("{[1,2,3]}"),
            io_to_db(Item, OldBinTerm, binterm, Len, undefined, Def, RW, <<"\"{[1,2,3]}\"">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {boolean, <<"\"false\"">>}}}},
            io_to_db(Item, OldTerm, boolean, Len, undefined, Def, RW, <<"\"false\"">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {boolean, <<"something">>}}}},
            io_to_db(Item, OldTerm, boolean, Len, Prec, Def, RW, <<"something">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {boolean, <<"TRUE">>}}}},
            io_to_db(Item, OldTerm, boolean, Len, Prec, Def, RW, <<"TRUE">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {tuple, Len, <<"[a]">>}}}},
            io_to_db(Item, OldTerm, tuple, Len, Prec, Def, RW, <<"[a]">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {tuple, Len, <<"{a}">>}}}},
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {list, Len, <<"[a]">>}}}},
            io_to_db(Item, OldTerm, list, Len, Prec, Def, RW, <<"[a]">>)
        ),
        ...
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {map, <<"[a]">>}}}},
            io_to_db(Item, OldTerm, map, Len, Prec, Def, RW, <<"[a]">>)
        ),
        ...
        ?assertEqual(
            {255, 255, 255, 255},
            io_to_db(Item, OldTerm, ipaddr, undefined, Prec, Def, RW, <<"255.255.255.255">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {ipaddr, undefined, "1.2.3.4.5"}}}},
            io_to_db(Item, OldTerm, ipaddr, 0, 0, Def, RW, <<"1.2.3.4.5">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {ipaddr, 4, "1.2.-1.4"}}}},
            io_to_db(Item, OldTerm, ipaddr, 4, 0, Def, RW, <<"1.2.-1.4">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {ipaddr, 6, "1.256.1.4"}}}},
            io_to_db(Item, OldTerm, ipaddr, 6, 0, Def, RW, <<"1.256.1.4">>)
        ),
        ?assertException(
            throw,
            {ClEr, {"Data conversion format error", {0, {ipaddr, 8, "1.2.1.4"}}}},
            io_to_db(Item, OldTerm, ipaddr, 8, 0, Def, RW, <<"1.2.1.4">>)
        ),
        ...
        ?assertEqual(
            {{2000, 1, 28}, {12, 12, 14}},
            offset_datetime('-', {{2000, 1, 28}, {12, 13, 14}}, 1.0 / 24.0 / 60.0)
        ),
        ?assertEqual(
            {{2000, 1, 28}, {12, 13, 13}},
            offset_datetime('-', {{2000, 1, 28}, {12, 13, 14}}, 1.0 / 24.0 / 3.6e3)
        ),
        ENow = ?TIME_UID,
        ...
        ?assertEqual(ENow, offset_timestamp('+', offset_timestamp('-', ENow, 1.0e-5), 1.0e-5)),
