...
exec(SKey, {'alter user', Name, {spec, Specs}}, _Stmt, _StmtOpts, IsSec) ->
    ...
    case lists:keyfind('identified by', 1, Specs) of
        {'identified by', NewPasswordMayBeDoubleQuoted} ->
            ...
            try begin PasswordStrengthFun = imem_seco:password_strength_fun(), PasswordStrengthFun(NewPassword) end of
                strong ->
                ...
                Error when Error /= strong ->
                    ?ClientError(lists:flatten(io_lib:format("New password insufficiently complex ('~p')", [Error])));
                Other ->
                    ?Error(
                        "Bad return from isPasswordComplex function '~p'" " (should be strong|short|medium|weak)",
                        [Other]
                    ),
...
grant_sys_priv(SKey, PA, GBin) when is_atom(PA) ->
    P =
        case imem_role:exists(SKey, PA) of
            true -> {role, PA};
            false -> {perm, PA}
        end,
    G =
        case (catch list_to_existing_atom(binary_to_list(GBin))) of
            GA when is_atom(GA) ->
...
revoke_obj_priv(SKey, PA, GBin, OBin) when is_atom(PA) ->
    ...
    O =
        case (catch list_to_existing_atom(Name)) of
            ...
            _ -> ?SecurityException({"Object does not exist", {SKey, OBin}})
        end,
    G =
        case (catch list_to_existing_atom(binary_to_list(GBin))) of
            GA when is_atom(GA) ->
