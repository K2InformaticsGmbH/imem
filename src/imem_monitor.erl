...
-define(
    GET_MONITOR_EXTRA,
    ?GET_CONFIG(monitorExtra, [], true, "Does monitor call an Extra function to augment the monitored status data?")
).
-define(
    GET_MONITOR_EXTRA_FUN,
    ?GET_CONFIG(
        monitorExtraFun,
        [],
        ?BIN(
            fun
                (Row) ->
                    ...
                    MemFreePerCent = 0.01 * erlang:round(10000 * FreeMemory / TotalMemory),
                    {_, VOutMem} = imem:get_vm_memory(),
                    #{free_virtual_memory := FreeVirtualMemory, total_virtual_memory := TotalVirtualMemory} =
                        imem:get_virtual_memory(),
        ...
        "Function which can be called in every monitor cycle to augment the monitor data."
    )
).
-define(GET_MONITOR_DUMP, ?GET_CONFIG(monitorDump, [], true, "Should the monitor status be written to a file on disk?")).
-define(
    GET_MONITOR_DUMP_FUN,
    ?GET_CONFIG(monitorDumpFun, [], <<"">>, "Function used to dump the monitor data to disk.")
).
...
-export([write_monitor/0, write_monitor/2, write_dump_log/3]).
...
init(_Args) ->
    try
        ...
        create_check_table(
            ?MONITOR_TABLE,
            {record_info(fields, ddMonitor), ?ddMonitor, #ddMonitor{}},
            ?MONITOR_TABLE_OPTS,
            system
        ),
...
handle_info(
    imem_monitor_loop,
    #state{extraFun = ExtraFun, extraHash = ExtraHash, dumpFun = DumpFun, dumpHash = DumpHash} = State
) ->
...
get_mon_fun(extra, Fun, Hash) -> get_mon_fun({extra, ?GET_MONITOR_EXTRA}, Fun, Hash);
get_mon_fun({extra, true}, Fun, Hash) -> get_mon_fun(?GET_MONITOR_EXTRA_FUN, Fun, Hash);
get_mon_fun(dump, Fun, Hash) -> get_mon_fun({dump, ?GET_MONITOR_DUMP}, Fun, Hash);
get_mon_fun({dump, true}, Fun, Hash) -> get_mon_fun(?GET_MONITOR_DUMP_FUN, Fun, Hash);
...
write_monitor(ExtraFun, DumpFun) ->
    try
        ...
        Moni1 =
            case ExtraFun of
                undefined -> Moni0;
                ExtraFun -> try Moni0#ddMonitor{extra = ExtraFun(Moni0)} catch
                        _:ExtraError ->
        ...
        case DumpFun of
            undefined -> ok;
            DumpFun -> try DumpFun(Moni1) catch _:DumpError -> ?Error("cannot dump monitor ~p", [DumpError]) end
        end
    catch
        _:Err ->
...
write_dump_log(File, Format, Args) ->
    case application:get_env(lager, crash_log) of
        ...
        {ok, CrashLogFile} ->
            ...
            LogPath = filename:join(lists:sublist(LogPathParts, length(LogPathParts) - 1)),
            file:write_file(filename:join(LogPath, File), list_to_binary(lists:flatten(io_lib:format(Format, Args))))
    end.
