...
%% for cmp/2 and cmp/3 (Token Comparison)

-define(CMP_EQUAL_STR, $=).

%% for cmp/2 and cmp/3 (Token Comparison)

-define(CMP_NAV, <<>>).

%% for cmp/2 and cmp/3 (one or both sides ?nav, not a value)

-define(CMP_SMALLER_LEFT, <<"<">>).
-define(CMP_SMALLER_RIGHT, <<">">>).
-define(CMP_WHITE_LEFT, <<"w=">>).

%% added whitespace on the left

-define(CMP_WHITE_RIGHT, <<"=w">>).

%% added whitespace on the right

-define(CMP_WHITE, <<"w=w">>).

%% added whitespace on both sides

-define(CMP_CASE, <<"c">>).

%% different casing on both sides (frames earlier result)

-define(CMP_DQUOTE, <<"\"">>).

%% dropped double quotes on both sides (frames earlier result)

-define(CMP_SPACE, 32).

%% Stereotype for white space

-define(CMP_CHAR, $@).

%% Stereotype for non-white space
...
-export(
    [
        %% compare two terms (LeftKey,RightKey)
        cmp/2,
        %% compare two terms (LeftKey,RightKey, Opts)
        cmp/3,
        cmp_trim/2,
        %% is the argument a string or a binary containing only white space?
        cmp_whitespace/1,
        %% reduce whitespace to the mminimum without corrupting (Erlang/SQL) code
        norm_whitespace/1,
        %% converts upper case to lower case in binary or string
        norm_casing/1,
...
cmp(_, ?nav, _) -> ?CMP_NAV;
cmp(L, L, _) -> ?CMP_EQUAL;
cmp(L, R, Opts) when is_binary(L) -> cmp(binary_to_list(L), R, Opts);
cmp(L, R, Opts) when is_binary(R) -> cmp(L, binary_to_list(R), Opts);
cmp(L, R, _Opts) when is_list(L), is_list(R) ->
    case
    {
        ...
        norm_casing(L),
        norm_casing(R),
        norm_casing(norm_dquotes(L)),
        norm_casing(norm_dquotes(R))
    } of
        {L1, R1, L2, R2, L3, R3} ->
            Res0 = cmp_white(L, R, []),
            ...
            case {IsEq0, IsEq1, IsEq2, IsEq3} of
                ...
                {false, _, _, true} ->
                    <<?CMP_DQUOTE/binary, ?CMP_CASE/binary, Res3/binary, ?CMP_CASE/binary, ?CMP_DQUOTE/binary>>;
                {false, _, _, _} -> Res0
            end
    end;
...
norm_casing(S) when is_binary(S) ->
...
norm_ws(_Last, [Ch | List], Acc) when Ch > ?CMP_SPACE -> norm_ws(Ch, List, [Ch | Acc]);
