...
accept_connection(AcceptPid, Socket, MyNode, Allowed, SetupTime) ->
    R = inet_tcp_dist:accept_connection(AcceptPid, Socket, MyNode, Allowed,
                                        SetupTime),
    ?MODULE ! {accept_connection, {{AcceptPid, Socket, MyNode, Allowed,
                                   SetupTime}, R}},
    R.
...
init([]) -> {ok, #state{}}.
reg_info() ->
    {Name,{ok,{_,{net_address,{Ip,Port},Host,tcp,inet},_}}}
    = proplists:get_value(listen, getq()),
    {Name, {Ip,Port}, Host}.
getq() -> lists:reverse(gen_server:call(?MODULE, get)).
handle_call(get, _From, State) ->
    {reply, State#state.calls, State};
