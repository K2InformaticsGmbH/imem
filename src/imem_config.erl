...
-export([lookup/3, get_config_hlk/5, get_config_hlk/6, put_config_hlk/6, put_config_hlk/7]).
-export([encrypt/1, decrypt/1, reference_resolve/1, reference_resolve/2, val/2]).
...
get_config_hlk({_Schema, Table}, Key, Owner, Context, Default) -> get_config_hlk(Table, Key, Owner, Context, Default);
get_config_hlk(Table, Key, Owner, Context, Default) when is_atom(Table), is_list(Context), is_atom(Owner) ->
    ...
    reference_resolve(
        ...
        case (catch imem_meta:read_hlk(Table, [Key | Context])) of
            ...
            [#ddConfig{val = Default, hkl = [Key]}] -> Default;
...
put_config_hlk(Table, Key, Owner, Context, Value, Remark) when is_atom(Table), is_list(Context), is_binary(Remark) ->
    imem_meta:dirty_write(Table, #ddConfig{hkl = [Key | Context], val = Value, remark = Remark, owner = Owner}).

encrypt(Val) ->
    {_, EVal} = crypto:stream_encrypt(crypto:stream_init(rc4, atom_to_list(erlang:get_cookie())), term_to_binary(Val)),
    [base64:encode(EVal) | {enc, 0}].

decrypt([B64Val | {enc, 0}]) ->
    Val = base64:decode(B64Val),
    {_, ValBin} = crypto:stream_decrypt(crypto:stream_init(rc4, atom_to_list(erlang:get_cookie())), Val),
    binary_to_term(ValBin);
decrypt(UnEncryptedVal) -> UnEncryptedVal.

reference_resolve(Term) -> reference_resolve(?CONFIG_TABLE, Term).

reference_resolve(Table, Term) -> reference_resolve(Table, Term, []).
    ...
    case lists:member(ConfigKey, Resolved) of
        false ->
            case (catch imem_meta:read_hlk(Table, ConfigKey)) of
                [#ddConfig{val = ResolvedRef}] -> reference_resolve(Table, ResolvedRef, [ConfigKey | Resolved]);
                _ -> ?ClientError({"Reference not found", ConfigKey})
            end;
        true -> ?ClientError({"Circular reference detected", ConfigKey})
    end;
reference_resolve(Table, [_ | {enc, _}] = Val, Resolved) -> reference_resolve(Table, decrypt(Val), Resolved);
