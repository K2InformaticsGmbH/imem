...
-export([finalize/2, fold/5, init/1, match/2]).

match(ParseTree, InFields) -> sqlparse_fold:top_down(imem_prune_fields, ParseTree, InFields).

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Setting up parameters.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-spec init(InFields :: [binary()]) -> InFields :: [binary()].
init(InFields) when is_list(InFields) -> InFields.

% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Postprocessing of the result.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
-spec finalize(InFields :: [binary()], [binary()] | tuple()) -> [binary()] | tuple().
finalize(_InFields, CtxIn) when is_list(CtxIn) -> lists:usort(CtxIn).
...
% binary
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold(InFields, _FunState, Ctx, PTree, FoldState) when is_binary(PTree), element(2, FoldState) == start ->
    add_if(InFields, PTree, Ctx);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% {atom, binary}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold(InFields, _FunState, Ctx, {Elem1, Elem2} = _PTree, FoldState)
when is_atom(Elem1), Elem1 =/= extra, is_binary(Elem2), element(2, FoldState) == start ->
    add_if(InFields, Elem2, Ctx);
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% {atom, binary, binary}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold(InFields, _FunState, Ctx, {Elem1, Elem2, Elem3} = _PTree, FoldState)
when is_atom(Elem1), is_binary(Elem2), is_binary(Elem3), element(2, FoldState) == start ->
...
% {atom, binary, _, _}
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold(InFields, _FunState, Ctx, {Elem1, Elem2, _Elem3, _Elem4} = _PTree, FoldState)
when is_atom(Elem1), is_binary(Elem2), element(2, FoldState) == start ->
...
% NO ACTION.
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
fold(_InFields, _FunState, Ctx, _PTree, _FoldState) -> Ctx.
