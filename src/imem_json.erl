...
-export([find/2, get/2, get/3, keys/1, put/3, remove/2, size/1, values/1, find_in_list/3]).
...
-export(
    [
        %% Constructs a data object prescribed by json path (using jpparse)
        eval/2,
        %% Recursively expands 'inline_*' references in a JSON object
        expand_inline/2,
        %% Lists differences of two JSON objects
        diff/2
    ]
).
...
when not
     (
         is_atom(Property)
         orelse (is_binary(Property) andalso byte_size(Property) > 0)
         orelse (is_list(Property) andalso length(Property) > 0)
     ) ->
    error(bad_property);
find_in_list(_Property, DataObjects, _Default) when not is_list(DataObjects) -> error(bad_object);
...
-spec find(nonempty_string() | atom() | binary(), jsx:json_term()) -> jsx:json_term() | no_return().
...
when not
     (
         is_map(DataObject)
         orelse (is_list(DataObject) andalso length(DataObject) > 0)
         orelse (is_binary(DataObject) andalso byte_size(DataObject) > 0)
     ) ->
    error(bad_object);
find(Property, DataObject) when is_atom(Property) -> find(atom_to_list(Property), DataObject);
find(Property, DataObject) when is_binary(Property) -> find(binary_to_list(Property), DataObject);
find([First | _] = Property, DataObject) when is_list(Property), is_binary(DataObject) ->
    % Duplicate subpattern numbers
    % End of object or property TODO newline check
    ReExp =
        case {First, lists:last(Property)} of
            {$", $"} -> Property;
            {_, $"} -> [$" | Property];
            {$", _} -> Property ++ "\"";
            {_, _} -> [$" | Property ++ "\""]
        end ++ ":" ++ "(?|" ?JRENUMBR "|" ?JRESTRNG "|" ?JRECONST ")" "[,}\r\n]",
    case re:run(DataObject, ReExp, [{capture, [1], list}, ungreedy]) of
        nomatch -> error({not_found, DataObject, ReExp});
        {match, [Got]} ->
            case [Re || Re <- ["^" ?JRENUMBR "$", "^" ?JRESTRNG "$", "^" ?JRECONST "$"], re:run(Got, Re) /= nomatch] of
                ["^" ?JRESTRNG "$"] -> re:replace(Got, "^\"|\"$", "", [{return, binary}, global]);
                ["^" ?JRECONST "$"] -> list_to_existing_atom(Got);
                ["^" ?JRENUMBR "$"] -> try jsx:decode(list_to_binary(Got)) catch _:Error -> error({Error, Got}) end;
                [] -> error({badmatch, Got})
            end
    end;
find(Property, DataObject) when is_map(DataObject) ->
    ...
    maps:fold(
        fun
            (K, V, '$not_found') when K == Prop -> V;
            (_K, V, '$not_found') when is_map(V) -> find(Property, V);
            (_K, _V, AccIn) -> AccIn
        end,
        '$not_found',
        DataObject
        ...
        [_ | _] = Value ->
            lists:foldl(
                fun (#{} = Val, '$not_found') -> find(Property, Val); (_, AccIn) -> AccIn end,
                '$not_found',
                Value
            );
        Value -> Value
...
find(Property, DataObject) when is_list(DataObject) ->
    ...
    lists:foldl(
        ...
        [_ | _] = Value ->
            lists:foldl(
                fun ([{_, _} | _] = Val, '$not_found') -> find(Property, Val); (_, AccIn) -> AccIn end,
                '$not_found',
                Value
            );
        Value -> Value
...
get(Key, DataObject, Default) when is_list(DataObject) -> get_deep(DataObject, Key, Default, []);
...
get(Key, DataObject, Default) -> ?MODULE:get(Key, decode(DataObject), Default).
...
get_deep(Obj, Key, Default, _) -> proplists:get_value(Key, Obj, Default).
...
put(Key, Value, DataObject) when is_map(DataObject) -> maps:put(Key, Value, DataObject);
put(Key, Value, DataObject) -> encode(?MODULE:put(Key, Value, decode(DataObject))).

%% @doc Remove a key from a data object

-spec remove(key(), data_object()) -> data_object().
remove(Key, DataObject) when is_list(DataObject) -> proplists:delete(Key, DataObject);
remove(Key, DataObject) when is_map(DataObject) -> maps:remove(Key, DataObject);
remove(Key, DataObject) -> encode(remove(Key, decode(DataObject))).

%% @doc Size of a data object, ignoring null values

-spec size(data_object()) -> integer().
size(DataObject) when is_list(DataObject) -> length(clean_null_values(DataObject));
size(DataObject) when is_map(DataObject) -> maps:size(clean_null_values(DataObject));
size(DataObject) -> ?MODULE:size(decode(DataObject)).

%% @doc Returns a complete list of values, in arbitrary order, from data object

-spec values(data_object()) -> list().
values(DataObject) when is_list(DataObject) -> [V || {_, V} <- DataObject];
values(DataObject) when is_map(DataObject) -> maps:values(DataObject);
values(DataObject) -> values(decode(DataObject)).

%% @doc Convert any format of DataObject to proplist

-spec to_proplist(data_object()) -> list().
to_proplist(DataObject) when is_list(DataObject) -> DataObject;
to_proplist(DataObject) when is_map(DataObject) -> maps:to_list(DataObject);
to_proplist(DataObject) -> to_proplist(decode(DataObject)).
...
-spec eval(JPPTree :: list() | binary() | tuple(), [{binary(), data_object()}]) ->
    % See below for spec of Details
    % Either parsetree/bind list is malformed
    %  or given jppath can not be folded
    % Happy path return
    % spec for Details in {error, {nomatch, Details}}
    % this is only for debugging fold function,
    % which shouldn't happen otherwise
    % While walking sub-objects
    % While walking sub-lists
    {nomatch, NoMatches}
    | {error, Details}
    | list(value()) when Details :: {operation_not_supported, any()}
    | {unimplemented, any()}
    | {unbound, any()}
    | {malformed, any()} , NoMatches :: {path_not_found, any(), any()}
    | {property_not_found, any(), any()}
    | {index_out_of_bounds, any(), any()}.
eval(JsonPath, Binds) when is_binary(JsonPath) orelse is_list(JsonPath) ->
    {ok, Tree} = jpparse:parsetree(JsonPath),
    eval(Tree, Binds);
eval(Tree, [{Name, Object} | _] = Binds) when is_binary(Name), is_binary(Object) ->
    case eval(Tree, [{N, decode(O)} || {N, O} <- Binds]) of
        {nomatch, Reason} -> {nomatch, Reason};
        {error, Reason} -> {error, Reason};
        MatchedObject -> encode(MatchedObject)
    end;
eval(Tree, Binds) ->
    case jpparse_fold:foldbu(fun jpp_walk/3, Binds, Tree) of
...
-define(TRACE, io:format(user, "[~p] ~p~n PT   ~p~n BIND ~p~n", [?LINE, _Depth, Pt, Binds])).
-endif.
    ...
    case proplists:get_value(Arg, Binds) of
        ...
        Obj ->
            case Op of
                ...
                Op -> exit({operation_not_supported, list_to_binary(["#", Op])})
            end
    end;
...
jpp_walk(_Depth, {'[]', L, Idxs} = Pt, Binds) ->
    ...
    case proplists:get_value(L, Binds) of
        ...
        List ->
            ...
            NewBinds = jpp_walk_list(_Depth, List, lists:reverse(Idxs), Pt, Binds),
            % TODO: To be rewritten for maps
            case {NewBinds, Idxs, List} of
                {Binds, [], []} -> [{Pt, []} | Binds];
                {Binds, [], [[_ | _] | _]} -> [{Pt, []} | Binds];
                {Binds, [], _} -> exit({nomatch, {path_not_found, Pt, List}});
...
jpp_walk(_Depth, {'{}', L, Props} = Pt, Binds) ->
    ...
    case proplists:get_value(L, Binds) of
        ...
        Obj ->
            ...
            % to append to head of building list
            NewBinds = jpp_walk_sub_obj(_Depth, Obj, lists:reverse(Props), Pt, Binds),
            % TODO: To be rewritten for maps
            case {NewBinds, Props, Obj} of
                {Binds, [], [{_, _} | _]} -> [{Pt, [{}]} | Binds];
                {Binds, [], _} -> exit({nomatch, {path_not_found, Pt, Obj}});
                {NewBinds, _, _} -> NewBinds
            end
    end;
jpp_walk(_Depth, {':', R, L} = Pt, Binds) when is_binary(R) ->
    ?TRACE,
    case proplists:get_value(L, Binds) of
        undefined -> exit({unbound, L});
        Obj ->
            V = ?MODULE:get(R, Obj),
            if
                V =:= undefined -> exit({nomatch, {property_not_found, R, Obj}});
                true -> [{Pt, V} | Binds]
            end
    end;
jpp_walk(_Depth, Pt, Binds) when is_binary(Pt); is_integer(Pt) ->
    ?TRACE,
    Binds.
...
jpp_walk_list(_Depth, List, Idxs, Pt, Binds) ->
    ...
    lists:foldl(
        fun
            (I, _) when is_integer(I) andalso ((I =< 0) orelse (I > length(List))) ->
                exit({nomatch, {index_out_of_bound, I, List}});
            (I, BindAcc) when is_integer(I) ->
                case BindAcc of
                    [{Pt, Lst} | RestBindAcc] -> [{Pt, [lists:nth(I, List) | Lst]} | RestBindAcc];
            ...
            (I, BindAcc) ->
                case eval(I, BindAcc) of
                    {M, BindAcc1} when is_integer(M) andalso (M > 0) andalso (M =< length(List)) ->
                        case BindAcc1 of
                            [{I, Lst} | RestBindAcc1] -> [{I, [lists:nth(M, List) | Lst]} | RestBindAcc1];
                            _ -> [{I, [lists:nth(M, List)]} | BindAcc1]
                        end;
                    _ -> exit({nomatch, {path_not_found, I, BindAcc}})
                end
        end,
        Binds,
        Idxs
        ...
        fun
            (P, BindAcc) when is_binary(P) ->
                case ?MODULE:get(P, Obj) of
                    undefined -> exit({nomatch, {property_not_found, P, Obj}});
                    V ->
                        case BindAcc of
                            [{Pt, OldBind} | RestBindAcc] -> [{Pt, [{P, V} | OldBind]} | RestBindAcc];
                            _ -> [{Pt, [{P, V}]} | BindAcc]
                        end
                end;
                ...
                case eval(P, BindAcc) of
                    {M, BindAcc1} when is_binary(M) ->
                        case ?MODULE:get(M, Obj) of
                            undefined -> exit({nomatch, {property_not_found, M, Obj}});
                            V ->
                                case BindAcc1 of
                                    [{P, OldBind1} | RestBindAcc1] -> [{P, [{M, V} | OldBind1]} | RestBindAcc1];
                                    _ -> [{P, [{M, V}]} | BindAcc1]
                                end
                        end;
                    _ -> exit({nomatch, {path_not_found, P, BindAcc}})
                end
        end,
        Binds,
        Props
...
clean_null_values([{_, _} | _] = DataObject) -> [{K, V} || {K, V} <- DataObject, V =/= null];
clean_null_values(DataObject) when is_list(DataObject) -> DataObject;
clean_null_values(DataObject) when is_map(DataObject) ->
    maps:fold(fun (_, null, OutMap) -> OutMap; (K, V, OutMap) -> maps:put(K, V, OutMap) end, #{}, DataObject);
clean_null_values(DataObject) -> encode(clean_null_values(decode(DataObject))).
...
-spec expand_inline(Root :: data_object(), Binds :: [{any(), data_object()}]) -> data_object().
expand_inline(Root, Binds) when is_binary(Root) -> encode(expand_inline(decode(Root), Binds));
            ...
            fun
                (K, V, AccIn) ->
                    case re:run(K, "^inline_*") of
                        nomatch -> AccIn;
                        _ ->
                            case proplists:get_value(V, Binds, '$not_found') of
...
diff(Data1, Data2) when is_list(Data1), is_list(Data2) -> diff(lists:sort(Data1), lists:sort(Data2), []);
diff(Data1, undefined) when is_list(Data1) -> diff(lists:sort(Data1), []);
diff(Data1, Data2) when is_map(Data1), is_map(Data2) -> maps:from_list(diff(maps:to_list(Data1), maps:to_list(Data2)));
diff(Data1, undefined) when is_map(Data1) -> maps:from_list(diff(maps:to_list(Data1), []));
diff(Data1, Data2) when is_map(Data1), is_list(Data2) -> maps:from_list(diff(maps:to_list(Data1), Data2));
diff(Data1, Data2) when is_map(Data1), is_binary(Data2) -> maps:from_list(diff(maps:to_list(Data1), decode(Data2)));
diff(Data1, Data2) when is_list(Data1), is_map(Data2) -> diff(Data1, maps:to_list(Data2));
diff(Data1, Data2) when is_list(Data1), is_binary(Data2) -> diff(Data1, decode(Data2));
diff(Data1, Data2) when is_binary(Data1), is_binary(Data2) -> encode(diff(decode(Data1), decode(Data2)));
diff(Data1, undefined) when is_binary(Data1) -> encode(diff(decode(Data1), []));
diff(Data1, Data2) when is_binary(Data1), is_list(Data2) -> encode(diff(decode(Data1), Data2));
diff(Data1, Data2) when is_binary(Data1), is_map(Data2) -> encode(diff(decode(Data1), maps:to_list(Data2)));
diff(undefined, Data2) when is_list(Data2) -> diff([], lists:sort(Data2), []);
diff(undefined, Data2) when is_map(Data2) -> maps:from_list(diff([], maps:to_list(Data2)));
diff(undefined, Data2) when is_binary(Data2) -> encode(diff([], decode(Data2)));
...
diff([{A, null} | R1], [{B, BV} | R2], Acc) when A < B ->
    % missing equals null
    diff(R1, [{B, BV} | R2], Acc);
diff([{A, AV} | R1], [{B, BV} | R2], Acc) when A < B ->
    diff(R1, [{B, BV} | R2], [{A, [{<<"v1">>, AV}, {<<"v2">>, null}]} | Acc]);
diff([{A, AV} | R1], [{B, null} | R2], Acc) when A > B ->
    % missing equals null
    diff([{A, AV} | R1], R2, Acc);
diff([{A, AV} | R1], [{B, BV} | R2], Acc) when A > B ->
    diff([{A, AV} | R1], R2, [{B, [{<<"v1">>, null}, {<<"v2">>, BV}]} | Acc]).
...
-define(
    TEST_JSON,
    <<"{\"surname\":\"Doe\",\"name\":\"John\",\"foo\":\"bar\"," "\"earthling\":true,\"age\":981,\"empty\":null}">>
).
-define(
    TEST_PROP,
    [
...
-define(
    TEST_LIST_PROP,
    [[{<<"surname">>, <<"Doe">>}, {<<"name">>, <<"John">>}], [{<<"surname">>, <<"Jabe">>}, {<<"name">>, <<"John">>}]]
).
...
find_in_list_test_() ->
        ...
        ],
    {
        ...
        [
            ...
            } || {I, {K, L, D, R}} <- lists:zip(lists:seq(1, length(Tests)), Tests)
        ]
    }.
...
expand_inline_test_() ->
    {
        inparallel,
        [
            {Title, ?_assertEqual(Expected, expand_inline(Root, Binds))} || {Title, Expected, Root, Binds} <- [
                % Mixed format tests
                {
                    "map_mixed_bind",
                    #{<<"a">> => 1, <<"b">> => 2, <<"c">> => 3, <<"d">> => 4},
                    #{<<"a">> => 1, <<"inline_bin">> => [1], <<"inline_pl">> => [2], <<"inline_map">> => [3]},
                    [{[1], <<"{\"b\":2}">>}, {[2], [{<<"c">>, 3}]}, {[3], #{<<"d">> => 4}}]
                },
                {
                    "pl_mixed_bind",
...
find_test_() ->
        ...
        ],
    {
        ...
        [
            ...
            } || {I, {B, K, V}} <- lists:zip(lists:seq(1, length(Tests)), Tests)
        ]
    }.
...
get_test_() ->
    {
        ...
        [
            ...
            {"list_prop_list", ?_assertEqual([<<"Doe">>, <<"Jabe">>], ?MODULE:get(<<"surname">>, ?TEST_LIST_PROP))}
        ]
    }.

keys_test_() ->
    % lists:sort
    Keys = [<<"surname">>, <<"name">>, <<"foo">>, <<"earthling">>, <<"age">>, <<"empty">>],
    {
        inparallel,
        [
...
eval_test_() ->
    ...
    Bind = [{<<"root">>, Object}],
    {
        inparallel,
        [
            {P, ?_assertEqual(O, err_to_atom(eval(P, Bind)))} || {P, O} <- [
                % basic hirarchical walk
                {"root{}", <<"{}">>},
                {"root[]", path_not_found},
                {"root#keys", <<"[\"a1\",\"a2\",\"a3\",\"a4\"," "\"a5\",\"a6\"]">>},
                {"root{a6}#keys", <<"[\"a6\"]">>},
