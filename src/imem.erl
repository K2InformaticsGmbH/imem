...
-safe([get_os_memory/0, get_vm_memory/0, get_swap_space/0, get_virtual_memory/0]).
...
start() -> {ok, _} = application:ensure_all_started(?MODULE).

stop() -> ok = application:stop(?MODULE).
...
start(_Type, StartArgs) ->
    ...
    RunningMnesia = lists:member(mnesia, [A || {A, _} <- proplists:get_value(running, AppInfo)]),
    if
        RunningMnesia ->
            ?Error("Mnesia already started"),
            {error, mnesia_already_started};
        true ->
            LoadedMnesia = lists:member(mnesia, [A || {A, _, _} <- proplists:get_value(loaded, AppInfo)]),
            if
                LoadedMnesia -> ok;
                true -> application:load(mnesia)
            end,
            config_start_mnesia(),
...
wait_remote_imem() ->
    ...
    [
        % failures (pang) can be ignored
        % removing self
        net_adm:ping(N1) || N1 <- lists:usort(
            % removing duplicates
            lists:flatten([rpc:call(N, erlang, nodes, []) || N <- AllNodes])
        ) -- [node()]
    ],
    % Building lists of nodes already running IMEM
    RunningImemNodes =
        [N || N <- AllNodes, true == lists:keymember(imem, 1, rpc:call(N, application, which_applications, []))],
    ...
    LoadedButNotRunningImemNodes =
        [N || N <- AllNodes, true == lists:keymember(imem, 1, rpc:call(N, application, loaded_applications, []))]
        --
        RunningImemNodes,
    ?Info("Nodes ~p loaded but not running IMEM", [LoadedButNotRunningImemNodes]),
            ...
            fun
                (Node, Nodes) ->
                    case rpc:call(Node, application, get_env, [?MODULE, start_time]) of
                        {ok, {StartTime, _}} when StartTime < SelfStartTime -> [Node | Nodes];
        ...
        RemoteStartedApplications ->
            case lists:keymember(imem, 1, RemoteStartedApplications) of
                false ->
                    ...
                    ?Info("Node ~p starting IMEM, waiting for it to finish starting", [Node]),
...
config_start_mnesia() ->
    ...
    SchemaDir =
        case ((length(RootParts) > 0) andalso filelib:is_dir(filename:join(RootParts))) of
...
set_start_time(Node) ->
    ...
    case rpc:call(Node, application, get_env, [imem, start_time]) of
        ...
        undefined ->
            ok = application:set_env(?MODULE, start_time, {rpc:call(Node, imem_if_mnesia, timestamp, []), Node})
    end.

-spec get_virtual_memory() -> map().
get_virtual_memory() -> get_virtual_memory(os:type()).

-spec get_virtual_memory(tuple()) -> map().
get_virtual_memory({win32, _}) ->
    case re:split(os:cmd("wmic OS get FreeVirtualMemory, TotalVirtualMemorySize"), "\r\r\n", [{return, list}]) of
        [_, Mems | _] ->
            [FreeVMStr, TotalVMStr] = string:tokens(Mems, " "),
            #{free_virtual_memory => list_to_integer(FreeVMStr), total_virtual_memory => list_to_integer(TotalVMStr)};
        _ -> #{free_virtual_memory => 0, total_virtual_memory => 0}
    end;
get_virtual_memory(_) -> #{free_virtual_memory => 0, total_virtual_memory => 0}.
...
get_os_memory() ->
    ...
    FreeMem =
        lists:foldl(
            fun
                ({T, M}, A) when T =:= free_memory; T =:= buffered_memory; T =:= cached_memory -> A + M;
                (_, A) -> A
            end,
            0,
            SysData
...
get_vm_memory({unix, _} = Unix) ->
    {
        ...
        erlang:round(
            ...
            *
            list_to_float(
                re:replace(os:cmd("ps -p " ++ os:getpid() ++ " -o pmem="), "[[:space:]]*", "", [global, {return, list}])
            )
            /
            100
        )
...
get_swap_space() ->
    case maps:from_list(memsup:get_system_memory_data()) of
        #{free_swap := FreeSwap, total_swap := TotalSwap} -> {TotalSwap, FreeSwap};
        _ -> {0, 0}
    end.
...
spawn_sync_mfa(M, F, A) ->
    ...
    receive Result -> Result after 60000 -> {error, timeout} end.
...
all_apps_version_info(Opts) when is_list(Opts) -> all_apps_version_info(maps:from_list(Opts));
all_apps_version_info(Opts) ->
        ...
        [
            {
                Mod,
                if
                    is_atom(ModPath) -> atom_to_list(ModPath);
                    true -> ModPath
                end
    ...
    AllApps =
        [
            {App, list_to_binary(AppVsn), element(2, application:get_key(App, modules))}
        ...
        case Opts of
            #{apps := FilterApps} ->
                FilterAppsBin =
                    [
                        if
                            is_atom(A) -> atom_to_binary(A, utf8);
                            true -> A
                        end || A <- FilterApps
                    ],
                FilteredAllApps =
                    [App || {A, _, _} = App <- AllApps, lists:member(atom_to_binary(A, utf8), FilterAppsBin)],
                FilteredAllModules = lists:flatten([Modules || {_, _, Modules} <- FilteredAllApps]),
                FilteredAllModulesDetails =
                    lists:filter(fun ({Mod, _}) -> lists:member(Mod, FilteredAllModules) end, AllModulesDetails),
                {FilteredAllApps, FilteredAllModulesDetails};
            _ -> {AllApps, AllModulesDetails}
        end,
    Blacklist =
        lists:filtermap(
            fun
                (BL) when is_binary(BL) -> {true, string:trim(binary_to_list(BL))};
                (BL) when is_list(BL) -> {true, string:trim(BL)};
                (BL) when is_atom(BL) -> {true, string:trim(atom_to_list(BL))};
                (_) -> false
            end,
            ?GET_CONFIG(
                itemBlacklist,
                [],
                [node_modules, dist, swagger, dev],
                "Leaf files/folders to exclude from hash processing for ddVersion"
            )
        ),
    all_apps_version_info(merge(FAllApps, FAllModulesDetails), Opts#{blacklist => Blacklist}).

merge(AllApps, AllModulesDetails) -> merge(AllApps, AllModulesDetails, _Acc = []).

merge([], [], Acc) -> lists:usort(Acc);
merge([], ModulesDetails, Acc) -> merge([], [], [{undefined, undefined, ModulesDetails} | Acc]);
merge([{App, AppVsn, Modules} | AllApps], ModulesDetails, Acc) ->
    {ProcessModuleDetails, RestModulesDetails} =
        lists:partition(fun ({Mod, _}) -> lists:member(Mod, Modules) end, ModulesDetails),
    merge(AllApps, RestModulesDetails, [{App, AppVsn, ProcessModuleDetails} | Acc]).

all_apps_version_info(AllApps, Opts) ->
    all_apps_version_info(AllApps, Opts#{git => os:find_executable("git")}, _ProcessPriv = true, _Acc = []).
...
all_apps_version_info(
    ...
    #{blacklist := Blacklist} = Opts,
    ProcessPriv,
    Acc
) ->
    case lists:member(filename:basename(ModPath), Blacklist) of
        true -> all_apps_version_info([{App, AppVsn, Rest} | AllApps], Opts, _ProcessPriv = false, Acc);
        false ->
            ...
            DDRec =
                #ddVersion{
                    app =
                        if
                            App /= undefined -> atom_to_binary(App, utf8);
                            true -> App
                        end,
                    appVsn = AppVsn,
                    ...
                    NewAcc =
                        case filelib:is_dir(PrivDir) of
                            true ->
                                ...
                                ?Debug("~p files @ ~s of ~p-~s~n", [length(PrivFiles), PathRoot, App, AppVsn]),
                                walk_priv(Opts, DDRec1#ddVersion.app, DDRec1#ddVersion.appVsn, PrivDir, PrivFiles, Acc1);
                            _ -> Acc1
                        end,
                    all_apps_version_info(
                        [{App, AppVsn, Rest} | AllApps],
                        Opts2,
                        _ProcessPriv = false,
                        [DDRec1 | NewAcc]
                    );
...
process_app(#ddVersion{app = App} = DDRec, Opts, Module) ->
    case filename:dirname(proplists:get_value(source, Module:module_info(compile))) of
        Dir when is_list(Dir) ->
            ...
            case file_phash2(FilePath) of
                ...
                FileHash ->
                    ...
                    [DDRec#ddVersion{file = File, fileVsn = FileHash, filePath = FilePath, fileOrigin = FileOrigin}]
            end;
        _ -> []
    end.
...
git_info(#{git := GitExe} = Opts, Path) when is_list(Path); is_binary(Path) ->
    ...
    case os_cmd(GitExe, Dir, ["rev-parse", "HEAD"]) of
        <<"fatal:", _>> -> {<<>>, <<>>};
        Revision ->
            case re:run(os_cmd(GitExe, Dir, ["remote", "-v"]), "(http[^ ]+)", [{capture, [1], list}]) of
                {match, [Url | _]} -> git_info(Opts, {Url, Revision});
                _ -> {<<>>, <<>>}
            end
    end;
git_info(_Opts, {Url, Revision}) ->
    CleanUrl = re:replace(Url, "\\.git", "", [{return, list}]),
    {list_to_binary([CleanUrl, "/raw/", string:trim(Revision)]), lists:last(filename:split(CleanUrl))}.

git_file(#{gitRepo := Repo} = Opts, Path) when is_list(Repo) -> git_file(Opts#{gitRepo := list_to_binary(Repo)}, Path);
git_file(#{git := GitExe, gitRepo := Repo} = Opts, Path) when GitExe /= false ->
    case re:run(Path, <<Repo/binary, "(.*)">>, [{capture, [1], list}]) of
        {match, [RelativePath]} -> git_file_low(Opts#{absPath => Path}, RelativePath);
        _ -> <<>>
    end;
...
git_file_low(#{git := GitExe, gitRoot := GitRoot, absPath := Path} = Opts, BaseName, RelativePath) ->
    case
    {
        os_cmd(GitExe, filename:dirname(Path), ["ls-files", "--error-unmatch", BaseName]),
        filename:extension(RelativePath)
    } of
        {<<"error: pathspec", _/binary>>, ".erl"} ->
            git_file_low(Opts, Path, re:replace(RelativePath, "\\.erl", "\\.yrl", [{return, list}]));
        {<<"error: pathspec", _/binary>>, ".yrl"} ->
            git_file_low(Opts, re:replace(RelativePath, "\\.yrl", "\\.xrl", [{return, list}]));
        {<<"error: pathspec", _/binary>>, _} ->
            #{gitRepo := Repo} = Opts,
            case re:run(Path, <<"lib/", Repo/binary, "(.*)">>, [{capture, [1], list}]) of
                nomatch -> notfound;
                {match, [RelativePath1]} -> list_to_binary([GitRoot, RelativePath1])
            end;
        _ -> list_to_binary([GitRoot, RelativePath])
    end.

walk_priv(_, _, _, _, [], Acc) -> Acc;
walk_priv(#{blacklist := Blacklist} = Opts, App, AppVsn, Path, [FileOrFolder | Rest], Acc) ->
        ...
        false ->
            ...
            case filelib:is_dir(FullPath) of
                true ->
                    ...
                    {match, [PathRoot]} = re:run(FullPath, <<"(", App/binary, ".*)">>, [{capture, [1], list}]),
                    ?Debug("~p files @ ~s of ~s-~s~n", [length(PrivFiles), PathRoot, App, AppVsn]),
                    walk_priv(Opts, App, AppVsn, Path, Rest, walk_priv(Opts, App, AppVsn, FullPath, PrivFiles, Acc));
                _ ->
                    walk_priv(
                        Opts,
                        App,
...
file_phash2(FilePath) ->
    case file:read_file(FilePath) of
        {ok, FBin} -> <<"ph2:", (integer_to_binary(erlang:phash2(FBin))) / binary>>;
        _ -> <<>>
    end.

os_cmd(Exe, Dir, Args) ->
    case filelib:is_dir(Dir) of
        true -> os_cmd(Exe, [{cd, Dir}, {args, Args}]);
        _ -> <<"fatal:badpath">>
    end.

os_cmd(Exe, Args) ->
    Port = open_port({spawn_executable, Exe}, [binary, exit_status, stderr_to_stdout | Args]),
    (
        fun
            Rcv(Buf) ->
                receive
