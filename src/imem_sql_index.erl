...
exec(SKey, {'drop index', {}, TableName} = _ParseTree, _Stmt, _Opts, IsSec) ->
    ...
    Table =
        if
            TableSchema =:= undefined -> {imem_meta:schema(), list_to_existing_atom(binary_to_list(Tbl))};
            true -> {list_to_existing_atom(binary_to_list(TableSchema)), list_to_existing_atom(binary_to_list(Tbl))}
        end,
...
exec(SKey, {'drop index', IndexName, TableName} = _ParseTree, _Stmt, _Opts, IsSec) ->
    ...
    case lists:filter(Pred, imem_meta:table_columns(Table)) of
        [] ->
            % Index = external index name
            if_call_mfa(IsSec, drop_index, [SKey, Table, Index]);
        [Field] ->
            % Field = internal index atom
            if_call_mfa(IsSec, drop_index, [SKey, Table, Field])
    end;
exec(SKey, {'create index', {}, {}, TableName, [], {}, {}} = _ParseTree, _Stmt, _Opts, IsSec) ->
    ...
    ExistingIndexDefs =
        case imem_meta:read(ddTable, {MySchema, Table}) of
            [#ddTable{} = D] ->
...
exec(
    SKey,
    {'create index', IndexType, IndexName, TableName, IndexDefn, NormWithFun, FilterWithFun} = _ParseTree,
    _Stmt,
    _Opts,
    IsSec
) ->
    ...
    NewIdx =
        #ddIdxDef{
            id = MaxIdx + 1,
            name = Index,
            type = imem_index:index_type(IndexType),
            pl = IndexDefn,
            vnf = Vnf,
            iff = Iff
        },
