...
init() ->
    imem_meta:init_create_check_table(
        ...
        [{trigger, <<"fun(__OR,__NR,__T,__U,__O) ->" " imem_tracer:trigger(__OR,__NR,__T,__U,__O) " "end.">>}],
        system
    ).
...
trigger(OldRec, NewRec, ddTrace, User, _TrOpts) ->
    case {OldRec, NewRec} of
        {{}, {}} ->
            ...
            dbg:stop_clear();
        {{}, #ddTrace{enable = Enabled, mod = M, func = F, args = A, match_spec = MS} = Trace} ->
        ...
        {#ddTrace{enable = true, mod = M, func = F, args = A}, #ddTrace{enable = false}} ->
            ...
            end;
        {
            ...
            #ddTrace{enable = true, mod = M1, func = F1, args = A1, match_spec = MS} = Trace
        } ->
            case is_trace_running() of
                true ->
                    check_start_trace(User, Trace),
...
tracer(Trace, #tracer_cb_st{last_s = LastSec, count = Count} = St) ->
    ...
    if
        NewCount > ?MAX_MSG_PER_SEC ->
            ...
            % dropped
            St;
        true -> send_trace_event(Trace, St#tracer_cb_st{count = NewCount, last_s = Now})
    end.

handler_fun() -> {ok, fun tracer/2}.
    ...
    NewState =
        receive
            {tracer_pid, TracerPid} ->
                ?Info("imem_tracer ~p, dbg:tracer ~p, imem_statement ~p~n", [self(), TracerPid, ReplyPid]),
                true = erlang:link(TracerPid),
                State;
            {trace_event, Trace} -> process_trace(Trace, State);
            {From, {add, M, F, A, Rate}} ->
                From ! ok,
                State#state{
                    filters = (State#state.filters) #{{M, F, A} => #{rate => Rate, stat => #{}, overflow => false}}
                };
            {From, {del, M, F, A}} ->
                From ! ok,
                State#state{filters = maps:remove({M, F, A}, State#state.filters)};
            {_, _, process, _, _} = Mon -> ?SystemExceptionNoLogging({process_died, Mon});
            Other ->
...
% internal functions
%------------------------------------------------------------------------------
check_start_trace(_User, #ddTrace{trace_node = Node, enable = true}) when Node /= node() -> check_start_tracer(Node);
check_start_trace(User, #ddTrace{trace_node = Node, enable = true, mod = M, func = F, args = A}) when Node == node() ->
    case
    imem_meta:select_count(
        ddTrace,
        [{#ddTrace{event_type = register, enable = true, trace_node = '$1', _ = '_'}, [{'/=', {node}, '$1'}], [true]}]
    )
    >
    0 of
        true -> ?ClientErrorNoLogging("one or more remote traces are active");
        _ -> check_start_tracer(Node)
    end,
    LocalSafe = trace_whitelist(User, []),
    case LocalSafe -- lists:usort([M, {M, F}, {M, '_'}, {M, F, A}, {M, '_', '_'}]) of
        LocalSafe -> ?ClientErrorNoLogging("mod / func / args unsafe for local node tracing" " (not whitelisted)");
        _ -> true
    end;
            ...
            case whereis(?MODULE) of
                Pid when is_pid(Pid) ->
                    case is_process_alive(Pid) of
                        ...
                        false -> ?ClientErrorNoLogging("tracer process not alive")
                    end;
...
check_start_tracer(Node, Pid) ->
    ...
    case dbg:tracer(Node, process, {F, #tracer_cb_st{pid = Pid}}) of
        {ok, Node} ->
            ...
            Pid ! {tracer_pid, TPid};
        {error, Error} -> ?ClientErrorNoLogging({"on trace activate", Node, Error})
    end.

process_trace({trace, From, Type, MFA}, State) -> process_trace({trace, From, Type, MFA, <<>>}, State);
process_trace({trace, FromTo, Type, {M, F, ArityArgs}, Extra}, State) ->
    case apply_filter(M, F, ArityArgs, State) of
        {ok, NewState} ->
            ...
            State#state.reply ! {mnesia_table_event, {write, ddTrace, Row, [], undefined}},
            NewState;
        {overflow, NewState} ->
            Row =
                #ddTrace{
...
send_trace_event(Trace, #tracer_cb_st{pid = Pid} = State) ->
    ...
    if
        length(Messages) < ?MAX_TRACER_PROC_MQ_LEN ->
        ...
        length(Messages) < ?MAX_TRACER_PROC_MQ_LEN ->
            case catch ?MODULE ! {self(), Message} of
                {'EXIT', Error} ->
                    ...
                    dbg:stop_clear();
                _ -> receive Reply -> Reply after 1000 -> ?SystemExceptionNoLogging("tracer reply timeout") end
            end;
...
ddTraceDef() ->
    (#ddTrace{})
    #ddTrace{trace_key = <<"fun(First, R) ->" " imem_tracer:trace_key_default(First, R) " "end.">>, trace_node = node()}.

trace_key_default(_, #ddTrace{trace_key = TK}) when is_integer(TK) -> TK;
trace_key_default(_, Rec) ->
    PosList = [#ddTrace.event_type, #ddTrace.mod, #ddTrace.func, #ddTrace.args, #ddTrace.trace_node],
...
apply_filter(Mod, Fun, Args, State) when is_list(Args) -> apply_filter(Mod, Fun, length(Args), State);
            ...
            case (Last < Now) of
                true ->
                    % New Second, reset everything
                    {
                        ...
                        State#state{
                            filters =
                                Filters#{Key => #{rate => Rate, stat => #{last => Now, count => 1}, overflow => false}}
                        }
                    };
...
disable_tps() ->
    {Rows, _} = imem_meta:select(ddTrace, [{#ddTrace{event_type = register, enable = true, _ = '_'}, [], ['$_']}]),
    lists:foreach(
        fun
            (Row) ->
...
trace_whitelist(U, {M, F} = W) when is_atom(M), is_atom(F) -> trace_whitelist(U, [W]);
trace_whitelist(U, {M, F, A} = W) when is_atom(M) andalso is_atom(F) andalso (is_integer(A) orelse A == '_') ->
    trace_whitelist(U, [W]);
trace_whitelist(U, W) when is_list(W) ->
    lists:foreach(
        fun
            (Wi) when is_atom(Wi) -> ok;
            ({M, F}) when is_atom(M), is_atom(F) -> ok;
            ({M, F, A}) when is_atom(M) andalso is_atom(F) andalso (is_integer(A) orelse A == '_') -> ok;
        ...
        Wl when is_list(Wl) ->
            ...
            case lists:usort(Wlus ++ W) of
                Wlus ->
                    % already added
                    Wlus;
                NewWlus -> ?GET_CONFIG(trace_local_wl, [U], NewWlus, "list of MFAs safe to trace on local node")
            end;
        _ -> ?GET_CONFIG(trace_local_wl, [U], lists:usort(W), "list of MFAs safe to trace on local node")
    end.
...
timestamp({win32, nt}) ->
    ...
    imem_win32:queryPerformanceCounter() * 1000000 div imem_win32:queryPerformanceFrequency();
timestamp(_) -> erlang:system_time(microsecond).
...
F =
    fun
        (T, _) -> io:format("~p -- ~p~n", [T, catch imem_tracer:tracer_dummy(T, ok)])
