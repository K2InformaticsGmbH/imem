...
-export(
    [
        ...
        update_prepare/5,
        %% stateful creation of update plan (stored in state)
        update_cursor_prepare/4,
        %% stateful execution of update plan (fetch aborted first)
        update_cursor_execute/4,
        %% apply FilterSpec and SortSpec and return new SQL and SortFun
        filter_and_sort/5,
        %% apply FilterSpec and SortSpec and return new SQL and SortFun
        filter_and_sort/6,
        %% simulation of synchronous fetch
        fetch_recs/5,
        %% simulation of synchronous fetch followed by a lists:sort
        fetch_recs_sort/5,
        %% async streaming fetch
        fetch_recs_async/4,
        %% async streaming fetch with options ({tail_mode,)
        fetch_recs_async/5,
        fetch_close/3,
        close/2
    ]
).
-export([create_stmt/3, receive_raw/0, receive_raw/1, receive_recs/2, receive_recs/3, recs_sort/2, result_tuples/2]).

% Functions applied with Common Test
-export(
    [
        if_call_mfa/3,
        %% table name from physical_name={Schema,Name} or cluster_name={Node,Schema,Name}
        table_name/1
    ]
).

-record(
    %% state for fetch process
    fetchCtx,
    {
        pid :: pid(),
        %% fetch monitor ref
        monref :: any(),
        %% undefined | waiting | fetching | done | tailing | aborted
        status :: atom(),
        metarec :: tuple(),
        %% could be adaptive
        blockSize = 100 :: integer(),
        %% next rownum to be used, if any, element(1,MetaRec)
        rownum :: undefined | integer,
        %% rows remaining to be fetched. initialized to Limit and decremented
        remaining = 0 :: integer(),
        %% fetch options like {tail_mode,true}
        opts = [] :: list(),
        %% compiled matchspec for master table condition (bound with MetaRec)
        tailSpec :: any(),
        %% filter specification {Guard,Binds}
        filter :: any(),
        recName :: atom(),
        lastmeta :: tuple()
    }
...
-record(
    %% state for statment process, including fetch subprocess
    state,
    {
        statement,
        %% parent monitor reference (DOWN -> kill statement)
        parmonref = undefined :: reference(),
        isSec = false :: boolean(),
        seco = none :: integer(),
        fetchCtx = #fetchCtx{},
        %% reply destination TCP socket or Pid
        reply,
        %% bulk execution plan (table updates/inserts/deletes)
        updPlan = [] :: list()
    }
).
...
create_stmt(Statement, SKey, IsSec) ->
    ...
    case {IsSec, node()} of
        ...
        {false, _} ->
            % meta, remote
            rpc:call(Node, gen_server, start, [?MODULE, [Statement, self()], [{spawn_opt, [{fullsweep_after, 0}]}]]);
        {true, Node} ->
            % sec, local
...
fetch_recs(SKey, Pid, Sock, Timeout, IsSec) -> fetch_recs(SKey, Pid, Sock, Timeout, [], IsSec).
...
fetch_recs_async(SKey, StmtResult, Sock, IsSec) -> fetch_recs_async(SKey, StmtResult, Sock, [], IsSec).
...
fetch_close(SKey, #stmtResults{stmtRefs = StmtRefs}, IsSec) -> fetch_close(SKey, hd(StmtRefs), IsSec);
fetch_close(SKey, StmtRefs, IsSec) when is_list(StmtRefs) -> fetch_close(SKey, hd(StmtRefs), IsSec);
fetch_close(SKey, Pid, IsSec) when is_pid(Pid) ->
    LocalNode = node(),
    case node(Pid) of
        LocalNode -> gen_server:call(Pid, {fetch_close, IsSec, SKey});
        RemoteNode -> rpc:call(RemoteNode, gen_server, call, [Pid, {fetch_close, IsSec, SKey}])
    end.
...
filter_and_sort(SKey, Pid, FilterSpec, SortSpec, Cols, IsSec) when is_pid(Pid) ->
    LocalNode = node(),
    case node(Pid) of
        LocalNode -> gen_server:call(Pid, {filter_and_sort, IsSec, FilterSpec, SortSpec, Cols, SKey});
        RemoteNode ->
...
update_cursor_prepare(SKey, Pid, IsSec, ChangeList) when is_pid(Pid) ->
    ...
    case node(Pid) of
        LocalNode ->
            case
            gen_server:call(Pid, {update_cursor_prepare, IsSec, SKey, ChangeList}, ?CALL_TIMEOUT(update_cursor_prepare)) of
                ok -> ok;
                Error -> throw(Error)
            end;
        RemoteNode ->
            ...
            rpc:call(
                ...
                gen_server,
                call,
                [Pid, {update_cursor_prepare, IsSec, SKey, ChangeList}, ?CALL_TIMEOUT(update_cursor_prepare)]
            ) of
                ok -> ok;
                Error -> throw(Error)
            end
...
update_cursor_execute(SKey, StmtRefs, IsSec, Lock) when is_list(StmtRefs) ->
    update_cursor_execute(SKey, hd(StmtRefs), IsSec, Lock);
update_cursor_execute(SKey, Pid, IsSec, Lock) when is_pid(Pid) -> update_cursor_exec(SKey, Pid, IsSec, Lock).
        ...
        case node(Pid) of
            ...
            RemoteNode ->
                rpc:call(
                    ...
                    gen_server,
                    call,
                    [Pid, {update_cursor_execute, IsSec, SKey, Lock}, ?CALL_TIMEOUT(update_cursor_execute)]
                )
        end,
    case Result of
...
close(SKey, #stmtResults{stmtRefs = StmtRefs}) -> close(SKey, hd(StmtRefs));
close(SKey, StmtRefs) when is_list(StmtRefs) -> close(SKey, hd(StmtRefs));
close(SKey, Pid) when is_pid(Pid) ->
    LocalNode = node(),
    case node(Pid) of
        LocalNode -> gen_server:cast(Pid, {close, SKey});
        RemoteNode -> rpc:call(RemoteNode, gen_server, cast, [Pid, {close, SKey}])
    end.
...
handle_call({set_seco, SKey}, _From, State) ->
    % store internal SKey in statement process, may be needed to authorize join functions
    ?IMEM_SKEY_PUT(SKey),
...
handle_call({update_cursor_prepare, IsSec, _SKey, ChangeList}, _From, #state{statement = Stmt, seco = SKey} = State) ->
    STT = ?TIMESTAMP,
    {Reply, UpdatePlan1} =
        try {ok, update_prepare(IsSec, SKey, Stmt#statement.tables, Stmt#statement.colMap, ChangeList)} catch
            _:Reason ->
                imem_meta:log_to_db(
                    error,
                    ?MODULE,
                    handle_call,
                    [{reason, Reason}, {changeList, ChangeList}],
                    "update_cursor_prepare error"
                ),
                {{error, Reason}, []} end,
    imem_meta:log_slow_process(
        ?MODULE,
        update_cursor_prepare,
        STT,
        100,
        4000,
        [{table, hd(Stmt#statement.tables)}, {rows, length(ChangeList)}]
    ),
    {reply, Reply, State#state{updPlan = UpdatePlan1}};
handle_call({update_cursor_execute, _IsSec, _SKey, _Lock}, _From, #state{updPlan = UpdatePlan} = State)
when UpdatePlan == [] ->
    % nothing to do, return empty keylist
    {reply, [], State};
handle_call(
    {update_cursor_execute, IsSec, _SKey, Lock},
    _From,
    #state{seco = SKey, fetchCtx = FetchCtx0, updPlan = UpdatePlan, statement = Stmt} = State
) ->
    ...
    Reply =
        try
            ...
            case length(Stmt#statement.tables) of
                ...
                _ ->
                    %% position of last tuple
                    LastIdx = ?TableIdx(length(Stmt#statement.tables)),
                    WrapOut =
                        fun
                            ({Tag, {}}) ->
            ...
            _:Reason ->
                imem_meta:log_to_db(
                    ...
                    ?MODULE,
                    handle_call,
                    [{reason, Reason}, {updPlan, UpdatePlan}],
                    "update_cursor_execute error"
                ),
    ...
    imem_meta:log_slow_process(
        ?MODULE,
        update_cursor_execute,
        STT,
        100,
        4000,
        [{table, hd(Stmt#statement.tables)}, {rows, length(UpdatePlan)}]
    ),
    {reply, Reply, State};
handle_call({filter_and_sort, _IsSec, FilterSpec, SortSpec, Cols0, _SKey}, _From, #state{statement = Stmt} = State) ->
    ...
    Reply =
        try
            ...
            % sql_box:flat_from_pt({select,NewSections2}),
            ...
            _:Reason ->
                imem_meta:log_to_db(
                    ...
                    ?MODULE,
                    handle_call,
                    [{reason, Reason}, {filter_spec, FilterSpec}, {sort_spec, SortSpec}, {cols, Cols0}],
                    "filter_and_sort error"
                ),
                {error, Reason}
        end,
    ...
    imem_meta:log_slow_process(
        ...
        100,
        4000,
        [{table, hd(Stmt#statement.tables)}, {filter_spec, FilterSpec}, {sort_spec, SortSpec}]
    ),
    {reply, Reply, State};
handle_call({fetch_close, _IsSec, _SKey}, _From, #state{statement = Stmt, fetchCtx = FetchCtx0} = State) ->
    ...
    case Status of
        undefined ->
            ...
            % normal close after completed fetch
            ok;
        waiting ->
            % client stops fetch
        ...
        fetching ->
            ...
            kill_fetch(MonitorRef, Pid);
        tailing ->
            % client stops tail mode
            unsubscribe(Stmt);
        aborted ->
            % client acknowledges abort
            ok
    end,
    FetchCtx1 = FetchCtx0#fetchCtx{pid = undefined, monref = undefined, metarec = undefined, status = undefined},
    {reply, ok, State#state{fetchCtx = FetchCtx1}}.

% client may restart the fetch now
handle_cast({fetch_recs_async, _IsSec, _SKey, Sock, _Opts}, #state{fetchCtx = #fetchCtx{status = done}} = State) ->
    ...
    imem_meta:log_to_db(
        ...
        ?MODULE,
        handle_cast,
        [{sock, Sock}, {opts, _Opts}, {status, done}],
        "fetch_recs_async rejected"
    ),
    send_reply_to_client(
        Sock,
        {error, {'ClientError', "Fetch is completed, execute fetch_close before fetching from start again"}}
    ),
    {noreply, State};
handle_cast({fetch_recs_async, _IsSec, _SKey, Sock, _Opts}, #state{fetchCtx = #fetchCtx{status = aborted}} = State) ->
    ...
    imem_meta:log_to_db(
        ...
        ?MODULE,
        handle_cast,
        [{sock, Sock}, {opts, _Opts}, {status, aborted}],
        "fetch_recs_async rejected"
    ),
    send_reply_to_client(
        Sock,
        {error, {'SystemException', "Fetch is aborted, execute fetch_close before fetching from start again"}}
    ),
    {noreply, State};
handle_cast({fetch_recs_async, _IsSec, _SKey, Sock, _Opts}, #state{fetchCtx = #fetchCtx{status = tailing}} = State) ->
    ...
    imem_meta:log_to_db(
        ...
        ?MODULE,
        handle_cast,
        [{sock, Sock}, {opts, _Opts}, {status, tailing}],
        "fetch_recs_async rejected"
    ),
    send_reply_to_client(
        Sock,
        {error, {'ClientError', "Fetching in tail mode, execute fetch_close before fetching from start again"}}
    ),
    {noreply, State};
handle_cast(
    {fetch_recs_async, IsSec, _SKey, Sock, Opts},
    #state{statement = Stmt, seco = SKey, fetchCtx = FetchCtx0} = State
) ->
    ...
    case {lists:member({fetch_mode, skip}, Opts), FetchCtx0#fetchCtx.pid} of
        {Skip, undefined} ->
            %% {SkipFetch, Pid} = {true|false, uninitialized} -> skip fetch
            Params1 =
                case lists:keyfind(params, 1, Opts) of
                    false ->
                        %% from statement exec, only used on first fetch
                        Params0;
                    {_, P} ->
                        %% from fetch_recs, only used on first fetch
                        P
                end,
            RecName =
                try
                    imem_meta:table_record_name(Table)
                catch
                    {'ClientError', {"Table does not exist", _}} -> undefined
                end,
            RowNum = 1,
            ...
            case Skip of
                true ->
                    ...
                    FetchSkip =
                        #fetchCtx{
                            ...
                            blockSize = BlockSize,
                            rownum = RowNum,
                            remaining = MainSpec#scanSpec.limit,
                            opts = Opts,
                            tailSpec = TailSpec,
                            filter = FilterFun,
                            recName = RecName
                ...
                false ->
                    try
                        {TailSpec, FilterFun, FetchStartResult} =
                            case imem_meta:is_virtual_table(Table) of
                                false ->
                                    ...
                                    %?Info("Filter Fun after meta bind:~n~p", [FF]),
                                    {
                                        TS,
                                        FF,
                                        if_call_mfa(IsSec, fetch_start, [SKey, self(), Table, SS, BlockSize, Opts])
                                    };
                                true ->
                                    ...
                                    Rows =
                                        [
                                            ...
                                            {I, K}
                                            <-
                                            generate_virtual_data(RecName, {MR}, SGuard, MainSpec#scanSpec.limit)
                                        ],
                                    % ?LogDebug("Generated virtual scan data ~p~n~p", [Table,Rows]),
                                    {
                                        ...
                                        if_call_mfa(
                                            IsSec,
                                            fetch_start_virtual,
                                            [SKey, self(), Table, Rows, BlockSize, RowNum, Opts]
                                        )
                                    }
                            end,
                            ...
                            TransPid when is_pid(TransPid) ->
                                ...
                                RecName =
                                    try
                                        ...
                                        {'ClientError', {"Table does not exist", _TableName}} -> undefined
                                    end,
                                FetchStart =
                                    #fetchCtx{
                                        pid = TransPid,
        ...
        {true, _Pid} ->
            %% skip ongoing fetch (and possibly go to tail_mode)
            ...
            % ?LogDebug("Filter Fun after meta bind:~n~p~n", [FilterFun]),
            FetchSkipRemaining =
                FetchCtx0#fetchCtx{metarec = MR, lastmeta = MR, opts = Opts, tailSpec = TailSpec, filter = FilterFun},
...
handle_info(
    {mnesia_table_event, {write, _Table, {schema, _TableName, _TableNewProperties}, _OldRecords, _ActivityId}},
    State
) ->
...
handle_info({mnesia_table_event, {delete, schema, _What, _SchemaInfo, _ActivityId}}, State) -> {noreply, State};
...
handle_info(
    {row, Rows0},
    #state{reply = Sock, isSec = IsSec, seco = SKey, fetchCtx = FetchCtx0, statement = Stmt} = State
) ->
    #fetchCtx{metarec = MR0, rownum = RowNum, remaining = Rem0, status = Status, filter = FilterFun, opts = Opts} =
        FetchCtx0,
    %?Info("received ~p rows ~n~p", [length(Rows0)-2,Rows0]),
    {Rows1, Complete} =
        case {Status, Rows0} of
            {waiting, [?sot, ?eot | R]} ->
    ...
    Result =
        case {length(Stmt#statement.tables), FilterFun, RowNum} of
            {_, false, _} -> [];
            {1, true, undefined} -> take_remaining(Rem0, lists:map(Wrap, Rows1));
            {1, _, undefined} -> take_remaining(Rem0, lists:filter(FilterFun, lists:map(Wrap, Rows1)));
            {_, true, undefined} -> join_rows(lists:map(Wrap, Rows1), FetchCtx0, Stmt);
            {_, _, undefined} -> join_rows(lists:filter(FilterFun, lists:map(Wrap, Rows1)), FetchCtx0, Stmt);
            {1, true, FirstRN} -> update_row_num(MR1, FirstRN, take_remaining(Rem0, lists:map(Wrap, Rows1)));
            {_, true, FirstRN} ->
        ...
        true ->
            ...
            case length(Result) >= Rem0 of
                ...
                false ->
                    ...
                    if
                        Complete -> handle_fetch_complete(State#state{fetchCtx = FetchCtx1});
                        PushMode ->
                            gen_server:cast(self(), {fetch_recs_async, IsSec, SKey, Sock, Opts}),
                            {noreply, State#state{fetchCtx = FetchCtx1}};
                        true -> {noreply, State#state{fetchCtx = FetchCtx1}}
                    end
            end;
        false ->
...
handle_info(
    {'DOWN', Ref, process, _Pid, {no_exists, {TableName, _}} = _Reason},
    #state{reply = Sock, fetchCtx = #fetchCtx{monref = Ref}} = State
) ->
            ...
            case catch if_call_mfa(false, subscribe, [none, {table, Table, detailed}]) of
                ...
                Error ->
                    ...
                    imem_meta:log_to_db(
                        error,
                        ?MODULE,
                        handle_fetch_complete,
                        [{table, Table}, {error, Error}, {sock, Sock}],
                        "Cannot subscribe to table changes"
                    ),
                    send_reply_to_client(
                        Sock,
                        {error, {'SystemException', {"Cannot subscribe to table changes", {Table, Error}}}}
                    ),
                    {noreply, State#state{fetchCtx = FetchCtx0#fetchCtx{status = done}}}
            end
    end.
    ...
    case catch if_call_mfa(false, unsubscribe, [none, {table, Table, detailed}]) of
        ...
        Error ->
            ...
            imem_meta:log_to_db(
                ...
                "Cannot unsubscribe table changes"
            )
    end.
...
process_tail_row(R0, #state{isSec = IsSec, seco = SKey, fetchCtx = FetchCtx0, statement = Stmt} = State) ->
    ...
    #fetchCtx{
        ...
        =
        FetchCtx0,
    %% Params cannot change any more after first fetch
    MR1 = imem_sql:meta_rec(IsSec, SKey, Stmt#statement.metaFields, [], MR0),
    MR2 =
        case RowNum of
            undefined -> MR1;
            _ -> setelement(?RownumIdx, MR1, RowNum)
        end,
    %% Main Tail Record (record name recovered)
    R1 = erlang:setelement(?RecIdx, R0, RecName),
    % ?LogDebug("processing tail : record / tail / filter~n~p~n~p~n~p~n", [R1,TailSpec,FilterFun]),
    RawRecords =
        case {Status, TailSpec, FilterFun} of
            {tailing, false, _} -> [];
            {tailing, _, false} -> [];
            ...
            STF ->
                imem_meta:log_to_db(
                    ...
                    handle_info,
                    [{status, STF}, {rec, R1}],
                    "Unexpected status in tail event"
                ),
                []
        end,
    case {RawRecords, length(Stmt#statement.tables), RowNum} of
        {[], _, _} ->
            ...
            {[], State};
        {_, 1, undefined} ->
            %% single table select, avoid join overhead
            if
                Rem0 =< 1 ->
        ...
        {_, 1, _FirstRN} ->
            %% single table select, avoid join overhead but update RowNum meta field
            if
                Rem0 =< 1 ->
                    % ?LogDebug("send~n~p~n", [{RawRecords,true}]),
                    unsubscribe(Stmt),
                ...
                true ->
                    ...
                    {
                        {RawRecords, tail},
                        State#state{fetchCtx = FetchCtx0#fetchCtx{rownum = RowNum + 1, remaining = Rem0 - 1}}
                    }
            end;
        {_, _, undefined} ->
            %% join raw result of main scan with remaining tables
                ...
                Result ->
                    if
                        ...
                        true ->
                            ...
                            {
                                {Result, tail},
                                State#state{fetchCtx = FetchCtx0#fetchCtx{remaining = Rem0 - length(Result)}}
                            }
                    end
            end;
        {_, _, _} ->
            %% join raw result of main scan with remaining tables, correct RowNum
            case join_rows(RawRecords, FetchCtx0, Stmt) of
                [] -> {[], State};
                JoinedRows ->
                    if
                        (Rem0 =< length(JoinedRows)) ->
                            ...
                            unsubscribe(Stmt),
                            {
                                {update_row_num(MR2, RowNum, JoinedRows), true},
                                State#state{fetchCtx = FetchCtx0#fetchCtx{status = done}}
                            };
                        true ->
                            % ?LogDebug("send~n~p~n", [{Result,tail}]),
                            {
                                {update_row_num(MR2, RowNum, JoinedRows), tail},
                                State#state{
                                    fetchCtx =
                                        FetchCtx0#fetchCtx{
                                            rownum = RowNum + length(JoinedRows),
                                            remaining = Rem0 - length(JoinedRows)
                                        }
                                }
                            }
                    end
...
zip_row_num(FirstRN, Rows) -> lists:zip(lists:seq(FirstRN, FirstRN + length(Rows) - 1), Rows).

join_rows(MetaAndMainRows, FetchCtx0, Stmt) ->
    #fetchCtx{blockSize = BlockSize, remaining = RemainingRowQuota} = FetchCtx0,
    %% {Schema,Name,Alias} for each table to join
    JoinTables = tl(Stmt#statement.tables),
...
join_rows([], _, _, _, _, Acc) ->
    %% lists:reverse(Acc);
    Acc;
join_rows(_, _, RRowQuota, _, _, Acc) when RRowQuota < 1 ->
    %% lists:reverse(Acc);
    Acc;
join_rows([Row | Rows], BlockSize, RRowQuota, JoinTables, JoinSpecs, Acc) ->
    Rec =
        erlang:make_tuple(
            length(JoinTables) + 2,
            undefined,
            [{?MetaIdx, element(?MetaIdx, Row)}, {?MainIdx, element(?MainIdx, Row)}]
        ),
...
join_row(Recs0, BlockSize, Ti, [{_S, JoinTable} | Tabs], [JS | JSpecs]) ->
    ...
    Recs1 =
        case lists:member(JoinTable, ?DataTypes) of
...
join_table(Rec, _BlockSize, Ti, Table, #scanSpec{limit = Limit} = JoinSpec) ->
    ...
    %% TODO: Move away from single shot join fetch, use async block fetch here as well.
    MaxSize = Limit + 1000,
...
generate_virtual_data(boolean, _Rec, true, _MaxSize) -> [{false, <<"false">>}, {true, <<"true">>}];
generate_virtual_data(_Table, _Rec, true, _MaxSize) -> ?ClientError({"Invalid virtual filter guard", true});
...
vals_from_or_equal(Tag, OrEqual) -> vals_from_or_equal(Tag, OrEqual, []).

vals_from_or_equal(Tag, {'==', Tag, V}, Acc) -> lists:reverse([V | Acc]);
vals_from_or_equal(Tag, {'or', {'==', Tag, V}, OrEqual}, Acc) -> vals_from_or_equal(Tag, OrEqual, [V | Acc]);
vals_from_or_equal(_, SGuard, _Acc) -> ?UnimplementedException({"Unsupported virtual in() filter guard", SGuard}).
...
generate_virtual(list = Table, {list, Items}, MaxSize) -> generate_virtual(Table, Items, MaxSize);
% generate_virtual(Table, {const,Items}, MaxSize) when is_tuple(Items) ->
%     generate_virtual(Table, tuple_to_list(Items), MaxSize);
generate_virtual(tuple, {const, I}, _) when is_tuple(I) -> [{I, imem_datatype:term_to_io(I)}];
...
generate_virtual(datetime = Table, Items, MaxSize) when is_list(Items) ->
    generate_limit_check(Table, length(Items), MaxSize),
    Pred = fun ({{_, _, _}, {_, _, _}}) -> true; (_) -> false end,
    [{I, imem_datatype:datetime_to_io(I)} || I <- lists:filter(Pred, Items)];
generate_virtual(datetime, {{_, _, _}, {_, _, _}} = I, _) -> [{I, imem_datatype:datetime_to_io(I)}];
...
generate_virtual(ipaddr = Table, Items, MaxSize) when is_list(Items) ->
    generate_limit_check(Table, length(Items), MaxSize),
    %% ToDo: IpV6
    Pred = fun ({A, B, C, D}) when is_integer(A), is_integer(B), is_integer(C), is_integer(D) -> true; (_) -> false end,
    [{I, imem_datatype:ipaddr_to_io(I)} || I <- lists:filter(Pred, Items)];
generate_virtual(ipaddr, {A, B, C, D} = I, _) when is_integer(A), is_integer(B), is_integer(C), is_integer(D) ->
    [{I, imem_datatype:ipaddr_to_io(I)}];
generate_virtual(ipaddr, _, _) ->
    %% ToDo: IpV6
    [];
...
generate_virtual(timestamp = Table, Items, MaxSize) when is_list(Items) ->
    generate_limit_check(Table, length(Items), MaxSize),
    Pred = fun ({Meg, Sec, Mic}) when is_number(Meg), is_integer(Sec), is_integer(Mic) -> true; (_) -> false end,
...
generate_virtual(userid = Table, Items, MaxSize) when is_list(Items) ->
    generate_limit_check(Table, length(Items), MaxSize),
    Pred = fun (X) -> is_integer(X) end,
    %% ToDo: filter in imem_account
    [{I, imem_datatype:term_to_io(I)} || I <- lists:filter(Pred, Items)];
generate_virtual(userid, I, _) when is_integer(I) ->
    %% ToDo: filter in imem_account
    [{I, imem_datatype:integer_to_io(I)}];
generate_virtual(userid, system, _) -> [{system, imem_datatype:atom_to_io(system)}];
...
update_prepare(IsSec, SKey, [{Node, Schema, Table} | _], ColMap, ChangeList) ->
    ...
    % No trigger options supported for now
    TableInfo = {Node, Schema, Table, TableType, list_to_tuple(DefRec), Trigger, User, []},
...
node_from_recs(Recs) -> element(2, element(?MetaIdx, Recs)).

% second item in MetaRec
...
update_prepare(IsSec, SKey, TableInfo, ColMap, [CItem | CList], Acc) ->
    ...
    case node_from_recs(lists:nth(3, CItem)) of
        ...
        _ ->
            %?Info("update_prepare skipped ~p ~p",[TableInfo,CItem]),
...
update_prepare_local(
    IsSec,
    SKey,
    {_, S, Tab, Typ, _, Trigger, User, TrOpts} = TableInfo,
    ColMap,
    [[Item, nop, Recs | _] | CList],
    Acc
) ->
...
update_prepare_local(
    IsSec,
    SKey,
    {_, S, Tab, Typ, DefRec, Trigger, User, TrOpts} = TableInfo,
    ColMap,
    [[Item, upd, Recs | Values] | CList],
    Acc
) ->
                    ...
                    #bind{tind = 0, cind = 0, func = Proj, btree = BTree} when is_function(Proj) ->
                        case BTree of
                                    ...
                                    fun
                                        (X) ->
                                            ...
                                            case {Value, OldVal} of
                                                {?navio, ?nav} ->
                                                    % attribute still not present
                                                    X;
                                                {?navio, _} ->
                                                    % remove head
                                                    ?replace(X, Cx, tl(?BoundVal(B, X)));
                                                _ ->
                                                    case
                                                    imem_datatype:io_to_db(
                                                        Item,
                                                        OldVal,
                                                        list_type(Type),
                                                        undefined,
                                    ...
                                    fun
                                        (X) ->
                                            ...
                                            case {Value, OldVal} of
                                                ...
                                                {?navio, _} ->
                                                    % remove last element
                                                    ?replace(X, Cx, lists:sublist(?BoundVal(B, X), Pos - 1));
                                                _ ->
                                                    case
                                                    imem_datatype:io_to_db(
                                                        Item,
                                                        ...
                                                        NewVal ->
                                                            ?replace(
                                                                X,
                                                                Cx,
                                                                lists:sublist(?BoundVal(B, X), Pos - 1) ++ NewVal
                                                            )
                                                    end
                                            end
                                    end,
                            ...
                            {element, PosBind, #bind{tind = ?MainIdx, cind = Cx, type = Type} = B} ->
                                ...
                                {Cx, Pos, Fx};
                            {bytes, #bind{tind = ?MainIdx, cind = Cx, type = T} = B, StartBind, LenBind}
                            when T == bitstring; T == binary; T == term; T == binterm ->
                                        ...
                                        (X) ->
                                            ...
                                            imem_datatype:io_to_db(
                                                ...
                                                NewVal when size(NewVal) /= size(OldVal) ->
                                                    ?ClientError(
                                                        {"Wrong byte size of replacement binary", {Item, size(NewVal)}}
                                                    );
                                                NewVal when Start >= 0 ->
                                                    Prefix = binary:part(OldBin, 0, Start),
                                                    Suffix =
                                                        binary:part(OldBin, Start + Len, size(OldBin) - Len - Start),
                                ...
                                {Cx, abs(Start) + 1, Fx};
                            {bytes, #bind{tind = ?MainIdx, cind = Cx, type = T} = B, StartBind}
                            when T == bitstring; T == binary; T == term; T == binterm ->
                                Start = imem_sql_expr:bind_tree(StartBind, Recs),
                                Fx =
                                    fun
                                        (X) ->
                                            ...
                                            imem_datatype:io_to_db(
                                                ...
                                                NewVal when size(OldBin) + Start >= 0 ->
                                                    Prefix = binary:part(OldBin, 0, size(OldBin) + Start),
                                                    ?replace(X, Cx, <<Prefix/binary, NewVal/binary>>);
                                                NewVal when size(OldBin) + Start < 0 -> ?replace(X, Cx, NewVal)
                                            end
                                    end,
                                {Cx, abs(Start) + 1, Fx};
                            {bits, #bind{tind = ?MainIdx, cind = Cx, type = T} = B, StartBind, LenBind}
                            when T == bitstring; T == binary; T == term; T == binterm ->
                                Start = imem_sql_expr:bind_tree(StartBind, Recs),
                                Len = imem_sql_expr:bind_tree(LenBind, Recs),
                                Fx =
                                ...
                                {Cx, abs(Start) + 1, Fx};
                            {bits, #bind{tind = ?MainIdx, cind = Cx, type = T} = B, StartBind}
                            when T == bitstring; T == binary; T == term; T == binterm ->
                                Start = imem_sql_expr:bind_tree(StartBind, Recs),
                                Fx =
                                    fun
                                                    ...
                                                    case Pos of
                                                        ...
                                                        _ ->
                                                            ?replace(
                                                                X,
                                                                Cx,
                                                                lists:sublist(Old, Pos - 1) ++ lists:nthtail(Pos, Old)
                                                            )
                                                    end;
                                                _ ->
                                                    case
                                                    imem_datatype:io_to_db(
                                                        ...
                                                        NewVal2 ->
                                                            ...
                                                            ?replace(
                                                                X,
                                                                Cx,
                                                                lists:sublist(Old, Pos - 1)
                                                                ++
                                                                NewVal2
                                                                ++
                                                                lists:nthtail(Pos, Old)
                                                            )
                                                    end
                                            end
                                    end,
                                                    ...
                                                    case Value of
                                                        ...
                                                        _ ->
                                                            try
                                                                ...
                                                                _:_ ->
                                                                    case
                                                                    imem_json:decode(imem_datatype:add_dquotes(Value)) of
                                                                        OldVal -> X;
                                                                        NewVal2 ->
                                                                            ?replace(
                                                                                X,
                                        ...
                                        (X) ->
                                            ...
                                            case {Value, OldVal} of
                                                {?navio, ?nav} ->
                                                    % attribute still not present
                                                    X;
                                                {?navio, _} -> ?replace(X, Cx, maps:remove(Key, ?BoundVal(B, X)));
                                                _ ->
                                                    case
                                                    imem_datatype:io_to_db(
                                                        Item,
                                                        ...
                                                        OldVal -> X;
                                                        NewVal ->
                                                            ?replace(X, Cx, maps:put(Key, NewVal, ?BoundVal(B, X)))
                                                    end
                                            end
                                    end,
                                {Cx, 1, Fx};
                                                        ...
                                                        NewVal ->
                                                            NewParent =
                                                                maps:put(
                                                                    Key,
                                                                    NewVal,
                                                                    maps:get(ParentKey, ?BoundVal(B, X))
                                                                ),
                                                            ?replace(
                                                                X,
                                                                Cx,
                                                                maps:put(ParentKey, NewParent, ?BoundVal(B, X))
                                                            )
                                                    end
                                            end
                                    end,
                                            ...
                                            case {Value, OldVal} of
                                                ...
                                                {<<>>, _} ->
                                                    NewParent =
                                                        imem_json:remove(
                                                            AttName,
                                                            imem_json:get(ParentName, ?BoundVal(B, X))
                                                        ),
                                                    ?replace(
                                                        X,
                                                        Cx,
                                                        imem_json:put(ParentName, NewParent, ?BoundVal(B, X))
                                                    );
                                                _ ->
                                                    case Value of
                                                        OldVal -> X;
                            ...
                            fun
                                (X) ->
                                    ?replace(
                                        X,
                                        Cx,
                                        imem_datatype:io_to_db(Item, ?BoundVal(CMap, X), T, L, P, D, false, Value)
                                    )
                            end,
                        {Cx, 0, Fx}
                end || {#bind{readonly = R} = CMap, Value} <- lists:zip(ColMap, Values),
                % , Value /= ?navio
                R == false
            ]
        ),
            ...
            [
                case CMap of
                    #bind{tind = 0, cind = 0, func = Proj, btree = BTree} when is_function(Proj) ->
                        case BTree of
                            {hd, #bind{tind = ?MainIdx, cind = Cx, type = Type}} ->
                                ...
                                {Cx, 1, Fx};
                            {element, Pos, #bind{tind = ?MainIdx, cind = Cx, name = Name, type = Type}}
                            when is_number(Pos) ->
                                Fx =
                                    fun
                                        (X) ->
                                                    ...
                                                    try
                                                        ...
                                                        _:_ ->
                                                            ?replace(
                                                                ...
                                                                imem_json:put(
                                                                    AttName,
                                                                    imem_json:decode(imem_datatype:add_dquotes(Value)),
                                                                    ?BoundVal(B, X)
                                                                )
                                                            )
                                                    end
                    ...
                    #bind{tind = ?MainIdx, cind = Cx, type = T, len = L, prec = P, default = D} ->
                        Fx =
                            fun
                                (X) -> ?ins_repl(X, Cx, imem_datatype:io_to_db(Item, ?nav, T, L, P, D, false, Value))
                            end,
                        {Cx, 0, Fx}
                end || {#bind{readonly = R} = CMap, Value} <- lists:zip(ColMap, Values),
                R == false,
...
receive_raw() -> receive_raw(50, []).

receive_raw(Timeout) -> receive_raw(Timeout, []).

receive_raw(Timeout, Acc) ->
    case receive
        R ->
            ?Debug("receive_raw got:~n~p~n", [R]),
            R
    after
        Timeout -> stop
    end of
...
receive_recs(StmtResult, Complete) -> receive_recs(StmtResult, Complete, 50, []).

receive_recs(StmtResult, Complete, Timeout) -> receive_recs(StmtResult, Complete, Timeout, []).

receive_recs(#stmtResults{stmtRefs = [StmtRef | _]} = StmtResult, Complete, Timeout, Acc) ->
    case receive
        R ->
            % ?Debug("~p got:~n~p~n", [?TIMESTAMP,R]),
            R
    after
        Timeout -> stop
    end of
        stop ->
            Unchecked =
                case Acc of
                    ...
                    [{StmtRef, {_, Complete}} | _] -> lists:reverse(Acc);
                    [{StmtRef, {L1, C1}} | _] -> throw({bad_complete, {StmtRef, {L1, C1}}});
                    Res -> throw({bad_receive, lists:reverse(Res)})
                end,
            case lists:usort([element(1, SR) || SR <- Unchecked]) of
                [StmtRef] ->
                    List = lists:flatten([element(1, element(2, T)) || T <- Unchecked]),
                    if
                        length(List) =< 10 -> ?Debug("Received:~n~p~n", [List]);
                        true ->
        ...
        {_, Result} -> receive_recs(StmtResult, Complete, Timeout, [Result | Acc])
    end.
