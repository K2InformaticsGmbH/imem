...
% erlang:_/1 (default)
-safe(['+', '-', 'bnot', float, size, bit_size, byte_size, md5, throw, hd, tl, round, length, whereis, phash2]).
...
safe(Mod) when is_atom(Mod) ->
    ...
    lists:usort(
        lists:flatten(
            lists:foldl(
                fun
                    ({safe, [all]}, Acc) -> check_export(Mod) ++ Acc;
                    ({safe, SL}, Acc) when is_list(SL) ->
                        lists:map(
                            fun
                                (F) when is_atom(F) ->
                                    if
                                        DefMod /= erlang -> check_export(DefMod, [F]);
                                        true -> {DefMod, F, 1}
                                    end;
                                ({F, A}) when is_atom(F), is_integer(A) ->
                                    if
                                        DefMod /= erlang -> check_export(DefMod, [{F, A}]);
                                        true -> {DefMod, F, A}
                                    end;
                                ({M, F, A}) when is_atom(M), is_atom(F), is_integer(A) -> check_export(M, F, A);
                                (#{m := M, f := F}) when is_atom(M) -> check_export(M, F);
                                (#{m := M}) when is_atom(M) -> check_export(M)
                            end,
                            SL
                        ) ++ Acc;
                    (_, Acc) -> Acc
...
check_export(_, []) -> [];
check_export(M, [F | Fs]) -> check_export(M, F) ++ check_export(M, Fs);
check_export(M, {R, A}) when is_list(R), is_integer(A) ->
    [{M, Fn, A} || {Fn, Art} <- M:module_info(exports), re:run(atom_to_list(Fn), R) /= nomatch, Art == A];
check_export(M, {F, A}) when is_atom(F), is_integer(A) -> check_export(M, F, A);
check_export(M, F) when is_atom(F) ->
    case lists:filtermap(fun ({Fun, A}) when Fun == F -> {true, {M, F, A}}; (_) -> false end, M:module_info(exports)) of
...
compile(String, Bindings) when is_binary(String), is_list(Bindings) -> compile(binary_to_list(String), Bindings);
compile(String, Bindings) when is_list(String), is_list(Bindings) ->
    Code =
        case [lists:last(string:strip(String))] of
            "." -> String;
            _ -> String ++ "."
        end,
    {ok, ErlTokens, _} = erl_scan:string(Code),
    {ok, ErlAbsForm} = erl_parse:parse_exprs(ErlTokens),
    case catch erl_eval:exprs(ErlAbsForm, Bindings, none, {value, fun ?MODULE:nonLocalHFun/2}) of
        {value, Value, _} -> Value;
...
nonLocalHFun(FSpec, Args) -> nonLocalHFun(FSpec, Args, safe()).
            ...
            case lists:keymember(Mod, 1, SafeFuns) of
                false ->
                    case safe(Mod) of
                        ...
                        ModSafe -> nonLocalHFun(FSpec, Args, SafeFuns ++ ModSafe)
                    end;
        ...
        [
            {
                C,
                case O of
                    'SystemException' -> ?_assertException(throw, {'SecurityException', _}, compile(C));
                    'ClientError' -> ?_assertException(throw, {'ClientError', _}, compile(C));
                    runtime ->
                        Fun = compile(C),
                        ?_assertException(throw, {'SecurityException', _}, Fun());
                    _ -> ?_assertEqual(O, compile(C))
                end
            } ||
            {C, O}
            <-
