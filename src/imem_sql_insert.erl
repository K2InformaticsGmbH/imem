...
exec(SKey, {insert, TableName, {}, {}, _Returning} = ParseTree, _Stmt, Opts, IsSec) ->
    ...
    [Tbin] =
        [{TS, TN} || #bind{tind = Ti, cind = Ci, schema = TS, table = TN} <- FullMap0, Ti /= ?MetaIdx, Ci == ?FirstIdx],
...
exec(SKey, {insert, TableName, {_, Columns}, {_, Values}, _Returning} = ParseTree, _Stmt, Opts, IsSec) ->
    ...
    ColBTrees2 =
        [
            {
                case imem_sql_funs:expr_fun(T) of
...
merge_values([{V, #bind{cind = Ci, type = Type, len = Len, prec = Prec, default = Def}} | Values], Rec) ->
    case imem_datatype:type_check(V, Type, Len, Prec, Def) of
        ok -> merge_values(Values, erlang:setelement(Ci, Rec, V));
        {error, Reason} -> ?ClientError(Reason)
    end.
