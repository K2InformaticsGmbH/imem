...
-export([filter_funs/0, expr_fun/1, filter_fun/1]).
-export(
    [
        unary_fun_bind_type/1,
...
-export([re_compile/1, re_match/2, like_compile/1, like_compile/2]).
-export(
    [
        to_integer/1,
        to_float/1,
...
unary_fun_bind_type([$#, _]) ->
    % #key(s), #value(s) for now
    #bind{type = json, default = <<>>};
unary_fun_bind_type([$v, $n, $f, $_ | _]) ->
    % vnf_.... value normalizing functions
    #bind{type = term, default = <<>>};
...
unary_fun_result_type([$#, _]) ->
    % #key(s), #value(s) for now
    #bind{type = json, default = []};
unary_fun_result_type([$v, $n, $f, $_ | _]) ->
    % vnf_.... value normalizing functions
    #bind{type = list, default = []};
...
unary_fun_result_type("to_time") ->
    % ToDo: Type=time
    #bind{type = timestamp, default = ?nav};
...
ternary_fun_bind_type2("preview") ->
    % or integer index id accepted
    #bind{type = list, default = ?nav};
ternary_fun_bind_type2("preview_keys") ->
    % or integer index id accepted
    #bind{type = list, default = ?nav};
...
ternary_fun_bind_type3("preview") ->
    % or integer search token accepted
    #bind{type = binstr, default = ?nav};
ternary_fun_bind_type3("preview_keys") ->
    % or integer search token accepted
    #bind{type = binstr, default = ?nav};
...
%% create a list
expr_fun({list, L}) when is_list(L) -> list_fun(lists:reverse([expr_fun(E) || E <- L]), []);
expr_fun(L) when is_list(L) -> list_fun(lists:reverse([expr_fun(E) || E <- L]), []);
...
expr_fun({Op, A, B}) when Op == '=='; Op == '>'; Op == '>='; Op == '<'; Op == '=<'; Op == '/=' -> comp_fun({Op, A, B});
%% Mathematical expressions
expr_fun({pi}) -> math:pi();
expr_fun({Op, A}) when Op == '+'; Op == '-' -> math_fun({Op, A});
expr_fun({Op, A}) when Op == sqrt; Op == log; Op == log10; Op == exp; Op == erf; Op == erfc ->
    module_fun(math, {Op, A});
expr_fun({Op, A}) when Op == sin; Op == cos; Op == tan; Op == asin; Op == acos; Op == atan -> module_fun(math, {Op, A});
expr_fun({Op, A}) when Op == sinh; Op == cosh; Op == tanh; Op == asinh; Op == acosh; Op == atanh ->
    module_fun(math, {Op, A});
expr_fun({Op, A, B}) when Op == '+'; Op == '-'; Op == '*'; Op == '/'; Op == 'div'; Op == 'rem' -> math_fun({Op, A, B});
expr_fun({Op, A, B}) when Op == pow; Op == atan2 -> module_fun(math, {Op, A, B});
%% Erlang module
...
expr_fun({Op, A, B}) when Op == integer_to_binary; Op == integer_to_list; Op == list_to_binary; Op == list_to_bitstring ->
    module_fun(erlang, {Op, A, B});
expr_fun({Op, A, B}) when Op == list_to_integer; Op == max; Op == min; Op == phash2 -> module_fun(erlang, {Op, A, B});
expr_fun({Op, A, B}) when Op == crc32; Op == float_to_binary; Op == float_to_list -> module_fun(erlang, {Op, A, B});
...
expr_fun({Op, A}) when Op == last; Op == reverse; Op == sort; Op == usort -> module_fun(lists, {Op, A});
expr_fun({Op, A})
when Op == vnf_identity;
     Op == vnf_lcase_ascii;
     Op == vnf_lcase_ascii_ne;
     Op == vnf_tokens;
     Op == vnf_integer;
     Op == vnf_float;
     Op == vnf_datetime;
...
expr_fun({Op, A}) when Op == integer_uid; Op == time_uid; Op == time -> module_fun(imem_meta, {Op, A});
expr_fun({Op, A}) when Op == cmp_white_space; Op == norm_white_space -> module_fun(imem_cmp, {Op, A});
expr_fun({Op, A, B})
when Op == nth; Op == member; Op == merge; Op == nthtail; Op == seq; Op == sublist; Op == subtract; Op == usort ->
    module_fun(lists, {Op, A, B});
%% maps module
expr_fun({Op, A}) when Op == map_size -> module_fun(maps, {size, A});
...
expr_fun({'and', A, B}) ->
    ...
    case {Fa, Fb} of
        ...
        {_, false} -> false;
        {true, _} ->
            %% may be ?nav or a fun evaluating to ?nav
            Fb;
        {_, true} ->
            %% may be ?nav or a fun evaluating to ?nav
            Fa;
...
when Op == is_binstr;
     ...
     Op == is_timestamp ->
    unary_fun({Op, A});
expr_fun({Op, A}) when Op == round; Op == trunc; Op == upper; Op == lower -> unary_fun({Op, A});
expr_fun({Op, A}) when Op == '#keys'; Op == '#key'; Op == '#values'; Op == '#value'; Op == json_to_list ->
    unary_json_fun({Op, A});
expr_fun({Op, A}) -> ?UnimplementedException({"Unsupported expression operator", {Op, A}});
...
when Op == is_member;
    ...
    binary_fun({Op, A, B});
expr_fun({Op, A, B})
when Op == to_decimal; Op == from_decimal; Op == add_dt; Op == add_ts; Op == slice; Op == bits; Op == bytes; Op == split ->
    binary_fun({Op, A, B});
expr_fun({Op, A, B})
when Op == nvl_atom;
     Op == nvl_binary;
    ...
    binary_fun({Op, A, B});
expr_fun({Op, A, B})
when Op == json_arr_proj; Op == json_obj_proj; Op == json_value; Op == json_diff; Op == cmp; Op == diff; Op == diff_only ->
    binary_fun({Op, A, B});
expr_fun({Op, A, B}) -> ?UnimplementedException({"Unsupported expression operator", {Op, A, B}});
...
expr_fun({Op, A, B, C}) -> ?UnimplementedException({"Unsupported function arity 3", {Op, A, B, C}});
expr_fun({Op, A, B, C, D}) -> ?UnimplementedException({"Unsupported function arity 4", {Op, A, B, C, D}});
expr_fun(Value) -> Value.

bind_action(P) when is_function(P) ->
    %% parameter already bound to function
    true;
bind_action(#bind{tind = 0, cind = 0} = P) -> ?SystemException({"Unexpected expression binding", P});
bind_action(#bind{} = P) ->
    %% find bind by tag name or return false for value prameter
    P;
bind_action(_) -> false.
...
module_fun(Mod, {Op, {const, A}}) when is_tuple(A) -> module_fun_final(Mod, {Op, A});
module_fun(Mod, {Op, A}) -> module_fun_final(Mod, {Op, expr_fun(A)});
module_fun(Mod, {Op, {const, A}, {const, B}}) when is_tuple(A), is_tuple(B) -> module_fun_final(Mod, {Op, A, B});
module_fun(Mod, {Op, {const, A}, B}) when is_tuple(A) -> module_fun_final(Mod, {Op, A, expr_fun(B)});
module_fun(Mod, {Op, A, {const, B}}) when is_tuple(B) -> module_fun_final(Mod, {Op, expr_fun(A), B});
...
mod_op_1(Mod, Op, A) -> try Mod:Op(A) catch _:_ -> ?nav end.

mod_op_2(?MODULE, Op, ?nav, B)
when Op == nvl_atom;
     Op == nvl_binary;
     Op == nvl_binstr;
     Op == nvl_float;
...
mod_op_2(Mod, Op, A, B) -> try Mod:Op(A, B) catch _:_ -> ?nav end.

mod_op_3(_, _, _, _, ?nav) -> ?nav;
mod_op_3(_, _, _, ?nav, _) -> ?nav;
mod_op_3(_, _, ?nav, _, _) -> ?nav;
mod_op_3(Mod, Op, A, B, C) -> try Mod:Op(A, B, C) catch _:_ -> ?nav end.

math_fun({Op, A}) -> math_fun_unary({Op, expr_fun(A)});
math_fun({Op, A, B}) ->
    Fa = expr_fun(A),
    Fb = expr_fun(B),
...
-define(
    MathOpBlockBinary(__Op, __A, __B),
    case __Op of
...
comp_fun({Op, {const, A}, {const, B}}) when is_tuple(A), is_tuple(B) -> comp_fun_final({Op, A, B});
comp_fun({Op, {const, A}, B}) when is_tuple(A) -> comp_fun_final({Op, A, expr_fun(B)});
comp_fun({Op, A, {const, B}}) when is_tuple(B) -> comp_fun_final({Op, expr_fun(A), B});
comp_fun({Op, A, B}) ->
    Fa = expr_fun(A),
    Fb = expr_fun(B),
...
unary_fun({Op, {const, A}}) when is_tuple(A) -> unary_fun_final({Op, A});
unary_fun({Op, A}) -> unary_fun_final({Op, expr_fun(A)});
...
is_binstr(A) -> is_binary(A).

% TODO: not precise enough
is_binterm(A) -> is_binary(A).

% TODO: not precise enough
is_datetime({{A, B, C}, {D, E, F}})
...
% TODO: not precise enough
is_json(A) -> is_binary(A).

% TODO: not precise enough
is_name(A) -> is_binary(A).
...
is_timestamp({A, B, C}) when is_integer(A), is_integer(B), is_integer(C) ->
    % remove later
    true;
is_timestamp(_) -> false.

% TODO: not precise enough
upper(S) when is_binary(S) ->
    unicode:characters_to_binary(string:uppercase(unicode:characters_to_list(S, utf8)), unicode, utf8);
upper(S) when is_list(S) -> string:uppercase(S);
upper(S) -> S.

lower(S) when is_binary(S) ->
    unicode:characters_to_binary(string:lowercase(unicode:characters_to_list(S, utf8)), unicode, utf8);
...
to_text(T) when is_binary(T) -> to_text(binary_to_list(T));
...
to_text(T) -> imem_datatype:term_to_io(T).

to_tuple(B) when is_binary(B) -> imem_datatype:io_to_tuple(B, 0);
to_tuple(T) when is_tuple(T) -> T.

to_list(B) when is_binary(B) -> try imem_datatype:io_to_list(B, 0) catch _:_ -> ?nav end;
...
to_map(B) when is_binary(B) ->
    case catch imem_datatype:io_to_map(B) of
        M when is_map(M) -> M;
        _ -> imem_json:decode(B, [return_maps])
    end.

to_term(B) when is_binary(B) -> try imem_datatype:io_to_term(B) catch _:_ -> ?nav end;
to_term(T) -> T.
...
to_json(false) -> false;
to_json(null) -> null;
to_json(B) when is_binary(B) -> try imem_json:decode(B) catch _:_ -> B end;
to_json(L) when is_list(L) -> L;
% case catch imem_json:encode(L) of
%     JO when is_binary(JO) -> JO;
%     _ ->    ?nav
% end;
to_json(M) when is_map(M) -> try imem_json:to_proplist(M) catch _:_ -> ?nav end;
to_json(_) -> ?nav.

to_pid(T) when is_pid(T) -> T;
to_pid(B) -> try imem_datatype:io_to_pid(B) catch _:_ -> ?nav end.

to_existing_atom(A) when is_atom(A) -> A;
to_existing_atom(B) when is_binary(B) -> try ?binary_to_existing_atom(B) catch _:_ -> ?nav end;
to_existing_atom(L) when is_list(L) -> try list_to_existing_atom(L) catch _:_ -> ?nav end;
to_existing_atom(_) -> ?nav.

to_integer(B) when is_binary(B) -> try to_integer(binary_to_list(B)) catch _:_ -> ?nav end;
to_integer(I) when is_integer(I) -> I;
to_integer(F) when is_float(F) -> erlang:round(F);
to_integer(L) when is_list(L) -> try list_to_integer(L) catch _:_ -> ?nav end;
to_integer(_) -> ?nav.
...
to_number(B) when is_binary(B) -> try to_number(binary_to_list(B)) catch _:_ -> ?nav end;
to_number(F) when is_float(F) -> F;
to_number(I) when is_integer(I) -> I;
to_number(L) when is_list(L) ->
    case (catch list_to_integer(L)) of
        I when is_integer(I) -> I;
        _ -> try list_to_float(L) catch _:_ -> ?nav end
    end.
...
list_to_binstr(X) -> try list_to_binary(io_lib:format("~s", [X])) catch _:_ -> ?nav end.
...
to_timestamp({Megas, Secs, Micros}) ->
    % remove later
    {1000000 * Megas + Secs, Micros, node(), 0};
to_timestamp({Secs, Micros, Node, Cnt}) -> {Secs, Micros, Node, Cnt}.

to_time(B) when is_binary(B) ->
    Time = imem_datatype:io_to_timestamp(B),
    {element(1, Time), element(2, Time)};
to_time(L) when is_list(L) ->
    Time = imem_datatype:io_to_timestamp(L),
    {element(1, Time), element(2, Time)};
to_time({Secs, Micros}) -> {Secs, Micros};
to_time({Megas, Secs, Micros}) ->
    % remove later
    {1000000 * Megas + Secs, Micros};
...
prefix_ul(L) when is_list(L) -> L ++ <<255>>.

% improper list [...|<<255>>]
unary_json_fun({_, {const, A}}) when is_tuple(A) -> ?nav;
unary_json_fun({Op, A}) -> unary_json_fun_final({Op, expr_fun(A)});
...
binary_fun({Op, {const, A}, {const, B}}) when is_tuple(A), is_tuple(B) -> binary_fun_final({Op, A, B});
binary_fun({Op, {const, A}, B}) when is_tuple(A) -> binary_fun_final({Op, A, expr_fun(B)});
binary_fun({Op, A, {const, B}}) when is_tuple(B) -> binary_fun_final({Op, expr_fun(A), B});
binary_fun({Op, A, B}) ->
    FA = expr_fun(A),
    FB = expr_fun(B),
...
binary_fun_final(BTree) -> ?UnimplementedException({"Unsupported filter function", {BTree}}).

-spec add_dt(ddDatetime(), number()) -> ddDatetime().
add_dt(DT, Offset) when is_tuple(DT), is_number(Offset) -> imem_datatype:offset_datetime('+', DT, Offset).

%% Offset by (fractions of) days

-spec diff_dt(ddDatetime(), ddDatetime()) -> float().
diff_dt(A, B) when is_tuple(A), is_tuple(B) -> imem_datatype:datetime_diff(B, A).

%% Difference in (fractions of) days

-spec add_ts(ddTimeUID() | ddTimestamp() | {integer(), integer(), integer()}, number()) -> ddTimestamp().
add_ts(TS, Offset) when is_tuple(TS), is_number(Offset) -> imem_datatype:offset_timestamp('+', TS, Offset).
...
-spec diff_ts(
    ...
    float().
diff_ts(TS1, TS2) -> imem_datatype:usec_diff(TS1, TS2) / 8.64e10.
...
trunc({Sec, Micro, Node, I}, N) when is_integer(Sec), is_integer(Micro), is_integer(N), N > 0 ->
    {erlang:trunc(Sec / N) * N, 0, Node, I};
...
json_obj_proj(A, B) when is_list(B) ->
    % filter json object A with names in B
    L = json_to_list(A),
...
json_value(A, B) when is_binary(A), is_binary(B) -> json_value(A, json_to_list(B));
json_value(_, []) -> ?nav;
json_value(A, [{_, _} | _] = B) when is_binary(A) ->
    % pick value of attribute A in json object B
    safe_value(A, B);
json_value(A, B) when is_binary(A), is_map(B) ->
    % pick value of attribute A in json object B
    safe_value(A, B);
json_value(A, [[{_, _} | _]] = [B]) when is_binary(A) ->
    % pick value of attribute A in array with one object B
    safe_value(A, B);
json_value(A, [B]) when is_binary(A), is_map(B) ->
    % pick value of attribute A in array with one object B
    safe_value(A, B);
json_value(A, [[{_, _} | _] | _] = L) when is_binary(A) ->
    % pick value of attribute A in array of objects L
    [safe_value(A, B) || B <- L];
json_value(A, [#{} | _] = L) when is_binary(A) ->
    % pick value of attribute A in array of objects L
    [safe_value(A, B) || B <- L];
...
to_decimal(B, P) when is_integer(P), (P > 0) -> erlang:round(math:pow(10, P) * to_number(B)).

ternary_fun({Op, {const, A}, B, C}) when is_tuple(A) -> ternary_fun({Op, A, B, C});
ternary_fun({Op, A, {const, B}, C}) when is_tuple(B) -> ternary_fun({Op, A, B, C});
ternary_fun({Op, A, B, {const, C}}) when is_tuple(C) -> ternary_fun({Op, A, B, C});
ternary_fun({Op, A, B, C}) ->
    FA = expr_fun(A),
    FB = expr_fun(B),
    FC = expr_fun(C),
...
ternary_fun_final(BTree) -> ?UnimplementedException({"Unsupported filter function", BTree}).
...
slice(<<>>, _) -> <<>>;
slice(B, Start) when is_binary(B) -> unicode:characters_to_binary(slice(unicode:characters_to_list(B, utf8), Start));
slice([], _) -> [];
...
slice(L, Start) when is_list(L) ->
    if
        length(L) + Start >= 0 -> lists:nthtail(length(L) + Start, L);
        true -> L
    end;
...
slice(L, Start, Len) when is_list(L) ->
    if
        length(L) + Start >= 0 -> lists:sublist(L, length(L) + Start + 1, Len);
        true -> lists:sublist(L, Len)
    end;
