...
-define(TABLE_OPTS, [{record_name, skvhTable}, {type, ordered_set}]).
-define(
    AUDIT_OPTS,
    [
        {record_name, skvhAudit},
        {type, ordered_set},
        %% 430000 = 5 Days - 2000 sec
        {purge_delay, 430000}
    ]
).
-define(HIST_OPTS, [{record_name, skvhHist}, {type, ordered_set}]).

-record(
    %% table provisioning context
    skvhCtx,
    {
        %% main table alias
        mainAlias :: atom(),
...
-record(
    ...
    skvhCL,
    {
        ...
        time :: ddTimeUID(),
        %% old value
        ovalue :: binary() | undefined,
        %% new value
        nvalue :: binary() | undefined,
        cuser = unknown :: ddEntityId() | unknown
    }
).
-record(
    %% sorted key value hash table
    skvhHist,
    {ckey = ?nav :: binary() | ?nav, cvhist :: list(#skvhCL{})}
).

-define(skvhHist, [binterm, list]).

-record(
    %% sorted key value hash audit table
    skvhAudit,
    {
        %% ?TIME_UID
        time = ?ERL_MIN_TERM :: ddTimeUID(),
        ckey = ?nav :: binary() | ?nav,
        %% old value
        ovalue :: binary() | undefined,
        %% new value
...
-record(
    ...
    skvhTable,
    {
        ...
        chash =
            <<
                "fun(_,__Rec) -> list_to_binary(io_lib:format(\"~.36B\",[erlang:phash2({element(2,__Rec),element(3,__Rec)})])) end."
            >>
            ::
            binary()
    }
...
-define(
    GET_PURGE_HIST_DAYS(__HIST_TABLE),
    ?GET_CONFIG(purgeHistDayThreshold, [__HIST_TABLE], 100, "Days after which history record is deleted")
).

-export(
    [
        %% (Channel)                    return table name as binstr
        table_name/1,
        %% (Channel)                    return table name as atom
        atom_table_name/1,
        %% (Channel)                    return audit alias as bisntr
        audit_alias/1,
        %% (Channel)                    return history alias as atom
        atom_history_alias/1,
        %% (Name,[],Opts,Owner)         create empty table / audit table / history table (Name as binary or atom)
        create_table/4,
        %% (Name)
        drop_table/1,
        %% (Name)                       return #skvhCtx{mainAlias=Tab, auditAlias=Audit, histAlias=Hist}
        channel_ctx/1,
        %% (Channel)                    create empty table / audit table / history table if necessary (Name as binary or atom)
        create_check_channel/1,
        %% (Channel,Options)            create empty table / audit table / history table if necessary (Name as binary or atom)
        create_check_channel/2,
        %% (User, Channel)              create empty table / audit table / history table if necessary (Name as binary or atom)
        create_check_skvh/2,
        %% (Channel,Options)            create empty table / audit table / history table if necessary (Name as binary or atom)
        create_check_skvh/3,
        %% (User, Channel, KVTable)             resource may not exist, will be created, return list of hashes
        write/3,
        %% (User, Channel, Key, Value)          resource may not exist, will be created, map of inserted row
        write/4,
        %% (User, Channel, MapList)             resources should not exist will be created, retrun list of maps with inserted rows
        insert/3,
        %% (User, Channel, Key, Value)          resource should not exist will be created, return map with inserted row
        insert/4,
        %% (user, Channel, ChangeList)          update a list of resources, it will fail if the old value was modified by someone else
        update/3,
        %% (User, Channel, KeyList)             return empty Arraylist if none of these resources exists, the list is returning as a map
        read/3,
        %% (User, Channel, KeyList)             return list of maps
        read_siblings/3,
        %% (User, Channel, KeyList)             return list of maps
        read_shallow/3,
        %% (User, Channel, KeyList)             return empty Arraylist if none of these resources exists, the list is returning as a map
        read_deep/3,
        %% (User, Channel, Item, KeyTable)      return empty Arraylist if none of these resources exists
        read/4,
        %% (User, Channel, CKey1, CKey2, L)     from key at or after CKey1 to last key before CKey2, result as map, fails if more than L rows
        readGELT/5,
        %% (User, Channel, Item, CKey1, CKey2, L)   from key at or after CKey1 to last key before CKey2, fails if more than L rows
        readGELT/6,
        %% (User, Channel, CKey1, CKey2, L)     from key at or after CKey1 to last key before CKey2, list of keys
        readGELTKeys/5,
        %% (User, Channel, CKey1, CKey2, L)     from key at or after CKey1 to last key before CKey2, list of {key, hash}
        readGELTHashes/5,
        %% (User, Channel, CKey1, CKey2, L)     from key at or after CKey1 to last key before CKey2, list of skvhmaps
        readGELTMap/5,
        %% (User, Channel, CKey1, L)            start with first key after CKey1, return result as list of maps of lenght L or less
        readGT/4,
        %% (User, Channel, Item, CKey1, Limit)  start with first key after CKey1, return Limit results or less
        readGT/5,
        %% (User, Channel, CKey1, Limit)        start with first key at or after CKey1, return Limit results or less as maps
        readGE/4,
        %% (User, Channel, Item, CKey1, Limit)  start with first key at or after CKey1, return Limit results or less
        readGE/5,
        %% (User, Channel, TS1, Limit)          read audit info after Timestamp1, resturn a list of maps with Limit results or less
        audit_readGT/4,
        %% (User, Channel, Item, TS1, Limit)    read audit info after Timestamp1, return Limit results or less
        audit_readGT/5,
        %% (User, Channel, KeyList)             return history list as maps for given keys
        hist_read/3,
        %% (User, Channel, Key)               return the oldvalue of the deleted object
        hist_read_deleted/3,
        %% (User, Channel)                      keeps the last non noop history states and cverifies history state exists for all the keys
        prune_history/2,
        %% (HistTables)                    keeps the first and last entries, all other entries older than or equal to purgeHistDayThreshold are removed
        purge_history_tables/1,
        %% (User, Channel, RowList)             delete a resource will fail if it was modified, rows should be in map format
        remove/3,
        %% (User, Channel, RowList, Opts)       delete a resource will fail if it was modified, rows should be in map format, with trigger options
        remove/4,
        %% (User, Channel, KeyTable)            do not complain if keys do not exist
        delete/3,
        %% (User, Channel, CKey1, CKey2, L)     delete range of keys >= CKey1 and < CKey2, fails if more than L rows
        deleteGELT/5,
        %% (User, Channel, CKey1, CKey2, L)     delete range of keys > CKey1 and < CKey2, fails if more than L rows
        deleteGTLT/5,
        get_longest_prefix/4,
        %% (User, Channel, Key, TS1)      returns the records if there is any for the key after the timestamp TS1
        check_age_audit_entry/4,
        %% (User, Channel, Key)                creates an entry in audit table for channel wehere nvalue and ovalue are the same
        audit_write_noop/3,
        %% (Channel, Key)                       gets the next key after Key and returns decoded key
        dirty_next/2
    ]
).
-export(
    [
        build_aux_table_info/1,
        audit_info/6,
        write_audit/1,
        audit_recs_time/1,
...
-safe(
    [
        ...
        map_to_skvh_rec,
        expand_inline_key,
        expand_inline_key_map,
        expand_inline,
        audit_recs_time,
        write_history,
        write_audit,
        audit_info,
        build_aux_table_info
    ]
).
-safe(
    [
...
-spec foldl(User :: any(), FoldFun :: function(), InputAcc :: any(), Channel :: binary()) -> OutPutAcc :: any().
foldl(_User, FoldFun, InputAcc, Channel) when is_function(FoldFun, 2) ->
    Tab = atom_table_name(Channel),
    imem_meta:foldl(
        fun (Rec, Acc) when is_record(Rec, skvhTable) -> FoldFun(skvh_rec_to_map(Rec), Acc); (_Rec, Acc) -> Acc end,
        InputAcc,
        Tab
    ).
            ...
            case re:run(TrgrFun, ModHookCodeRe, [dotall, {capture, all, list}]) of
                ...
                {match, _} ->
                    imem_meta:create_or_replace_trigger(
                        Tab,
                        re:replace(TrgrFun, ModHookCodeRe, PreparedHook, [{return, binary}, dotall])
                    );
        ...
        TrgrFun when is_binary(TrgrFun) ->
            ...
            case re:replace(TrgrFun, ModHookCodeRe, "", [{return, binary}, dotall]) of
                TrgrFun ->
                    % No previous hook
                    ok;
                NewTrgrFun -> imem_meta:create_or_replace_trigger(Tab, NewTrgrFun)
            end;
        _ -> {error, notrigger}
    end.

-spec expand_inline_key(User :: any(), Channel :: binary(), Key :: any()) -> Json :: binary().
...
-spec expand_inline_key_map(User :: any(), Channel :: binary(), Key :: any()) -> Json :: map().
...
-spec expand_inline(User :: any(), Channel :: binary(), Json :: binary()) -> ExpandedJson :: binary().
expand_inline(User, Channel, Json) when is_binary(Json) ->
    imem_json:encode(expand_inline(User, Channel, imem_json:decode(Json, [return_maps])));
...
audit_table_name(Channel, Key) when is_list(Channel) -> imem_meta:partitioned_table_name(Channel ++ ?AUDIT_SUFFIX, Key).
...
audit_table_time({Sec, Mic, Node, TCnt} = TransTime, CH) ->
    ...
    case imem_meta:last(ATName) of
        '$end_of_table' ->
            % audit is empty, any TransTime accepted
            {ATName, TransTime};
        {_, _, _} ->
            % audit is of old format, TransTime accepted
            {ATName, TransTime};
        TransTime ->
            % I see my own audit write. May not occur
            {ATName, {Sec, Mic, Node, ?INTEGER_UID}};
        LastLog when (LastLog >= TransTime) andalso (element(2, LastLog) < 999999) ->
            % count usec up
            {ATName, {element(1, LastLog), element(2, LastLog) + 1, Node, TCnt}};
        LastLog when (LastLog >= TransTime) ->
            % partition might change
            audit_table_time({element(1, LastLog) + 1, 0, Node, TCnt}, CH);
        _ ->
            % last audit key is smaller than TransTime, accepted
            {ATName, TransTime}
    end.

audit_table_next(ATName, {Sec, Mic, Node, _Cnt}, _CH) -> {ATName, {Sec, Mic, Node, ?INTEGER_UID}}.

% next bigger key for double transaction
is_row_type(map, R) when is_map(R) -> R;
is_row_type(map, R) when is_list(R) ->
    [
        if
            not is_map(H) -> ?ClientError({"Bad datatype, expected map", R});
            true -> H
        end || H <- R
    ];
...
channel_ctx(Channel) when is_binary(Channel); is_atom(Channel) ->
    ...
    Tab =
        try
            ...
            %% throws if master table is not locally resident
            imem_meta:check_local_table_copy(T),
            T
        catch
            throw : {'ClientError', {"Table does not exist", _}} -> ?ClientError({"Channel does not exist", Channel});
            throw : Throw -> throw(Throw);
            Class:Error -> ?SystemException({"Error creating channel context", {Class, Error}})
        end,
    Audit =
        try
            A = list_to_existing_atom(?AUDIT(Channel)),
            AP = imem_meta:partitioned_table_name_str(A, ?TIME_UID),
            %% throws if table is not locally resident
            imem_meta:check_local_table_copy(list_to_existing_atom(AP)),
            A
        catch
            _:_ -> ignored
        end,
    Hist =
        try
            ...
            %% throws if history table is not locally resident
            imem_meta:check_local_table_copy(H),
            H
        catch
            _:_ -> ignored
        end,
...
create_check_skvh(_UserId, Channel, Options) -> create_check_channel(Channel, Options).

-spec create_check_channel(binary() | atom()) -> ok.
create_check_channel(Channel) -> create_check_channel(Channel, [audit, history]).
...
create_check_channel(Channel, Options) ->
            ...
            Class1:_ when Class1 =:= error; Class1 =:= throw ->
                ...
                {NewCValue, NewTypes} =
                    case proplists:get_value(type, Options, binary) of
                        binary -> {<<"fun(R) -> imem_dal_skvh:is_row_type(binary,R) end.">>, ?skvhTable};
                        list ->
                            {
                ...
                imem_meta:create_check_table(
                    TC,
                    {record_info(fields, skvhTable), NewTypes, #skvhTable{cvalue = NewCValue}},
                    ?TABLE_OPTS,
                    system
                ),
                TC
        ...
        CreateAudit ->
            try
                ...
                Class2:_ when Class2 =:= error; Class2 =:= throw ->
                    ...
                    imem_meta:create_check_table(
                        AC,
                        {record_info(fields, skvhAudit), ?skvhAudit, #skvhAudit{}},
                        ?AUDIT_OPTS,
                        system
                    ),
                    AC
        ...
        CreateAudit andalso CreateHistory ->
            try
                ...
                Class:_Reason when Class =:= error; Class =:= throw ->
                    ...
                    imem_meta:create_check_table(
                        HC,
                        {record_info(fields, skvhHist), ?skvhHist, #skvhHist{}},
                        ?HIST_OPTS,
                        system
                    ),
                    HC
    ...
    case catch imem_meta:get_trigger(Tab) of
        undefined -> imem_meta:create_or_replace_trigger(Tab, TiggerFun);
        Trigger when is_binary(Trigger) ->
            case re:run(Trigger, ?EXTEND_CODE) of
                nomatch -> no_op;
                _ -> imem_meta:create_or_replace_trigger(Tab, TiggerFun)
            end;
        _ -> no_op
    end,
    ok.
...
create_table(Channel, [], _TOpts, Owner) when is_binary(Channel) ->
    ...
    {ok, QTN} =
        imem_meta:create_table(Tab, {record_info(fields, skvhTable), ?skvhTable, #skvhTable{}}, ?TABLE_OPTS, Owner),
    AC = list_to_atom(?AUDIT(Channel)),
    {ok, _} =
        imem_meta:create_table(AC, {record_info(fields, skvhAudit), ?skvhAudit, #skvhAudit{}}, ?AUDIT_OPTS, Owner),
...
drop_table(Name) when is_atom(Name) -> drop_table(list_to_binary(atom_to_list(Name)));
...
build_aux_table_info(Table) ->
    ...
    HistoryTable = try ?HIST_FROM_STR(Channel) of H -> H catch _:_ -> '$no_history' end,
...
audit_info(User, _Channel, AuditTable, TransTime, OldRec, NewRec) when element(2, OldRec) == element(2, NewRec) ->
...
audit_info(User, Channel, AuditTable, TransTime, OldRec, NewRec) ->
    ...
    [
        {
            AuditTable,
            #skvhAudit{time = TransTime, ckey = OldKey, ovalue = element(3, OldRec), nvalue = undefined, cuser = User}
        },
        {
            AuditTable1,
            #skvhAudit{time = TransTime1, ckey = NewKey, ovalue = undefined, nvalue = element(3, NewRec), cuser = User}
        }
    ].
...
dirty_next(Channel, Key) ->
    case imem_meta:dirty_next(atom_table_name(Channel), sext:encode(Key)) of
        ...
        NextEncKey -> sext:decode(NextEncKey)
    end.
...
write_history(HistoryTable, [{_AuditTable, #skvhAudit{time = T, ckey = K, ovalue = O, nvalue = N, cuser = U}} | Rest]) ->
    if
        O == N andalso N == undefined -> ok = imem_meta:truncate_table(HistoryTable);
        true -> ok
    end,
    ok =
        imem_meta:write(
...
io_key_to_term(Key) when is_binary(Key) -> imem_datatype:io_to_binterm(Key).

io_to_integer(I) when is_integer(I) -> I;
io_to_integer(Key) when is_binary(Key) -> imem_datatype:io_to_integer(Key, 0, 0).

io_value_to_term(V) when is_binary(V) -> V.

%% ToDo: Maybe convert to map datatype when available

binterm_to_term_key(MaybeDecodedKey) when is_binary(MaybeDecodedKey) -> imem_datatype:binterm_to_term(MaybeDecodedKey);
binterm_to_term_key(MaybeDecodedKey) when is_list(MaybeDecodedKey) -> MaybeDecodedKey.
...
term_key_to_binterm(MaybeEncodedKey) when is_binary(MaybeEncodedKey) ->
    try
        ...
        _:_ -> imem_datatype:term_to_binterm(MaybeEncodedKey)
    end;
term_key_to_binterm(TermKey) -> imem_datatype:term_to_binterm(TermKey).
...
io_kv_pair_to_tuple(KVPair) ->
    case binary:split(KVPair, <<9>>, [global]) of
        %% split at tab position
        [K, V] -> {io_key_to_term(K), io_value_to_term(V)};
        Other -> ?ClientError(?E102(Other))
    end.

io_key_table_to_term_list(KeyTable) ->
    %% split at line feed positions
    BList = binary:split(io_clean_table(KeyTable), <<10>>, [global]),
    [io_key_to_term(Key) || Key <- BList].

io_kv_table_to_tuple_list(KVTable) ->
    %% split at line feed positions
    BList = binary:split(io_clean_table(KVTable), <<10>>, [global]),
...
term_value_to_io(undefined) ->
    %% ToDo: Maybe convert from map datatype when available
    <<"undefined">>;
term_value_to_io(V) when is_binary(V) -> V.
...
skvh_rec_to_map(#skvhHist{ckey = _, cvhist = _} = HistRec) -> skvh_hist_to_map(HistRec).
...
read(Cmd, SkvhCtx, Item, [Key | Keys], Acc) ->
    KVP =
        case imem_meta:dirty_read(SkvhCtx#skvhCtx.mainAlias, Key) of
...
delete(User, Cmd, SkvhCtx, [Key | Keys], Acc) ->
    Hash =
        case imem_meta:read(SkvhCtx#skvhCtx.mainAlias, Key) of
            [] -> undefined;
            [Rec] ->
                try
...
read_siblings(User, Channel, [Key | Keys]) ->
    ...
    read_shallow(User, Channel, [lists:reverse(ParentKeyRev)]) ++ read_siblings(User, Channel, Keys).

read_shallow(User, Channel, Keys) ->
    ReadShallowFun = fun () -> read_shallow_internal(User, Channel, Keys) end,
...
read_shallow_internal(User, Channel, [Key | Keys]) ->
    ...
    TableName = atom_table_name(Channel),
    read_shallow_single(Channel, TableName, StartKey, EndKey, KeyLen) ++ read_shallow_internal(User, Channel, Keys).
        ...
        NextKey ->
            case length(imem_datatype:binterm_to_term(NextKey)) =:= KeyLen of
                ...
                true ->
                    ...
                    [skvh_rec_to_map(Row) | read_shallow_single(Channel, TableName, NextKey, EndKey, KeyLen)]
            end
    end.

read_deep(User, Channel, Keys) ->
    ReadDeepFun = fun () -> read_deep_internal(User, Channel, Keys) end,
...
read_deep_internal(User, Channel, [Key | Keys]) ->
    ...
    EndKey = term_key_to_binterm(binterm_to_term_key(Key) ++ <<255>>),
    TableName = atom_table_name(Channel),
    read_deep_single(Channel, TableName, StartKey, EndKey) ++ read_deep_internal(User, Channel, Keys).
...
-spec get_longest_prefix(User :: any(), Channel :: binary(), StartKey :: [list()], EndKey :: [list()]) ->
...
get_longest_prefix(User, Channel, StartKey, EndKey) when is_binary(Channel) ->
    ...
    case
    imem_if_mnesia:return_atomic(
        imem_meta:transaction(
            fun () -> get_longest_prefix(User, {table, atom_table_name(Channel)}, StartKeyEnc, EndKeyEnc) end
        )
    ) of
...
get_longest_prefix(User, {table, Table}, StartKeyEnc, EndKeyEnc) ->
    case imem_meta:next(Table, StartKeyEnc) of
        ...
        NextKeyEnc ->
            if
                NextKeyEnc == EndKeyEnc -> NextKeyEnc;
                NextKeyEnc < EndKeyEnc -> get_longest_prefix(User, {table, Table}, NextKeyEnc, EndKeyEnc);
                true -> StartKeyEnc
            end
    end.
...
read(User, Channel, [DecodedKey | Keys]) ->
    ...
    case imem_meta:read(TableName, Key) of
        [ReadResult] -> [skvh_rec_to_map(ReadResult) | read(User, Channel, Keys)];
...
remove(User, Channel, Row, Opts) -> remove_single(User, Channel, Row, Opts).
...
hist_read(User, Channel, [DecodedKey | DecodedKeys]) ->
    ...
    case imem_meta:read(HistTableName, Key) of
        [HistResult] -> [skvh_hist_to_map(HistResult) | hist_read(User, Channel, DecodedKeys)];
...
prune_history(User, Channel) ->
    ...
    PruneFun =
        fun
            (#{ckey := Key, cvalue := Value}, _) ->
                EKey = imem_datatype:term_to_binterm(Key),
                {HistTime, OValue, NValue, CUser} =
                    case hist_read(User, Channel, [Key]) of
                ...
                imem_meta:write(
                    ...
                    #skvhHist{
                        ckey = EKey,
                        cvhist = [#skvhCL{time = HistTime, ovalue = OValue, nvalue = NValue, cuser = CUser}]
                    }
                )
        end,
...
purge_history_tables([HistTable | HistTables]) ->
    ...
    DateBefore = calendar:gregorian_days_to_date(calendar:date_to_gregorian_days(date()) - Days),
...
purge_history_table(#skvhHist{cvhist = Hist} = Rec, {DateBefore, HistTable}) when length(Hist) >= 3 ->
    case purge_hist_rows(DateBefore, Hist) of
        Hist -> no_op;
        PurgedHist -> imem_meta:write(HistTable, Rec#skvhHist{cvhist = PurgedHist})
    end,
    {DateBefore, HistTable};
purge_history_table(_, Acc) -> Acc.

purge_hist_rows(DateBefore, [First | Hists]) -> purge_hist_rows(DateBefore, Hists, [First]).

purge_hist_rows(_DateBefore, [Last], Acc) -> lists:reverse([Last | Acc]);
purge_hist_rows(DateBefore, [Hist | Hists], Acc) ->
    NewAcc =
        case imem_datatype:timestamp_to_local_datetime(Hist#skvhCL.time) of
...
prune_cvhist([#{nvalue := V, ovalue := V} | Rest], Value, Time, User) -> prune_cvhist(Rest, Value, Time, User);
prune_cvhist([#{nvalue := NVal, ovalue := OVal, time := HistTime, cuser := CUser} | _], NVal, _, _) ->
    {HistTime, OVal, NVal, CUser};
prune_cvhist([#{nvalue := NVal} | _], Value, Time, User) -> {Time, NVal, Value, User}.
...
audit_readGT(User, AuditAliasAtom, TimeStamp, Limit) when is_binary(TimeStamp) ->
    audit_readGT(User, AuditAliasAtom, imem_datatype:io_to_timestamp(TimeStamp), Limit);
audit_readGT(User, AuditAliasAtom, TimeStamp, Limit) when is_atom(AuditAliasAtom), is_tuple(TimeStamp) ->
    audit_readGT(User, audit_partitions(AuditAliasAtom, TimeStamp), TimeStamp, Limit);
audit_readGT(_User, AuditPartitions, TimeStamp, Limit) when is_list(AuditPartitions), is_tuple(TimeStamp) ->
    audit_part_readGT(
        AuditPartitions,
        {#skvhAudit{time = '$1', _ = '_'}, [{'>', '$1', match_val(TimeStamp)}], ['$_']},
        Limit
    ).
...
audit_read_limited(Cmd, _, Item, _MatchFunction, 0, Acc) ->
    %% enough audits collected
    audit_project_result(Cmd, Acc, Item);
...
project_result(Cmd, L, <<"key">>) -> return_stringlist(Cmd, [term_key_to_io(K) || #skvhTable{ckey = K} <- L]);
project_result(Cmd, L, <<"value">>) -> return_stringlist(Cmd, [term_value_to_io(V) || #skvhTable{cvalue = V} <- L]);
project_result(Cmd, L, <<"hash">>) -> return_stringlist(Cmd, [term_hash_to_io(H) || #skvhTable{chash = H} <- L]);
...
audit_project_result(Cmd, L, <<"time">>) -> return_stringlist(Cmd, [term_time_to_io(T) || #skvhAudit{time = T} <- L]);
audit_project_result(Cmd, L, <<"key">>) -> return_stringlist(Cmd, [term_key_to_io(K) || #skvhAudit{ckey = K} <- L]);
audit_project_result(Cmd, L, <<"value">>) ->
    return_stringlist(Cmd, [term_value_to_io(V) || #skvhAudit{nvalue = V} <- L]);
audit_project_result(Cmd, L, <<"new">>) -> return_stringlist(Cmd, [term_value_to_io(V) || #skvhAudit{nvalue = V} <- L]);
audit_project_result(Cmd, L, <<"old">>) -> return_stringlist(Cmd, [term_value_to_io(V) || #skvhAudit{ovalue = V} <- L]);
audit_project_result(Cmd, L, <<"user">>) -> return_stringlist(Cmd, [term_user_to_io(U) || #skvhAudit{cuser = U} <- L]);
...
audit_project_result(Cmd, L, <<"tkvquadruple">>) ->
    return_stringlist(
        Cmd,
        [term_tkv_triple_to_io({T, K, O, N}) || #skvhAudit{time = T, ckey = K, ovalue = O, nvalue = N} <- L]
    );
audit_project_result(Cmd, L, <<"tkvuquadruple">>) ->
    return_stringlist(
        Cmd,
        [term_tkvu_quadruple_to_io({T, K, V, U}) || #skvhAudit{time = T, ckey = K, nvalue = V, cuser = U} <- L]
    );
audit_project_result(Cmd, L, <<"tkvuquintuple">>) ->
    return_stringlist(
        ...
        [
            term_tkvu_quintuple_to_io({T, K, O, N, U})
            || #skvhAudit{time = T, ckey = K, ovalue = O, nvalue = N, cuser = U} <- L
        ]
    ).
...
debug(Cmd, Resp) -> lager:debug([{cmd, hd(Cmd)}], "PROVISIONING EVENT for ~p ~p returns ~p", [hd(Cmd), tl(Cmd), Resp]).
