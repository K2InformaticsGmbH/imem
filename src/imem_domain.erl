...
-record(
    %% imem helper table for domain samples, used in snapshot transforms to anonymize
    ddDomSample,
    {domKey :: term(), domPattern :: term(), domMin :: term(), domMax :: term(), opts :: list()}
).
...
-record(
    ...
    ddDomTrans,
    {domKey :: term(), domTrans :: term(), domPattern :: term(), opts :: list()}
).
...
-export(
    [
        ...
        string_pattern/1,
        %% StrDomain::atom(), ChrDomain::atom(), binstr() -> RandomizedString::string()
        string_random/3
...
string_pattern(B) when is_binary(B) -> string_pattern(unicode:characters_to_list(B, utf8), [], 0).

string_pattern([], Acc, Uni) -> {lists:reverse(Acc), Uni};
string_pattern([32 | Rest], Acc, Uni) ->
    % emit space
    string_pattern(Rest, [32 | Acc], Uni);
...
string_new_random(StrDomain, _ChrDomain, <<>>) ->
    ok =
        imem_meta:write(
            ddDomTrans,
            #ddDomTrans{domKey = {StrDomain, <<>>}, domTrans = {StrDomain, <<>>}, domPattern = [], opts = []}
        ),
    <<>>;
string_new_random(StrDomain, ChrDomain, BStr) ->
    ...
    ok =
        imem_meta:write(
            ddDomTrans,
            #ddDomTrans{domKey = {StrDomain, BStr}, domTrans = {StrDomain, Res}, domPattern = DP, opts = Opts}
        ),
    Res.

string_random_chars(StrDomain, ChrDomain, Opts, DP) -> string_random_chars(StrDomain, ChrDomain, Opts, DP, []).
