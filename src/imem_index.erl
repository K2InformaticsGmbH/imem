...
-export(
    [
        %% identity transformation, no change of value
        vnf_identity/1,
        %% lower case ascii, allow empty strings
        vnf_lcase_ascii/1,
        %% lower case ascii non-empty
        vnf_lcase_ascii_ne/1,
        %% lower case asccii, tokenized, empty binaries removed (words)
        vnf_tokens/1,
        %% accept integers (convert if necessary) return ?nav on failure
        vnf_integer/1,
        %% accept floats (convert if necessary) return ?nav on failure
        vnf_float/1,
        %% accept Date as string in JSON format converts to erlang datetime
        vnf_datetime/1,
        %% accept Date as a non-empty string in JSON format converts to erlang datetime
        vnf_datetime_ne/1
    ]
).
-export([vnf_binterm_list_1/2, vnf_binterm_list_2/2, vnf_binterm_list_3/2, vnf_binterm_list_4/2, vnf_binterm_list_5/2]).
...
-export(
    [
        iff_true/1,
        %% true for binstrings representing a list
        iff_binterm_list/1,
        %% true for binstrings representing a list with one element (root object type)
        iff_binterm_list_1/1,
        %% used in generated iff
        iff_list_pattern/2
    ]
).
-export(
    [
        %% used to generate iff fun from key pattern
        gen_iff_binterm_list_pattern/1,
        gen_iff_binterm_list_patterns/1
    ]
).
-export(
    [
        %% (IndexTable,ID,Type,SearchStrategies,SearchTerm,Limit,Iff,Vnf) -> [{Strategy,Key,Value,Stu}]
        preview/8,
        %% (IndexTable,ID,Type,SearchStrategies,SearchTerm,Limit,Iff,Vnf,Cont) -> [{Strategy,Key,Value,Stu}]
        preview/9,
        %% (IndexTable,{ID,...},SearchTerm) -> [{Strategy,Key,Value,Stu}]
        preview/3,
        %% (IndexTable,{ID,Limit,Type,SearchStrategies},SearchTerm) -> [Key] unique keys, sorted by match occurence
        preview_keys/3
    ]
).
-export(
    [
        ...
        binstr_only_valid/1,
        binstr_only_valid/2,
        binstr_only_latin1/1,
        binstr_only_latin1/2,
...
-define(HASH_RANGES, [255, 2147483647, 4294967295]).

%% giving 8(3)/27(6)/32(8) bit/(byte) hashes
...
remove(IndexTable, [{ID, iv_h, Key, Value} | Items]) ->
    case imem_if_mnesia:read(IndexTable, {ID, Value}) of
        ...
        [#ddIndex{lnk = Hash}] -> imem_if_mnesia:delete(IndexTable, {ID, Hash, Key})
    end,
    remove(IndexTable, Items);
...
lookup(IndexTable, Stu) ->
    {Keys, true} = imem_if_mnesia:select(IndexTable, [{#ddIndex{stu = Stu, lnk = '$1'}, [], ['$1']}]),
    lists:merge(Keys).
...
new_hash(Value, IndexTable, ID) -> new_hash(Value, IndexTable, ID, ?HASH_RANGES).

new_hash(Value, IndexTable, ID, []) -> ?SystemException({"Cannot create hash", {IndexTable, ID, Value}});
...
vnf_lcase_ascii(B) when is_binary(B) ->
    ...
    [binstr_only_ascii(binstr_accentfold(binstr_to_lower(B)))];
vnf_lcase_ascii(Val) ->
    % unicode_string_only_ascii(io_lib:format("~p",[Val])).
    BinStr = try io_lib:format("~s", [Val]) catch error : badarg -> io_lib:format("~p", [Val]) end,
    [binstr_only_ascii(binstr_accentfold(binstr_to_lower(unicode:characters_to_binary(BinStr))))].

vnf_tokens(Val) -> lists:usort(binary:split(vnf_lcase_ascii(Val), ?TOKENIZERS, [global])) -- [<<>>].
...
vnf_datetime(B) -> try imem_datatype:io_to_datetime(B) of D -> [D] catch _T:_E -> [?nav] end.
...
%% return true without looking at Key or Value
...
iff_list_pattern(Key, Key) ->
    %% includes [],[]
    true;
iff_list_pattern(_, ['*']) ->
    %% Key matches zero or more list elements
    true;
...
gen_iff_binterm_list_patterns(Patterns) -> fun ({Key, _}) -> iff_binterm_list_patterns(Key, Patterns) end.

iff_binterm_list_patterns(_Key, []) -> false;
iff_binterm_list_patterns(Key, [Pattern | Patterns]) ->
    imem_index:iff_list_pattern(imem_datatype:binterm_to_term(Key), Pattern)
    orelse iff_binterm_list_patterns(Key, Patterns).
...
preview(IndexTable, ID, SearchTerm) when is_integer(ID) -> preview(IndexTable, [ID, 10], SearchTerm);
preview(IndexTable, [ID], SearchTerm) -> preview(IndexTable, [ID, 10], SearchTerm);
preview(IndexTable, [ID, Limit], SearchTerm) -> preview(IndexTable, [ID, Limit, ivk], SearchTerm);
...
preview(IndexTable, ID, Type, _SearchStrategies, {RangeStart, RangeEnd}, Limit, Iff, Vnf, FromStu) ->
    case {Vnf(RangeStart), Vnf(RangeEnd)} of
        ...
        {[?nav], _} -> [];
        {_, [?nav]} -> [];
        {[NormStart], [NormEnd]} -> preview_range_init(IndexTable, ID, Type, {NormStart, NormEnd}, Limit, Iff, FromStu)
    end;
preview(IndexTable, ID, Type, SearchStrategies, SearchTerm, Limit, Iff, Vnf, FromStu) ->
    case Vnf(SearchTerm) of
        ...
        [NormalizedTerm | _] ->
            case is_regexp_search(SearchStrategies, NormalizedTerm) of
                true -> preview_regexp_init(IndexTable, ID, Type, NormalizedTerm, Limit, Iff, FromStu);
                false ->
...
preview_regexp(IndexTable, ID, iv_k, Pattern, {ID, Value} = Stu, Limit, Iff) ->
    Partial =
        case imem_if_mnesia:read(IndexTable, Stu) of
            [#ddIndex{stu = {ID, Value}, lnk = Key} = Entry] ->
                case imem_sql_funs:re_match(Pattern, Value) andalso Iff({Key, Value}) of
                    true -> [build_result_entry(Entry#ddIndex.stu, re_match, Key, Value)];
                    false -> []
                end;
            _ -> []
        end,
    case imem_if_mnesia:next(IndexTable, Stu) of
        ...
        {ID, _NextValue} = NextStu -> Partial ++ preview_regexp(IndexTable, ID, iv_k, Pattern, NextStu, Limit - 1, Iff);
        _ -> Partial
    end;
        ...
        case imem_if_mnesia:read(IndexTable, Stu) of
            [#ddIndex{stu = {ID, Value}, lnk = KeyList} = Entry] ->
                case imem_sql_funs:re_match(Pattern, Value) of
                    true -> preview_expand_kl(re_match, Entry#ddIndex.stu, KeyList, Value, Iff);
                    false -> []
                end;
            _ -> []
        end,
...
preview_regexp(IndexTable, ID, iv_h, Pattern, {ID, Value} = Stu, Limit, Iff) ->
    Partial =
        case imem_if_mnesia:read(IndexTable, Stu) of
            [#ddIndex{stu = {ID, Value}, lnk = Hash}] ->
                case imem_sql_funs:re_match(Pattern, Value) of
                    true -> preview_expand_hash(re_match, IndexTable, ID, Hash, Value, ?SMALLEST_TERM, Iff);
                    false -> []
                end;
            _ -> []
        end,
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, _NextValue} = NextStu -> Partial ++ preview_regexp(IndexTable, ID, iv_h, Pattern, NextStu, Limit - 1, Iff);
        _ -> Partial
    end.
...
preview_range_init(IndexTable, ID, Type, SearchTerm, Limit, Iff, FromStu) ->
    ...
    imem_if_mnesia:return_atomic(
        imem_if_mnesia:transaction(
            fun () -> preview_range(IndexTable, ID, Type, SearchTerm, StartingStu, Limit, Iff) end
        )
    ).
    ...
    case imem_if_mnesia:next(IndexTable, PrevStu) of
        ...
        {ID, Value, Key} = Stu when Value >= RangeStart, Value =< RangeEnd ->
            case Iff({Key, Value}) of
                ...
                false -> preview_range_unique(IndexTable, ID, RangeStart, RangeEnd, Stu, Limit, Iff, Acc)
            end;
        _ -> Acc
    end.
...
preview_range(IndexTable, ID, iv_k, {RangeStart, RangeEnd} = SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_k, IndexTable, range_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, NextValue} = NextStu when NextValue >= RangeStart, NextValue =< RangeEnd ->
            Partial ++ preview_range(IndexTable, ID, iv_k, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_range(IndexTable, ID, iv_kl, {RangeStart, RangeEnd} = SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_kl, IndexTable, range_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, NextValue} = NextStu when NextValue >= RangeStart, NextValue =< RangeEnd ->
            Partial ++ preview_range(IndexTable, ID, iv_kl, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_range(IndexTable, ID, iv_h, {RangeStart, RangeEnd} = SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_h, IndexTable, range_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, NextValue} = NextStu when NextValue >= RangeStart, NextValue =< RangeEnd ->
            Partial ++ preview_range(IndexTable, ID, iv_h, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end.
...
preview_execute(IndexTable, ID, Type, [exact_match | SearchStrategies], SearchTerm, Limit, Iff, undefined, FromStu) ->
    ResultExact =
        imem_if_mnesia:return_atomic(
            imem_if_mnesia:transaction(
                fun () -> preview_exact(IndexTable, ID, Type, SearchTerm, ?SMALLEST_TERM, Limit, Iff, FromStu) end
            )
        ),
    ResultExact
    ++
...
preview_execute(IndexTable, ID, Type, [head_match | SearchStrategies], SearchTerm, Limit, Iff, PrevStrategy, FromStu) ->
    ...
    ResultHead =
        imem_if_mnesia:return_atomic(
            imem_if_mnesia:transaction(
                fun () -> preview_head(IndexTable, ID, Type, SearchTerm, StartingStu, Limit, IffAndNotAdded) end
            )
        ),
...
preview_execute(IndexTable, ID, Type, [body_match | _SearchStrategies], SearchTerm, Limit, Iff, PrevStrategy, FromStu) ->
    ...
    ResultBody =
        imem_if_mnesia:return_atomic(
            imem_if_mnesia:transaction(
                fun () -> preview_body(IndexTable, ID, Type, SearchTerm, StartingStu, Limit, IffNotAdded) end
            )
        ),
    ...
    case imem_if_mnesia:next(IndexTable, {ID, SearchTerm, Key}) of
        ...
        {ID, SearchTerm, NextKey} = Stu ->
            case Iff({NextKey, SearchTerm}) of
                true ->
                    NewLimit =
                        case lists:keyfind(SearchTerm, 3, Acc) of
                            false -> Limit - 1;
                            _ -> Limit
                        end,
                ...
                false -> preview_exact_unique(IndexTable, ID, SearchTerm, NextKey, Limit, Iff, Acc)
            end;
        _ -> Acc
    end.
    ...
    case imem_if_mnesia:read(IndexTable, {ID, SearchTerm}) of
        [#ddIndex{stu = {ID, SearchTerm}, lnk = Key} = Entry] ->
            case Iff({Key, SearchTerm}) of
                true -> [build_result_entry(Entry#ddIndex.stu, exact_match, Key, SearchTerm)];
                false -> []
            end;
        _ -> []
    end;
    ...
    case imem_if_mnesia:next(IndexTable, PrevStu) of
        ...
        {ID, <<SearchTerm:SizeSearch/binary, _/binary>> = Value, Key} = Stu ->
            case Iff({Key, Value}) of
                ...
                false -> preview_exact_unique(IndexTable, ID, SearchTerm, Stu, Limit, Iff, Acc)
            end;
        _ -> Acc
    end.
...
preview_head(IndexTable, ID, iv_k, SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_k, IndexTable, head_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    SizeSearch = size(SearchTerm),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, <<SearchTerm:SizeSearch/binary, _/binary>>} = NextStu ->
            Partial ++ preview_head(IndexTable, ID, iv_k, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_head(IndexTable, ID, iv_kl, SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_kl, IndexTable, head_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    SizeSearch = size(SearchTerm),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, <<SearchTerm:SizeSearch/binary, _/binary>>} = NextStu ->
            Partial ++ preview_head(IndexTable, ID, iv_kl, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_head(IndexTable, ID, iv_h, SearchTerm, Stu, Limit, Iff) ->
    {NewLimit, Partial} =
        get_partial(iv_h, IndexTable, head_match, Stu, imem_if_mnesia:read(IndexTable, Stu), Iff, Limit),
    SizeSearch = size(SearchTerm),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, <<SearchTerm:SizeSearch/binary, _/binary>>} = NextStu ->
            Partial ++ preview_head(IndexTable, ID, iv_h, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end.
    ...
    case imem_if_mnesia:next(IndexTable, PrevStu) of
        ...
        {ID, Value, Key} = Stu ->
            case binary:match(Value, SearchTerm) =/= nomatch andalso Iff({Key, Value}) of
                ...
                false -> preview_body_unique(IndexTable, ID, SearchTerm, Stu, Limit, Iff, Acc)
            end;
        _ -> Acc
    end.
...
preview_body(IndexTable, ID, iv_k, SearchTerm, Stu, Limit, Iff) ->
    NewIff = fun ({Key, Value}) -> binary:match(Value, SearchTerm) =/= nomatch andalso Iff({Key, Value}) end,
    {NewLimit, Partial} =
        get_partial(iv_k, IndexTable, body_match, Stu, imem_if_mnesia:read(IndexTable, Stu), NewIff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, _NextValue} = NextStu -> Partial ++ preview_body(IndexTable, ID, iv_k, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_body(IndexTable, ID, iv_kl, SearchTerm, Stu, Limit, Iff) ->
    NewIff = fun ({Key, Value}) -> binary:match(Value, SearchTerm) =/= nomatch andalso Iff({Key, Value}) end,
    {NewLimit, Partial} =
        get_partial(iv_kl, IndexTable, body_match, Stu, imem_if_mnesia:read(IndexTable, Stu), NewIff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, _NextValue} = NextStu ->
            Partial ++ preview_body(IndexTable, ID, iv_kl, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end;
preview_body(IndexTable, ID, iv_h, SearchTerm, Stu, Limit, Iff) ->
    NewIff = fun ({Key, Value}) -> binary:match(Value, SearchTerm) =/= nomatch andalso Iff({Key, Value}) end,
    {NewLimit, Partial} =
        get_partial(iv_h, IndexTable, body_match, Stu, imem_if_mnesia:read(IndexTable, Stu), NewIff, Limit),
    case imem_if_mnesia:next(IndexTable, Stu) of
        '$end_of_table' -> Partial;
        {ID, _NextValue} = NextStu -> Partial ++ preview_body(IndexTable, ID, iv_h, SearchTerm, NextStu, NewLimit, Iff);
        _ -> Partial
    end.
...
preview_expand_kl(Type, Stu, [Key | KeyList], SearchTerm, Iff) ->
    case Iff({Key, SearchTerm}) of
        true ->
            [build_result_entry(Stu, Type, Key, SearchTerm) | preview_expand_kl(Type, Stu, KeyList, SearchTerm, Iff)];
        false -> preview_expand_kl(Type, Stu, KeyList, SearchTerm, Iff)
    end.
    ...
    case imem_if_mnesia:next(IndexTable, {ID, Hash, Key}) of
        ...
        {ID, Hash, NextKey} = Stu ->
            case Iff({NextKey, SearchTerm}) of
                ...
                false -> preview_expand_hash(Type, IndexTable, ID, Hash, SearchTerm, NextKey, Iff)
            end;
        _ -> []
    end.

-spec build_result_entry(integer(), atom(), binary(), term()) ->
    {atom(), binary(), term(), {integer(), term(), binary()}}.
...
add_filter_duplicated(exact_match, SearchTerm, Iff) ->
    fun ({Key, Value}) -> Value =/= SearchTerm andalso Iff({Key, Value}) end;
...
-spec get_partial(atom(), atom(), atom(), integer(), list, fun(), integer()) -> {integer(), list()}.
get_partial(iv_k, _IndexTable, MatchType, {Id, Value}, [#ddIndex{stu = {Id, Value}, lnk = Key} = Entry], Iff, Limit) ->
    case Iff({Key, Value}) of
        true -> {Limit - 1, [build_result_entry(Entry#ddIndex.stu, MatchType, Key, Value)]};
        false -> {Limit, []}
    end;
get_partial(
    iv_kl,
    ...
    case preview_expand_kl(MatchType, Entry#ddIndex.stu, KeyList, Value, Iff) of
        [] -> {Limit, []};
        Partial -> {Limit - 1, Partial}
    end;
get_partial(iv_h, IndexTable, MatchType, {Id, Value}, [#ddIndex{stu = {Id, Value}, lnk = Hash}], Iff, Limit) ->
    case preview_expand_hash(MatchType, IndexTable, Id, Hash, Value, ?SMALLEST_TERM, Iff) of
        [] -> {Limit, []};
        Partial -> {Limit - 1, Partial}
    end;
get_partial(_IndexType, _IndexTable, _MatchType, _Id, _ReadResult, _Iff, Limit) -> {Limit, []}.
...
index_type({}) ->
    %% parser's default type today, ToDo: remove when parser returns 'undefined'
    ivk;
index_type(A) when is_atom(A) -> index_type(list_to_binary(atom_to_list(A)));
index_type(<<"undefined">>) ->
    %% ToDo: default type 'undefined' should come from the parser
    ivk;
index_type(<<"unique">>) -> iv_k;
index_type(<<"keylist">>) -> iv_kl;
index_type(<<"hashmap">>) -> iv_h;
index_type(<<"bintree">>) ->
    %% might come from the parser in the future
    ivk;
...
binstr_accentfold(Str) when is_binary(Str) -> b_convert(Str, <<>>).

b_convert(<<>>, A) -> A;
b_convert(<<195, C, R/binary>>, A) when C >= 128, C =< 133 ->
    % À Á Â Ã Ä Å
    b_convert(R, <<A/binary, $A>>);
b_convert(<<195, C, R/binary>>, A) when C >= 136, C =< 139 ->
    % È É Ê Ë
    b_convert(R, <<A/binary, $E>>);
b_convert(<<195, C, R/binary>>, A) when C >= 140, C =< 143 ->
    % Ì Í Î Ï
    b_convert(R, <<A/binary, $I>>);
b_convert(<<195, C, R/binary>>, A) when C >= 146, C =< 150; C =:= 152 ->
    % Ò Ó Ô Õ Ö Ø
    b_convert(R, <<A/binary, $O>>);
b_convert(<<195, C, R/binary>>, A) when C >= 153, C =< 156 ->
    % Ù Ú Û Ü
    b_convert(R, <<A/binary, $U>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 157 ->
    % Ý
    b_convert(R, <<A/binary, $Y>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 184 ->
    % CAPITAL LETTER Y WITH DIAERESIS
    b_convert(R, <<A/binary, $Y>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 135 ->
    % Ç
    b_convert(R, <<A/binary, $C>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 145 ->
    % Ñ
    b_convert(R, <<A/binary, $N>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 134 ->
    % Æ -> AE
    b_convert(R, <<A/binary, $A, $E>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 158 ->
    % Þ -> TH
    b_convert(R, <<A/binary, $T, $H>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 144 ->
    % Ð -> D
    b_convert(R, <<A/binary, $D>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 160 ->
    % S WITH CARON
    b_convert(R, <<A/binary, $S>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 189 ->
    % Z WITH CARON
    b_convert(R, <<A/binary, $Z>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 146 ->
    % OE LIGATURE -> OE
...
b_convert(<<195, C, R/binary>>, A) when C >= 172, C =< 175 ->
    % ì í î ï
    b_convert(R, <<A/binary, $i>>);
b_convert(<<195, C, R/binary>>, A) when C >= 178, C =< 182; C =:= 184 ->
    % ò ó ô õ ö ø
    b_convert(R, <<A/binary, $o>>);
...
b_convert(<<195, C, R/binary>>, A) when C =:= 177 ->
    % ñ
    b_convert(R, <<A/binary, $n>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 161 ->
    % s WITH CARON
    b_convert(R, <<A/binary, $s>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 190 ->
    % z WITH CARON
    b_convert(R, <<A/binary, $z>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 159 ->
    % ß -> ss
    b_convert(R, <<A/binary, $s, $s>>);
b_convert(<<197, C, R/binary>>, A) when C =:= 147 ->
    % oe LIGATURE -> oe
    b_convert(R, <<A/binary, $o, $e>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 190 ->
    % þ -> th
    b_convert(R, <<A/binary, $t, $h>>);
b_convert(<<195, C, R/binary>>, A) when C =:= 176 ->
    % ð -> d
    b_convert(R, <<A/binary, $d>>);
b_convert(<<H, R/binary>>, A) -> b_convert(R, <<A/binary, H>>).

%% @doc lowercases all alphabetical characters supported by ISO 8859-15

-spec binstr_to_lower(binary()) -> binary().
binstr_to_lower(Str) when is_binary(Str) -> b_lower(Str, <<>>).

b_lower(<<>>, A) -> A;
b_lower(<<195, C, R/binary>>, A) when C >= 128, C =< 150; C >= 152, C =< 158 ->
    % Standard range
    NC = C + 32,
    b_lower(R, <<A/binary, 195, NC>>);
b_lower(<<197, C, R/binary>>, A) when C =:= 160; C =:= 146; C =:= 189 ->
    % Non-standard range
    NC = C + 1,
    b_lower(R, <<A/binary, 197, NC>>);
b_lower(<<197, C, R/binary>>, A) when C =:= 184 ->
    % Special case CAPITAL LETTER Y WITH DIAERESIS
    b_lower(R, <<A/binary, 195, 191>>);
b_lower(<<C, R/binary>>, A) when C >= 65, C =< 90 ->
    NC = C + 32,
    b_lower(R, <<A/binary, NC>>);
b_lower(<<H, R/binary>>, A) -> b_lower(R, <<A/binary, H>>).

%% @doc uppercases all alphabetical characters supported by ISO 8859-15

-spec binstr_to_upper(binary()) -> binary().
binstr_to_upper(Str) when is_binary(Str) -> b_upper(Str, <<>>).

b_upper(<<>>, A) -> A;
b_upper(<<195, C, R/binary>>, A) when C >= 160, C =< 182; C >= 184, C =< 190 ->
    % Standard range
    NC = C - 32,
    b_upper(R, <<A/binary, 195, NC>>);
b_upper(<<197, C, R/binary>>, A) when C =:= 161; C =:= 147; C =:= 190 ->
    % Non-standard range
    NC = C - 1,
    b_upper(R, <<A/binary, 197, NC>>);
b_upper(<<195, C, R/binary>>, A) when C =:= 191 ->
    % Special case CAPITAL LETTER Y WITH DIAERESIS
    b_upper(R, <<A/binary, 197, 184>>);
b_upper(<<C, R/binary>>, A) when C >= 97, C =< 122 ->
    NC = C - 32,
    b_upper(R, <<A/binary, NC>>);
b_upper(<<H, R/binary>>, A) -> b_upper(R, <<A/binary, H>>).

%% @doc Walks binary string, keeps only valid ascii characters

-spec binstr_only_ascii(binary()) -> binary().
binstr_only_ascii(BinStr) when is_binary(BinStr) -> binstr_only_ascii(BinStr, "");
binstr_only_ascii(BinStr) when is_list(BinStr) -> binstr_only_ascii(list_to_binary(BinStr), "").

binstr_only_ascii(BinStr, PlaceHolder) -> b_afilter(BinStr, PlaceHolder, <<>>).

b_afilter(<<>>, _, A) -> A;
% Unprintable characters:0 - 9, 11, 12, 14 - 31, 127
b_afilter(<<C, R/binary>>, PH, A) when C >= 0, C =< 9; C =:= 11; C =:= 12; C >= 14, C =< 31; C >= 127 ->
    b_vfilter(R, PH, ?BIN_APP(A, PH));
b_afilter(<<C, R/binary>>, PH, A) -> b_afilter(R, PH, <<A/binary, C>>).

%% @doc Walks binary string, keeps only valid and displayable) utf8 characters

-spec binstr_only_valid(binary()) -> binary().
binstr_only_valid(Binstr) -> binstr_only_valid(Binstr, "").

binstr_only_valid(BinStr, PH) when is_binary(BinStr) -> b_vfilter(BinStr, PH, <<>>).
...
b_vfilter(<<M, C, R/binary>>, PH, A) when M >= 195, M =< 223, C >= 128, C =< 191; M >= 194, C >= 160, C =< 191 ->
    b_vfilter(R, PH, <<A/binary, M, C>>);
% Three-byte UTF8 224-239, 128-191, 128-191
b_vfilter(<<M, C1, C2, R/binary>>, PH, A) when M >= 224, M =< 239, C1 >= 128, C1 =< 191, C2 >= 128, C2 =< 191 ->
    b_vfilter(R, PH, <<A/binary, M, C1, C2>>);
% Four-byte UTF8 240-247, 128-191, 128-191, 128-191
b_vfilter(<<M, C1, C2, C3, R/binary>>, PH, A)
when M >= 240, M =< 247, C1 >= 128, C1 =< 191, C2 >= 128, C2 =< 191, C3 >= 128, C3 =< 191 ->
    b_vfilter(R, PH, <<A/binary, M, C1, C2, C3>>);
% Five-byte UTF8 248-251, 128-191, 128-191, 128-191, 128-191
b_vfilter(<<M, C1, C2, C3, C4, R/binary>>, PH, A)
when M >= 248, M =< 251, C1 >= 128, C1 =< 191, C2 >= 128, C2 =< 191, C3 >= 128, C3 =< 191, C4 >= 128, C4 =< 191 ->
    b_vfilter(R, PH, <<A/binary, M, C1, C2, C3, C4>>);
% Six-byte UTF8 252-253, 128-191, 128-191, 128-191, 128-191, 128-191,
b_vfilter(<<M, C1, C2, C3, C4, C5, R/binary>>, PH, A)
when M >= 252,
     M =< 253,
     C1 >= 128,
     C1 =< 191,
...
b_vfilter(<<_, R/binary>>, PH, A) -> b_vfilter(R, PH, ?BIN_APP(A, PH)).

%% @doc Walks binary string, keeps only valid and displayable) utf8 characters
%% also present in the latin1 characterset

-spec binstr_only_latin1(binary()) -> binary().
binstr_only_latin1(Binstr) -> binstr_only_latin1(Binstr, "").

binstr_only_latin1(BinStr, PH) when is_binary(BinStr) -> b_lfilter(BinStr, PH, <<>>).
...
b_lfilter(<<M, C, R/binary>>, PH, A) when M >= 195, M =< 197, C >= 128, C =< 191; M >= 194, C >= 160, C =< 191 ->
    b_lfilter(R, PH, <<A/binary, M, C>>);
% Everything else (garbage)
b_lfilter(<<_, R/binary>>, PH, A) -> b_lfilter(R, PH, ?BIN_APP(A, PH)).
...
binstr_match_precompile(Pattern) -> binary:compile_pattern(Pattern).
...
string_test_() ->
    {setup, fun setup/0, fun teardown/1, {with, [fun string_operations/1, fun iff_functions/1, fun prune_list_test/1]}}.
    ...
    [
        ...
        {"from integer", ?_assertEqual([<<"12798">>], vnf_lcase_ascii(12798))},
        {
            ...
            ?_assertEqual(
                [<<"g:xr*a\\6r">>],
                vnf_lcase_ascii(
                    <<71, 191, 58, 192, 88, 82, 194, 42, 223, 65, 187, 19, 92, 145, 228, 248, 26, 54, 196, 114>>
                )
            )
        }
    ].
...
binstr_only_valid_test_() ->
    ...
    [
        ...
        {"random", ?_assertEqual(<<"J^f^BWm\rk$D:A)OX!ß³V7|p\"%Ç°#s)h0\\">>, binstr_only_valid(Random))}
    ].
...
binstr_only_latin1_test_() ->
    ...
    [
        ...
        {"random", ?_assertEqual(<<"J^f^BWm\rk$D:A)OX!ß³V7|p\"%Ç°#s)h0\\">>, binstr_only_latin1(Random))}
    ].
