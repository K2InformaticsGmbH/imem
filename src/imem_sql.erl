...
-export([exec/5, parse/1, prune_fields/2]).
-export(
    [
        escape_sql/1,
        un_escape_sql/1,
...
statement_class(TableAlias) ->
    case
    {
        imem_meta:is_time_partitioned_alias(TableAlias),
        imem_meta:is_node_sharded_alias(TableAlias),
        imem_meta:is_local_alias(TableAlias)
    } of
        {false, false, false} ->
            % remote query of a simple table/partition on another node
            "R";
        {false, false, true} ->
...
parse_sql_name([], _, Temp, Acc) -> lists:reverse([lists:reverse(Temp) | Acc]);
parse_sql_name([$. | Rest], L, Temp, Acc) when L rem 2 == 0 -> parse_sql_name(Rest, L, [], [lists:reverse(Temp) | Acc]);
parse_sql_name([$" | Rest], L, Temp, Acc) -> parse_sql_name(Rest, L + 1, [$" | Temp], Acc);
parse_sql_name([Ch | Rest], L, Temp, Acc) -> parse_sql_name(Rest, L, [Ch | Temp], Acc).
...
prune_fields(InFields, ParseTree) -> imem_prune_fields:match(ParseTree, InFields).
...
exec(SKey, select, ParseTree, Stmt, Opts, IsSec) -> imem_sql_select:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, union, ParseTree, Stmt, Opts, IsSec) -> imem_sql_select:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'union all', ParseTree, Stmt, Opts, IsSec) -> imem_sql_select:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, minus, ParseTree, Stmt, Opts, IsSec) -> imem_sql_select:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, intersect, ParseTree, Stmt, Opts, IsSec) -> imem_sql_select:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, insert, ParseTree, Stmt, Opts, IsSec) -> imem_sql_insert:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'create user', ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'alter user', ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'drop user', ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'create role', ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'drop role', ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, grant, ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, revoke, ParseTree, Stmt, Opts, IsSec) -> imem_sql_account:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'create table', ParseTree, Stmt, Opts, IsSec) -> imem_sql_table:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'drop table', ParseTree, Stmt, Opts, IsSec) -> imem_sql_table:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'truncate table', ParseTree, Stmt, Opts, IsSec) -> imem_sql_table:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'create index', ParseTree, Stmt, Opts, IsSec) -> imem_sql_index:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, 'drop index', ParseTree, Stmt, Opts, IsSec) -> imem_sql_index:exec(SKey, ParseTree, Stmt, Opts, IsSec);
exec(SKey, Command, ParseTree, _Stmt, _Opts, _IsSec) ->
    ?UnimplementedException({"SQL command unimplemented", {SKey, Command, ParseTree}}).

escape_sql(Str) when is_list(Str) -> re:replace(Str, "(')", "''", [global, {return, list}]);
escape_sql(Bin) when is_binary(Bin) -> re:replace(Bin, "(')", "''", [global, {return, binary}]).

un_escape_sql(Str) when is_list(Str) -> re:replace(Str, "('')", "'", [global, {return, list}]);
un_escape_sql(Bin) when is_binary(Bin) -> re:replace(Bin, "('')", "'", [global, {return, binary}]).
...
meta_rec(_, _, [], Params, undefined) ->
    list_to_tuple(
        [imem_datatype:io_to_db(N, undefined, T, 0, P, undefined, false, Value) || {N, T, P, [Value | _]} <- Params]
    );
meta_rec(IsSec, SKey, MetaFields, Params, undefined) ->
...
parse_sql_name_test_() ->
    [
        ...
        {
            "double",
            ?_assertEqual({"\"Schema.1\"", "\"Complex.$@#Name\""}, parse_sql_name("\"Schema.1\".\"Complex.$@#Name\""))
        }
    ].
