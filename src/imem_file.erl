...
-define(DEFAULT_SFTP_OPTS, [{silently_accept_hosts, true}, {user_interaction, false}]).
-define(
    SFTP_CONNECT_TIMEOUT(__HOST, __PORT),
    ?GET_CONFIG(
        sfhSftpConnectTimeout,
        [__HOST, __PORT],
        3000,
...
connect(#{proto := sftp, host := Host, port := Port, user := User, password := Password, opts := Opts} = Ctx) ->
    EOpts =
        ?DEFAULT_SFTP_OPTS ++ [{user, User}, {password, Password}, {timeout, ?SFTP_CONNECT_TIMEOUT(Host, Port)} | Opts],
    case ssh_sftp:start_channel(Host, Port, EOpts) of
        {ok, ChannelPid, ConnectionRef} -> Ctx#{chn_pid => ChannelPid, conn_ref => ConnectionRef};
        {error, _} = Error -> Error
    end;
connect(
    #{
...
connect(#{proto := sftp, key := _, user := _, key_cb_mod := _} = Ctx) -> connect(Ctx#{key_cb_args => #{}});
connect(#{proto := sftp, key := _, user := _} = Ctx) -> connect(Ctx#{key_cb_mod => ?MODULE});
...
-spec list_dir(map(), filename(), time_out()) -> {ok, [filename()]} | {error, error()}.
list_dir(#{proto := sftp, chn_pid := ChannelPid, path := Path}, Dir, Timeout) ->
    ssh_sftp:list_dir(ChannelPid, filename:join(Path, Dir), Timeout);
list_dir(#{path := Path}, Dir, _Timeout) -> file:list_dir(filename:join(Path, Dir)).
...
make_dir(#{path := Path}, Dir, _Timeout) -> file:make_dir(filename:join(Path, Dir)).

-spec open(map(), filename(), modes(), time_out()) -> {ok, handle()} | {error, error()}.
...
open(#{path := Path}, File, Modes, _Timeout) -> file:open(filename:join(Path, File), lists:usort([binary | Modes])).

-spec close(map(), handle(), time_out()) -> ok | {error, error()}.
close(#{proto := sftp, chn_pid := ChannelPid}, Handle, Timeout) -> ssh_sftp:close(ChannelPid, Handle, Timeout);
close(_Ctx, IoDevice, _Timeout) -> file:close(IoDevice).

-spec read(map(), handle(), integer(), time_out()) -> {ok, binary()} | eof | {error, error()}.
read(#{proto := sftp, chn_pid := ChannelPid}, Handle, Len, Timeout) -> ssh_sftp:read(ChannelPid, Handle, Len, Timeout);
read(_Ctx, IoDevice, Len, _Timeout) -> file:read(IoDevice, Len).

-spec pread(map(), handle(), integer(), integer(), time_out()) -> {ok, binary()} | eof | {error, error()}.
pread(#{proto := sftp, chn_pid := ChannelPid}, Handle, Position, Len, Timeout) ->
    ssh_sftp:pread(ChannelPid, Handle, Position, Len, Timeout);
pread(_Ctx, IoDevice, Position, Len, _Timeout) -> file:pread(IoDevice, Position, Len).

-spec read_file(map(), filename(), time_out()) -> {ok, binary()} | {error, error()}.
...
read_file(#{path := Path}, File, _Timeout) -> file:read_file(filename:join(Path, File)).

-spec read_line(map(), handle(), time_out()) -> {ok, binary()} | eof | {error, error()}.
read_line(#{proto := sftp}, _Handle, _Timeout) ->
    %% to be implemented with read and position
    {error, unimplemented};
read_line(_Ctx, IoDevice, _Timeout) -> file:read_line(IoDevice).
...
write(_Ctx, IoDevice, Bytes, _Timeout) -> file:write(IoDevice, Bytes).

-spec write_file(map(), filename(), binary(), time_out()) -> ok | {error, error()}.
write_file(#{proto := sftp, chn_pid := ChannelPid, path := Path}, File, Iolist, Timeout) ->
    ssh_sftp:write_file(ChannelPid, filename:join(Path, File), Iolist, Timeout);
write_file(#{path := Path}, File, Bytes, _Timeout) -> file:write_file(filename:join(Path, File), Bytes).

-spec read_file_info(map(), filename(), time_out()) -> {ok, file:file_info()} | {error, error()}.
...
read_file_info(#{path := Path}, File, _Timeout) -> file:read_file_info(filename:join(Path, File)).
...
is_dir(#{path := Path}, Dir, _Timeout) -> filelib:is_dir(filename:join(Path, Dir)).

-spec rename(map(), filename(), filename(), timeout()) -> ok | {error, error()}.
rename(#{proto := sftp, chn_pid := ChannelPid}, Source, Destination, Timeout) ->
    ssh_sftp:rename(ChannelPid, Source, Destination, Timeout);
rename(_Ctx, Source, Destination, _Timeout) -> file:rename(Source, Destination).
...
delete(#{path := Path}, File, _Timeout) -> file:delete(filename:join(Path, File)).
...
pfx2pem(PfxFile, Password, OutKeyFile, OutCertFile) ->
    ...
    if
        OpenSsl == false -> error("openssl executable is not found");
        true -> ok
    end,
    ...
    log_cmd(
        ...
        erlang:open_port(
            ...
            [{line, 128}, {args, KeyArgs}, exit_status, stderr_to_stdout, {parallelism, true}]
        )
    ),
    case filelib:is_regular(PassKeyFile) of
        true -> ?Debug("extracted private key into ~s from ~s", [PassKeyFile, PfxFile]);
        false -> error("failed to extract private key")
    end,
    ...
    log_cmd(
        ...
        erlang:open_port(
            {spawn_executable, OpenSsl},
            [{line, 128}, {args, PassRemoveArgs}, exit_status, stderr_to_stdout, {parallelism, true}]
        )
    ),
    case filelib:is_regular(OutKeyFile) of
        true -> ?Debug("removed password from private key ~s to ~s", [PassKeyFile, OutKeyFile]);
        false -> error("failed to remove password from private key")
    end,
    case file:delete(PassKeyFile) of
        ok -> ?Debug("deleted private key ~s", [PassKeyFile]);
        {error, Reason} -> error({"failed to delete private key", Reason})
    end,
    ...
    log_cmd(
        ...
        erlang:open_port(
            {spawn_executable, OpenSsl},
            [{line, 128}, {args, CertArgs}, exit_status, stderr_to_stdout, {parallelism, true}]
        )
    ),
...
log_cmd(Cmd, Args, Port, Buf) when is_port(Port) ->
    receive
        ...
        {Port, {exit_status, Status}} ->
            if
                Status == 0 -> ?Debug("~s ~p finished successfully", [Cmd, Args]);
...
connect_test_() ->
    {
        ...
        [
            ...
            <-
            [
                {"cifs_list_str", "//localhost/share", #{proto => cifs, path => "//localhost/share"}},
                {"cifs_bin_str", <<"//localhost/share">>, #{proto => cifs, path => <<"//localhost/share">>}},
                {"cifs_list_str_win_slash", "\\\\localhost\\share", #{proto => cifs, path => "\\\\localhost\\share"}},
                {
                    "cifs_bin_str_win_slash",
                    <<"\\\\localhost\\share">>,
                    #{proto => cifs, path => <<"\\\\localhost\\share">>}
                },
                {"local_list_str_big", "C:/localhost/share", #{proto => local, path => "C:/localhost/share"}},
                {"local_list_str_small", "c:/localhost/share", #{proto => local, path => "c:/localhost/share"}},
                {"local_bin_str_big", <<"C:/localhost/share">>, #{proto => local, path => <<"C:/localhost/share">>}},
                {"local_bin_str_small", <<"c:/localhost/share">>, #{proto => local, path => <<"c:/localhost/share">>}},
                {"unix_bin_str", <<"/home/user/share">>, #{proto => local, path => <<"/home/user/share">>}},
                {"unix_list_str", "/home/user/share", #{proto => local, path => "/home/user/share"}},
