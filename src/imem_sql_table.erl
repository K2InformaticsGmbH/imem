...
exec(_SKey, {'drop table', {tables, Tables}, _Exists, _RestrictCascade}, _Stmt, _Opts, _IsSec) ->
    % old parser compatibility
    exec(_SKey, {'drop table', {tables, Tables}, _Exists, _RestrictCascade, []}, _Stmt, _Opts, _IsSec);
exec(_SKey, {'drop table', {tables, []}, _Exists, _RestrictCascade, _TableTypeOpts}, _Stmt, _Opts, _IsSec) ->
    % typed table drops
    ok;
exec(
    SKey,
    {'drop table', {tables, [TableName | Tables]}, Exists, RestrictCascade, TableTypeOpts} = _ParseTree,
    Stmt,
    Opts,
    IsSec
) ->
...
create_table(SKey, Table, TOpts, [], IsSec, ColMap) ->
    ...
    %% {Schema,Table} not converted to atom yet !!!
...
create_table(SKey, Table, TOpts, [{Name, Type, COpts} | Columns], IsSec, ColMap) when is_binary(Name) ->
    ...
    {T, L, P} =
        case Type of
            ...
            B when is_binary(B) ->
                case
                imem_datatype:imem_type(
                    imem_datatype:io_to_term(imem_datatype:strip_squotes(string:lowercase(binary_to_list(B))))
                ) of
            ...
            {Typ, SLen} ->
                {
                    imem_datatype:imem_type(binary_to_existing_atom(string:lowercase(Typ), utf8)),
                    binary_to_integer(SLen),
                    undefined
                };
            {Typ, SLen, SPrec} ->
                {
                    imem_datatype:imem_type(binary_to_existing_atom(string:lowercase(Typ), utf8)),
                    binary_to_integer(SLen),
                    binary_to_integer(SPrec)
                };
            Else -> ?SystemException({"Unexpected parse tree structure", Else})
        end,
    {Default, Opts} =
        case lists:keyfind(default, 1, COpts) of
            false ->
                    ...
                    case re:run(Str, "fun\\((.*)\\)[ ]*->(.*)end.", [global, {capture, [1, 2], list}]) of
                        {match, [[_Params, _Body]]} ->
                            ...
                            try
                                ...
                                _:Reason -> ?ClientError({"Bad default fun", Reason})
                            end;
                        % end;
                        nomatch -> imem_datatype:io_to_term(Str)
                    end,
                {FT, lists:keydelete(default, 1, COpts)}
        end,
    C =
        #ddColumn{
            %% not converted to atom yet !!!
            name = Name,
            type = T,
            len = L,
            prec = P,
