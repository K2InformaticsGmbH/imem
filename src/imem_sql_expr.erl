...
-export(
    [column_map_tables/3, column_map_columns/2, column_map_items/2, expr/3, bind_scan/3, bind_virtual/3, bind_tree/2]
).
-export([main_spec/2, join_specs/3, sort_fun/3, sort_spec/3, filter_spec_where/3, sort_spec_order/3, sort_spec_fun/3]).
-export([binstr_to_qname2/1, to_guard/1]).
-export([rownum_limit/0]).

% Functions applied with Common Test
-export([bind_subtree_const/1, binstr_to_qname3/1, qname3_to_binstr/1, uses_filter/1]).

-define(
    GET_ROWNUM_LIMIT,
    imem_config:get_config_hlk(
        ?CONFIG_TABLE,
        {imem, imem_sql_expr, rownumDefaultLimit},
        imem_sql_expr,
        [node()],
        200000,
...
bind_scan(Ti, X, ScanSpec0) ->
    ...
    case {STree0, FTree0} of
        {true, true} ->
            %% use pre-calculated SSpec0
            {SSpec0, TailSpec0, FilterFun0};
        {_, true} ->
            %% no filter fun (pre-calculated to true)
        ...
        {_, _} ->
            %% both filter funs needs to be evaluated
            [{SHead, [undefined], [Result]}] = SSpec0,
            STree1 = bind_table(Ti, STree0, X),
...
bind_virtual(Ti, X, ScanSpec0) ->
    ...
    case {STree0, FTree0} of
        ...
        {_, true} ->
            ...
            % ?LogDebug("STree after scan (~p) bind :~n~p~n", [Ti,to_guard(STree1)]),
            % to_guard(STree1)
            SSpec1 = [{SHead, [STree1], [Result]}],
            {SSpec1, TailSpec0, FilterFun0};
        {_, _} ->
            %% filter fun needs to be evaluated
            [{SHead, [undefined], [Result]}] = SSpec0,
            STree1 = bind_table(Ti, STree0, X),
...
uses_bind(Ti, [A | Rest]) ->
    case uses_bind(Ti, A) of
        true -> true;
        false -> uses_bind(Ti, Rest)
    end;
...
uses_bind(Ti, {_, A, B, C, D}) ->
    uses_bind(Ti, A) orelse uses_bind(Ti, B) orelse uses_bind(Ti, C) orelse uses_bind(Ti, D);
uses_bind(_, _) -> false.
...
uses_operator(Op, {_, A, B, C, D}) ->
    uses_operator(Op, A) orelse uses_operator(Op, B) orelse uses_operator(Op, C) orelse uses_operator(Op, D);
...
uses_filter(BTree) -> uses_filter(BTree, imem_sql_funs:filter_funs()).
...
bind_done({list, []}) ->
    % ToDo: may need to abandon concept of tagged lists and use plain lists instead
    true;
...
bind_eval({list, L}) when is_list(L) ->
    ...
    case lists:usort([bind_done(El) || El <- BTL]) of
        [false | _] ->
            %% cannot simplify tree list here
            {list, BTL};
        [true] ->
            %% BTree evaluates to a list of values
            BTL
    end;
...
bind_eval({'or', Left, false}) ->
    % bind_eval(Left);
    Left;
bind_eval({'or', false, Right}) ->
    % bind_eval(Right);
    Right;
bind_eval({'and', false, _}) -> false;
bind_eval({'and', _, false}) -> false;
bind_eval({'and', true, true}) -> true;
bind_eval({'and', Left, true}) ->
    % bind_eval(Left);
    Left;
bind_eval({'and', true, Right}) ->
    % bind_eval(Right);
    Right;
...
bind_eval({'or', Same, Same}) ->
    % bind_eval(Same);
    Same;
bind_eval({'and', Same, Same}) ->
    % bind_eval(Same);
    Same;
...
bind_eval(BTree) ->
    case bind_done(BTree) of
        false ->
            %% cannot simplify BTree here
            BTree;
        true ->
            %% BTree evaluates to a value
            bind_fun(imem_sql_funs:expr_fun(BTree))
    end.

bind_fun(L) when is_list(L) -> [bind_fun(I) || I <- L];
bind_fun(BTF) when is_function(BTF) -> bind_value(BTF(anything));
bind_fun(Value) -> bind_value(Value).
...
bind_tab(Ti, {Op, A}, X) ->
    ...
    bind_eval({Op, bind_tab(Ti, A, X)});
bind_tab(Ti, {Op, A, {from_binterm, #bind{tind = Ti, cind = 2, btree = undefined} = B}}, X)
when Op == '=='; Op == '>'; Op == '>='; Op == '<'; Op == '=<'; Op == '=' ->
    bind_eval({Op, bind_tab(Ti, {to_binterm, A}, X), bind_tab(Ti, B, X)});
bind_tab(Ti, {Op, {from_binterm, #bind{tind = Ti, cind = 2, btree = undefined} = A}, B}, X)
when Op == '=='; Op == '>'; Op == '>='; Op == '<'; Op == '=<'; Op == '=' ->
    bind_eval({Op, bind_tab(Ti, A, X), bind_tab(Ti, {to_binterm, B}, X)});
bind_tab(Ti, {Op, A, B}, X) ->
    %% binary functions/op.
    bind_eval({Op, bind_tab(Ti, A, X), bind_tab(Ti, B, X)});
bind_tab(Ti, {Op, A, B, C}, X) -> bind_eval({Op, bind_tab(Ti, A, X), bind_tab(Ti, B, X), bind_tab(Ti, C, X)});
bind_tab(Ti, {Op, A, B, C, D}, X) ->
    bind_eval({Op, bind_tab(Ti, A, X), bind_tab(Ti, B, X), bind_tab(Ti, C, X), bind_tab(Ti, D, X)});
...
% TODO: may need to bind lists here too
...
bind_subtree_const(BTree) -> BTree.
...
prune_walk(Ti, {Op, A, B, C, D}) ->
    prune_eval({Op, prune_walk(Ti, A), prune_walk(Ti, B), prune_walk(Ti, C), prune_walk(Ti, D)});
prune_walk(Ti, L) when is_list(L) -> [prune_walk(Ti, I) || I <- L];
prune_walk(_, BTree) -> BTree.

% ToDo: Maybe need to prune_walk lists too
...
main_spec(?EmptyWhere, FullMap) -> scan_spec(?MainIdx, true, FullMap);
...
join_specs(Ti, WBTree, FullMap) -> join_specs(Ti, WBTree, FullMap, []).

join_specs(?MainIdx, _, _, Acc) ->
    %% done when looking at main table
    Acc;
...
scan_spec(Ti, STree0, FullMap) ->
    ...
    Limit =
        case rownum_match(STree0) of
            ...
            {'==', L, ?RownumBind} when is_integer(L) -> L;
            {'==', ?RownumBind, L} when is_integer(L) -> L;
            Else -> ?UnimplementedException({"Unsupported use of rownum", {Else}})
        end,
    ...
    case {uses_generator(STree0), uses_bind(Ti - 1, STree0), uses_filter(STree0)} of
        {false, false, true} ->
            ...
            TailSpec =
                if
                    Ti == ?MainIdx -> ets:match_spec_compile(SSpec);
                    true -> true
                end,
            %% TODO: Use bind tree and implicit binding
            FilterFun = imem_sql_funs:filter_fun(FTree),
            #scanSpec{
                sspec = SSpec,
                stree = true,
                tailSpec = TailSpec,
        ...
        {true, false, true} ->
            ...
            %% will be split and reworked at fetch time
...
binstr_to_qname(Bin) when is_binary(Bin) ->
    case re:run(Bin, "^(?<PART>((\"[^\"]*\")|([^\".]*)))[.]*(?<REST>.*)$", [{capture, ['PART', 'REST'], binary}]) of
        {match, [<<>>, <<>>]} -> [];
        {match, [Part, <<>>]} -> [Part];
...
binstr_to_qname3(Bin) when is_binary(Bin) ->
    case binstr_to_qname(Bin) of
        [N] ->
            %% may need a left shift for a table name
...
column_map_items(Map, tag) -> [C#bind.tag || C <- Map];
column_map_items(Map, schema) -> [C#bind.schema || C <- Map];
column_map_items(Map, table) -> [C#bind.table || C <- Map];
column_map_items(Map, alias) -> [C#bind.alias || C <- Map];
column_map_items(Map, name) -> [C#bind.name || C <- Map];
column_map_items(Map, qname) -> [qname3_to_binstr({C#bind.schema, C#bind.table, C#bind.name}) || C <- Map];
column_map_items(Map, tind) -> [C#bind.tind || C <- Map];
column_map_items(Map, cind) -> [C#bind.cind || C <- Map];
column_map_items(Map, type) -> [C#bind.type || C <- Map];
column_map_items(Map, len) -> [C#bind.len || C <- Map];
column_map_items(Map, prec) -> [C#bind.prec || C <- Map];
column_map_items(Map, ptree) -> [C#bind.ptree || C <- Map];
column_map_items(_Map, Item) -> ?ClientError({"Invalid item", Item}).
...
is_readonly(#bind{tind = ?MainIdx, cind = 0}) ->
    %% Vector field can be edited ??
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {_, #bind{tind = ?MainIdx, cind = 0}}}) ->
    %% Vector field can be edited ??
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {_, _, #bind{tind = ?MainIdx, cind = 0}}}) ->
    %% Vector field can be edited ??
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {Op, #bind{}}}) when Op == hd; Op == last ->
    %% editable projections
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {Op, _, #bind{}}})
when Op == element; Op == nth; Op == json_value; Op == map_get ->
    %% editable projections
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {Op, _, #bind{}}}) when Op == bytes; Op == bits ->
    %% editable projections
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {Op, _, #bind{}, #bind{}}}) when Op == bytes; Op == bits ->
    %% editable projections
    false;
is_readonly(#bind{tind = 0, cind = 0, btree = {from_binterm, _Bind}}) -> false;
is_readonly(#bind{tind = 0, cind = 0, type = json}) -> false;
is_readonly(_BTree) -> true.

%%?Info("is_readonly ~p",[_BTree]),
...
column_map_meta_fields([Name | Names], Ti, Acc) ->
    %% Ci of next meta field (starts with 1, not 2)
    Cindex = length(Acc) + 1,
...
column_map_param_fields([Param | Params], Ti, Acc) ->
    case imem_datatype:is_datatype(element(?ParamTypeIdx, Param)) of
        true ->
            %% Parameter name as binary in first element of triple
            N = element(?ParamNameIdx, Param),
            %% Parameter type (imem datatype) as second element
            Type = element(?ParamTypeIdx, Param),
            %% Parameter precision (for decimals)
            Prec = element(?ParamPrecisionIdx, Param),
            %% Ci of next param field, appended to meta fields
            Cindex = length(Acc) + 1,
            Tag = list_to_atom(lists:flatten([$$, integer_to_list(?MetaIdx), integer_to_list(Cindex)])),
            Bind =
                #bind{
        ...
        false -> ?ClientError({"Invalid data type for parameter", {element(1, Param), element(2, Param)}})
    end.
...
column_map_table_fields([{as, Table, Alias} | Tables], Ti, Acc) when is_binary(Table), is_binary(Alias) ->
    ...
    % binstr_to_qname2(Table),
    {S, T} = imem_meta:qualified_alias(Table),
    ...
    Cols =
        case S of
            ...
            _ ->
                ...
                imem_meta:column_infos({?binary_to_atom(S), ?binary_to_atom(T)})
        end,
    Binds =
        [
            #bind{
                ...
                name = to_binstr(N),
                default = D,
                tag = list_to_atom(lists:flatten([$$, integer_to_list(Ti), integer_to_list(Ci)]))
            }
            ||
...
column_map_columns([#bind{schema = undefined, table = undefined, name = ?Star} | Columns], FullMap, Acc) ->
    ...
    Cmaps =
        [
            case length([N || #bind{name = N} <- FullMap, N == Name]) of
                1 -> binterm_arg_conv(Bind#bind{table = A, alias = Name, ptree = Name});
                _ ->
                    binterm_arg_conv(
                        Bind#bind{
                            table = A,
                            alias = qname3_to_binstr({undefined, A, Name}),
                            ptree = qname3_to_binstr({undefined, A, Name})
                        }
                    )
            end || #bind{tind = Ti, name = Name, alias = A} = Bind <- FullMap,
            Ti /= ?MetaIdx
        ],
...
column_map_columns([#bind{schema = Schema, table = Table, name = ?Star} = _Cmap0 | Columns], FullMap, Acc) ->
    ...
    Prefix =
        case ?atom_to_binary(imem_meta:schema()) of
            Schema -> undefined;
            _ -> Schema
        end,
    Cmaps =
            ...
            case length([N || #bind{name = N} <- FullMap, N == Name]) of
                ...
                _ ->
                    binterm_arg_conv(
                        Bind#bind{
                            table = A,
                            alias = qname3_to_binstr({Prefix, A, Name}),
                            ptree = qname3_to_binstr({Prefix, A, Name})
                        }
                    )
            end || #bind{tind = Ti, schema = S, name = Name, alias = A} = Bind <- FullMap,
            Ti /= ?MetaIdx,
            S == Schema,
            A == Table
        ],
    column_map_columns(Cmaps ++ Columns, FullMap, Acc);
column_map_columns(
    [#bind{schema = Schema, table = Table, name = Name, alias = Alias, ptree = PTree} | Columns],
    FullMap,
    Acc
) ->
...
column_map_lookup(QN3, FullMap) ->
    case field_map_lookup(QN3, FullMap) of
        #bind{type = term, btree = {from_binterm, Bind}} ->
            %% represent as binterm to the rowfuns
            Bind;
        Other -> Other
    end.
...
field_map_lookup({Schema, Table, NameIn} = QN3, FullMap) ->
    ...
    NameInString =
        case is_binary(NameIn) of
            true -> string:to_lower(binary_to_list(NameIn));
            false -> NameIn
        end,
    Pred =
        fun
    ...
    if
        ...
        (Tcount == 0) ->
            case imem_datatype:strip_dquotes(Name) of
                ...
                UQN ->
                    ?LogDebug("column_map lookup ~p ~p ~p~n", [Schema, Table, UQN]),
                    field_map_lookup({Schema, Table, UQN}, FullMap)
            end;
        (Tcount > 1) -> ?ClientError({"Ambiguous field or table name", qname3_to_binstr(QN3)});
        (Name == undefined) ->
            Bind = hd(Bmatch),
            %% bind to whole table record
            Bind#bind{type = tuple, cind = 0};
        true -> binterm_arg_conv(hd(Bmatch))
    end.
    ...
    case {imem_datatype:strip_squotes(PTree), BindTemplate} of
        {PTree, _} ->
            ...
            case (catch list_to_float(binary_to_list(PTree))) of
                V when is_float(V) -> #bind{tind = 0, cind = 0, type = float, readonly = true, btree = V};
                _ ->
                    case (catch list_to_integer(binary_to_list(PTree))) of
                        I when is_integer(I) -> #bind{tind = 0, cind = 0, type = integer, readonly = true, btree = I};
                        _ ->
                                    ...
                                    case BindTemplate#bind.type of
                                        json ->
                                            case (catch field_map_lookup({S, T, N}, FullMap)) of
                                                #bind{} = B ->
                                                    % binding for resolved relational name has priority
                                                    B;
                                                _ ->
                                                    % leave json attribute name as binary
                                                    PTree
                                            end;
                                        _ ->
                                            %% N could be a table name here
                                            field_map_lookup({S, T, N}, FullMap)
                                    end
                            end
                    end
            end;
        {B, Tbind} when Tbind == #bind{} ->
            %% assume binstr, use to_<datatype>() to override
        ...
        {B, #bind{type = binstr}} ->
            %% just take the literal value from SQL text
            %?Info("~p guessing for ~p -> ~p ~p",[binstr,B,binstr,nowrap]),
            #bind{tind = 0, cind = 0, type = binstr, default = <<>>, readonly = true, btree = imem_sql:un_escape_sql(B)};
        {B, #bind{type = T, len = L, prec = P, default = D, tag = Tag}} ->
            %% best effort conversion to proposed type
...
expr({param, Name}, FullMap, _) when is_binary(Name) -> field_map_lookup({undefined, ?ParamTab, Name}, FullMap);
    ...
    case imem_datatype:is_rowfun_extension(Fname, 1) of
        ...
        false ->
            case imem_sql_funs:unary_fun_bind_type(Fname) of
                undefined -> ?UnimplementedException({"Unsupported unary sql function", Fname});
                BT ->
                    try
                        ...
                        _ : {'ClientError', Reason} -> ?ClientError(Reason);
                        _:_ -> ?UnimplementedException({"Bad parameter for unary sql function", Fname})
                    end
            end
    end;
expr({'fun', <<"regexp_like">>, [A, B]}, FullMap, BT) -> expr({'fun', <<"is_regexp_like">>, [A, B]}, FullMap, BT);
expr({'||', A, B}, FullMap, _) ->
...
expr({'fun', Fname, [A, B]}, FullMap, _) ->
    CMapA =
        case imem_sql_funs:binary_fun_bind_type1(Fname) of
            undefined -> ?UnimplementedException({"Unsupported binary sql function", Fname});
            BA -> expr(A, FullMap, BA)
        end,
    CMapB =
        case imem_sql_funs:binary_fun_bind_type2(Fname) of
            undefined -> ?UnimplementedException({"Unsupported binary sql function", Fname});
...
expr({'fun', Fname, [A, B, C]}, FullMap, _) ->
    CMapA =
        case imem_sql_funs:ternary_fun_bind_type1(Fname) of
            undefined -> ?UnimplementedException({"Unsupported ternary sql function", Fname});
            BA -> expr(A, FullMap, BA)
        end,
    CMapB =
        case imem_sql_funs:ternary_fun_bind_type2(Fname) of
            undefined -> ?UnimplementedException({"Unsupported ternary sql function", Fname});
            BB -> expr(B, FullMap, BB)
        end,
    CMapC =
        case imem_sql_funs:ternary_fun_bind_type3(Fname) of
            undefined -> ?UnimplementedException({"Unsupported ternary sql function", Fname});
...
expr({Op, A, B}, FullMap, BT) when Op == '+'; Op == '-'; Op == '*'; Op == '/'; Op == 'div'; Op == 'rem' ->
    ...
    case {CMapA#bind.tind, CMapB#bind.tind} of
        {0, 0} -> expr_math(Op, CMapA, CMapB, BT);
        {0, _} when CMapB#bind.type == datetime; CMapB#bind.type == timestamp ->
            case CMapA#bind.type of
                ...
                _ ->
                    CMapA1 = expr(A, FullMap, #bind{type = number, default = ?nav}),
        ...
        {_, 0} when CMapA#bind.type == datetime; CMapA#bind.type == timestamp ->
            case CMapB#bind.type of
                ...
                number -> expr_time(Op, CMapA, CMapB, BT);
                _ ->
                    CMapB1 = expr(B, FullMap, #bind{type = number, default = ?nav}),
        ...
        {_, _} when CMapA#bind.type /= datetime, CMapA#bind.type /= timestamp -> expr_math(Op, CMapA, CMapB, BT);
        {_, _} -> expr_time(Op, CMapA, CMapB, BT)
    end;
...
expr({between, A, Low, High}, FullMap, BT) -> expr({'and', {'>=', A, Low}, {'<=', A, High}}, FullMap, BT);
...
expr_concat(#bind{type = T} = CMapA, #bind{type = T} = CMapB) -> #bind{type = T, btree = {concat, CMapA, CMapB}};
...
expr_math(Op, CMapA, CMapB, BT) ->
    case C = {CMapA#bind.type, CMapB#bind.type, Op, BT#bind.type} of
        {decimal, _, _, _} -> ?UnimplementedException({"Unsupported number conversion", C});
        {_, decimal, _, _} -> ?UnimplementedException({"Unsupported number conversion", C});
        {_, _, _, decimal} -> ?UnimplementedException({"Unsupported number conversion", C});
        {_, _, '/', _} -> #bind{type = float, btree = {Op, CMapA, CMapB}};
        {integer, integer, _, _} -> #bind{type = integer, btree = {Op, CMapA, CMapB}};
        {float, _, _, _} -> #bind{type = float, btree = {Op, CMapA, CMapB}};
        {_, float, _, _} -> #bind{type = float, btree = {Op, CMapA, CMapB}};
        {list, _, _, _} -> #bind{type = list, btree = {Op, CMapA, CMapB}};
        {map, _, _, _} -> #bind{type = map, btree = {Op, CMapA, CMapB}};
        {_, _, _, _} -> #bind{type = number, btree = {Op, CMapA, CMapB}}
    end.
...
expr_time(Op, CMapA, CMapB, BT) ->
    case C = {CMapA#bind.type, CMapB#bind.type, Op, BT#bind.type} of
        ...
        {T, timestamp, '-', _} when T == integer; T == float; T == number ->
            #bind{type = timestamp, btree = {add_ts, CMapB, {'-', CMapA}}};
        {_, _, _, _} -> ?UnimplementedException({"Unsupported time arithmetic", C})
    end.
...
expr_comp(Op, #bind{type = T} = CMapA, #bind{type = T} = CMapB) ->
    %% equal types, direct comparison
    {Op, CMapA, CMapB};
expr_comp(Op, #bind{type = binstr, btree = BTA}, #bind{type = string} = CMapB) -> {Op, binstr_to_string(BTA), CMapB};
expr_comp(Op, #bind{type = decimal, prec = 0} = CMapA, #bind{type = integer} = CMapB) -> {Op, CMapA, CMapB};
expr_comp(Op, #bind{type = decimal} = CMapA, #bind{type = integer} = CMapB) ->
    %% convert integer to decimal before comparing
    {Op, CMapA, integer_to_decimal(CMapB, CMapA#bind.prec)};
expr_comp(Op, #bind{type = decimal} = CMapA, #bind{type = T} = CMapB) when T == float; T == integer; T == number ->
    %% convert decimal to float before comparing
    {Op, decimal_to_float(CMapA, CMapA#bind.prec), CMapB};
expr_comp(Op, #bind{type = T} = CMapA, #bind{type = number} = CMapB) when T == float; T == integer ->
    %% compatible types, direct comparison
    {Op, CMapA, CMapB};
expr_comp(Op, CMapA, CMapB) -> {Op, CMapA, CMapB}.

%% erlang can compare anything
...
integer_to_decimal(I, 0) when is_integer(I) -> I;
integer_to_decimal(I, Prec) when is_integer(I), is_integer(Prec), Prec >= 0 -> erlang:round(math:pow(10, Prec)) * I;
integer_to_decimal(#bind{btree = BTree} = I, 0) -> I#bind{type = integer, btree = BTree};
...
decimal_to_float(D, 0) when is_integer(D) -> D;
decimal_to_float(D, Prec) when is_integer(D), is_integer(Prec), Prec >= 0 -> math:pow(10, - Prec) * D;
decimal_to_float(#bind{btree = BTree} = D, 0) -> D#bind{type = float, btree = BTree};
...
sort_spec_item(Expr, <<>>, FullMap, ColMap) -> sort_spec_item(Expr, <<"asc">>, FullMap, ColMap);
sort_spec_item(Expr, Direction, _FullMap, ColMap) ->
    ...
    IDs =
        case (catch list_to_integer(binary_to_list(Expr))) of
            CP when is_integer(CP) ->
                %% Index to select column given
                [Tag || #bind{tag = Tag} <- ColMap, Tag == CP];
            _ ->
                case [Tag || #bind{tag = Tag, alias = A} <- ColMap, A == Expr] of
                    [] ->
                        case [Tag || #bind{tag = Tag, ptree = PTree} <- ColMap, PTree == Expr] of
                            ...
                            TT ->
                                %% parse tree found (identical to select expression)
                                TT
                        end;
                    TA ->
                        %% select column alias given
                        TA
                end
        end,
    case IDs of
...
sort_fun_item(Expr, <<>>, FullMap, ColMap) -> sort_fun_item(Expr, <<"asc">>, FullMap, ColMap);
sort_fun_item(Expr, Direction, FullMap, ColMap) ->
    ML =
        case (catch list_to_integer(binary_to_list(Expr))) of
            CP when is_integer(CP) ->
                %% Index to select column given
                [B || #bind{tag = Tag} = B <- ColMap, Tag == CP];
            _ ->
    ...
    case ML of
        ...
        [#bind{tind = 0, cind = 0, type = Type, func = Func}] ->
            %% TODO: constant, could be ignored in sort
            F = fun (_) -> Func end,
            sort_fun_impl(Type, F, Direction);
        [#bind{type = Type} = Bind] ->
            Func = fun (X) -> ?BoundVal(Bind, X) end,
            sort_fun_impl(Type, Func, Direction);
        _ -> ?ClientError({"Ambiguous column name in order by clause", Expr})
    end.
...
filter_spec_where(?NoMoreFilter, _, WhereTree) -> WhereTree;
...
filter_spec_where(?NoMoreFilter, _, ?EmptyWhere, LeftTree) -> LeftTree;
filter_spec_where(?NoMoreFilter, _, WhereTree, LeftTree) -> {'and', LeftTree, WhereTree};
...
filter_value_tree(like, _, _, _, _, _, Val) -> imem_datatype:add_squotes(imem_sql:escape_sql(Val));
...
filter_value_tree(in, _Tag, binstr, _L, _P, _D, Val) -> imem_datatype:add_squotes(imem_sql:escape_sql(Val));
filter_value_tree(in, _, T, _, _, _, Val) when T == integer; T == float; T == number; T == userid -> Val;
...
sort_spec_order([], _, _) -> [];
sort_spec_order(SortSpec, FullMap, ColMap) -> sort_spec_order(SortSpec, FullMap, ColMap, []).

sort_spec_order([], _, _, Acc) -> lists:reverse(Acc);
...
sort_order({Ti, Ci, Direction}, FullMap, _ColMap) ->
    ...
    case
    [
        {S, T, A, N} || #bind{tind = Tind, cind = Cind, schema = S, table = T, alias = A, name = N} <- FullMap,
        Tind == Ti,
        Cind == Ci
    ] of
        [{_, Tab, Tab, Name}] -> {Name, Direction};
        [{_, _, Alias, Name}] -> {qname2_to_binstr({Alias, Name}), Direction};
...
sort_order({CName, Direction}, _, _) -> {CName, Direction}.

sort_spec_fun([], _, _) -> fun (_X) -> {} end;
...
sort_fun_any({Ti, Ci, Direction}, FullMap, _) ->
    ...
    case [B || #bind{tind = Tind, cind = Cind} = B <- FullMap, Tind == Ti, Cind == Ci] of
        ...
        Else -> ?ClientError({"Bad sort field binding", Else})
    end;
sort_fun_any({Cp, Direction}, _, ColMap) when is_integer(Cp) ->
    %% SortSpec given referencing ColMap position
    case lists:nth(Cp, ColMap) of
        #bind{tind = 0, cind = 0, type = T, func = Func} when is_function(Func) -> sort_fun_impl(T, Func, Direction);
...
sort_fun_impl(atom, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                A when is_atom(A) -> [- Item || Item <- atom_to_list(A)] ++ [?MaxChar];
                V -> V
            end
    end;
sort_fun_impl(binstr, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                B when is_binary(B) -> [- Item || Item <- binary_to_list(B)] ++ [?MaxChar];
                V -> V
            end
    end;
...
sort_fun_impl(ipadr, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                {A, B, C, D} when is_integer(A), is_integer(B), is_integer(C), is_integer(D) -> {- A, - B, - C, - D};
                {A, B, C, D, E, F, G, H}
                when is_integer(A),
                     is_integer(B),
                     is_integer(C),
...
sort_fun_impl(string, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                [H | T] when is_integer(H) -> [- Item || Item <- [H | T]] ++ [?MaxChar];
                V -> V
            end
    end;
sort_fun_impl(timestamp, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                {Sec, Micro} when is_integer(Sec), is_integer(Micro) -> {- Sec, - Micro};
...
sort_fun_impl({atom, number}, F, <<"desc">>) ->
    fun
        (X) ->
            case F(X) of
                {T, N} when is_atom(T), is_number(N) -> {[- Item || Item <- atom_to_list(T)] ++ [?MaxChar], - N};
                V -> V
            end
    end;
        ...
        (X) ->
            case F(X) of
                ...
                when is_atom(T),
                     ...
                     is_integer(H) ->
                    {[- Item || Item <- atom_to_list(T)] ++ [?MaxChar], - A, - B, - C, - D, - E, - F, - G, - H};
                {T, R} when is_atom(T) -> {[- Item || Item <- atom_to_list(T)] ++ [?MaxChar], R};
                V -> V
            end
    end;
