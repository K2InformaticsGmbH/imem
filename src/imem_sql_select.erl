...
-define(DefaultRendering, str).

%% gui (strings when necessary) | str (always strings) | raw (erlang terms)

-define(
    GET_DATE_FORMAT(__IsSec),
    ?GET_CONFIG(dateFormat, [__IsSec], eu, "Default date format (eu/us/iso/raw) to return in SQL queries.")
).

%% eu | us | iso | raw

-define(
    GET_NUM_FORMAT(__IsSec),
    ?GET_CONFIG(numberFormat, [__IsSec], {prec, 2}, "Default number formats and precision (not used yet).")
).

%% not used yet

-define(
    GET_STR_FORMAT(__IsSec),
    ?GET_CONFIG(stringFormat, [__IsSec], [], "Default string format to return in SQL queries (not used yet).")
).

%% not used yet

-define(
    GET_ALLOW_CLUSTER_QUERY(__AccountId),
    ?GET_CONFIG(
        allowClusterQuery,
        [__AccountId],
        false,
        "Is user allowed to perform cluster queries (C = across all nodes)?"
    )
).
-define(
    GET_ALLOW_RANGE_QUERY(__AccountId),
    ?GET_CONFIG(
        allowRangeQuery,
        [__AccountId],
        false,
        "Is user allowed to perform partition queries (P = across all partitions)?"
    )
).

-export([exec/5, flatten_tables/1]).
...
exec(SKey, {select, SelectSections} = ParseTree, Stmt, Opts, IsSec) ->
    ...
    case {?GET_ALLOW_CLUSTER_QUERY(AccountId), lists:member($C, Class)} of
        ...
        {false, true} ->
            ?ClientError(
                {"You are not allowed to run cluster queries. Please select on each node separately.", AccountId}
            )
    end,
    case {?GET_ALLOW_RANGE_QUERY(AccountId), lists:member($P, Class)} of
        ...
        {false, true} ->
            ?ClientError(
                {"You are not allowed to run range queries. Please select each time partition separately.", AccountId}
            )
    end,
    ...
    MetaFields =
        lists:usort(imem_sql:prune_fields(imem_meta:meta_field_list(), ParseTree)) -- [?META_ROWNUM, ?META_NODE],
    ...
    Tables =
        [
            imem_meta:qualified_table_name({TS, TN}) || #bind{tind = Ti, cind = Ci, schema = TS, table = TN} <- FullMap,
            Ti /= ?MetaIdx,
            Ci == ?FirstIdx
        ],
    ...
    RowCols =
        [
            #rowCol{tag = Tag, alias = A, type = T, len = L, prec = P, readonly = R}
            || #bind{tag = Tag, alias = A, type = T, len = L, prec = P, readonly = R} <- ColMap0
        ],
    ...
    %% start with last join table, proceed to first
    JoinSpecs = imem_sql_expr:join_specs(?TableIdx(length(Tables)), WBTree0, FullMap),
    %?Info("JoinSpecs:~n~p~n", [JoinSpecs]),
    ColMap1 =
        [
            if
                (Ti == 0) and (Ci == 0) -> CMap#bind{func = imem_sql_funs:expr_fun(BTree)};
                true -> CMap
            end || #bind{tind = Ti, cind = Ci, btree = BTree} = CMap <- ColMap0
        ],
    ...
    RowFun =
        case ?DefaultRendering of
            ...
            str ->
                imem_datatype:select_rowfun_str(
                    ColMap1,
                    ?GET_DATE_FORMAT(IsSec),
                    ?GET_NUM_FORMAT(IsSec),
                    ?GET_STR_FORMAT(IsSec)
                )
        end,
...
span_tables(FullTableNames, ClusterTableNames) -> span_tables(FullTableNames, ClusterTableNames, []).
...
flatten_tables(L, D, N, Acc) -> flatten_tables(L, D, N + 1, [flatten_one(L, D, N + 1) | Acc]).

flatten_one(L, D, N) -> [flatten_pick(L, D, N, I) || I <- lists:seq(1, length(L))].
