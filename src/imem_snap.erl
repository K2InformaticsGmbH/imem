...
-export(
    [init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3, format_status/2, start_link/1]
).
...
-define(
    GET_SNAPSHOT_CYCLE_WAIT,
    ?GET_CONFIG(snapshotCycleWait, [], 10000, "Wait time between snapshot cycles in msec.")
).
-define(
    GET_SNAPSHOT_CHUNK_MAX_SIZE,
    ?GET_CONFIG(snapshotChunkMaxSize, [], 500, "Maximum snapshot chunk size (number of rows).")
).
-define(
    GET_SNAPSHOT_CHUNK_FETCH_TIMEOUT,
    ?GET_CONFIG(snapshotChunkFetchTimeout, [], 20000, "Timeout in msec for fetching the next chunk from DB.")
).
-define(
    GET_SNAPSHOT_SCRIPT,
    ?GET_CONFIG(snapshotScript, [], true, "Do we want to use a specialized snapshot script function?")
).
-define(
    GET_SNAPSHOT_EXCLUSION_PATTERNS,
    ?GET_CONFIG(
        snapshotExclusuionPatterns,
        [],
...
-define(
    PUT_SNAPSHOT_EXCLUSION_PATTERNS(__TablePatterns, __Remark),
    ?PUT_CONFIG(snapshotExclusuionPatterns, [], __TablePatterns, __Remark)
).
-define(
    GET_SNAPSHOT_SCRIPT_FUN,
    ?GET_CONFIG(
        snapshotScriptFun,
        [],
        <<
        ...
        >>,
        "Function to perform customized snapshotting."
    )
).
-define(
    GET_CLUSTER_SNAPSHOT,
    ?GET_CONFIG(snapshotCluster, [], true, "Do we need periodic snapshotting of important tables an all nodes?")
).
-define(
    GET_CLUSTER_SNAPSHOT_TABLES,
    ?GET_CONFIG(
        snapshotClusterTables,
        [],
        [ddAccount, ddRole, ddConfig],
        "List of important tables to be regularily snapshotted."
    )
).
-define(
    GET_CLUSTER_SNAPSHOT_TOD,
    ?GET_CONFIG(snapshotClusterHourOfDay, [], 14, "Hour of (00..23)day in which important tables must be snapshotted.")
).
-define(
    GET_SNAPSHOT_COPY_FILTER(_Table),
    ?GET_CONFIG(snapshotCopyFilter, [_Table], [["*"]], "Filters to apply during copying cluster snapshot.")
).
-define(B(_B), <<??_B>>).
-define(
    GET_SNAPSHOT_COPY_FILTER_FUN(_Table),
    ?GET_CONFIG(
        snapshotCopyFilterFun,
        [_Table],
        ?B(
            fun
...
start_snap_loop() -> erlang:whereis(?MODULE) ! imem_snap_loop.

-endif.

-spec suspend_snap_loop() -> term().
suspend_snap_loop() -> erlang:whereis(?MODULE) ! imem_snap_loop_cancel.
            ...
            case filelib:ensure_dir(SnapshotDir) of
                ok ->
                    case file:make_dir(SnapDir) of
                        ok -> ok;
                        {error, eexists} -> ok;
                        {error, Error} -> ?Warn("unable to create snapshot directory ~p : ~p~n", [SnapDir, Error])
                    end;
                {error, Error} -> ?Warn("unable to create snapshot directory ~p : ~p~n", [SnapDir, Error])
            end;
        _ ->
...
create_clean_dir(Prefix) ->
    ...
    case filelib:is_dir(BackupDir) of
        true -> lists:foreach(fun (F) -> ok = file:delete(F) end, filelib:wildcard("*.*", BackupDir));
        false ->
...
cluster_snap(Tabs, '$replace_with_timestamp', Dir) -> cluster_snap(Tabs, ?TIMESTAMP, Dir);
cluster_snap([], StartTime, Dir) ->
    ...
    case zip:zip(ZipFile, ZipCandidates, [{cwd, Dir}]) of
        {error, Reason} -> ?Error("cluster snapshot ~s failed reason : ~p", [ZipFile, Reason]);
        _ ->
            lists:foreach(fun (F) -> ok = file:delete(filename:join(Dir, F)) end, filelib:wildcard("*.*", Dir)),
            ok = file:del_dir(Dir),
...
handle_info({cluster_snap, Tables, StartTime, Dir}, State) ->
    {
        ...
        case ?GET_CLUSTER_SNAPSHOT of
            true ->
                ...
                {_, SnapDir} = application:get_env(imem, imem_snapshot_dir),
                ZipFilePattern =
                    lists:flatten(io_lib:format(?BKP_ZIP_PREFIX "~4..0B~2..0B~2..0B_~2..0B*.zip", [Y, M, D, H])),
                ...
                case filelib:wildcard(ZipFilePattern, BackupDir) of
                    ...
                    _ ->
                        case ?GET_CLUSTER_SNAPSHOT_TOD of
                            H ->
                                if
                                    Dir == '$create_when_needed' -> ?Info("cluster snapshot ~p", [Tables]);
                                    true -> ok
                                end,
            ...
            false ->
                erlang:send_after(5000, ?MODULE, {cluster_snap, '$replace_with_timestamp', '$create_when_needed'}),
                State
        end
    };
handle_info(imem_snap_loop, #state{snapFun = SFun, snapHash = SHash} = State) ->
    case ?GET_SNAPSHOT_CYCLE_WAIT of
        MCW when (is_integer(MCW) andalso (MCW >= 100)) ->
            {SnapHash, SnapFun} =
                case ?GET_SNAPSHOT_SCRIPT of
                    false -> {undefined, undefined};
                    true ->
                        case ?GET_SNAPSHOT_SCRIPT_FUN of
                            <<"">> -> {undefined, undefined};
                            SFunStr ->
...
-spec zip(all | {re, string()} | {files, [string()]}) -> ok | string().
zip(all) -> zip({re, "*" ++ ?BKP_EXTN});
zip({re, MatchPattern}) ->
    {_, SnapDir} = application:get_env(imem, imem_snapshot_dir),
    SnapFiles = [filename:basename(File) || File <- filelib:wildcard(filename:join([SnapDir, MatchPattern]))],
    zip({files, SnapFiles});
zip({files, SnapFiles}) ->
    {_, SnapDir} = application:get_env(imem, imem_snapshot_dir),
    ZipCandidates = [SF || SF <- SnapFiles, filelib:file_size(filename:join([SnapDir, SF])) > 0],
    if
        ZipCandidates =:= [] -> ok;
        true ->
            {Secs, Micros} = ?TIMESTAMP,
            ...
            case zip:zip(ZipFileFullPath, ZipCandidates, [{cwd, SnapDir}]) of
                {error, Reason} ->
                    lists:flatten(
                        io_lib:format("old snapshot backup to ~s failed reason : ~p", [ZipFileFullPath, Reason])
                    );
                _ -> lists:flatten(io_lib:format("old snapshots are backed up to ~s", [ZipFileFullPath]))
            end
    end.

% display information of existing snapshot or a snapshot bundle (.zip)
-spec info(bkp | zip) ->
    {bkp, [{dbtables | snaptables | restorabletables, list()}]} | {zip, list()} | {error, string()}.
info(bkp) ->
        ...
        true ->
            SnapTables =
                [
                    ...
                    end || F <- filelib:wildcard("*" ++ ?BKP_EXTN, SnapDir),
                    re:run(F, "(.*)\\" ++ ?BKP_EXTN, [{capture, [1], list}]) =/= nomatch
                ],
            STabs = [S || {S, _, _} <- SnapTables],
            RestorableTables =
                sets:to_list(sets:intersection(sets:from_list([atom_to_list(M) || M <- MTabs]), sets:from_list(STabs))),
            {
                bkp,
                [
...
info({zip, [Z | ZipFiles]}, ContentFiles) ->
    info(
        {zip, ZipFiles},
        case zip:foldl(fun (F, IF, _, Acc) -> [{F, IF()} | Acc] end, [], Z) of
            {ok, CntFiles} -> [{filename:absname(Z), CntFiles} | ContentFiles];
            {error, Error} ->
...
-spec take([all] | [local] | {tabs, [atom() | string()]}) ->
    {ok, atom() | string()} | {error, atom() | string() | term()}.
take([all]) ->
    % standard snapshot tables (no local time partitioned tables)
    take({tabs, all_snap_tables()});
take([local]) ->
    % local time partitioned tables
...
take({tabs, [_R | _] = RegExs}) when is_list(_R) ->
    ...
    SelectedSnapTables =
        lists:flatten([[T || R <- RegExs, re:run(atom_to_list(T), R, []) /= nomatch] || T <- FilteredSnapReadTables]),
    ?Debug("tables being snapshoted ~p~n", [SelectedSnapTables]),
    case SelectedSnapTables of
        [] ->
            {
                error,
                lists:flatten(io_lib:format(" ~p doesn't match any table in ~p~n", [RegExs, FilteredSnapReadTables]))
            };
        _ -> take({tabs, SelectedSnapTables})
    end;
take({tabs, Tabs}) ->
    % list of tables as atoms or snapshot transform maps
    lists:flatten(
        [
...
take(Tab) when is_atom(Tab) ->
    % single table as atom (internal use)
    take({tabs, [Tab]});
take(Tab) when is_map(Tab) ->
    % single table using transform map
    take({tabs, [Tab]}).

% snapshot restore interface
%  - periodic snapshoting timer is paused during a restore operation
-spec restore(bkp, [atom() | list()], destroy | replace, true | false) -> [{atom(), term()}].
...
%  - periodic snapshoting timer is paused during a restore operation
-spec restore_as(bkp, atom() | string(), atom() | binary() | string(), destroy | replace, true | false) ->
    ok | {error, term()}.
...
restore_as(bkp, SrcTab, DstTab, Strategy, Simulate) ->
    ...
    SnapFile =
        case filelib:is_file(DstSnapFile) of
            true -> DstSnapFile;
...
-spec restore(zip, string(), re:mp() | iodata() | unicode:charlist(), destroy | replace, true | false) ->
    list() | {error, string()}.
restore(zip, ZipFile, TabRegEx, Strategy, Simulate) when is_list(ZipFile) ->
    case filelib:is_file(ZipFile) of
        true ->
            ...
            Files = [F || F <- Fs, re:run(F, TabRegEx, [{capture, all, list}]) =/= nomatch],
...
-spec restore_chunked(atom() | string(), destroy | replace, true | false) ->
    ok | {Inserted :: integer(), Edited :: integer(), Appended :: integer} | {error, term()}.
...
restore_chunked(Tab, SnapFile, Strategy, Simulate) ->
    ...
    case file:open(SnapFile, [read, raw, binary]) of
        {ok, FHndl} ->
            if
                (Simulate /= true) andalso (Strategy =:= destroy) -> catch imem_meta:truncate_table(Tab);
                true -> ok
            end,
        ...
        mnesia:write_table_property(
            ...
            case P of
                #ddTable{columns = Cols, opts = TabOpts, owner = Owner} ->
                    ...
                    NewOpts =
                        case proplists:get_value(record_name, TOpts, enoent) of
                            enoent ->
                                [{record_name, list_to_atom(filename:rootname(filename:basename(SnapFile)))} | TOpts];
                            _ -> TOpts
                        end,
                    ...
                    lists:foldl(
                        fun
                            (Row, {I, E, A}) ->
                                UpdatedRows = E + A,
                                if
                                    (UpdatedRows > 0 andalso UpdatedRows rem 500 == 0) ->
                                        ?Info("restoring ~p updated ~p rows~n", [Tab, UpdatedRows]);
                                    true -> ok
                                end,
                                if
                                    (TableSize > 0) andalso (TableType =/= bag) ->
                                        K = element(2, Row),
                                        case imem_meta:read(Tab, K) of
                                            [Row] ->
                                                % found identical existing row
                                                {I + 1, E, A};
                                            [_] ->
                                                % existing row with different content,
                                                case Strategy of
                                                    replace ->
                                                        if
                                                            Simulate /= true -> ok = imem_meta:write(Tab, Row);
                                                            true -> ok
                                                        end,
                                                        {I, E + 1, A};
                                                    destroy ->
...
all_snap_tables() ->
    lists:filter(
        fun (T) -> imem_meta:is_readable_table(T) andalso not imem_meta:is_local_time_partitioned_table(T) end,
...
all_local_time_partitioned_tables() ->
    lists:filter(
        fun (T) -> imem_meta:is_readable_table(T) andalso imem_meta:is_local_time_partitioned_table(T) end,
        imem_meta:all_tables()
    ).
...
take_fun(Me, Tab, FetchFunPid, RTrans, RowCount, ByteCount, FHndl) ->
    ...
    receive
        ...
        {row, [?sot, ?eot | Rows]} ->
            ...
            ?Debug(
                "snap ~p all, total ~p rows ~p bytes~n",
                [Tab, RowCount + length(Rows), ByteCount + byte_size(RowsBin)]
            ),
            write_close_file(Me, FHndl, RowsBin);
        {row, [?eot | Rows]} ->
            RowsBin = term_to_binary([RTrans(R) || R <- Rows]),
            ?Debug(
                "snap ~p last, total ~p rows ~p bytes~n",
                [Tab, RowCount + length(Rows), ByteCount + byte_size(RowsBin)]
            ),
            write_close_file(Me, FHndl, RowsBin);
        {row, [?sot | Rows]} ->
...
take_chunked(Tab, SnapDir) when is_atom(Tab) -> take_chunked(#{table => Tab}, SnapDir);
    ...
    _Pid =
        spawn(
            fun
                () ->
                    AvgRowSize =
                        case imem_meta:table_size(Tab) of
                    ...
                    ChunkSize =
                        lists:min(
                            [
                                erlang:round((element(2, imem:get_os_memory()) / 2) / AvgRowSize),
                                ?GET_SNAPSHOT_CHUNK_MAX_SIZE
                            ]
                        ),
                    ?Debug(
                        "[~p] snapshoting ~p of ~p rows ~p bytes~n",
                        [self(), Tab, imem_meta:table_size(Tab), imem_meta:table_memory(Tab)]
                    ),
                    FHndl =
                        case
                        file:open(
                            ...
                            [
                                ...
                                {
                                    delayed_write,
                                    erlang:round(ChunkSize * AvgRowSize),
...
-spec timestamp({integer(), integer(), integer()}) -> integer().
timestamp({Mega, Secs, Micro}) -> Mega * 1000000000000 + Secs * 1000000 + Micro.

-spec del_dirtree(file:name_all()) -> ok | {error, term()}.
del_dirtree(Path) ->
...
do_snapshot() -> do_snapshot(imem_compiler:compile(?GET_SNAPSHOT_SCRIPT_FUN)).

-spec do_snapshot(function()) -> ok | term() | {error, {string(), term()}}.
do_snapshot(SnapFun) ->
    try
        ok =
            case SnapFun of
...
set_snap_properties(Prop) -> ets:insert(?SNAP_ETS_TAB, Prop#snap_properties{last_snap = ?TIMESTAMP}).
...
exclude_table_pattern(TablePattern) when is_binary(TablePattern) -> exclude_table_pattern(binary_to_list(TablePattern));
    ...
    case {application:get_env(imem, cold_start_recover), imem_meta:nodes()} of
        {{ok, true}, []} ->
            case lists:reverse(lists:sort(filelib:wildcard(?BKP_ZIP_PREFIX "*.zip", SnapDir))) of
                [] ->
                    ?Warn(
                        "Cold Start : unable to auto restore, no "
                        ?BKP_ZIP_PREFIX "*.zip found in snapshot directory ~s",
                        [SnapDir]
                    ),
                    false;
                [ZipFile | _] ->
...
filter_cluster_snapshot(Target) ->
    ...
    case lists:reverse(lists:sort(filelib:wildcard(?BKP_ZIP_PREFIX "*.zip", SnapDir))) of
        [] ->
            ?Warn(
                "unable to filter cluster snapshot : no " ?BKP_ZIP_PREFIX "*.zip" " found in snapshot directory ~s",
                [SnapDir]
            );
        [ZipFile | _] ->
            Source = filename:join(SnapDir, ZipFile),
            ?Info("creating filtered copy of clusterd snapshot from ~s to ~s", [Source, Target]),
            Path = filename:dirname(Target),
            File = filename:basename(Target),
            TempDir = filename:join(Path, "_temp" ++ filename:rootname(File)),
            ...
            {ok, Files} =
                zip:foldl(
                    fun
                        (FileInArchive, GetInfoFun, GetBinFun, Files) ->
                            Table = filename:basename(FileInArchive, filename:extension(FileInArchive)),
                            case ?GET_SNAPSHOT_COPY_FILTER(Table) of
                                [] -> Files;
                                Filters ->
                                    FoldFun = imem_compiler:compile(?GET_SNAPSHOT_COPY_FILTER_FUN(Table)),
                                    ...
                                    Stats =
                                        try
                                            Sts = fold_snap(GetBinFun(), FoldFun, Filters, TargetFileHandle),
        ...
        {true, _} ->
            case file:del_dir(DirOrFile) of
                ...
                {error, eexist} ->
                    ...
                    lists:foreach(fun (F) -> del_all_dir_tree(filename:join(DirOrFile, F)) end, Filenames),
                    del_all_dir_tree(DirOrFile)
            end
    end.
...
fold_snap(<<Length : 32, TermAndRest/binary>>, FoldFun, Filters, Ctx, TargetFileHandle) ->
    <<ChunkBin:Length/binary, Rest/binary>> = TermAndRest,
    ChunkTerm = binary_to_term(ChunkBin),
    {NewChunkTerm, Ctx1} = process_chunk(ChunkTerm, FoldFun, Ctx#{filters => Filters}),
    Ctx2 =
        case NewChunkTerm of
            skip -> incr(chunks, skipped, 1, Ctx1);
            ChunkTerm ->
                ok = file:write(TargetFileHandle, <<(byte_size(ChunkBin)) : 32, ChunkBin/binary>>),
                incr(chunks, copied, 1, Ctx1);
            NewChunkTerm ->
                NewChunkBin = term_to_binary(NewChunkTerm),
                ok = file:write(TargetFileHandle, <<(byte_size(NewChunkBin)) : 32, NewChunkBin/binary>>),
                incr(chunks, changed, 1, Ctx1)
        end,
    fold_snap(Rest, FoldFun, Filters, incr(chunks, total, 1, Ctx2), TargetFileHandle).

process_chunk(RawRows, FoldFun, #{matchSpecCompiled := CompiledMatchSpec, skvh := Skvh} = Ctx) when is_list(RawRows) ->
    Rows =
        if
            Skvh -> [imem_dal_skvh:skvh_rec_to_map(SkvhRec) || SkvhRec <- RawRows];
            true -> RawRows
        end,
    lists:foldr(
        fun
            (Row, {NewRows, ICtx}) ->
                case process_chunk(Row, FoldFun, ICtx) of
                    {skip, ICtx1} -> {NewRows, incr(rows, skipped, 1, ICtx1)};
                    {copy, ICtx1} -> {[Row | NewRows], incr(rows, copied, 1, ICtx1)};
                    {write, Row, ICtx1} -> {[Row | NewRows], incr(rows, copied, 1, ICtx1)};
                    {write, NewRow, ICtx1} -> {[NewRow | NewRows], incr(rows, changed, 1, ICtx1)}
                end
        end,
        {[], incr(rows, total, length(Rows), Ctx)},
        ets:match_spec_run(Rows, CompiledMatchSpec)
    );
process_chunk({prop, [#ddTable{opts = Opts}]} = Prop, FoldFun, #{filters := Filters} = Ctx) ->
    Skvh = is_skvh(Opts),
    {MatchSpec, MatchSpecCompiled} =
        case catch ets:match_spec_compile(Filters) of
            {'EXIT', _} ->
    ...
    case
    apply_fold_fun(Prop, FoldFun, Ctx#{skvh => Skvh, matchSpec => MatchSpec, matchSpecCompiled => MatchSpecCompiled}) of
        {copy, Ctx1} -> {Prop, Ctx1};
        {write, Prop1, Ctx1} -> {{prop, Prop1}, Ctx1}
    end;
process_chunk(SkvhMap, FoldFun, #{skvh := true} = Ctx) ->
    case apply_fold_fun(SkvhMap, FoldFun, Ctx) of
        {write, NewSkvhMap, Ctx1} -> {write, imem_dal_skvh:map_to_skvh_rec(NewSkvhMap), Ctx1};
        Other -> Other
    end;
process_chunk(Rec, FoldFun, Ctx) -> apply_fold_fun(Rec, FoldFun, Ctx).
...
filters2ms(false, Filters) -> [{'$1', f2mc(Filter, {element, 2, '$1'}), ['$_']} || Filter <- Filters].

apply_fold_fun(Term, FoldFun, Ctx) ->
    case FoldFun(Term, Ctx) of
        {true, #{foldFnAcc := Priv}} -> {copy, Ctx#{foldFnAcc => Priv}};
        {false, #{foldFnAcc := Priv}} -> {skip, Ctx#{foldFnAcc => Priv}};
        {true, NewTerm, #{foldFnAcc := Priv}} -> {write, NewTerm, Ctx#{foldFnAcc => Priv}};
        Other ->
...
f2mc_list([H | T], C, M) ->
    ...
    if
        M == undefined -> f2mc_list(T, {tl, C}, L);
...
f2mc_tuple([], _C, M) -> M;
f2mc_tuple(['*'], _C, M) -> M;
f2mc_tuple(['_' | T], {element, I, C}, M) -> f2mc_tuple(T, {element, I + 1, C}, M);
f2mc_tuple([H | T], {element, I, C}, M) ->
    L =
        case io_lib:printable_unicode_list(H) of
            false when is_list(H) -> f2mc_list(H, {element, I, C});
            false when is_tuple(H), tuple_size(H) > 0 -> f2mc_tuple(H, {element, I, C});
            _ -> {'==', {element, I, C}, {const, H}}
        end,
    C1 = {element, I + 1, C},
    if
        M == undefined -> f2mc_tuple(T, C1, L);
...
do_cluster_snapshot() ->
    ...
    case zip:zip(ZipFile, ZipCandidates, [{cwd, Dir}]) of
        {error, Reason} -> ?Error("zip ~s failed reason : ~p", [ZipFile, Reason]);
        _ -> ?Info("zipped to ~s", [ZipFile])
    end,
    ...
    Min = TMin rem 60,
    Sec = TSec rem 60,
    ?Info("finished : added ~p tables (~p bytes) in ~2..0B:~2..0B:~2..0B", [length(Tables), Bytes, Hour, Min, Sec]).
        ...
        10000 ->
            ...
            if
                length(NewSnappers) /= length(Snappers) ->
                    ?Warn("workers ~p, alive ~p", [length(Snappers), length(NewSnappers)]);
                true -> ?Info("pending snapshot on ~p table(s)", [length(NewSnappers)])
            end,
...
f2mc_test_() ->
    {
        ...
        [
                ...
                fun
                    () ->
                        case catch f2mc(Filter, '$1') of
                            {'EXIT', Exception} ->
                                ?debugFmt(
                                    "~n~s : imem_snap:f2mc(~p, '$1').~n" "Expected   : ~p~n" "Exception  : ~p",
                                    [Title, Filter, MatchSpec, Exception]
                                ),
                                error(failed);
                            CalculatedMs ->
                                if
                                    CalculatedMs /= MatchSpec ->
                                        ?debugFmt(
                                            "~n~s : imem_snap:f2mc(~p, '$1').~n" "Expected   : ~p~n" "Got        : ~p",
            ...
            [
                ...
                {"2 list both", ["one", "two"], [{'==', '$1', {const, ["one", "two"]}}]},
                {
                    "2 list second",
                    ['_', "two"],
                    [{'andalso', {'==', {length, '$1'}, 2}, {'==', {hd, {tl, '$1'}}, {const, "two"}}}]
                },
                {
...
msrun_test_() ->
    Rows = [["a"], ["a", 1], ["a", 1, c], ["b"], ["b", 1], ["b", 2], {"a"}, {"a", 1}, 1, 1.234, a, <<"a">>, "a"],
    {
        inparallel,
        [
            {
