...
get_apps([App | Apps], Confs) -> get_apps(Apps, Confs#{App => get_app(App)}).
...
find(App, Mod, {tree, form_list, {attr, 0, [], none}, Comps}) -> find(App, Mod, Comps, []).
...
find(
    App,
    Mod,
    [{call, _, {remote, _, {atom, _, imem_config}, {atom, _, get_config_hlk}}, [_, Key, _, _, Default | Rest]} | Comps],
    Acc
) ->
    find(
        App,
        ...
        lists:usort(
            [
                list_to_tuple(
                    [
                        ast2term(App, Mod, Key),
                        ast2term(App, Mod, Default) | case Rest of
...
find(App, Mod, [C | Comps], Acc) when is_atom(C); is_integer(C); is_float(C); is_map(C) -> find(App, Mod, Comps, Acc);
