...
-export([get_metric/2, get_metric/3, request_metric/4]).
...
start_link(Mod) -> gen_server:start_link({local, Mod}, ?MODULE, [Mod], []).

-spec get_metric(atom(), term()) -> term() | {error, timeout}.
get_metric(Mod, MetricKey) -> get_metric(Mod, MetricKey, ?DEFAULT_REQ_TIMEOUT).
...
get_metric(Mod, MetricKey, Timeout) ->
    ...
    receive {metric, ReqRef, _Timestamp, _Node, Metric} -> Metric after Timeout -> timeout end.
...
request_metric(Mod, MetricKey, ReqRef, ReplyTo) ->
    ...
    case Mod:request_metric(MetricKey) of
        noreply -> metric_cast(Mod, MetricKey, ReplyFun);
        {noreply, NewMetricKey} -> metric_cast(Mod, NewMetricKey, ReplyFun);
        {ok, Reply} ->
            ReplyFun(Reply),
            ok
    end.
...
metric_cast(Mod, MetricKey, ReplyFun) ->
    case whereis(Mod) of
        P when not is_pid(P) ->
            ...
            ok;
        _ -> gen_server:cast(Mod, {request_metric, MetricKey, ReplyFun})
    end.
...
terminate(Reason, #state{mod = Mod, impl_state = ImplState}) -> Mod:terminate(Reason, ImplState).
        ...
        {_, eval_crash_suspend} ->
            case is_crash_suspend_timeout((Time - State#state.last_request) / 1000000) of
                true ->
                    ...
                    State;
                false -> internal_get_metric(MetricKey, ReplyFun, State#state{system_state = normal})
            end;
        {true, normal} ->
            {NewImplState, NewSysState} = safe_request_metric(Mod, MetricKey, ReplyFun, ImplState),
            State#state{impl_state = NewImplState, system_state = NewSysState, last_request = Time};
        {true, SysState} ->
            ReplyFun(SysState),
        ...
        {false, _} ->
            MaxReductions =
                ?GET_CONFIG(
                    ...
                    [Mod],
                    100000000,
                    "Max number of reductions per second before considering the system as overloaded."
                ),
            ...
            {NewImplState, NewSysState} =
                case {ReductionsRate > MaxReductions, PctMemoryUsed > MaxMemory} of
                    {true, _} ->
                    ...
                    _ -> safe_request_metric(Mod, MetricKey, ReplyFun, ImplState)
                end,
            State#state{
...
safe_request_metric(Mod, MetricKey, ReplyFun, ImplState) ->
    try Mod:handle_metric_req(MetricKey, ReplyFun, ImplState) of NewImplState -> {NewImplState, normal} catch
        Error:Reason ->
            ?Error(
                "~p:~p crash on metric request, called as ~p:handle_metric_req(~p, ~p, ~p)~n~p~n",
                [Error, Reason, Mod, MetricKey, ReplyFun, ImplState, erlang:get_stacktrace()]
            ),
            ReplyFun({error, {eval_crash, Reason}}),
            {ImplState, eval_crash_suspend} end.
...
build_reply_fun(ReqRef, Sock) when is_tuple(Sock) ->
    fun (Result) -> imem_server:send_resp({imem_async, {metric, ReqRef, ?TIMESTAMP, node(), Result}}, Sock) end.
...
is_crash_suspend_timeout(ElapsedSeconds) when ElapsedSeconds > 2 ->
    ...
    SuspendTimeout =
        ?GET_CONFIG(
            ...
            [],
            10,
            "Seconds the agent will refuse to respond after an evaluation crash, minimum value 2."
        ),
