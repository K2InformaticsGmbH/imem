...
% {socket_opts,[]}]}
-define(
    HTTP_PROFILE(__AppId),
    ?GET_CONFIG(
        smsTokenValidationHttpProfile,
        [__AppId],
        smsott,
        "HTTP profile name to be used for SMS one time token validation."
    )
).
-define(
    HTTP_OPTS(__AppId),
    ?GET_CONFIG(smsTokenValidationHttpOpts, [__AppId], [], "HTTP options for SMS one time token validation.")
).

-export([send_sms_token/3, verify_sms_token/4]).

-spec send_sms_token(atom(), binary(), ddCredRequest()) -> ok | no_return().
...
send_sms_token(_AppId, _To, _DDCredRequest) -> error("Unimplemented").

-spec verify_sms_token(atom(), binary(), list() | integer() | binary(), ddCredRequest()) -> ok | no_return().
...
verify_sms_token(_AppId, _To, _Token, _DDCredRequest) -> error("Unimplemented").

% @doc
sc_soap_send_sms_token(AppId, To) ->
    sc_send_sms_token(
        ?GET_CONFIG(smsTokenSendServiceUrl, [AppId], "https://host:port/sendSmsToken", "SMS token sender service URL."),
...
sc_soap_verify_sms_token(AppId, To, Token) ->
    sc_verify_sms_token(
        ?GET_CONFIG(
            smsTokenValidationServiceUrl,
            [AppId],
            "https://host:port/validateSmsToken",
            "SMS One Time Token service URL"
        ),
        ?GET_CONFIG(smsTokenServiceParams, [AppId], {"user", "password", "xmlns"}, "SMS token service parameters."),
        To,
        Token,
...
sc_send_sms_token(AppId, To, Text, TokenType, ExpireTime, TokenLength, Profile) ->
    sc_send_sms_token(
        ?GET_CONFIG(
            ...
            "SMS token validation service URL."
        ),
    ...
    case lists:member(TokenType, ?TOKEN_TYPES) of
        true ->
            ...
            Req =
                imem_json:encode(
                    if
                        TraceId /= <<>> -> maps:put(traceId, TraceId, ReqMap);
                        true -> ReqMap
                    end
                ),
            ...
            httpc:request(
                ...
                {
                    Url,
                    [{"client_id", ClientId}, {"Accept", "application/json; charset=utf-8"}],
        ...
        _ -> error({"Invalid token type", TokenType})
    end.

sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, TTES, TTLS, TL, P) when is_integer(TTES) ->
    sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, integer_to_list(TTES), TTLS, TL, P);
sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, TTES, TTLS, TL, P) when is_integer(TTLS) ->
    sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, TTES, integer_to_list(TTLS), TL, P);
sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, TTES, TTLS, TL, P) when is_integer(TL) ->
    sc_send_sms_token(Url, Cred, From, To, Tex, TokTyp, TTES, TTLS, integer_to_list(TL), P);
sc_send_sms_token(Url, {User, Password, XMLNs}, From, To, Text, TokenType, TTESec, TTLSec, TokenLength, Profile) ->
    case lists:member(TokenType, ?TOKEN_TYPES) of
        true ->
            Req =
                list_to_binary(
                    [
                        ...
                        "</text>" "<tokenType>",
                        TokenType,
                        "</tokenType>" "<timeToExpiredSeconds>",
                        TTESec,
                        "</timeToExpiredSeconds>" "<timeToLiveSeconds>",
                        TTLSec,
                        "</timeToLiveSeconds>" "<tokenlength>",
                        TokenLength,
                        "</tokenlength>" "<oneTime>true</oneTime>" "</sendSmsToken>"
                    ]
                ),
            ...
            httpc:request(
                ...
                {
                    Url,
                    [{"Authorization", Authorization}, {"Accept", "application/xml;charset=utf-8"}],
...
sc_verify_sms_token(AppId, To, Token) when is_binary(To) -> sc_verify_sms_token(AppId, binary_to_list(To), Token);
sc_verify_sms_token(AppId, To, Token) when is_integer(Token) -> sc_verify_sms_token(AppId, To, integer_to_list(Token));
sc_verify_sms_token(AppId, To, Token) when is_binary(Token) -> sc_verify_sms_token(AppId, To, binary_to_list(Token));
sc_verify_sms_token(AppId, To, Token) ->
    sc_verify_sms_token(
        ...
        ?GET_CONFIG(
            smsTokenValidationClientId,
            [AppId],
            "RokAOeF59nkcFg2GtgxgOdZzosQW1MPQ",
            "SMS token validation client ID."
        ),
        To,
        Token,
        imem_client:get_profile(httpc, ?HTTP_PROFILE(AppId), ?HTTP_OPTS(AppId))
...
sc_verify_sms_token(Url, {User, Password, XMLNs}, To, Token, Profile) ->
    Req =
        list_to_binary(
            [
                "<validateSmsToken xmlns=\"",
                XMLNs,
                "\">" "<msisdn>",
                To,
                "</msisdn>" "<validationToken>",
                Token,
                "</validationToken>" "</validateSmsToken>"
            ]
        ),
    Authorization = "Basic " ++ binary_to_list(base64:encode(User ++ ":" ++ Password)),
...
sc_verify_sms_token(Url, ClientId, To, Token, Profile) ->
    ...
    httpc:request(
        get,
        {string:join([Url, To, Token], "/"), [{"client_id", ClientId}, {"Accept", "application/json; charset=utf-8"}]},
    ...
    case catch xmerl_scan:string(Resp) of
        {
            #xmlElement{
                ...
                content = [#xmlElement{name = 'ValidationResultType', content = [#xmlText{value = "OK"}]}]
            },
            _
        } ->
            ok;
        {
            #xmlElement{
                name = validationResult,
                content = [#xmlElement{name = 'ValidationResultType', content = [#xmlText{value = Other}]}]
            },
            _
        } ->
            Other;
