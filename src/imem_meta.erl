...
-define(DDNODE_TIMEOUT, 3000).

% RPC timeout for ddNode evaluation
-define(
    META_TABLES,
    [?CACHE_TABLE, ?LOG_TABLE, ?MONITOR_TABLE, ?CONFIG_TABLE, dual, ddNode, ddSnap, ddSchema, ddSize, ddAlias, ddTable]
).
-define(
    META_FIELDS,
    [?META_ROWNUM, ?META_NODE, <<"systimestamp">>, <<"user">>, <<"username">>, <<"sysdate">>, <<"schema">>]
).
-define(META_OPTS, [purge_delay, trigger]).

% table options only used in imem_meta and above
-define(
    VIRTUAL_TABLE_ROW_LIMIT,
    ?GET_CONFIG(
        virtualTableRowLimit,
        [],
        1000,
        "Maximum number of rows which can be generated in first (and only) result block"
    )
).
-define(
    LOG_TABLE_OPTS,
...
-define(DD_CACHE_OPTS, [{scope, local}, {local_content, true}, {record_name, ddCache}]).
-define(
    DD_INDEX_OPTS,
    [
        {record_name, ddIndex},
...
% ToDo: Implement also for sharded and partitioned tables
-define(BAD_NAME_CHARACTERS, "!?#*:+-.\\<|>/").

%% invalid chars for tables and columns

% -define(RecIdx, 1).       %% Record name position in records
-define(FirstIdx, 2).

%% First field position in records (matching to hd(ColumnInfos))
...
%% placeholder for JSON path pointing to the whole document (record position)

-define(EmptyJP, {}).

%% placeholder for compiled JSON path pointing to the whole document (record position)

-define(PartEndDigits, 10).

% Number of digits representing the partition end (seconds since epoch) in partition names
-define(
    GET_SNAP_NAME_TRANS(__TABLE),
    ?GET_CONFIG(
        snapNameTrans,
        [__TABLE],
        <<"fun(N) -> atom_to_list(N) end">>,
        "Name translation for snapshot files, '.bkp' added"
    )
).
-define(
    GET_SNAP_PROP_TRANS(__TABLE),
    ?GET_CONFIG(snapPropTrans, [__TABLE], <<"fun(P) -> P end">>, "Table property translation for snapshot files")
).
-define(
    GET_SNAP_ROW_TRANS(__TABLE),
...
-export(
    [
        ...
        truncate_table/1,
        truncate_table/2,
        %% dump local table to snapshot directory
        snapshot_table/1,
        %% replace local table by version in snapshot directory
        restore_table/1,
        %% replace/create local table by version in snapshot directory
        restore_table_as/2,
        %% read whole table, only use for small tables
        read/1,
        %% read by key
        read/2,
        %% read using hierarchical list key
        read_hlk/2,
        %% select without limit, only use for small result sets
        select/2,
        %% select dirty without any trynsaction locks
        dirty_select/2,
        %% select virtual table without limit, only use for small result sets
        select_virtual/3,
        %% select with limit
        select/3,
        select_sort/2,
        ...
        ets/2,
        select_count/2,
        %% parameterized insert/update/merge/remove
        modify/8,
        %% parameterized insert/update/merge/remove with trigger options
        modify/9,
        %% apply defaults, write row if key does not exist, apply trigger
        insert/2,
        %% apply defaults, write row if key does not exist, apply trigger
        insert/3,
        %% apply defaults, write row if key does not exist, apply trigger with trigger options
        insert/4,
        %% apply defaults, write row if key exists, apply trigger (bags not supported)
        update/2,
        %% apply defaults, write row if key exists, apply trigger (bags not supported)
        update/3,
        %% apply defaults, write row if key exists, apply trigger with trigger options (bags not supported)
        update/4,
        %% apply defaults, write row, apply trigger (bags not supported)
        merge/2,
        %% apply defaults, write row, apply trigger (bags not supported)
        merge/3,
        %% apply defaults, write row, apply trigger (bags not supported) with trigger options
        merge/4,
        %% delete row if key exists (if bag row exists), apply trigger
        remove/2,
        %% delete row if key exists (if bag row exists), apply trigger
        remove/3,
        %% delete row if key exists (if bag row exists), apply trigger with trigger options
        remove/4,
        %% write row for single key, no defaults applied, no trigger applied
        write/2,
        write_log/1,
        dirty_read/1,
        dirty_read/2,
        ...
        %% delete row by key
        dirty_delete/2,
        %% delete single row in bag table
        delete_object/2,
        %% abort transaction with a message
        abort/1
    ]
).
-export(
    [
        %% stateless creation of update plan from change list
        update_prepare/3,
        %% take change list and generate update plan (stored in state)
        update_cursor_prepare/2,
        %% take update plan from state and execute it (fetch aborted first)
        update_cursor_execute/2,
        %% apply arity/0 funs of default record to ?nav values of current record
        apply_defaults/2,
        %% apply any arity funs of default record to current record
        apply_validators/3,
        %% apply any arity funs of default record to current record
        apply_validators/4,
        fetch_recs/3,
...
-export(
    [
        ...
        update_tables/2,
        %% (Old,New,Tab,User,TrOpts,IdxDef)
        update_index/6,
...
-export(
    [transaction/1, transaction/2, transaction/3, return_atomic_list/1, return_atomic_ok/1, return_atomic/1, lock/2]
).
-export([first/1, dirty_first/1, next/2, dirty_next/2, last/1, dirty_last/1, prev/2, dirty_prev/2, foldl/3]).
-export(
    [
        %% merge two data tables into a bigger one, presenting the differences side by side
        merge_diff/3,
        %% merge two data tables into a bigger one, presenting the differences side by side
        merge_diff/4,
        %% merge two data tables into a bigger one, presenting the differences side by side
        merge_diff/5,
        %% take (LeftType, LeftData, RightType, RightData, Opts, User) and produce data for a side-by-side view
        term_diff/6
    ]
).
...
init_create_table(TableName, RecDef, Opts) -> init_create_table(TableName, RecDef, Opts, #ddTable{} #ddTable.owner).
...
init(_Args) ->
    Result =
        try
            % read node_shard config, evaluate and write back (cache)
            application:set_env(imem, node_shard, node_shard()),
            %% may not be able to register in ddTable
            init_create_table(ddAlias, {record_info(fields, ddAlias), ?ddAlias, #ddAlias{}}, ?DD_ALIAS_OPTS, system),
            %% may not be able to register in ddTable
            init_create_table(
                ?CACHE_TABLE,
                {record_info(fields, ddCache), ?ddCache, #ddCache{}},
                ?DD_CACHE_OPTS,
                system
            ),
            init_create_table(ddTable, {record_info(fields, ddTable), ?ddTable, #ddTable{}}, ?DD_TABLE_OPTS, system),
            %% register in ddTable if not done yet
            init_create_table(
                ?CACHE_TABLE,
                {record_info(fields, ddCache), ?ddCache, #ddCache{}},
                ?DD_CACHE_OPTS,
...
create_partitioned_table_sync(TableAlias, TableName) when is_atom(TableAlias), is_atom(TableName) ->
    ...
    case self() of
        ...
        _ -> gen_server:call(?MODULE, {create_partitioned_table, TableAlias, TableName}, 35000)
    end.
...
-spec dictionary_trigger(#ddAlias{} | #ddTable{}, #ddAlias{} | #ddTable{}, ddMnesiaTable(), ddEntityId(), ddOptions()) ->
    ok.
dictionary_trigger(OldRec, NewRec, T, _User, _TrOpts) when T == ddTable; T == ddAlias ->
    ...
    case {OldRec, NewRec} of
        {{}, {}} ->
            ...
            ok;
        {{}, _} ->
            %% write new rec (maybe fixing something)
        ...
        {_, _} ->
            %% update dict rec (maybe fixing something)
            {S, TO} = element(2, OldRec),
...
-spec seconds_since_epoch(ddTimestamp() | ddTimeUID() | ddDatetime() | {integer(), integer(), integer()}) ->
    undefined | integer().
...
drop_meta_tables() -> drop_meta_tables(?META_TABLES).
...
meta_field(Name) when is_atom(Name) -> meta_field(?atom_to_binary(Name));
meta_field(Name) -> lists:member(Name, ?META_FIELDS).

meta_field_info(Name) when is_atom(Name) -> meta_field_info(?atom_to_binary(Name));
meta_field_info(<<"sysdate">> = N) -> #ddColumn{name = N, type = datetime, len = 20, prec = 0};
meta_field_info(<<"systimestamp">> = N) -> #ddColumn{name = N, type = timestamp, len = 20, prec = 0};
meta_field_info(<<"schema">> = N) -> #ddColumn{name = N, type = atom, len = 10, prec = 0};
meta_field_info(?META_NODE = N) -> #ddColumn{name = N, type = atom, len = 100, prec = 0};
meta_field_info(<<"user">> = N) -> #ddColumn{name = N, type = userid, len = 20, prec = 0};
meta_field_info(<<"username">> = N) -> #ddColumn{name = N, type = binstr, len = 20, prec = 0};
meta_field_info(?META_ROWNUM = N) -> #ddColumn{name = N, type = integer, len = 10, prec = 0};
meta_field_info(Name) -> ?ClientError({"Unknown meta column", Name}).
...
%meta_field_value(user) ->           unknown;
meta_field_value(Name) -> imem_if_mnesia:meta_field_value(Name).

column_info_items(Info, name) -> [C#ddColumn.name || C <- Info];
column_info_items(Info, type) -> [C#ddColumn.type || C <- Info];
column_info_items(Info, default) -> [C#ddColumn.default || C <- Info];
column_info_items(Info, default_fun) ->
    lists:map(fun imem_datatype:to_term_or_fun/1, [C#ddColumn.default || C <- Info]);
column_info_items(Info, len) -> [C#ddColumn.len || C <- Info];
column_info_items(Info, prec) -> [C#ddColumn.prec || C <- Info];
column_info_items(Info, opts) -> [C#ddColumn.opts || C <- Info];
column_info_items(_Info, Item) -> ?ClientError({"Invalid item", Item}).

column_names(Infos) -> [list_to_atom(lists:flatten(io_lib:format("~p", [N]))) || #ddColumn{name = N} <- Infos].

-spec column_infos(ddTable()) -> ddTableMeta().
column_infos(TableAlias) when is_atom(TableAlias) -> column_infos({schema(), TableAlias});
column_infos({_Node, Schema, TableAlias}) -> column_infos({Schema, TableAlias});
...
column_infos({Schema, TableAlias}) when is_binary(Schema), is_binary(TableAlias) ->
    S = try ?binary_to_existing_atom(Schema) catch _:_ -> ?ClientError({"Schema does not exist", Schema}) end,
    T = try ?binary_to_existing_atom(TableAlias) catch _:_ -> ?ClientError({"Table does not exist", TableAlias}) end,
    column_infos({S, T});
...
column_infos(Names, Types, Defaults) ->
    ...
    if
        (NamesLength =/= TypesLength) orelse (NamesLength =/= DefaultsLength) orelse (TypesLength =/= DefaultsLength) ->
            ?ClientError(
                {
                    "Column definition params length mismatch",
                    {{"Names", NamesLength}, {"Types", TypesLength}, {"Defaults", DefaultsLength}}
                }
            );
        true -> ok
    end,
    [
        #ddColumn{name = list_to_atom(lists:flatten(io_lib:format("~p", [N]))), type = T, default = D}
        || {N, T, D} <- lists:zip3(Names, Types, Defaults)
    ].
...
create_table(TableAlias, Columns, Opts) -> create_table(TableAlias, Columns, Opts, #ddTable{} #ddTable.owner).
...
create_sys_conf(Path) -> imem_if_sys_conf:create_sys_conf(Path).
...
create_check_physical_table({Schema, TableAlias}, ColumnInfos, Opts0, Owner) when is_atom(TableAlias), is_atom(Schema) ->
    ...
    case lists:member(Schema, [schema(), ddSysConf]) of
        ...
        _ -> ?UnimplementedException({"Create/check table in foreign schema", {Schema, TableAlias}})
    end.
...
create_physical_table(TableAlias, ColInfos, Opts0, Owner) ->
    ...
    case is_reserved_for_tables(TableAlias) of
        false -> ok;
        true ->
            ?Info("Reserved table name detected", [TableAlias]),
            ok
    ...
    case {TypeMod, length(ColInfos)} of
        ...
        _ ->
            % no such check for module defined tables
            ok
    end,
    ...
    case lists:keyfind(true, 1, ReservedCheck) of
        false -> ok;
        {_, BadC} ->
            ?Info("Reserved column name '~p' detected", [BadC]),
            ok
...
module_from_type_opts(Opts) ->
    case lists:keyfind(type, 1, Opts) of
        false ->
            % normal table
            undefined;
        {type, T} when T == set; T == ordered_set; T == bag ->
            % normal table
            undefined;
...
module_with_table_api(M) ->
    case catch M:module_info(exports) of
        {'EXIT', _} -> ?ClientError({"Unknown module name for table type", {type, M}});
        Exports ->
        ...
        ddAlias ->
            ...
            case (catch imem_if_mnesia:read(ddTable, {MySchema, PTN})) of
                [] ->
                    ...
                    % register ddAlias table in dictionary
                    imem_if_mnesia:write(ddTable, DDTableRow),
                    catch
                    imem_if_mnesia:create_table(
                        PTN,
                        column_names(ColInfos),
                        if_opts(Opts0) ++ [{user_properties, [DDTableRow]}]
                    );
                _ ->
                    % dictionary exists or ddTable does not exist yet
                    imem_if_mnesia:create_table(
                        PTN,
                        column_names(ColInfos),
                        if_opts(Opts0) ++ [{user_properties, [DDTableRow]}]
         ...
         ?CACHE_TABLE ->
            ...
            case (catch imem_if_mnesia:read(ddTable, {MySchema, PTN})) of
                ...
                _ ->
                    % entry exists or ddTable does not exist yet
                    imem_if_mnesia:create_table(
                        PTN,
                        column_names(ColInfos),
                        if_opts(Opts0) ++ [{user_properties, [DDTableRow]}]
            ...
            try
                ...
                throw : {'ClientError', {"Table already exists", PTN}} = Reason ->
                    case imem_if_mnesia:read(ddTable, {MySchema, PTN}) of
                        [] ->
                            % ddTable dictionary data was missing
                            imem_if_mnesia:write(ddTable, DDTableRow);
                        _ -> ok
                    end,
                    throw(Reason)
        ...
        _ ->
            ...
            Opts =
                case imem_if_mnesia:read(ddAlias, {MySchema, TableAlias}) of
                    [#ddAlias{columns = ColInfos, opts = Opts1, owner = Owner}] ->
                        %% ddAlias options override the given values for new partitions
                        Opts1;
                    [#ddAlias{}] ->
                        ?ClientError(
                            {"Create table fails because columns or owner do not match with ddAlias", TableAlias}
                        );
                    [] ->
                        %% [Schema, ".", Name, "_", Period, "@", Node]
                        [_, _, BaseName, _, _, _, _] = parse_table_name(TableAlias),
                        case
                        [
                            parse_table_name(TA) || #ddAlias{qname = {S, TA}} <- imem_if_mnesia:read(ddAlias),
                            S == MySchema
                        ] of
                            [] ->
                                % No matching aliases registered so far for MySchema
                                ok;
                            ParsedTNs ->
                                case length([BB || [_, _, BB, _, _, _, _] <- ParsedTNs, BB == BaseName]) of
                                    ...
                                    _ ->
                                        ?ClientError(
                                            {"Name conflict (different rolling period) in ddAlias", TableAlias}
                                        )
                                end
                        end,
            ...
            try
                imem_if_mnesia:create_table(
                    PTN,
                    column_names(ColInfos),
                    if_opts(Opts) ++ [{user_properties, [DDTableRow]}]
                ),
                imem_if_mnesia:write(ddTable, DDTableRow),
                ...
                throw : {'ClientError', {"Table already exists", PTN}} = Reason ->
                    case imem_if_mnesia:read(ddTable, {MySchema, PTN}) of
                        [] ->
                            % ddTable meta data was missing
                            imem_if_mnesia:write(ddTable, DDTableRow);
                        _ -> ok
                    end,
                    case imem_if_mnesia:read(ddAlias, {MySchema, TableAlias}) of
                        [] ->
                            % ddAlias meta data was missing
                            imem_if_mnesia:write(ddAlias, DDAliasRow);
                        _ -> ok
                    end,
                    throw(Reason)
...
get_trigger(Table) when is_binary(Table) -> get_trigger(binary_to_atom(Table, utf8));
get_trigger(Table) when is_list(Table) -> get_trigger(list_to_atom(Table));
...
create_trigger(Table, TFunStr) when is_binary(Table) -> create_trigger(binary_to_atom(Table, utf8), TFunStr);
create_trigger(Table, TFunStr) when is_list(Table) -> create_trigger(list_to_atom(Table), TFunStr);
            ...
            case lists:keysearch(trigger, 1, D#ddTable.opts) of
                ...
                {value, {_, Trig}} ->
                    try
                        ...
                        throw : {'ClientError', _} ->
                            ?Warn(
                                "migrating trigger on table ~p results in ~p",
                                [Table, create_or_replace_trigger(Table, TFunStr)]
                            )
                    end
            end;
        [] -> ?ClientError({"Table dictionary does not exist for", Table})
...
create_or_replace_trigger(Table, TFun) when is_list(Table) -> create_or_replace_trigger(list_to_atom(Table), TFun);
create_or_replace_trigger(Table, TFun) when is_atom(Table) ->
    imem_datatype:io_to_fun(TFun, ?DD_TRIGGER_ARITY),
    case read(ddTable, {schema(), Table}) of
        [#ddTable{} = D] ->
            case lists:keysearch(trigger, 1, D#ddTable.opts) of
                {value, {trigger, TFun}} -> ok;
                _ ->
...
create_index_table(IndexTable, ParentOpts, Owner) ->
    IndexOpts =
        case lists:keysearch(purge_delay, 1, ParentOpts) of
            false -> ?DD_INDEX_OPTS;
    ...
    case imem_if_mnesia:read(ddTable, {Schema, Table}) of
        ...
        [#ddTable{columns = ColumnInfos}] ->
            ...
            FoldFun =
                fun
                    (Row, _) ->
                        imem_meta:update_index({}, Row, Table, system, [], IdxPlan),
                        ok
                end,
        ...
        [#ddTable{} = D] ->
            case lists:keyfind(index, 1, D#ddTable.opts) of
                {index, Indices} ->
                    {IndexToDrop, RestIndices} =
                        lists:foldl(
                            fun
                                (I, {Drop, Rest}) ->
                                    case Index of
                                        Index when is_binary(Index) andalso I#ddIdxDef.name =:= Index -> {I, Rest};
                                        Index when is_integer(Index) andalso I#ddIdxDef.id =:= Index -> {I, Rest};
                                        _ -> {Drop, [I | Rest]}
                                    end
                            end,
                            {undefined, []},
                    ...
                    case IndexToDrop of
                        undefined -> ?ClientError({"Index does not exist for", PTN, Index});
                        _ ->
                            if
                                RestIndices =:= [] -> drop_index(PTN);
                                true -> create_or_replace_index(PTN, RestIndices)
                            end
                    end;
                false -> drop_index(PTN)
            end;
        [] -> ?ClientError({"Table dictionary does not exist for", PTN})
    end.
...
drop_index(Table) when is_binary(Table) -> drop_index(binary_to_atom(Table, utf8));
drop_index(Table) when is_list(Table) -> drop_index(list_to_atom(Table));
...
drop_trigger(Table) when is_binary(Table) -> drop_trigger(binary_to_atom(Table, utf8));
drop_trigger(Table) when is_list(Table) -> drop_trigger(list_to_atom(Table));
...
is_valid_table_name(Table) when is_atom(Table) -> is_valid_table_name(atom_to_list(Table));
is_valid_table_name(Table) when is_binary(Table) -> is_valid_table_name(binary_to_list(Table));
...
norm_opt({trigger, V}) ->
    % trigger is the only binary option for now
    {trigger, V};
...
if_opts([], _) -> [];
if_opts(Opts, []) -> Opts;
if_opts(Opts, [MO | Others]) -> if_opts(lists:keydelete(MO, 1, Opts), Others).

-spec truncate_table(ddTable()) -> ok.
truncate_table(TableAlias) -> truncate_table(TableAlias, meta_field_value(user)).

-spec truncate_table(ddTable(), ddEntityId()) -> ok.
truncate_table({Schema, TableAlias, _Alias}, User) -> truncate_table({Schema, TableAlias}, User);
...
truncate_table(TableAlias, User) -> truncate_table(qualified_table_name(TableAlias), User).
...
truncate_partitioned_tables([TableName | TableNames], User) ->
    ...
    Trans =
        case imem_if_mnesia:is_readable_table(IndexTable) of
            true ->
                fun
                    () ->
...
snapshot_table({_Schema, Table, _Alias}) -> snapshot_table({_Schema, Table});
...
snapshot_table(TableName) -> snapshot_table(qualified_table_name(TableName)).
...
restore_table({_Schema, Table, _Alias}) -> restore_table({_Schema, Table});
...
restore_table(TableName) -> restore_table(qualified_table_name(TableName)).

restore_table_as({_Schema, Table, _Alias}, NewTable) -> restore_table_as({_Schema, Table}, NewTable);
restore_table_as(Oldtable, {_Schema, Table, _Alias}) -> restore_table_as(Oldtable, {_Schema, Table});
...
restore_table_as(Alias, NewAlias) when is_atom(Alias) ->
    ...
    case lists:sort(simple_or_local_node_sharded_tables(Alias)) of
        ...
        [PTN] ->
            NewPTN =
                case catch binary_to_atom(NewAlias, utf8) of
                    % _existing
                    {'EXIT', _} -> NewAlias;
                    ...
                    NewAliasAtom ->
                        case lists:sort(simple_or_local_node_sharded_tables(NewAliasAtom)) of
                            [] -> NewAliasAtom;
                            [NPTN] -> NPTN;
                            NPTNs -> ?ClientError({"Too many target tables", NPTNs})
                        end
                end,
        ...
        PTNs -> ?ClientError({"Too many source tables", PTNs})
    end;
restore_table_as(TableName, NewTableName) ->
    ...
    case imem_sql_expr:binstr_to_qname2(NewTableName) of
        {MySchemaBin, Table} -> restore_table_as(qualified_table_name(TableName), {MySchema, Table});
        {Schema, Table} -> ?UnimplementedException({"Restore table as in foreign schema", {MySchema, Schema, Table}})
    end.
...
drop_table(TableAlias, TableTypeOpts) when is_atom(TableAlias), is_list(TableTypeOpts) ->
    case is_system_table(TableAlias) of
        ...
        false ->
            drop_tables_and_infos(
                TableAlias,
                lists:sort(simple_or_local_node_sharded_tables(TableAlias)),
                TableTypeOpts
            )
    end;
...
drop_tables_and_infos(TableName, [TableName], TableTypeOpts) -> drop_table_and_info(TableName, TableTypeOpts);
...
purge_table({Schema, TableAlias, _Alias}, Opts) -> purge_table({Schema, TableAlias}, Opts);
...
purge_table(TableAlias, Opts) ->
    case is_time_partitioned_alias(TableAlias) of
        false -> ?UnimplementedException({"Purge not supported on this table type", TableAlias});
        true -> purge_time_partitioned_table(TableAlias, Opts)
    end.
...
purge_time_partitioned_table(TableAlias, Opts) ->
    case lists:sort(simple_or_local_node_sharded_tables(TableAlias)) of
        ...
        [TableName | _] ->
            KeepTime =
                case proplists:get_value(purge_delay, Opts) of
                    undefined -> ?TIMESTAMP;
                    Seconds ->
                        {Secs, Micro} = ?TIMESTAMP,
                        {Secs - Seconds, Micro}
                end,
            KeepName = partitioned_table_name(TableAlias, KeepTime),
            if
                TableName >= KeepName ->
                    %% no memory could be freed
                    0;
                true ->
    ...
    case is_node_sharded_alias(TableAlias) of
        true ->
            case is_time_partitioned_alias(TableAlias) of
                ...
                false -> [physical_table_name(TableAlias)]
            end;
        false -> physical_table_names(TableAlias)
    end.

-spec is_local_alias(ddSimpleTable() | ddQualifiedTable()) -> boolean().
is_local_alias({_Schema, TableAlias}) -> is_local_alias(TableAlias);
is_local_alias(TableAlias) when is_atom(TableAlias) -> is_local_alias(atom_to_list(TableAlias));
is_local_alias(TableAlias) when is_binary(TableAlias) -> is_local_alias(binary_to_list(TableAlias));
...
is_node_sharded_alias(TableAlias) when is_atom(TableAlias) -> is_node_sharded_alias(atom_to_list(TableAlias));
is_node_sharded_alias(TableAlias) when is_binary(TableAlias) -> is_node_sharded_alias(binary_to_list(TableAlias));
...
is_time_partitioned_alias(TableAlias) when is_atom(TableAlias) -> is_time_partitioned_alias(atom_to_list(TableAlias));
...
is_local_node_sharded_table(Name) when is_atom(Name) -> is_local_node_sharded_table(atom_to_list(Name));
is_local_node_sharded_table(Name) when is_binary(Name) -> is_local_node_sharded_table(binary_to_list(Name));
is_local_node_sharded_table(Name) when is_list(Name) -> lists:suffix([$@ | node_shard()], Name).

-spec is_local_time_partitioned_table(ddSimpleTable()) -> boolean.
is_local_time_partitioned_table(Name) when is_atom(Name) -> is_local_time_partitioned_table(atom_to_list(Name));
is_local_time_partitioned_table(Name) when is_binary(Name) -> is_local_time_partitioned_table(binary_to_list(Name));
...
is_time_partitioned_table(Name) when is_atom(Name) -> is_time_partitioned_table(atom_to_list(Name));
is_time_partitioned_table(Name) when is_binary(Name) -> is_time_partitioned_table(binary_to_list(Name));
...
is_schema_time_partitioned_table(Name) when is_atom(Name) -> is_schema_time_partitioned_table(atom_to_list(Name));
is_schema_time_partitioned_table(Name) when is_binary(Name) -> is_schema_time_partitioned_table(binary_to_list(Name));
...
parse_table_name(TableName) when is_atom(TableName) -> parse_table_name(atom_to_list(TableName));
...
parse_table_name(TableName) when is_binary(TableName) -> parse_table_name(binary_to_list(TableName)).
...
index_table_name(Table) when is_atom(Table) -> index_table_name(atom_to_list(Table));
index_table_name(Table) when is_binary(Table) -> index_table_name(binary_to_list(Table));
...
time_to_partition_expiry(Table) when is_atom(Table) -> time_to_partition_expiry(atom_to_list(Table));
time_to_partition_expiry(Table) when is_binary(Table) -> time_to_partition_expiry(binary_to_list(Table));
time_to_partition_expiry(Table) when is_list(Table) ->
    case parse_table_name(Table) of
        [_Schema, _Dot, _BaseName, _, "", _Aterate, _Shard] -> ?ClientError({"Not a time partitioned table", Table});
...
time_of_partition_expiry(Table) when is_atom(Table) -> time_of_partition_expiry(atom_to_list(Table));
time_of_partition_expiry(Table) when is_binary(Table) -> time_of_partition_expiry(binary_to_list(Table));
time_of_partition_expiry(Table) when is_list(Table) ->
    case parse_table_name(Table) of
        [_Schema, _Dot, _BaseName, _, "", _Aterate, _Shard] -> ?ClientError({"Not a time partitioned table", Table});
        [_Schema, _Dot, _BaseName, "_", N, _Aterate, _Shard] -> {list_to_integer(N), 0}
    end.
...
physical_table_name(TableAlias) when is_binary(TableAlias) -> physical_table_name(binary_to_list(TableAlias));
physical_table_name(TableAlias) when is_list(TableAlias) ->
    case lists:reverse(TableAlias) of
        [$@ | _] ->
            % node sharded alias, maybe time sharded
            partitioned_table_name(TableAlias, ?TIMESTAMP);
        [$_, $@ | _] ->
            % time sharded only
            partitioned_table_name(TableAlias, ?TIMESTAMP);
        _ ->
            case lists:member($@, TableAlias) of
                ...
                true ->
                    % maybe node sharded alias
                    partitioned_table_name(TableAlias, ?TIMESTAMP)
            end
    end.

-spec physical_table_name(ddTable(), any()) ->
    % must work for any key
    ddMnesiaTable().
...
physical_table_name(TableAlias, Key) when is_binary(TableAlias) -> physical_table_name(binary_to_list(TableAlias), Key);
physical_table_name(TableAlias, Key) when is_list(TableAlias) ->
    case lists:reverse(TableAlias) of
        [$@ | _] ->
            % node sharded, maybe time sharded
...
table_alias_opts(TableAlias) -> table_alias_opts(schema(), TableAlias).

table_alias_opts("", TableAlias) when is_list(TableAlias) ->
    try table_alias_opts(schema(), list_to_existing_atom(TableAlias)) catch _:_ -> undefined end;
table_alias_opts(Schema, TableAlias) when is_list(TableAlias) ->
    try table_alias_opts(list_to_existing_atom(Schema), list_to_existing_atom(TableAlias)) catch _:_ -> undefined end;
...
table_opts(TableName) -> table_opts(schema(), TableName).

table_opts("", TableName) when is_list(TableName) ->
    try table_opts(schema(), list_to_existing_atom(TableName)) catch _:_ -> undefined end;
table_opts(Schema, TableName) when is_list(Schema), is_list(TableName) ->
    try table_opts(list_to_existing_atom(Schema), list_to_existing_atom(TableName)) catch _:_ -> undefined end;
...
qualified_alias(TableAlias) when is_binary(TableAlias) -> qualified_alias(binary_to_list(TableAlias));
qualified_alias(TableAlias) when is_list(TableAlias) ->
    ...
    {QS, QN} =
        case parse_table_name(TableAlias) of
            % [Schema, ".", Name, "_", Period, "@", Node]
            ["", "", N, "", "", "@", "_"] ->
                % plain shared table (e.g. audit)
                {SN, N ++ "@_"};
            [S, ".", N, "", "", "@", "_"] ->
                % plain shared table (e.g. audit)
                {S, N ++ "@_"};
            ["", "", N, "_", PT, "@", "_"] ->
                % particular shared time partition
                {SN, N ++ "_" ++ PT ++ "@_"};
            [S, ".", N, "_", PT, "@", "_"] ->
                % particular shared time partition
                {S, N ++ "_" ++ PT ++ "@_"};
            [[$c, $s, $v, $$ | Rest], _, N, "", "", "@", "local"] -> {[$c, $s, $v, $$ | Rest], N};
            [S, _, N, "", "", "@", "local"] when S == ""; S == SN -> {SN, N ++ "@" ++ NS};
            ["", _, N, "_", PT, "@", "local"] -> {SN, N ++ "_" ++ PT ++ "@" ++ NS};
            [SN, _, N, "_", PT, "@", "local"] -> {SN, N ++ "_" ++ PT ++ "@" ++ NS};
            [[$c, $s, $v, $$ | Rest], _, N, "", "", "@", _] -> {[$c, $s, $v, $$ | Rest], N};
            [S, _, N, "", "", "@", Shard] ->
                % handle invisible remote tables with {scope,local},{local_content,true} and optional @
                cluster_alias_name(S, N, Shard);
            [S, _, N, "_", PT, "@", ""] when S == ""; S == SN -> {SN, N ++ "_" ++ PT ++ "@"};
            [S, _, N, "_", PT, "@", Shard] when S == ""; S == SN -> {SN, N ++ "_" ++ PT ++ "@" ++ Shard};
            ["", "", N, "", "", "", ""] -> {SN, N};
            [[$c, $s, $v, $$ | Rest], _, N, "", "", "", ""] -> {[$c, $s, $v, $$ | Rest], N};
            [S, ".", N, "", "", "", ""] -> {S, N}
        end,
    ...
    case table_opts(Schema, BaseName ++ "@" ++ node_shard()) of
        undefined ->
            case table_opts(Schema, BaseName) of
                undefined -> [];
                _Opts1 ->
                    % table exists locally or is a virtual type table
...
physical_table_names(TableAlias) when is_binary(TableAlias) -> physical_table_names(binary_to_list(TableAlias));
physical_table_names(TableAlias) when is_list(TableAlias) ->
    ...
    case parse_table_name(TableAlias) of
        ...
        [_, _, N, "_", _P, "@", "local"] ->
            ...
            lists:filter(Pred, tables_starting_with(N ++ "_"));
        [_, _, N, "", "", "@", Shard] -> tables_starting_with(N ++ "@" ++ Shard);
        [_, _, N, "_", PT, "@", ""] when length(PT) >= ?PartEndDigits -> tables_starting_with(N ++ "_" ++ PT ++ "@");
        [_, _, N, "_", _P, "@", ""] ->
...
cluster_table_names(TableAlias) when is_list(TableAlias) ->
    ...
    case parse_table_name(TableAlias) of
        ...
        ["", "", N, "", "", "@", "_"] ->
            % plain shared table (e.g. audit)
            [{node(), schema(), list_to_atom(N ++ "@_")}];
        [S, ".", N, "", "", "@", "_"] ->
            % plain shared table (e.g. audit)
            [{node(), list_to_atom(S), list_to_atom(N ++ "@_")}];
        ["", "", N, "_", PT, "@", "_"] when length(PT) >= ?PartEndDigits ->
            % particular shared time partition
            [{node(), schema(), list_to_atom(N ++ "_" ++ PT ++ "@_")}];
        [S, ".", N, "_", PT, "@", "_"] when length(PT) >= ?PartEndDigits ->
            % particular shared time partition
            [{node(), list_to_atom(S), list_to_atom(N ++ "_" ++ PT ++ "@_")}];
        ...
        [[$c, $s, $v, $$ | Rest], _, N, "", "", "@", "local"] ->
            [{node(), list_to_binary([$c, $s, $v, $$ | Rest]), list_to_binary(N)}];
        [S, _, N, "", "", "@", "local"] when S == ""; S == SN -> [{node(), schema(), list_to_atom(N ++ "@" ++ NS)}];
        ...
        ["", "", N, "_", PT, "@", Shard] when length(PT) >= ?PartEndDigits ->
            ...
            [
                {node_from_table(T), schema(), T}
                || T <- lists:filter(Pred, tables_starting_with(N ++ "_" ++ PT ++ "@" ++ Shard))
            ];
        ...
        [S, _, N, "_", _P, "@", Shard] when S == ""; S == SN ->
            ...
            [{node_from_table(T), schema(), T} || T <- lists:filter(Pred, tables_starting_with(N ++ "_"))];
        ["", "", N, "", "", "", ""] -> [{node(), schema(), list_to_atom(N)}];
        [[$c, $s, $v, $$ | Rest], _, N, "", "", "", ""] ->
            [{node(), list_to_binary([$c, $s, $v, $$ | Rest]), list_to_binary(N)}];
        [S, ".", N, "", "", "", ""] -> [{node(), list_to_atom(S), list_to_atom(N)}]
    end.
...
node_from_shard(Shard) ->
    case node_shard() of
        ...
        _ ->
            ...
            case
            lists:filter(Pred, [{rpc:call(N, imem_meta, node_shard, []), N} || {_S, N} <- data_nodes(), N =/= node()]) of
...
cluster_table_names("", BaseName, "") -> [{N, S, TN} || {N, S, TN, _} <- cluster_table_names("", BaseName)];
...
partitioned_table_name(TableAlias, Key) -> list_to_atom(partitioned_table_name_str(TableAlias, Key)).
        ...
        [$@ | _] ->
            ...
            case catch list_to_integer(lists:reverse(RN)) of
                P when is_integer(P), P > 0, PL < ?PartEndDigits ->
                    ...
                    % {Sec,...}
                    PartitionEnd = integer_to_list(P * (seconds_since_epoch(Key) div P + 1)),
                    ...
                    case lists:member(S, node_shards()) of
                        ...
                        true ->
                            ...
                            case catch list_to_integer(lists:reverse(RN)) of
                                ...
                                _ ->
                                    % physical node sharded table name
                                    TableAlias
                            end
                    end
            end
...
qualified_table_name({_Node, Schema, Table}) -> qualified_table_name({Schema, Table});
...
qualified_table_name({undefined, T}) when is_binary(T) ->
    try {schema(), ?binary_to_existing_atom(T)} catch _:_ -> ?ClientError({"Unknown Table name", T}) end;
qualified_table_name({?CSV_SCHEMA_PATTERN = S, T}) when is_binary(T) -> {S, T};
...
qualified_table_name(Table) when is_binary(Table) -> qualified_table_name(imem_sql_expr:binstr_to_qname2(Table)).

-spec qualified_new_table_name(ddTable()) -> ddQualifiedTable().
qualified_new_table_name({_Node, Schema, Table}) -> qualified_new_table_name({Schema, Table});
...
tables_starting_with(Prefix) when is_atom(Prefix) -> tables_starting_with(atom_to_list(Prefix));
tables_starting_with(Prefix) when is_binary(Prefix) -> tables_starting_with(binary_to_list(Prefix));
tables_starting_with(Prefix) when is_list(Prefix) -> atoms_starting_with(Prefix, all_tables()).

-spec atoms_starting_with(ddString(), [atom()]) -> [atom()].
atoms_starting_with(Prefix, Atoms) -> atoms_starting_with(Prefix, Atoms, []).
...
tables_ending_with(Suffix) when is_atom(Suffix) -> tables_ending_with(atom_to_list(Suffix));
tables_ending_with(Suffix) when is_binary(Suffix) -> tables_ending_with(binary_to_list(Suffix));
tables_ending_with(Suffix) when is_list(Suffix) -> atoms_ending_with(Suffix, all_tables()).

-spec atoms_ending_with(ddString(), [atom()]) -> [atom()].
atoms_ending_with(Suffix, Atoms) -> atoms_ending_with(Suffix, Atoms, []).
...
abort(Reason) -> imem_if_mnesia:abort(Reason).

schema() -> imem_if_mnesia:schema().

schema(Node) -> imem_if_mnesia:schema(Node).

failing_function([]) -> {undefined, undefined, 0};
failing_function([{imem_meta, throw_exception, _, _} | STrace]) -> failing_function(STrace);
    ...
    case lists:prefix("imem", atom_to_list(M)) of
        true ->
            ...
            case re:run(NAsBin, <<"-(.+)/">>, [{capture, all_but_first, binary}]) of
                nomatch -> {M, N, Line};
                {match, [FunNameBin]} -> {M, binary_to_atom(FunNameBin, utf8), Line}
            end;
...
log_to_db(Level, Module, Function, Fields, Message) -> log_to_db(Level, Module, Function, Fields, Message, []).

log_to_db(Level, Module, Function, Fields, Message, Stacktrace) ->
    BinStr =
        try list_to_binary(Message) catch _:_ -> list_to_binary(lists:flatten(io_lib:format("~tp", [Message]))) end,
...
data_nodes() -> imem_if_mnesia:data_nodes().

all_tables() -> imem_if_mnesia:all_tables().
...
is_readable_table({_Schema, Table}) ->
    %% ToDo: may depend on schema
    is_readable_table(Table);
is_readable_table(Table) -> imem_if_mnesia:is_readable_table(Table).
...
is_virtual_table({_Schema, Table}) ->
    %% ToDo: may depend on schema
    is_virtual_table(Table);
is_virtual_table(Table) -> lists:member(Table, ?VirtualTables).
...
node_shard() ->
    case application:get_env(imem, node_shard) of
        ...
        {ok, node_shard_fun} -> try node_shard_value(application:get_env(imem, node_shard_fun), node()) catch
                _:_ ->
                    ?Error("bad config parameter ~p~n", [node_shard_fun]),
                    "nohost" end;
        ...
        _Else ->
            ?Error("bad config parameter ~p ~p~n", [node_shard, _Else]),
            node_hash(node())
    end.
...
node_shards() ->
    ...
    case application:get_env(imem, node_shard) of
        ...
        {ok, node_shard_fun} ->
            try [node_shard_value(application:get_env(imem, node_shard_fun), N2) || N2 <- DataNodes] catch
                _:_ ->
                    ?Error("bad config parameter ~p~n", [node_shard_fun]),
                    ["nohost"] end;
...
node_shard_value({ok, FunStr}, Node) ->
    ...
    Code =
        case [lists:last(string:strip(FunStr))] of
            "." -> FunStr;
            _ -> FunStr ++ "."
        end,
...
clean_host_name(Node) when is_atom(Node) -> clean_host_name(host_name(Node), []).

clean_host_name([], Acc) -> lists:reverse(Acc);
clean_host_name([Ch | Name], Acc) when Ch >= $@, Ch =< $Z -> clean_host_name(Name, [Ch | Acc]);
clean_host_name([Ch | Name], Acc) when Ch >= $a, Ch =< $z -> clean_host_name(Name, [Ch | Acc]);
clean_host_name([Ch | Name], Acc) when Ch >= $0, Ch =< $9 -> clean_host_name(Name, [Ch | Acc]);
clean_host_name([$_ | Name], Acc) -> clean_host_name(Name, [$_ | Acc]);
clean_host_name([_ | Name], Acc) -> clean_host_name(Name, Acc).
...
node_hash(Node) when is_atom(Node) -> io_lib:format("~6.6.0w", [erlang:phash2(Node, 1000000)]).
...
-spec trigger_infos(atom() | {atom(), atom()}) ->
    {TableType :: atom(), DefaultRecord :: tuple(), TriggerFun :: function()}.
trigger_infos(Table) when is_atom(Table) -> trigger_infos({schema(), Table});
        ...
        [] ->
            case imem_if_mnesia:read(ddTable, {Schema, Table}) of
                ...
                [#ddTable{columns = ColumnInfos, opts = Opts}] ->
                    ...
                    RecordName =
                        case lists:keyfind(record_name, 1, Opts) of
                            false -> Table;
                            {_, Name} -> Name
                        end,
                    ...
                    Trigger =
                        case {IdxPlan, lists:keyfind(trigger, 1, Opts)} of
                            {#ddIdxPlan{def = []}, false} ->
                                %% no trigger actions needed
                                fun (_Old, _New, _Tab, _User, _TrOpts) -> ok end;
                            {_, false} ->
                                %% only index maintenance trigger needed
                                fun
                                    (Old, New, Tab, User, _TrOpts) ->
                                        imem_meta:update_index(Old, New, Tab, User, [], IdxPlan)
                                end;
                            {_, {_, TFun}} ->
                                %% custom trigger and index maintenance needed
                                TriggerWithIndexing =
                                    trigger_with_indexing(TFun, <<"imem_meta:update_index">>, <<"IdxPlan">>),
                                %
...
compiled_index_plan(IdxDef, ColumnInfos) ->
    FieldMap =
        [
            {list_to_binary(atom_to_list(Name)), Pos + ?FirstIdx - 1}
    ...
    Def =
        [
            D#ddIdxDef{
                pl = compile_path_list(PL, FieldMap),
...
compile_or_generate(Source) when is_binary(Source) ->
    ...
    compile_or_generate(imem_datatype:io_to_fun(Source));
compile_or_generate(IdxFun) when is_function(IdxFun, 1) ->
    %% return the arity 1 function for vnf/1
    IdxFun;
compile_or_generate(IdxFun) when is_function(IdxFun, 2) ->
    %% return the arity 2 function for iff/2 or vnf/2
    IdxFun;
compile_or_generate(IdxFun) when is_function(IdxFun, 0) ->
    %% arity 0 function is a function generator
    IdxFun();
compile_or_generate(Bad) -> ?ClientError({"Bad index fun", Bad}).

json_pos_list(PathList) -> json_pos_list(PathList, []).
...
compile_json_path({jp, ParseTreeBinary}, FieldMap) when is_binary(ParseTreeBinary) ->
    ...
    case lists:keyfind(ParseTreeBinary, 1, FieldMap) of
        ...
        {_, Pos} ->
            %% represent field as record index (integer)
            Pos
    end;
...
compile_json_path(JsonPath, FieldMap) when is_tuple(JsonPath) -> compile_json_path({jp, JsonPath}, FieldMap).
...
table_type({_Node, _Schema, Table}) ->
    %% ToDo: may depend on schema
    table_type(Table);
...
table_record_name({ddSysConf, Table}) ->
    %% ToDo: may depend on schema
    imem_if_sys_conf:table_record_name(Table);
...
table_record_name({_Node, _Schema, Table}) ->
    %% ToDo: may depend on schema
    table_record_name(Table);
...
table_columns({_Node, _Schema, Table}) ->
    %% ToDo: may depend on schema
    table_columns(Table);
...
table_size({ddSysConf, _Table}) ->
    %% ToDo: implement imem_if_sys_conf:table_size(Table)
    0;
table_size({_Node, _Schema, Table}) ->
    %% ToDo: may depend on schema
    table_size(Table);
...
table_size(Table) ->
    %% ToDo: for an Alias, sum should be returned for all local time partitions
    imem_if_mnesia:table_size(physical_table_name(Table)).

-spec table_memory(ddTable()) -> integer().
table_memory({ddSysConf, _Table}) ->
    %% ToDo: implement imem_if_sys_conf:table_memory(Table)
    0;
table_memory({_Node, _Schema, Table}) ->
    %% ToDo: may depend on schema
    table_memory(Table);
table_memory({_Schema, Table}) ->
    %% ToDo: may depend on schema
    table_memory(Table);
table_memory(Table) -> imem_if_mnesia:table_memory(physical_table_name(Table)).

%% ToDo: sum should be returned for all local time partitions

exec(Statement, BlockSize, Schema) -> imem_sql:exec(none, Statement, BlockSize, Schema, false).

fetch_recs(Pid, Sock, Timeout) -> imem_statement:fetch_recs(none, Pid, Sock, Timeout, false).

fetch_recs_sort(Pid, Sock, Timeout) -> imem_statement:fetch_recs_sort(none, Pid, Sock, Timeout, false).

fetch_recs_async(Pid, Sock) -> imem_statement:fetch_recs_async(none, Pid, Sock, false).

fetch_recs_async(Opts, Pid, Sock) -> imem_statement:fetch_recs_async(none, Pid, Sock, Opts, false).

filter_and_sort(Pid, FilterSpec, SortSpec) -> imem_statement:filter_and_sort(none, Pid, FilterSpec, SortSpec, false).

filter_and_sort(Pid, FilterSpec, SortSpec, Cols) ->
    imem_statement:filter_and_sort(none, Pid, FilterSpec, SortSpec, Cols, false).

fetch_close(Pid) -> imem_statement:fetch_close(none, Pid, false).

update_prepare(Tables, ColMap, ChangeList) -> imem_statement:update_prepare(false, none, Tables, ColMap, ChangeList).

update_cursor_prepare(Pid, ChangeList) -> imem_statement:update_cursor_prepare(none, Pid, false, ChangeList).

update_cursor_execute(Pid, Lock) -> imem_statement:update_cursor_execute(none, Pid, false, Lock).

apply_defaults(DefRec, Rec) when is_tuple(DefRec) -> apply_defaults(tuple_to_list(DefRec), Rec);
apply_defaults(DefRec, Rec) when is_list(DefRec), is_tuple(Rec) -> apply_defaults(DefRec, Rec, 1).

apply_defaults([], Rec, _) -> Rec;
apply_defaults([D | DefRec], Rec0, N) ->
    Rec1 =
        case {element(N, Rec0), is_function(D), is_function(D, 0)} of
...
apply_validators(DefRec, Rec, Table) -> apply_validators(DefRec, Rec, Table, meta_field_value(user)).
...
apply_validators([D | DefRec], Rec0, Table, User, N) ->
    Rec1 =
        if
            is_function(D, 1) ->
                %% Params=[Field]
                setelement(N, Rec0, D(element(N, Rec0)));
            is_function(D, 2) ->
                %% Params=[Field,Rec]
                setelement(N, Rec0, D(element(N, Rec0), Rec0));
            is_function(D, 3) ->
                %% Params=[Field,Rec,Table]
                setelement(N, Rec0, D(element(N, Rec0), Rec0, Table));
            is_function(D, 4) ->
                %% Params=[Field,Rec,Table,User]
                setelement(N, Rec0, D(element(N, Rec0), Rec0, Table, User));
            true -> Rec0
        end,
...
fetch_start(Pid, Tab, MatchSpec, BlockSize, Opts)
when Tab == ddNode; Tab == ddSnap; Tab == ddSchema; Tab == ddSize; Tab == ddVersion ->
...
close(Pid) -> imem_statement:close(none, Pid).
...
read({_Node, Schema, Table}) -> read({Schema, Table});
read({_Schema, Table}) ->
    %% ToDo: may depend on schema
    read(Table);
read(ddNode) -> lists:flatten([read(ddNode, Node) || Node <- [node() | erlang:nodes()]]);
read(ddSnap) ->
...
read(ddSchema) -> [{ddSchema, {Schema, Node}, []} || {Schema, Node} <- data_nodes()];
read(ddSize) -> [hd(read(ddSize, Name)) || Name <- all_tables()];
read(ddVersion) -> imem:all_apps_version_info();
read(Table) -> imem_if_mnesia:read(physical_table_name(Table)).
...
read({_Node, Schema, Table}, Key) -> read({Schema, Table}, Key);
        ...
        {WC, WCDiff} when is_integer(WC), is_integer(WCDiff) ->
            case rpc:call(Node, imem_meta, time, [], ?DDNODE_TIMEOUT) of
                {Sec, Mic} when is_integer(Sec), is_integer(Mic) ->
                    [#ddNode{name = Node, wall_clock = WC, time = {Sec, Mic}, extra = []}];
                _ -> []
            end;
        _ -> []
    end;
read(ddNode, _) -> [];
read(ddSchema, Key) when is_tuple(Key) -> [S || #ddSchema{schemaNode = K} = S <- read(ddSchema), K == Key];
read(ddSchema, _) -> [];
read(ddVersion, App) -> imem:all_apps_version_info([{apps, [App]}]);
read(ddSize, Table) ->
    PTN = physical_table_name(Table),
    case is_time_partitioned_table(PTN) of
        true ->
            case
            (catch {table_size(PTN), table_memory(PTN), time_of_partition_expiry(PTN), time_to_partition_expiry(PTN)}) of
                {S, M, E, T} when is_integer(S), is_integer(M) -> [{ddSize, PTN, S, M, E, T}];
                _ -> [{ddSize, PTN, undefined, undefined, undefined, undefined}]
            end;
        false ->
            case (catch {table_size(PTN), table_memory(PTN)}) of
                {S, M} when is_integer(S), is_integer(M) -> [{ddSize, PTN, S, M, undefined, undefined}];
                _ -> [{ddSize, PTN, undefined, undefined, undefined, undefined}]
            end
    end;
read(Table, Key) -> imem_if_mnesia:read(physical_table_name(Table), Key).
...
dirty_index_read({_Node, Schema, Table}, SecKey, Index) -> dirty_index_read({Schema, Table}, SecKey, Index);
dirty_index_read({_Schema, Table}, SecKey, Index) ->
    %% ToDo: may depend on schema
    dirty_index_read(Table, SecKey, Index);
dirty_index_read(Table, SecKey, Index) -> imem_if_mnesia:dirty_index_read(physical_table_name(Table), SecKey, Index).

read_hlk({_Node, Schema, Table}, HListKey) -> read_hlk({Schema, Table}, HListKey);
read_hlk({_Schema, Table}, HListKey) ->
    %% ToDo: may depend on schema
    read_hlk(Table, HListKey);
read_hlk(Table, HListKey) -> imem_if_mnesia:read_hlk(Table, HListKey).
            ...
            case binary:longest_common_prefix([R, <<"auto_provisioned">>]) of
                16 ->
                    ...
                    case {DefOwner, Owner} of
                        _ when ((?MODULE =:= DefOwner) orelse (Owner =:= DefOwner) orelse (undefined =:= DefOwner)) ->
...
select({_Node, Schema, Table}, MatchSpec) -> select({Schema, Table}, MatchSpec);
select({_Schema, Table}, MatchSpec) ->
    %% ToDo: may depend on schema
    select(Table, MatchSpec);
select(Table, MatchSpec) -> imem_if_mnesia:select(physical_table_name(Table), MatchSpec).

ets(Fun, Args) -> imem_if_mnesia:ets(Fun, Args).
...
select_count({_Node, Schema, Table}, MatchSpec) -> select_count({Schema, Table}, MatchSpec);
select_count({_Schema, Table}, MatchSpec) ->
    %% ToDo: may depend on schema
    select_count(Table, MatchSpec);
select_count(Table, MatchSpec) -> imem_if_mnesia:select_count(physical_table_name(Table), MatchSpec).

dirty_select({_Node, Schema, Table}, MatchSpec) -> dirty_select({Schema, Table}, MatchSpec);
dirty_select({_Schema, Table}, MatchSpec) ->
    %% ToDo: may depend on schema
    dirty_select(Table, MatchSpec);
dirty_select(Table, MatchSpec) -> imem_if_mnesia:dirty_select(physical_table_name(Table), MatchSpec).

select(Table, MatchSpec, 0) -> select(Table, MatchSpec);
select({ddSysConf, Table}, _MatchSpec, _Limit) ->
    ?UnimplementedException({"Cannot select from ddSysConf schema, use DDerl GUI instead", Table});
select({_Node, Schema, Table}, MatchSpec, Limit) -> select({Schema, Table}, MatchSpec, Limit);
select({_Schema, Table}, MatchSpec, Limit) ->
    %% ToDo: may depend on schema
    select(Table, MatchSpec, Limit);
select(Tab, MatchSpec, Limit)
when Tab == ddNode; Tab == ddSnap; Tab == ddSchema; Tab == ddSize; Tab == ddVersion; Tab == integer ->
    select_virtual(Tab, MatchSpec, Limit);
select(Table, MatchSpec, Limit) -> imem_if_mnesia:select(physical_table_name(Table), MatchSpec, Limit).

select_integer_range(Min, Max, Limit) ->
    {[{integer, I} || I <- lists:seq(Min, erlang:min(Max, Min + Limit - 1))], true}.

select_virtual(_Table, [{_, [false], ['$_']}], _Limit) -> {[], true};
...
select_virtual(Table, [{_, [true], ['$_']}], _Limit) ->
    %% used in select * from virtual_table
    {read(Table), true};
...
select_virtual(Table, [{MatchHead, [Guard], ['$_']}] = MatchSpec, _Limit) ->
    ...
    Candidates =
        case operand_match(Tag, Guard) of
            false -> read(Table);
            {'==', Tag, {element, N, Tup1}} ->
                % ?Info("Virtual Select Key : ~p~n", [element(N,Tup1)]),
                read(Table, element(N, Tup1));
            {'==', {element, N, Tup2}, Tag} ->
                % ?Info("Virtual Select Key : ~p~n", [element(N,Tup2)]),
                read(Table, element(N, Tup2));
            {'==', Tag, Val1} ->
                % ?Info("Virtual Select Key : ~p~n", [Val1]),
                read(Table, Val1);
            {'==', Val2, Tag} ->
                % ?Info("Virtual Select Key : ~p~n", [Val2]),
                read(Table, Val2);
            _ -> read(Table)
        end,
...
write({_Node, Schema, TableAlias}, Record) -> write({Schema, TableAlias}, Record);
...
write(TableAlias, Record) ->
    ...
    try imem_if_mnesia:write(PTN, Record) catch
            ...
            case is_time_partitioned_alias(TableAlias) of
                true ->
                    case create_partitioned_table_sync(TableAlias, PTN) of
                        ok -> imem_if_mnesia:write(PTN, Record);
                        {error, recursive_call} ->
                            %% cannot create a new partition now, skip logging to database
                            ok;
                        E -> ?ClientError({"Table partition cannot be created", {PTN, E}})
                    end;
                false -> ?ClientError({"Table does not exist", T})
            end end.

dirty_write({ddSysConf, TableAlias}, _Record) ->
    ?UnimplementedException({"Cannot write to ddSysConf schema, use DDerl GUI instead", TableAlias});
dirty_write({_Node, Schema, TableAlias}, Record) -> dirty_write({Schema, TableAlias}, Record);
...
dirty_write(TableAlias, Record) ->
    ...
    PTN = physical_table_name(TableAlias, element(?KeyIdx, Record)),
    try imem_if_mnesia:dirty_write(PTN, Record) catch
        throw : {'ClientError', {"Table does not exist", T}} ->
            case is_time_partitioned_alias(TableAlias) of
                true ->
                    case create_partitioned_table_sync(TableAlias, PTN) of
                        ok -> imem_if_mnesia:dirty_write(PTN, Record);
                        {error, recursive_call} ->
                            %% cannot create a new partition now, skip logging to database
                            ok;
                        E -> ?ClientError({"Table partition cannot be created", {PTN, E}})
...
insert(TableAlias, Row) -> insert(TableAlias, Row, meta_field_value(user), []).

insert(TableAlias, Row, TrOpts) when is_list(TrOpts) -> insert(TableAlias, Row, meta_field_value(user), TrOpts);
insert(TableAlias, Row, User) -> insert(TableAlias, Row, User, []).

insert({ddSysConf, TableAlias}, _Row, _User, _TrOpts) ->
    ?UnimplementedException({"Cannot write to ddSysConf schema, use DDerl GUI instead", TableAlias});
insert({_Node, Schema, TableAlias}, Row, User, TrOpts) -> insert({Schema, TableAlias}, Row, User, TrOpts);
insert({_Schema, TableAlias}, Row, User, TrOpts) ->
    %% ToDo: may depend on schema
    insert(TableAlias, Row, User, TrOpts);
...
update(TableAlias, Row) -> update(TableAlias, Row, meta_field_value(user), []).

update(TableAlias, Row, TrOpts) when is_list(TrOpts) -> update(TableAlias, Row, meta_field_value(user), TrOpts);
update(TableAlias, Row, User) -> update(TableAlias, Row, User, []).

update({ddSysConf, TableAlias}, _Row, _User, _TrOpts) ->
    ?UnimplementedException({"Cannot write to ddSysConf schema, use DDerl GUI instead", TableAlias});
update({_Node, Schema, TableAlias}, Row, User, TrOpts) -> update({Schema, TableAlias}, Row, User, TrOpts);
update({_Schema, TableAlias}, Row, User, TrOpts) ->
    %% ToDo: may depend on schema
    update(TableAlias, Row, User, TrOpts);
...
merge(TableAlias, Row) -> merge(TableAlias, Row, meta_field_value(user), []).

merge(TableAlias, Row, TrOpts) when is_list(TrOpts) -> merge(TableAlias, Row, meta_field_value(user), TrOpts);
merge(TableAlias, Row, User) -> merge(TableAlias, Row, User, []).

merge({ddSysConf, TableAlias}, _Row, _User, _TrOpts) ->
    ?UnimplementedException({"Cannot write to ddSysConf schema, use DDerl GUI instead", TableAlias});
merge({_Node, Schema, TableAlias}, Row, User, TrOpts) -> merge({Schema, TableAlias}, Row, User, TrOpts);
merge({_Schema, TableAlias}, Row, User, TrOpts) ->
    %% ToDo: may depend on schema
    merge(TableAlias, Row, User, TrOpts);
...
remove(TableAlias, Row) -> remove(TableAlias, Row, meta_field_value(user), []).

remove(TableAlias, Row, TrOpts) when is_list(TrOpts) -> remove(TableAlias, Row, meta_field_value(user), TrOpts);
remove(TableAlias, Row, User) -> remove(TableAlias, Row, User, []).
...
remove({_Node, Schema, TableAlias}, Row, User, TrOpts) -> remove({Schema, TableAlias}, Row, User, TrOpts);
remove({_Schema, TableAlias}, Row, User, TrOpts) ->
    %% ToDo: may depend on schema
    remove(TableAlias, Row, User, TrOpts);
...
modify(Operation, TableType, TableAlias, DefRec, Trigger, ORow, NRow, User, TrOpts)
when is_atom(TableAlias), is_tuple(ORow), is_tuple(NRow), is_list(TrOpts) ->
    {Key, Row} =
        case {ORow, NRow} of
            %% Old Key / New Row Value if in doubt
            {{}, {}} -> ?ClientError({"Bad modify arguments, old and new rows are empty", Operation});
            {_, {}} -> {element(?KeyIdx, ORow), ORow};
            {{}, _} ->
                DefaultedRow = apply_defaults(DefRec, NRow),
    ...
    case ((TableAlias /= ddTable) and lists:member(?nav, tuple_to_list(Row))) of
        false ->
            %% may refer to a partition (of the old key)
            PTN = physical_table_name(TableAlias, Key),
            Trans =
                fun
                    () ->
                        case {Operation, TableType, read(PTN, Key)} of
                            %% TODO: Wrap in single transaction
                            {insert, bag, Bag} ->
                                case lists:member(Row, Bag) of
                            ...
                            {insert, _, []} ->
                                ...
                                Row;
                            {insert, _, [R]} ->
                                ?ConcurrencyException({"Insert failed, key already exists in", {PTN, R}});
                            {update, bag, _} ->
                                ?UnimplementedException(
                                    {"Update is not supported on bag tables, use delete and insert", TableAlias}
                                );
                            {update, _, []} -> ?ConcurrencyException({"Update failed, key does not exist", {PTN, Key}});
                            {update, _, [ORow]} ->
                                case element(?KeyIdx, NRow) of
                                    Key -> write(PTN, Row);
                                    OK ->
                                        %% key has changed
                                ...
                                Row;
                            {update, _, [R]} ->
                                case record_match(R, ORow) of
                                    true ->
                                        write(PTN, Row),
                                        Trigger(R, Row, TableAlias, User, TrOpts),
                                        Row;
                                    false -> ?ConcurrencyException({"Data is modified by someone else", {PTN, R}})
                                end;
                            {merge, bag, _} ->
                                ?UnimplementedException(
                                    {"Merge is not supported on bag tables, use delete and insert", TableAlias}
                                );
                            {merge, _, []} ->
                                write(PTN, Row),
                                Trigger({}, Row, TableAlias, User, TrOpts),
                            ...
                            {merge, _, [R]} ->
                                ...
                                Row;
                            {remove, bag, []} ->
                                ?ConcurrencyException({"Remove failed, object does not exist", {PTN, Row}});
        ...
        true -> ?ClientError({"Not null constraint violation", {TableAlias, Row}})
    end.
...
delete({_Node, Schema, TableAlias}, Key) -> delete({Schema, TableAlias}, Key);
delete({_Schema, TableAlias}, Key) ->
    %% ToDo: may depend on schema
    delete(TableAlias, Key);
delete(TableAlias, Key) -> imem_if_mnesia:delete(physical_table_name(TableAlias, Key), Key).

dirty_delete({_Node, Schema, TableAlias}, Key) -> dirty_delete({Schema, TableAlias}, Key);
dirty_delete({_Schema, TableAlias}, Key) ->
    %% ToDo: may depend on schema
    dirty_delete(TableAlias, Key);
dirty_delete(TableAlias, Key) -> imem_if_mnesia:dirty_delete(physical_table_name(TableAlias, Key), Key).

delete_object({_Node, Schema, TableAlias}, Row) -> delete_object({Schema, TableAlias}, Row);
...
update_tables([[{Schema, _, _} | _] | _] = UpdatePlan, Lock) -> update_tables(Schema, UpdatePlan, Lock, []).
...
update_tables(ddSysConf, [], Lock, Acc) -> imem_if_sys_conf:update_tables(Acc, Lock);
update_tables(_MySchema, [], Lock, Acc) -> imem_if_mnesia:update_tables(Acc, Lock);
...
update_index(
    ...
    User,
    TrOpts,
    [#ddIdxDef{id = ID, type = Type, pl = PL, vnf = Vnf, iff = Iff} | Defs],
    Removes0,
    Inserts0
) ->
...
index_items(Rec, RecJ, Table, User, ID, Type, [Pos | PL], Vnf, Iff, Changes0) when is_integer(Pos) ->
    %% index a field as a whole, no json path search
    Key = element(?KeyIdx, Rec),
    KVPs =
        case element(Pos, Rec) of
             ?nav -> [];
            RV ->
                case vnf_eval(Vnf, Key, RV) of
                    %% apply value normalising function
                     ?nav -> [];
                    [?nav] -> [];
                    [] -> [];
                    [NVal] -> [{Key, NVal}];
...
index_items(Rec, RecJ, Table, User, ID, Type, [{PT, FL} | PL], Vnf, Iff, Changes0) ->
    ...
    KVPs =
        case lists:keyfind(?nav, 2, Binds) of
            false ->
                ...
                case Match of
                    ...
                    {error, {unimplimented, E2}} -> ?UnimplementedException({"Index error", {unimplimented, E2}});
                    {error, {unbound, E3}} -> ?SystemException({"Index error", {unbound, E3}});
                    {error, {malformed, E4}} -> ?SystemException({"Index error", {malformed, E4}});
                    _NotSupported ->
                        %% For non supported datatypes, i.e. Maps/ports/pids
                        []
                end;
            _ ->
...
transaction(Function) -> imem_if_mnesia:transaction(Function).

transaction(Function, Args) -> imem_if_mnesia:transaction(Function, Args).

transaction(Function, Args, Retries) -> imem_if_mnesia:transaction(Function, Args, Retries).

return_atomic_list(Result) -> imem_if_mnesia:return_atomic_list(Result).

return_atomic_ok(Result) -> imem_if_mnesia:return_atomic_ok(Result).

return_atomic(Result) -> imem_if_mnesia:return_atomic(Result).
...
foldl(FoldFun, InputAcc, Table) -> imem_if_mnesia:foldl(FoldFun, InputAcc, Table).

lock(LockItem, LockKind) -> imem_if_mnesia:lock(LockItem, LockKind).
...
-spec sql_jp_bind(Sql :: string()) ->
    {NewSql :: string(), BindParamsMeta :: [{BindParam :: binary(), BindType :: atom(), JPPath :: string()}]}.
sql_jp_bind(Sql) ->
    ...
    ParamsMap =
        [
            {
                lists:flatten(Param),
                ":" ++ re:replace(lists:flatten(Param), "[:_\\[\\]{}]+", "", [global, {return, list}])
            }
        ...
        ],
    {
        lists:foldl(
            fun
                ({M, R}, Sql0) ->
                    M1 = re:replace(M, "([\\[\\]])", "\\\\&", [global, {return, list}]),
        ...
        [
            list_to_tuple(
                [
                    list_to_binary(R) | case re:run(M, "^:([a-zA-Z]+)_(.*)$", [{capture, [1, 2], list}]) of
                        {match, ["a", Jp]} -> [atom, Jp];
...
-spec sql_bind_jp_values(
    BindParamsMeta :: [{BindParam :: binary(), BindType :: atom(), JPPath :: string()}],
    JpPathBinds :: [{BindName :: binary(), BindObj :: any()}]
) ->
    {Values :: list()} | no_return().
sql_bind_jp_values(BindParamsMeta, JpPathBinds) -> [imem_json:eval(Jp, JpPathBinds) || {_, _, Jp} <- BindParamsMeta].
...
merge_diff(Left, Right, Merged, Opts, User) -> imem_merge:merge_diff(Left, Right, Merged, Opts, User).
        ...
        [
            {
                P,
                case R of
                    {error, E} -> ?_assertException(throw, {'ClientError', E}, ?MODULE:sql_jp_bind(S));
                    R ->
                        fun
                            () ->
