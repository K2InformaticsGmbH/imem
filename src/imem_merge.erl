...
-export(
    [
        ...
        %% merge two data tables into a bigger one, presenting the differences side by side
        merge_diff/5,
        %% present two terms side-by-side for comparison
        term_diff/4,
        %% present two terms side-by-side for comparison
        term_diff/5,
        %% present two terms side-by-side for comparison
...
term_diff(LeftType, LeftData, RightType, RightData) -> term_diff(LeftType, LeftData, RightType, RightData, []).
...
when LeftType == integer; LeftType == float; LeftType == decimal ->
    term_diff(number, LeftData, RightType, RightData, Opts, User);
term_diff(LeftType, LeftData, RightType, RightData, Opts, User)
when RightType == integer; RightType == float; LeftType == decimal ->
...
term_diff(number, LeftData, number, RightData, _Opts, _User) ->
    try
        ...
        _:_ -> ?ClientError({"Bad numbers as diff arguments", {LeftData, RightData}})
    end;
...
num_diff(Diff) when Diff >= 0 -> list_to_binary(["+" | lists:flatten(io_lib:format("~p", [Diff]))]);
num_diff(Diff) -> list_to_binary(lists:flatten(io_lib:format("~p", [Diff]))).

term_diff_out(_Opts, [], Acc) -> lists:reverse(Acc);
term_diff_out(Opts, [{_, []} | Diff], Acc) -> term_diff_out(Opts, Diff, Acc);
term_diff_out(Opts, [{eq, E} | Diff], Acc) -> term_diff_out(Opts, Diff, term_diff_add(E, E, Opts, Acc));
...
term_diff_out(Opts, [{del, [D]}, {ins, [I | Is]} | Diff], Acc) ->
    case lists:member($=, binary_to_list(imem_cmp:cmp(D, I, Opts))) of
        true ->
            % we have white space equality =w or w= or w=w
...
term_diff_out(Opts, [{ins, [I1]}], Acc) -> term_diff_out(Opts, [], term_diff_add(?nav, [I1], Opts, Acc));
term_diff_out(Opts, [{del, [D1]}], Acc) -> term_diff_out(Opts, [], term_diff_add([D1], ?nav, Opts, Acc)).

term_diff_add([], [], _Opts, Acc) -> Acc;
term_diff_add([], ?nav, _Opts, Acc) -> Acc;
term_diff_add(?nav, [], _Opts, Acc) -> Acc;
term_diff_add([L | LRest], ?nav, Opts, Acc) ->
    term_diff_add(LRest, ?nav, Opts, [#ddTermDiff{id = length(Acc) + 1, left = list_to_binary(L)} | Acc]);
term_diff_add(?nav, [R | RRest], Opts, Acc) ->
    term_diff_add(?nav, RRest, Opts, [#ddTermDiff{id = length(Acc) + 1, right = list_to_binary(R)} | Acc]);
...
merge_diff(Left, Right, Merged, Opts, User) ->
    imem_meta:log_to_db(
        ...
        ?MODULE,
        merge_diff,
        [{left, Left}, {right, Right}, {merged, Merged}],
        "merge_diff table"
    ),
    ...
    case {LeftQname, RightQname, MergedQname} of
        {{MySchema, L}, {MySchema, R}, {MySchema, _W}} ->
            case {merge_scan(Left), merge_scan(Right)} of
                {{0, _}, _} ->
                    % row_count=0
                    ?ClientError({"Empty table", Left});
                {_, {0, _}} ->
                    % row_count=0
                    ?ClientError({"Empty table", Right});
                {{_, Cols}, {_, Cols}} ->
...
merge_scan(Table, I, {Rows, 3}) -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 3});
merge_scan(Table, I, {Rows, 2}) ->
    [Row] = imem_meta:dirty_read(Table, I),
    case element(5, Row) of
        E3 when E3 == ?nav; E3 == <<>>; E3 == [] -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 2});
        _ -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 3})
    end;
    ...
    case element(5, Row) of
        E3 when E3 == ?nav; E3 == <<>>; E3 == [] ->
            case element(4, Row) of
                ...
                _ -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 2})
            end;
        _ -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 3})
    end;
            ...
            case element(4, Row) of
                ...
                _ ->
                    case element(3, Row) of
                        ...
                        _ -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 1})
                    end
            end;
        _ -> merge_scan(Table, imem_meta:dirty_next(Table, I), {Rows + 1, 3})
    end.
...
merge_write_out(Cols, Merged, Opts, User, K, [{_, []} | Diff]) -> merge_write_out(Cols, Merged, Opts, User, K, Diff);
...
merge_write(1, Merged, Opts, User, K, [L | Ls], [R | Rs]) ->
    ...
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, L, Cmp, R, <<>>, <<>>, <<>>, <<>>},
        User
    ),
...
merge_write(1, Merged, Opts, User, K, [L | Ls], ?nav) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, L, <<>>, ?nav, <<>>, <<>>, <<>>, <<>>},
        User
    ),
...
merge_write(1, Merged, Opts, User, K, ?nav, [R | Rs]) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, ?nav, <<>>, R, <<>>, <<>>, <<>>, <<>>},
        User
    ),
...
merge_write(2, Merged, Opts, User, K, [L | Ls], ?nav) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, K, L, <<>>, ?nav, ?nav, <<>>, <<>>},
        User
    ),
...
merge_write(2, Merged, Opts, User, K, ?nav, [R | Rs]) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, ?nav, ?nav, <<>>, K, R, <<>>, <<>>},
        User
    ),
...
merge_write(3, Merged, Opts, User, K, [L | Ls], [R | Rs]) ->
    ...
    imem_meta:insert(
        Merged,
        {
            imem_meta:physical_table_name(Merged),
            undefined,
            element(1, K),
            element(2, K),
            L,
...
merge_write(3, Merged, Opts, User, K, [L | Ls], ?nav) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, element(1, K), element(2, K), L, <<>>, ?nav, ?nav, ?nav},
        User
    ),
...
merge_write(3, Merged, Opts, User, K, ?nav, [R | Rs]) ->
    imem_meta:insert(
        Merged,
        {imem_meta:physical_table_name(Merged), undefined, ?nav, ?nav, ?nav, <<>>, element(1, K), element(2, K), R},
        User
    ),
    ...
    [
        {
            "TD_EQUAL1",
            ?_assertEqual(
                [#ddTermDiff{id = 1, left = <<"ABC">>, cmp = <<"=">>, right = <<"ABC">>}],
