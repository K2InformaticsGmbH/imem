...
trans_time_init() -> erlang:put(?TRANS_TIME_NAME, ?TIME_UID).
...
field_pick_pointers([$(, A, $) | Rest], Acc) -> field_pick_pointers(Rest, [(A - 48) | Acc]);
field_pick_pointers([$(, A, B, $) | Rest], Acc) -> field_pick_pointers(Rest, [10 * (A - 48) + (B - 48) | Acc]);
...
data_nodes() -> data_nodes(mnesia:system_info(running_db_nodes), []).

data_nodes([], Acc) -> lists:reverse(Acc);
data_nodes([Node | Nodes], Acc) ->
    case schema(Node) of
        {badrpc, _} -> data_nodes(Nodes, Acc);
        Schema -> data_nodes(Nodes, [{Schema, Node} | Acc])
    end.

all_tables() -> lists:delete(schema, mnesia:system_info(tables)).
...
table_type(Table) -> table_info(Table, type).

table_columns(Table) -> table_info(Table, attributes).

table_info(Table, InfoKey) ->
    Node = node(),
    try
        case mnesia:table_info(Table, where_to_read) of
            nowhere -> ?ClientErrorNoLogging({"Table info cannot be read", {Table, InfoKey}});
            Node -> mnesia:table_info(Table, InfoKey);
            Other ->
                case rpc:call(Other, mnesia, table_info, [Table, InfoKey], ?TABLE_INFO_RPC_TIMEOUT) of
                    {badrpc, Reason} -> ?ClientErrorNoLogging({"Table info is not accessible by rpc", Reason});
                    Result -> Result
                end
        end
    catch
...
table_record_name(Table) -> table_info(Table, record_name).

table_size(Table) -> table_info(Table, size).

table_memory(Table) -> table_info(Table, memory) * erlang:system_info(wordsize).
...
wait_table_tries(Tables, {0, _}) -> ?ClientErrorNoLogging({"Loading table(s) timeout~p", Tables});
...
create_index(Table, Column) when is_atom(Table) ->
    case mnesia:add_table_index(Table, Column) of
        {aborted, {no_exists, Table}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        {aborted, {already_exists, Table, _}} -> ?ClientErrorNoLogging({"Index already exists", {Table, Column}});
...
drop_index(Table, Column) when is_atom(Table) ->
    case mnesia:del_table_index(Table, Column) of
        {aborted, {no_exists, Table}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        {aborted, {no_exists, Table, _}} -> ?ClientErrorNoLogging({"Index does not exist", {Table, Column}});
        Result -> return_atomic_ok(Result)
    end.
...
dirty_read(Table, Key) when is_atom(Table) -> try mnesia:dirty_read(Table, Key) catch
        exit : {aborted, {no_exists, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        exit : {aborted, {no_exists, _, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        throw : Reason -> ?SystemExceptionNoLogging({"Mnesia dirty_read failure", Reason}) end.

dirty_index_read(Table, SecKey, Index) when is_atom(Table) -> try mnesia:dirty_index_read(Table, SecKey, Index) catch
        exit : {aborted, {no_exists, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        exit : {aborted, {no_exists, _, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        exit : {badarg, [Table, SecKey, Index]} -> ?ClientErrorNoLogging({"Index does not exist", {Table, Index}});
        throw : Reason -> ?SystemExceptionNoLogging({"Mnesia dirty_index_read failure", Reason}) end.
...
dirty_write(Table, Row) when is_atom(Table), is_tuple(Row) ->
    try mnesia:dirty_write(Table, Row) of ok -> ?TOUCH_SNAP(Table) catch
        exit : {aborted, {no_exists, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        exit : {aborted, {no_exists, _, _}} -> ?ClientErrorNoLogging({"Table does not exist", Table});
        throw : Reason -> ?SystemExceptionNoLogging({"Mnesia dirty_write failure", Reason}) end.
...
dirty_select(Table, MatchSpec) when is_atom(Table) -> mnesia:dirty_select(Table, MatchSpec).

dirty_delete(Table, Key) when is_atom(Table) -> mnesia:dirty_delete(Table, Key).
...
update_tables(UpdatePlan, Lock) ->
    Update =
        fun
            () ->
                [
                    update_xt(Table, Item, Lock, Old, New, Trigger, User, TrOpts)
                    || [Table, Item, Old, New, Trigger, User, TrOpts] <- UpdatePlan
                ]
        end,
...
when is_atom(Table), is_atom(Field), is_number(Incr), is_number(LimitMin), is_number(LimitMax) ->
...
update_bound_counter(Table, Field, Key, Incr, LimitMin, LimitMax)
when is_atom(Table), is_integer(Field), is_number(Incr), is_number(LimitMin), is_number(LimitMax) ->
    transaction(
        fun
            () ->
...
update_xt({_Table, bag}, _Item, _Lock, {}, {}, _, _, _) -> ok;
update_xt({Table, bag}, Item, Lock, Old, {}, Trigger, User, TrOpts) when is_atom(Table) ->
    ...
    if
        Exists ->
            ...
            {Item, {}};
        Lock == none -> {Item, {}};
        true -> ?ConcurrencyExceptionNoLogging({"Data is modified by someone else", {Item, Old}})
    end;
update_xt({Table, bag}, Item, Lock, {}, New, Trigger, User, TrOpts) when is_atom(Table) ->
    %% may be expensive
    Current = mnesia:read(Table, element(?KeyIdx, New)),
    Exists = lists:member(New, Current),
    if
        (Exists and (Lock == none)) -> {Item, New};
        Exists -> ?ConcurrencyExceptionNoLogging({"Record already exists", {Item, New}});
        true ->
...
update_xt({_Table, _}, _Item, _Lock, ?NoRec, ?NoRec, _, _, _) -> ok;
...
subscribe(system) ->
    ...
    ok;
subscribe(EventCategory) -> ?ClientErrorNoLogging({"Unsupported event category subscription", EventCategory}).
...
unsubscribe(EventCategory) -> ?ClientErrorNoLogging({"Unsupported event category unsubscription", EventCategory}).
...
foldl(FoldFun, InputAcc, Table) -> return_atomic(transaction(fun mnesia:foldl/3, [FoldFun, InputAcc, Table])).
...
init(_) ->
    ...
    case disc_schema_nodes(SchemaName) of
        [] ->
            case node() of
                nonode@nohost -> ok;
                _ ->
                    ?Warn("no node found at ~p for schema ~p cluster ~p~n", [node(), SchemaName, erlang:get_cookie()]),
                    {ok, _} = mnesia:change_config(extra_db_nodes, [node() | ClusterManagers])
            end;
        [DiscSchemaNode | _] ->
            ...
            {ok, _} = rpc:call(DiscSchemaNode, mnesia, change_config, [extra_db_nodes, MnesiaNodes])
    end,
    ...
    case is_pid(whereis(imem_inet_tcp_dist)) of
        ...
        _ -> ?Warn("imem_inet_tcp_dist not started, possibly missing -proto_dist" " erlang VM start parameter")
    end,
    {ok, #state{}}.
...
handle_info(check_epmd, State) ->
    case epmd_register() of
        {error, {already_registered, _OldPort}} -> ?Debug("EPMD has registration on ~p", [_OldPort]);
...
mnesia_down_stop_if_not_testing(State) -> {noreply, State}.
        ...
        case erl_epmd:names() of
            {error, address} ->
                spawn(
                    fun
                        () ->
                            open_port({spawn_executable, os:find_executable("epmd")}, [{args, ["-daemon"]}]),
            ...
            {ok, RegisteredNodes} ->
                ...
                case proplists:get_value(NodeName, RegisteredNodes) of
                    undefined ->
                        ...
                        ok;
                    OldPort -> {error, {already_registered, OldPort}}
                end
        end
    catch
...
mnesia_atomic(Fun, [_ | _] = Args) when Fun == write; Fun == delete; Fun == delete_object ->
...
ets(Fun, Args) -> mnesia:ets(Fun, Args).

select_count(Tab, MatchSpec) -> ets(fun ets:select_count/2, [Tab, MatchSpec]).
