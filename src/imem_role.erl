...
if_write(_SeCo, Table, #ddRole{} = Role) -> imem_meta:write(Table, Role).

if_read(_SeCo, Table, RoleId) -> imem_meta:read(Table, RoleId).

if_delete(_SeCo, Table, RoleId) -> imem_meta:delete(Table, RoleId).
...
exists(SeCo, #ddRole{id = RoleId} = Role) ->
    %% exists unchanged
    case if_read(SeCo, ddRole, RoleId) of
        [] -> false;
        [Role] -> true;
...
grant_role(SeCo, #ddRole{id = ToRoleId} = ToRole, RoleId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            case if_read(SeCo, ddRole, ToRoleId) of
    ...
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            case exists(SeCo, RoleId) of
                false -> ?ClientError({"Role does not exist", RoleId});
                true ->
                    ...
                    NewRoles =
                        case lists:member(RoleId, Roles) of
                            true -> Roles;
                            false ->
                                %% append because newer = seldom used
                                lists:append(Roles, [RoleId])
                        end,
...
revoke_role(SeCo, #ddRole{id = FromRoleId} = FromRole, RoleId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            case if_read(SeCo, ddRole, FromRoleId) of
...
revoke_role(SeCo, FromRoleId, RoleId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            #ddRole{roles = Roles} = FromRole = get(SeCo, FromRoleId),
...
grant_permission(SeCo, ToRoleId, PermissionId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            #ddRole{permissions = Permissions} = ToRole = get(SeCo, ToRoleId),
            NewPermissions =
                case lists:member(PermissionId, Permissions) of
                    true -> Permissions;
                    false ->
                        %% append because newer = seldom used
                        lists:append(Permissions, [PermissionId])
                end,
...
revoke_permission(SeCo, FromRoleId, PermissionId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            #ddRole{permissions = Permissions} = FromRole = get(SeCo, FromRoleId),
...
grant_quota(SeCo, ToRoleId, QuotaId, Value) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            #ddRole{quotas = Quotas} = ToRole = get(SeCo, ToRoleId),
...
revoke_quota(SeCo, FromRoleId, QuotaId) ->
    case imem_seco:have_permission(SeCo, manage_accounts) of
        true ->
            #ddRole{quotas = Quotas} = FromRole = get(SeCo, FromRoleId),
