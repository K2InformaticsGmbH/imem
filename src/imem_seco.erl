...
-define(
    GET_PASSWORD_LIFE_TIME(__AccountId),
    ?GET_CONFIG(passwordLifeTime, [__AccountId], 100, "Password expiry time in days.")
).
-define(SALT_BYTES, 32).
-define(PWD_HASH, scrypt).

%% target hash: pwdmd5,md4,md5,sha512,scrypt

-define(PWD_HASH_LIST, [scrypt, sha512, pwdmd5]).

%% allowed hash types

-define(REQUIRE_PWDMD5, <<"fun(Factors,NetCtx) -> [pwdmd5] -- Factors end">>).

% access | smsott | saml | pwdmd5
-define(AUTH_SMS_TOKEN_RETRY_DELAY, 1000).
-define(FULL_ACCESS, <<"fun(NetCtx) -> true end">>).
-define(
    PASSWORD_LOCK_TIME,
    ?GET_CONFIG(passwordLockTime, [], 900, "Password lock time in seconds after reaching the password lock count.")
).
-define(
    PASSWORD_LOCK_COUNT,
    ?GET_CONFIG(
        ...
        [],
        5,
        "Maximum number of wrong passwords tolerated before temporarily locking the account."
    )
).
...
-export(
    [
        drop_seco_tables/1,
        % parameters: (Name,Password) or (pwdmd5,Password)
...
-export([has_role/3, has_role/2, has_permission/3, has_permission/2]).
-export([have_role/2, have_permission/2]).

% Functions applied with Common Test
-export([normalized_msisdn/1]).

monitor_pid(SKey, Pid) when is_pid(Pid) -> gen_server:call(?MODULE, {monitor, SKey, Pid}).
...
init(_Args) ->
    try
        %% try creating system tables, may fail if they exist, then check existence
        ...
        case (catch imem_meta:table_columns(ddSeCo@)) of
            L when L == element(1, SDef) ->
                % field names in table match the new record
                ok;
            L when is_list(L) ->
                ?Info("dropping old version of table ddSeCo@", []),
        ...
        imem_meta:init_create_check_table(
            ddSeCo@,
            SDef,
            [{scope, local}, {local_content, true}, {record_name, ddSeCo}],
            system
        ),
        ...
        imem_meta:init_create_check_table(
            ddPerm@,
            PDef,
            [{scope, local}, {local_content, true}, {record_name, ddPerm}],
            system
        ),
        ...
        imem_meta:init_create_check_table(
            ddQuota@,
            QDef,
            [{scope, local}, {local_content, true}, {record_name, ddQuota}],
            system
        ),
            ...
            {[], true} ->
                ...
                Account =
                    #ddAccount{
                        ...
                        fullName = <<"DB Administrator">>,
                        lastPasswordChangeTime = LocalTime
                    },
...
if_select(_SKey, Table, MatchSpec) -> imem_meta:select(Table, MatchSpec).
...
if_select_perm_keys_by_skey(_SKeyM, SKey) ->
    %% M=Monitor / MasterContext
    MatchHead = #ddPerm{pkey = '$1', skey = '$2', _ = '_'},
    Guard = {'==', '$2', SKey},
    Result = '$1',
    if_select(SKey, ddPerm@, [{MatchHead, [Guard], [Result]}]).

if_check_table(_SeKey, Table) -> imem_meta:check_table(Table).

%% -- See similar Implementation in imem_account, imem_seco, imem_role --------------

if_dirty_index_read(_SeKey, Table, SecKey, Index) -> imem_meta:dirty_index_read(Table, SecKey, Index).

if_select_account_by_name(_SeKey, <<"system">>) -> {if_read(ddAccount, system), true};
if_select_account_by_name(_SeKey, Name) -> {if_dirty_index_read(_SeKey, ddAccount, Name, #ddAccount.name), true}.

%% --Interface functions  (calling imem_meta) ----------------------------------

if_drop_table(_SKey, Table) -> imem_meta:drop_table(Table).

if_truncate_table(_SKey, Table) -> imem_meta:truncate_table(Table).

if_write(_SKey, Table, Record) -> imem_meta:write(Table, Record).

if_read(Table, Key) -> imem_meta:read(Table, Key).

if_delete(_SKey, Table, RowId) -> imem_meta:delete(Table, RowId).
...
has_role(_RootRoleId, _RootRoleId) -> true;
...
has_permission(_RootRoleId, []) -> false;
has_permission(RootRoleId, PermissionList) when is_list(PermissionList) ->
    %% search for first match in list of permissions
    case if_read(ddRole, RootRoleId) of
        [#ddRole{permissions = [], roles = []}] -> false;
        [#ddRole{permissions = Permissions, roles = []}] -> list_member(PermissionList, Permissions);
...
has_permission(RootRoleId, PermissionId) ->
    ...
    case if_read(ddRole, RootRoleId) of
        [#ddRole{permissions = [], roles = []}] -> false;
        [#ddRole{permissions = Permissions, roles = []}] -> lists:member(PermissionId, Permissions);
        [#ddRole{permissions = Permissions, roles = ChildRoles}] ->
...
create_credentials(Name, Password) when is_binary(Name), is_binary(Password) ->
    % username/password credential for auth_start/3 and auth_add_credential/2
    {pwdmd5, {Name, erlang:md5(Password)}};
create_credentials(pwdmd5, Password) when is_list(Password) -> create_credentials(pwdmd5, list_to_binary(Password));
...
create_credentials(pwdmd5, Password) when is_binary(Password) -> {pwdmd5, erlang:md5(Password)}.

% for use in authenticate/3 and in raw credentials in ddAccount
cleanup_pid(Pid) ->
    ...
    case self() of
        ...
        _ -> ?SecurityViolation({"Cleanup unauthorized", {self(), Pid}})
    end.

list_member([], _Permissions) -> false;
...
drop_seco_tables(SKey) ->
    case have_permission(SKey, manage_system_tables) of
        ...
        false -> ?SecurityException({"Drop seco tables unauthorized", SKey})
    end.
...
%% security hash is returned back to caller

seco_unregister(#ddSeCo{skey = SKey, pid = Pid}) when Pid == self() -> catch if_delete(SKey, ddSeCo@, SKey).
...
seco_authenticated(SKey) ->
    case if_read(ddSeCo@, SKey) of
        [#ddSeCo{pid = Pid, authState = authenticated} = SeCo] when Pid == self() -> SeCo;
        [#ddSeCo{pid = Pid, authState = authorized} = SeCo] when Pid == self() -> SeCo;
        [#ddSeCo{}] ->
            % Not authenticated
            ?SecurityViolation({"Not authenticated", SKey});
...
seco_authorized(SKey) ->
    case if_read(ddSeCo@, SKey) of
        [#ddSeCo{pid = Pid, authState = authorized} = SeCo] when Pid == self() -> SeCo;
        [#ddSeCo{}] -> ?SecurityViolation({"Not logged in", SKey});
        [] -> ?SecurityException({"Not logged in", SKey})
    end.
...
seco_update(#ddSeCo{skey = SKey}, _) -> ?SecurityViolation({"Not logged in", SKey}).
...
fail_or_clear_password_lock(#ddSeCo{skey = SKey} = SeCo, AccountId) ->
    case if_read(ddAccountDyn, AccountId) of
        ...
        [#ddAccountDyn{lastFailureTime = undefined}] -> ok;
        [#ddAccountDyn{lastFailureTime = LastFailureTuple} = AD] ->
            ...
            % no need for monotony
            EffectiveSecs = 86400 * (366 * LY + 31 * LM + LD) + 3600 * LHr + 60 * LMin + LSec,
...
authenticate(SessionId, Name, {pwdmd5, Token}) ->
    % old direct API for simple password authentication, deprecated
        ...
        {[#ddAccount{id = AccountId} = Account], true} ->
            case if_read(ddAccountDyn, AccountId) of
                ...
                [#ddAccountDyn{lastFailureTime = undefined}] ->
                    ...
                    ok;
                [#ddAccountDyn{id = AccountId}] -> fail_or_clear_password_lock(SeCo, AccountId)
            end,
            ok = check_re_hash(SeCo, Account, Token, Token, true, ?PWD_HASH_LIST),
            % return SKey only
            seco_register(SeCo#ddSeCo{accountName = Name, accountId = AccountId, authFactors = [pwdmd5]}, authenticated);
        {[], true} -> authenticate_fail(SeCo, "Invalid account credentials. Please retry", true)
    end.

-spec auth_start(atom(), any(), ddCredential()) -> {ddSeCoKey(), [ddCredRequest()]} | no_return().
auth_start(AppId, SessionId, Credential) ->
    % access context / network parameters
...
auth_add_cred(SKey, Credential) -> auth_step(seco_existing(SKey), Credential).

-spec auth_abort(ddSeCoKey()) -> ok.
auth_abort(SKey) -> seco_unregister(seco_existing(SKey)).
...
auth_step(SeCo, {access, NetworkCtx}) when is_map(NetworkCtx) ->
    ...
    AccessCheckFunStr =
        ?GET_CONFIG(
            accessCheckFun,
            [SessionCtx#ddSessionCtx.appId],
            ?FULL_ACCESS,
            "Function to check network access parameters in preparation of authentication steps."
        ),
            ...
            [] ->
                case imem_datatype:io_to_fun(AccessCheckFunStr) of
                    ACF when is_function(ACF, 1) ->
                        ...
                        ACF;
                    _ -> authenticate_fail(SeCo, {"Invalid accessCheckFun", AccessCheckFunStr}, true)
                end;
    ...
    {AccountName1, AccountId1} =
        case AccessCheckFun(NewSessionCtx#ddSessionCtx.networkCtx) of
            true ->
                %% access granted
                {AccountName0, AccountId0};
            false ->
                %% access denied
                authenticate_fail(SeCo, {"Network access denied", NetworkCtx}, true);
            AccountName0 ->
                %% accountName confirmed by network
                {AccountName0, AccountId0};
            Name when is_binary(Name), AccountName0 == undefined ->
                %% accountName defined by network
                case if_select_account_by_name(SKey, Name) of
                    {[#ddAccount{locked = true}], true} ->
                        authenticate_fail(SeCo, "Account is locked. Contact a system administrator", true);
                    {[#ddAccount{id = AccId}], true} -> {Name, AccId};
                    {[], true} -> authenticate_fail(SeCo, {"Access denied for", Name}, true)
                end;
            _ -> authenticate_fail(SeCo, "Account name conflict", true)
        end,
    case lists:member(access, SeCo#ddSeCo.authFactors) of
        ...
        true ->
            AuthRequireFun = get_auth_fun(SeCo),
            {
                SKey,
                [
                    {A, #{accountName => AccountName0}}
                    || A <- AuthRequireFun(SeCo#ddSeCo.authFactors, SessionCtx#ddSessionCtx.networkCtx)
                ]
            }
    end;
auth_step(SeCo, {pwdmd5, {Name, Token}}) ->
    % may not yet exist in ddSeco@
    #ddSeCo{skey = SKey, accountId = AccountId0, authFactors = AFs} = SeCo,
    case if_select_account_by_name(SKey, Name) of
        {[#ddAccount{locked = true}], true} ->
            authenticate_fail(SeCo, "Account is locked. Contact a system administrator", true);
        ...
        {[#ddAccount{}], true} -> authenticate_fail(SeCo, "Account name conflict", true);
        {[], true} -> authenticate_fail(SeCo, "Invalid account credentials. Please retry", true)
    end;
auth_step(SeCo, {smsott, Token}) ->
    ...
    case sms_ott_mobile_phone(AccountId) of
        undefined -> authenticate_fail(SeCo, "Missing mobile phone number for SMS one time token", true);
        To ->
            case (catch imem_auth_smsott:verify_sms_token(SessionCtx#ddSessionCtx.appId, To, Token, {smsott, #{}})) of
                % TODO : smsott with parameters
                ok -> auth_step_succeed(SeCo#ddSeCo{authFactors = [smsott | AFs]});
                _ ->
                    case
                    ?GET_CONFIG(
                        smsTokenValidationRetry,
                        [SessionCtx#ddSessionCtx.appId],
                        true,
                        "Can a wrong SMS authentication token answer be retried (within time limit)?"
                    ) of
                        true ->
                            case
                            (catch imem_auth_smsott:send_sms_token(SessionCtx#ddSessionCtx.appId, To, {smsott, #{}})) of
                                ok ->
                                    timer:sleep(?AUTH_SMS_TOKEN_RETRY_DELAY),
                                    % re-ask for new token
                                    {seco_register(SeCo), [{smsott, #{accountName => AccountName, to => To}}]};
...
auth_step(SeCo, {saml, Name}) ->
    ...
    case if_select_account_by_name(SKey, Name) of
        ...
        {[#ddAccount{locked = true}], true} ->
            authenticate_fail(SeCo, "Account is locked. Contact a system administrator", true);
        {[#ddAccount{}], true} -> authenticate_fail(SeCo, "Account name conflict", true);
        {[], true} -> authenticate_fail(SeCo, "Not a valid user", true)
    end;
auth_step(SeCo, Credential) -> authenticate_fail(SeCo, {"Invalid credential type", element(1, Credential)}, true).
...
authenticate_fail(_SeCo, ErrorTerm, false) -> ?SecurityException(ErrorTerm).

get_auth_fun(#ddSeCo{sessionCtx = SessionCtx} = SeCo) ->
    AuthRequireFunStr =
        ?GET_CONFIG(
            authenticateRequireFun,
            [SessionCtx#ddSessionCtx.appId],
            ?REQUIRE_PWDMD5,
            "Function which defines authentication requirements depending on current authentication step."
        ),
        ...
        [] ->
            case imem_datatype:io_to_fun(AuthRequireFunStr) of
                CF when is_function(CF, 2) ->
                    ...
                    CF;
                _ -> authenticate_fail(SeCo, {"Invalid authenticatonRequireFun", AuthRequireFunStr}, true)
            end;
...
auth_step_succeed(
    #ddSeCo{skey = SKey, accountName = AccountName, accountId = AccountId, sessionCtx = SessionCtx, authFactors = AFs} =
        SeCo
) ->
    AuthRequireFun = get_auth_fun(SeCo),
    case AuthRequireFun(AFs, SessionCtx#ddSessionCtx.networkCtx) of
        [] ->
            case if_read(ddAccountDyn, AccountId) of
                [] ->
                    AD = #ddAccountDyn{id = AccountId},
                    % create dynamic account record if missing
                    if_write(SKey, ddAccountDyn, AD);
                [#ddAccountDyn{lastFailureTime = undefined}] -> ok;
                [#ddAccountDyn{} = AD] -> if_write(SKey, ddAccountDyn, AD#ddAccountDyn{lastFailureTime = undefined})
            end,
            % authentication success, return {SKey,[]}
            {seco_register(SeCo, authenticated), []};
        [smsott] ->
            case sms_ott_mobile_phone(AccountId) of
                undefined -> authenticate_fail(SeCo, "Missing mobile phone number for SMS one time token", true);
                To ->
                    case (catch imem_auth_smsott:send_sms_token(SessionCtx#ddSessionCtx.appId, To, {smsott, #{}})) of
                        ok ->
                            % request a SMS one time token
                            {seco_register(SeCo), [{smsott, #{accountName => AccountName, to => To}}]};
                        {'EXIT', {Err2, _StackTrace}} ->
                            case
                            ?GET_CONFIG(
                                smsTokenSendingErrorSkip,
                                [SessionCtx#ddSessionCtx.appId],
                                false,
                                "Should SMS token authentication be skipped when the token server is unavailable?"
                            ) of
                                true ->
                                    % authentication success, return {SKey,[]}
                                    {seco_register(SeCo, authenticated), []};
            ...
            case sms_ott_mobile_phone(AccountId) of
                ...
                To ->
                    case (catch imem_auth_smsott:send_sms_token(SessionCtx#ddSessionCtx.appId, To, {smsott, #{}})) of
                        ok ->
                            % request a SMS one time token
                            {seco_register(SeCo), [{smsott, #{accountName => AccountName, to => To}} | Rest]};
                        _ ->
                            % skip SMS one time token factor
                            {seco_register(SeCo), [{A, #{accountName => AccountName}} || A <- Rest]}
                    end
            end;
        OFs ->
            % ask for remaining factors to try
            {seco_register(SeCo), [{A, #{accountName => AccountName}} || A <- OFs]}
    end.
...
failure_count({{_, _, _}, {_, _, SF}}) -> SF rem 10.

% Failure count is packed into last second digit of a datetime tuple
failure_datetime(undefined) -> undefined;
failure_datetime({{Y, M, D}, {Hr, Mi, Ss}}) -> {{Y, M, D}, {Hr, Mi, 10 * (Ss div 10)}}.

failure_tuple({{Y, M, D}, {Hr, Mi, Ss}}, undefined) ->
    % first failure (last digit in seconds)
    {{Y, M, D}, {Hr, Mi, 10 * (Ss div 10) + 1}};
failure_tuple({{Y, M, D}, {Hr, Mi, Ss}}, {{_, _, _}, {_, _, SF}}) ->
    {{Y, M, D}, {Hr, Mi, 10 * (Ss div 10) + (SF + 1) rem 10}}.
    ...
    case if_read(ddAccountDyn, AccountId) of
        [] ->
            ...
            if_write(
                SKey,
                ddAccountDyn,
                #ddAccountDyn{id = AccountId, lastFailureTime = failure_tuple(LocalTime, undefined)}
            );
        [#ddAccountDyn{lastFailureTime = LFT} = AD] ->
    ...
    case lists:keyfind(Type, 1, Account#ddAccount.credentials) of
        {Type, {Salt, Hash}} ->
            case hash(Type, Salt, OldToken) of
                ...
                _ ->
                    % fail
                    check_re_hash(SeCo, Account, OldToken, NewToken, Unregister, [])
            end;
        false ->
            % continue
            check_re_hash(SeCo, Account, OldToken, NewToken, Unregister, Types)
...
find_re_hash(SeCo, Account, NewToken, []) -> re_hash(SeCo, undefined, undefined, NewToken, Account);
find_re_hash(SeCo, Account, NewToken, [Type | Types]) ->
    case lists:keyfind(Type, 1, Account#ddAccount.credentials) of
        false -> find_re_hash(SeCo, Account, NewToken, Types);
        FoundCred -> re_hash(SeCo, FoundCred, <<>>, NewToken, Account)
    end.

re_hash(_, {?PWD_HASH, _}, Token, Token, _) ->
    %% re_hash not needed, already using target hash
    ok;
...
login(SKey) ->
    ...
    PwdExpireDate =
        case {AccountId, ?GET_PASSWORD_LIFE_TIME(AccountId)} of
            {system, _} -> 0;
            {_, infinity} ->
                % sorts in after any date tuple
                0;
...
logout(SKey) -> seco_delete(SKey, SKey).
...
-spec password_strength_fun() -> fun((list() | binary()) -> short | weak | medium | strong).
