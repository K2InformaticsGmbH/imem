...
%% partition rolling (create new time partitions)
...
-ifdef(TEST).
-define(PROLL_FIRST_WAIT, 1000).

%% 1000 = 1 sec

-define(PROLL_ERROR_WAIT, 1000).

%% 1000 = 1 sec

-define(GET_PROLL_CYCLE_WAIT, ?GET_CONFIG(prollCycleWait, [], 1000, "Wait time in msec between partition rolls.")).

%% 1000 = 1 sec

-else.
-define(PROLL_FIRST_WAIT, 30000).
...
-define(GET_PROLL_CYCLE_WAIT, ?GET_CONFIG(prollCycleWait, [], 100000, "Wait time in msec between partition rolls.")).
...
-define(
    GET_PROLL_ITEM_WAIT,
    ?GET_CONFIG(prollItemWait, [], 10, "Wait time in msec between individual partition rolls within a cycle.")
).
...
-record(
    state,
    {
        %% list({TableAlias, TableName})
        prollList = [] :: list({atom(), atom()})
    }
).
...
handle_info(
    {roll_partitioned_tables, ProllCycleWait, ProllItemWait},
    State = #state{prollList = [{TableAlias, TableName} | Rest]}
) ->
    % process one proll candidate
    NextWait =
        case imem_if_mnesia:read(ddAlias, {imem_meta:schema(), TableAlias}) of
            [] ->
...
missing_partitions(AL, [Next | Times], IsHeadOfCluster, [TableAlias | Rest], Acc0) ->
    ...
    Acc1 =
        case catch (imem_meta:check_table(TableName)) of
            ok -> Acc0;
