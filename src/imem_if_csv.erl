...
-define(CSV_MAX_FILE_SIZE, 10000000).

% overridable in schema name: csv$eof$123 reads 123 bytes max per file
-define(CSV_DELIMITER, <<$">>).
-define(CSV_DELIMITER2, <<$", $">>).

% CSV escape of "
-define(CSV_DELIMITER3, <<$\\, $">>).

% Erlang escape of "
-define(ALL_CSV_OPTS, [lineSeparator, columnSeparator, columnCount, columns, header, encoding, skip]).
-define(
    ALL_CSV_SCHEMA_TOKENS,
    [
        {<<"crlf">>, lineSeparator, <<"\r\n">>},
        {<<"lf">>, lineSeparator, <<"\n">>},
        {<<"eof">>, lineSeparator, <<"">>},
...
-export([file_info/1, file_info/3, file_list/1, column_names/1, select/4, fetch_start/5]).
...
file_info(FilePattern, #{lineSeparator := _} = Opts, BinData) -> file_info_col_sep(FilePattern, Opts, BinData);
file_info(FilePattern, Opts, <<>>) -> file_info(FilePattern, Opts, file_sample(FilePattern, Opts));
...
file_info_col_sep(FilePattern, Opts, <<>>) -> file_info_col_sep(FilePattern, Opts, file_sample(FilePattern, Opts));
...
pick_col_separator(_Rows, #{columns := Cols} = Opts, []) -> Opts#{columnSeparator => <<>>, columnCount => length(Cols)};
pick_col_separator(_Rows, #{columnCount := ColumnCount} = Opts, []) ->
    Opts#{columnSeparator => <<>>, columns => default_columns(ColumnCount)};
pick_col_separator(_Rows, Opts, []) -> Opts#{columnSeparator => <<>>, columnCount => 1, columns => default_columns(1)};
pick_col_separator(Rows, Opts, [Sep | Seps]) ->
    ColumnCount =
        case maps:is_key(columnCount, Opts) of
            true -> maps:get(columnCount, Opts);
            false ->
    ...
    case {column_count(RowsSplitBySep), ColumnCount} of
        {false, _} -> pick_col_separator(Rows, Opts, Seps);
        {ColumnCount, ColumnCount} -> Opts#{columnSeparator => Sep};
        {CC, undefined} when is_integer(CC) -> Opts#{columnSeparator => Sep, columnCount => CC};
        {_, _} -> pick_col_separator(Rows, Opts, Seps)
    end.

file_info_col_count(FilePattern, #{columnCount := _} = Opts, BinData) ->
    file_info_col_names(FilePattern, Opts, BinData);
file_info_col_count(FilePattern, Opts, <<>>) -> file_info_col_names(FilePattern, Opts, file_sample(FilePattern, Opts));
file_info_col_count(FilePattern, #{lineSeparator := LineSeparator, columnSeparator := ColumnSeparator} = Opts, BinData) ->
    Rows = delimited_split(BinData, LineSeparator),
    RowsSplitBySep = split_cols(Rows, ColumnSeparator),
    NewOpts =
        case column_count(RowsSplitBySep) of
            false -> Opts#{columnCount => 1};
            CC when is_integer(CC) -> Opts#{columnCount => CC}
        end,
    file_info_col_names(FilePattern, NewOpts, BinData).

file_info_col_names(FilePattern, #{columns := _} = Opts, BinData) -> file_info_encoding(FilePattern, Opts, BinData);
file_info_col_names(FilePattern, Opts, <<>>) -> file_info_col_names(FilePattern, Opts, file_sample(FilePattern, Opts));
file_info_col_names(
    FilePattern,
    #{header := true, lineSeparator := LineSeparator, columnSeparator := ColumnSeparator, columnCount := ColumnCount} =
        Opts,
    BinData
) ->
    Rows = delimited_split(BinData, LineSeparator),
...
file_info_encoding(_FilePattern, #{encoding := _} = Opts, _BinData) -> Opts;
file_info_encoding(FilePattern, Opts, <<>>) -> file_info_encoding(FilePattern, Opts, file_sample(FilePattern, Opts));
file_info_encoding(_FilePattern, Opts, _BinData) -> Opts#{encoding => utf8}.

% ToDo: guess encoding
file_list(FilePattern) when is_binary(FilePattern) -> file_list(binary_to_list(FilePattern));
...
file_sample(FilePattern, Opts) -> file_sample(FilePattern, Opts, file_list(imem_datatype:strip_dquotes(FilePattern))).

file_sample(FilePattern, _Opts, []) -> ?ClientErrorNoLogging({"No files matching name pattern", FilePattern});
        ...
        case byte_size(D0) of
             ?CSV_BLOCK_SIZE ->
                ...
                case delimited_split(D1, LineSeparator) of
                    [<<>>] -> D0;
                    [D2 | _] ->
                        % ToDo: concat all except last for more statistics
                        <<D0/binary, D2/binary, LineSeparator/binary>>
                end;
            _ -> D0
        end,
    ok = file:close(Io),
...
schema_to_opts([Token | Tokens], Opts) ->
    case (catch list_to_integer(binary_to_list(Token))) of
        N when is_integer(N) -> schema_to_opts(Tokens, Opts#{columnCount => N});
        _ ->
            case Token of
                <<"skip", Skip/binary>> ->
                    case (catch list_to_integer(binary_to_list(Skip))) of
                        S when is_integer(S) -> schema_to_opts(Tokens, Opts#{skip => S});
                        _ -> ?ClientErrorNoLogging({"Invalid CSV skip number", Token})
                    end;
                _ ->
                    case lists:keyfind(Token, 1, ?ALL_CSV_SCHEMA_TOKENS) of
                        {Token, Key, Value} -> schema_to_opts(Tokens, Opts#{Key => Value});
                        _ -> ?ClientErrorNoLogging({"Invalid CSV schema token", Token})
                    end
            end
    end.
...
split_cols(Rows, SplitWith) ->
    SplitRows = lists:foldl(fun (Row, Acc) -> Acc ++ [delimited_split(Row, SplitWith)] end, [], Rows),
        ...
        true ->
            ...
            case {binary:part(Bin, {0, DelSize}), binary:part(Bin, {BinSize, - DelSize})} of
                {?CSV_DELIMITER, ?CSV_DELIMITER} ->
                    binary:replace(
                        binary:replace(binary:part(Bin, {1, BinSize - 2}), ?CSV_DELIMITER2, ?CSV_DELIMITER),
                        ?CSV_DELIMITER3,
                        ?CSV_DELIMITER
                    );
                _ -> Bin
...
default_columns(N) -> [list_to_binary(lists:flatten("col", integer_to_list(I))) || I <- lists:seq(1, N)].
...
column_names({Schema, FilePattern}) -> maps:get(columns, file_info(FilePattern, schema_to_opts(Schema))).
                        ...
                        case Contd0 of
                            undefined ->
                                ...
                                case select({Schema, UnquotedFN}, MatchSpec, RowLimit, read) of
                                    ...
                                    {[], {'$end_of_table'}} -> Pid ! {row, [?sot, ?eot]};
                                    {Rows, {'$end_of_table'}} -> Pid ! {row, [?sot, ?eot | Rows]};
                                    {Rows, Contd1} ->
                                        Pid ! {row, [?sot | Rows]},
                                        F(F, Contd1)
                                end;
                            Contd0 ->
                                ...
                                case select(Contd0) of
                                    ...
                                    {Rows, {'$end_of_table'}} -> Pid ! {row, [?eot | Rows]};
                                    {Rows, Contd1} ->
                                        Pid ! {row, Rows},
                                        F(F, Contd1)
                                end
...
read_blocks(
    ...
    case file:pread(Io, Skip, ColumnCount) of
        {ok, Bin} ->
            file:close(Io),
            AllNewRows =
                case ets:match_spec_run([list_to_tuple([?CSV_RECORD_NAME, File, Skip, byte_size(Bin), Bin])], CMS) of
                    [] -> Rows;
                    [Row] -> [Row | Rows]
                end,
            AllNewRowsCount = length(AllNewRows),
            if
                (AllNewRowsCount =< RowLimit) and LastFile ->
                    % last file read completely
                    {lists:reverse(AllNewRows), {'$end_of_table'}};
                (AllNewRowsCount < RowLimit) ->
                    % next file may add one more row
                    % open next file
                    {ok, Io1} = file:open(hd(Files), [raw, read, binary]),
                    read_blocks(Io1, Files, CMS, 0, BlockSize, MaxLineSize, RowLimit, RowsSkipped, CsvOpts, AllNewRows);
                (AllNewRowsCount == RowLimit) ->
                    % enough for now but keep next file open to read
                    % open next file
                    {ok, Io2} = file:open(hd(Files), [raw, read, binary]),
                    {
                        lists:reverse(AllNewRows),
                        continuation(Io2, Files, CMS, 0, BlockSize, MaxLineSize, RowLimit, RowsSkipped, CsvOpts)
                    }
            end;
        eof ->
            file:close(Io),
            case LastFile of
                true -> {lists:reverse(Rows), {'$end_of_table'}};
                false ->
                    % open next file
                    {ok, IoN} = file:open(hd(Files), [raw, read, binary]),
...
read_blocks(Io, [File | Files], CMS, Pos, BlockSize, MaxLineSize, RowLimit, RowsSkipped, CsvOpts, Rows) ->
    ...
    case read_line_or_more(Io, Pos, BlockSize, MaxLineSize, LineSeparator) of
        {ok, Bin} ->
            ...
            {Lines, FileRead} =
                case {binary_ends_with(Bin, LineSeparator), byte_size(Bin) rem BlockSize} of
                    {true, 0} ->
                        % drop trailing empty split
                        {lists:droplast(AllLines), false};
                    {true, _} ->
                        % drop trailing empty split
                        {lists:droplast(AllLines), true};
                    {false, 0} ->
                        % drop possibly incomplete row
                        {lists:droplast(AllLines), false};
                    {false, _} ->
                        % eof expected
                        {AllLines, true}
                ...
                case ColumnSeparator of
                    ...
                    <<>> when ColumnCount > 1 ->
                        fun
                            (Line, {P, Recs}) ->
                                LineSize = byte_size(Line) + LSL,
                                WithSizeFields =
                                    [P, LineSize, change_encoding(Line, Encoding, utf8)]
                                    ++
                                    lists:duplicate(ColumnCount - 1, <<>>),
                                {P + LineSize, [list_to_tuple([?CSV_RECORD_NAME, File | WithSizeFields]) | Recs]}
                        end;
                    CS when is_binary(CS) ->
                        fun
                            (Line, {P, Recs}) ->
                                StrFields =
                                    [unquote(change_encoding(L, Encoding, utf8)) || L <- delimited_split(Line, CS)],
                                LineSize = byte_size(Line) + LSL,
                                WithSizeFields =
                                    case length(StrFields) of
                                        ColumnCount -> [P | [LineSize | StrFields]];
                                        N when N < ColumnCount ->
                                            [P, LineSize | StrFields] ++ lists:duplicate(ColumnCount - N, <<>>);
                                        _ -> [P, LineSize | lists:sublist(StrFields, ColumnCount)]
                                    end,
                                {P + LineSize, [list_to_tuple([?CSV_RECORD_NAME, File | WithSizeFields]) | Recs]}
                        end
                end,
            ...
            {AllRecs, RowsSkipped1} =
                if
                    (RowsSkipped == Skip) ->
                        % skipping completed in previous block
                        {lists:reverse(RevRecs), RowsSkipped};
                    (RowsSkipped + RevRecsLength) =< Skip ->
                        % whole block result must be skipped
                        {[], RowsSkipped + RevRecsLength};
                    true ->
                        % skipping complete in this block
            ...
            AllNewRowsCount = length(AllNewRows),
            if
                (AllNewRowsCount =< RowLimit) and FileRead and LastFile ->
                    % last file read completely
                    file:close(Io),
                    {AllNewRows, {'$end_of_table'}};
                (AllNewRowsCount < RowLimit) ->
                    % next block may add more rows
                ...
                (AllNewRowsCount > RowLimit) ->
                    % too many rows read, clip and re-read from next result row
                ...
                (AllNewRowsCount == RowLimit) ->
                    % this file or next files may contain more data
                    {
                        AllNewRows,
                        continuation(
                            Io,
    ...
    case file:pread(Io, Pos, BlockSize) of
        {ok, D} ->
            case {byte_size(D), byte_size(LineSeparator)} of
                {0, _} -> list_to_binary(lists:reverse(Acc));
                ...
                {_, _} when Acc == [] -> {ok, D};
                {_, _} -> {ok, list_to_binary(lists:reverse([D | Acc]))}
            end;
        Error -> Error
    end.
...
change_encoding(Data, From, From) -> Data;
change_encoding(Data, ansi, To) ->
    case unicode:characters_to_binary(Data, latin1, To) of
        {incomplete, Encoded, Rest} -> Encoded ++ change_encoding(Rest, latin1, To);
