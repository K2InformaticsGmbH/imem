...
init([Channel, CtrlTable, OutputTable]) ->
    ...
    {ok, _} =
        imem_meta:create_table(
            CtrlTable,
            {record_info(fields, loadControl), ?loadControl, #loadControl{}},
            [{record_name, loadControl}],
            system
        ),
    ...
    {ok, _} =
        imem_meta:create_table(
            OutputTable,
            {record_info(fields, loadOutput), ?loadOutput, #loadOutput{}},
            [{record_name, loadOutput}],
            system
        ),
...
handle_call(Req, _From, State) ->
    io:format(user, "Unknown handle_call ~p~n", [Req]),
    {ok, Req, State}.

handle_cast(Msg, State) ->
    io:format(user, "Unknown handle_cast ~p~n", [Msg]),
    {noreply, State}.

handle_info(
    {
        mnesia_table_event,
        {write, CtrlTable, #loadControl{state = getkeys, operation = channel, keyregex = KRegx, limit = Limit}, _, _}
    },
    #state{ctrl_table = CtrlTable} = State
) ->
    {ok, _} = imem_if_mnesia:unsubscribe({table, State#state.ctrl_table, detailed}),
    Parent = self(),
    spawn(fun () -> keys_read_process(Parent, State#state.channel, KRegx, <<"-1.0e100">>, Limit) end),
    {noreply, State#state{ltrc = (State#state.ltrc) #loadOutput{keys = [], keycounter = 0}}};
handle_info(
    {mnesia_table_event, {write, CtrlTable, #loadControl{state = stop, operation = channel}, _, _}},
    #state{ctrl_table = CtrlTable} = State
) ->
    if
        State#state.reader_pid /= undefined ->
...
handle_info(
    {
        mnesia_table_event,
        {write, CtrlTable, #loadControl{state = run, operation = channel, readdelay = ReadDelay}, _, _}
    },
    #state{ctrl_table = CtrlTable} = State
) ->
    ReaderPid =
                    ...
                    fun
                        () ->
                            ...
                            random_read_process(
                                ...
                                State#state.channel,
                                ReadDelay,
                                (State#state.ltrc) #loadOutput.keys,
...
handle_info(
    {mnesia_table_event, {write, CtrlTable, #loadControl{state = stop, operation = audit}, _, _}},
    #state{ctrl_table = CtrlTable} = State
) ->
    if
        State#state.audit_reader_pid /= undefined ->
...
handle_info(
    {
        mnesia_table_event,
        {
            write,
            CtrlTable,
            #loadControl{state = run, operation = audit, keyregex = Key, readdelay = ReadDelay, limit = Limit},
            _,
            _
        }
    },
...
handle_info({keys, Keys}, #state{ltrc = LTRec} = State) ->
    NewLTRec =
        LTRec#loadOutput{keys = LTRec#loadOutput.keys ++ Keys, keycounter = LTRec#loadOutput.keycounter + length(Keys)},
...
handle_info({read, Count, TDiffSec, Key, Value}, State) ->
    NewLTRec =
        (State#state.ltrc)
        #loadOutput{time = ?TIMESTAMP, totalread = Count, rate = Count / TDiffSec, lastItem = Key, lastValue = Value},
    ok = imem_if_mnesia:write(State#state.output_table, NewLTRec),
    {noreply, State#state{ltrc = NewLTRec}};
handle_info({read_audit, Count, TDiffSec, Value}, State) ->
    NewLTRec =
        (State#state.ltra)
        #loadOutput{operation = audit, time = ?TIMESTAMP, totalread = Count, rate = Count / TDiffSec, lastValue = Value},
    ok = imem_if_mnesia:write(State#state.output_table, NewLTRec),
    {noreply, State#state{ltra = NewLTRec}};
handle_info({die, Reason}, State) -> {stop, Reason, State};
...
keys_read_process(Parent, Channel, KeyRegex, FromKey, Limit) ->
    ...
    if
        length(FilteredKeys) == Limit ->
            ...
            Parent ! resubscribe;
        length(FilteredKeys) == 0 -> Parent ! resubscribe;
        length(FilteredKeys) < Limit ->
            Parent ! {keys, FilteredKeys},
            keys_read_process(Parent, Channel, KeyRegex, lists:last(Keys), Limit - length(FilteredKeys));
...
random_read_process(Parent, Channel, ReadDelay, Keys, {StartTime, LastUpdate, Count}) ->
    ...
    {NewLastUpdate, NewCount} =
        if
            (TDiffUs > 1000000) ->
                Parent ! {read, Count + 1, ?TIMESTAMP_DIFF(Now, StartTime) / 1000000, Key, Value},
                {Now, Count + 1};
            true -> {LastUpdate, Count + 1}
        end,
    if
        ReadDelay > 0 -> timer:sleep(ReadDelay);
        true -> ok
    end,
    receive
        _ -> die
    after
        0 -> random_read_process(Parent, Channel, ReadDelay, Keys, {StartTime, NewLastUpdate, NewCount})
    end.
...
audit_read_process(Parent, Channel, ReadDelay, Key, Limit, {StartTime, LastUpdate, Count}) ->
    ...
    after
        0 ->
            NextKey =
                if
                    length(Values) > 0 -> lists:nth(1, re:split(lists:last(Values), "\t"));
                    true -> Key
                end,
