...
-define(GET_PURGE_CYCLE_WAIT, ?GET_CONFIG(purgeCycleWait, [], 10000, "Wait time in msec between table purge cycles.")).

%% 10000 = 10 sec

-define(
    GET_PURGE_ITEM_WAIT,
    ?GET_CONFIG(purgeItemWait, [], 10, "Wait time in msec between individual table purges within a cycle.")
).
...
-define(
    ...
    ?GET_CONFIG(
        ...
        [],
        false,
        "Do we want to use a special purge function to override the standard behaviour?"
    )
).
-define(
    GET_PURGE_SCRIPT_FUN,
    ?GET_CONFIG(
        purgeScriptFun,
        [],
        <<
...
-define(SHOULD_PURGE_HIST, ?GET_CONFIG(isHistPurgeEnabled, [], false, "Should hist records be purged periodically")).
-define(
    PURGE_HIST_HOUR,
    ?GET_CONFIG(purgeHistHourOfDay, [], 3, "Hour of (00..23) day in which history table are can be purged")
).

-behavior(gen_server).

-record(state, {purgeList = [] :: list(), purgeFun = undefined :: any(), purgeHash = undefined :: any()}).
        ...
        PCW when (is_integer(PCW) andalso PCW > 1000) ->
            ...
            case lists:sort(lists:filter(Pred, imem_meta:all_tables())) of
                ...
                PL ->
                    ...
                    handle_info(
                        {purge_partitioned_tables, PCW, ?GET_PURGE_ITEM_WAIT},
                        State#state{purgeFun = PFun, purgeHash = PHash, purgeList = PL}
                    )
            end;
        BadPCW ->
    ...
    case imem_if_mnesia:read(ddTable, {imem_meta:schema(), Tab}) of
        ...
        [#ddTable{opts = Opts}] ->
            case lists:keyfind(purge_delay, 1, Opts) of
                false ->
                    %% no purge delay in table create options, do not purge this file
                    ok;
                {purge_delay, PD} ->
                    {Sec, _} = ?TIMESTAMP,
                    PurgeEnd = Sec - PD,
                    ...
                    if
                        (PartitionEnd >= PurgeEnd) ->
                            %% too young, do not purge this file
                            ok;
                        true ->
        ...
        true ->
            ...
            case ?PURGE_HIST_HOUR of
                H ->
                    ...
                    spawn(fun () -> imem_dal_skvh:purge_history_tables(HistTables) end),
                    erlang:send_after(60 * 60 * 1000, self(), purge_history_tables);
                _ -> erlang:send_after(60 * 1000, self(), purge_history_tables)
            end;
        false -> erlang:send_after(60 * 1000, self(), purge_history_tables)
    end,
    {noreply, State};
...
try_cleanup(MinMemFreePerCent, TableExpiryMarginSec, MaxTableCountPercent, PartTables) ->
    %[{time_to_part_expiry,table_size,partition_time,table}]
    SortedPartTables =
        lists:sort(
            [
                {
                    imem_meta:time_to_partition_expiry(T),
    ...
    ?LogDebug("Possible purging canditate tables ~p", [SortedPartTables]),
    {Os, FreeMemory, TotalMemory} = imem:get_os_memory(),
    MemFreePerCent = FreeMemory / TotalMemory * 100,
    ?LogDebug("[~p] Free ~p%", [Os, MemFreePerCent]),
    if
        MemFreePerCent < MinMemFreePerCent ->
            ?Info("Free mem ~p% required min ~p%", [MemFreePerCent, MinMemFreePerCent]),
            DelCandidates =
                lists:foldl(
                    fun
                        ({TRemain, _RCnt, Class, _TName} = Itm, A) ->
                            if
                                TRemain < TableExpiryMarginSec ->
                                    ClassCnt = length([Spt || Spt <- SortedPartTables, element(3, Spt) =:= Class]),
                                    if
                                        ClassCnt > 1 -> [Itm | A];
                                        true -> A
                                    end;
            ...
            case DelCandidates of
                [] ->
                    ...
                    [{_, _, _, T} | _] =
                        lists:sort(
                            fun ({_, R1, _, _}, {_, R2, _, _}) when R1 > R2 -> true; (_, _) -> false end,
                            SortedPartTables
                        ),
                    imem_meta:log_to_db(
                        info,
        ...
        true ->
            ...
            if
                CurrentTableCount =< MaxTablesCount ->
                    % Nothing to drop yet
                    ?Debug(
                        "No Purge: CurrentTableCount ~p (used ~p% of ~p)",
                ...
                true ->
                    ...
                    if
                        SelectCount > 0 ->
                            ...
                            DropCandidates = [T || {_, _, _, T} <- lists:sublist(RSortedPartTables, SelectCount)],
