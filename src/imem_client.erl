...
get_auth_profile(Domain, AccountId) when is_binary(Domain) -> get_auth_profile(binary_to_list(Domain), AccountId);
get_auth_profile(Domain, AccountId) when is_list(Domain) -> ?APP_AUTH_PROFILE(Domain, AccountId).

fix_url(Url) ->
    case re:run(Url, "github.com/([^/]+)/([^/]+)/raw/([^/]+)/(.*)", [{capture, [1, 2, 3, 4], list}]) of
        {match, [Owner, Repo, Commit, Path]} -> fix_git_raw_url(Owner, Repo, Commit, Path);
        nomatch -> Url
    end.

fix_git_raw_url(Owner, Repo, Commit, Path) ->
    "https://" ++ filename:join(["raw.githubusercontent.com/", Owner, Repo, Commit, Path]).

http(Op, Url, ReqHeaders, Auth, Body) -> http(Op, Url, ReqHeaders, Auth, Body, [], []).
...
http(Op, Url, ReqHeaders, Auth, Body, HttpOptions, Options) when is_map(Body) ->
    http(Op, {Url, ReqHeaders, "application/json; charset=utf-8"}, Auth, imem_json:encode(Body), HttpOptions, Options);
http(Op, Url, ReqHeaders, Auth, Body, HttpOptions, Options) ->
    http(Op, {Url, ReqHeaders, "application/text"}, Auth, Body, HttpOptions, Options).

http(Op, {Url, ReqHeaders, ContentType}, {token, Token}, Body, HttpOptions, Options) ->
    ReqHeaders1 = [{"Authorization", "token " ++ Token} | ReqHeaders],
    http(Op, {Url, ReqHeaders1, ContentType, Body}, HttpOptions, Options);
http(Op, {Url, ReqHeaders, ContentType}, {basic, User, Password}, Body, HttpOptions, Options) ->
...
http(Method, {Url, ReqHeaders, _, _}, HttpOptions, Options) when Method == get; Method == delete ->
    http_req(Method, {Url, ReqHeaders}, HttpOptions, Options);
http(Method, {Url, ReqHeaders, ContentType, Body}, HttpOptions, Options)
...
http_req(Method, Request, HttpOptions, Options) ->
    case httpc:request(Method, Request, HttpOptions, lists:usort([{body_format, binary} | Options])) of
...
parse_http_resp([{_, _} | _] = Headers, Body) ->
    ...
    case MaybeContentType of
        [<<"application/json", _/binary>>] ->
            try imem_json:decode(Body, [return_maps]) catch _:_ -> {error, {invalid_json, Body}} end;
        _ -> Body
    end;
