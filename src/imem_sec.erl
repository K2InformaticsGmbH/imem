...
-export(
    [
        %% get single config value and put default if not found
        get_config_hlk/5,
        %% put single config value with remark
        put_config_hlk/6
    ]
).
-export(
    [authenticate/4, auth_start/4, auth_add_cred/2, auth_abort/1, login/1, change_credentials/3, logout/1, clone_seco/2]
).
-export(
    [
        create_table/4,
        ...
        restore_table/2,
        %% replace/create local table from snapshot backup
        restore_table_as/2,
        %% replace/create local table from snapshot backup
        restore_table_as/3,
        read/2,
        read/3,
        %% read hierarchical list key
        read_hlk/3,
        select/3,
        select/4,
        insert/3,
...
-export(
    [
        ...
        update_cursor_prepare/3,
        %% stateful execution of update plan (fetch aborted first)
        update_cursor_execute/3,
        %% apply any arity funs of default record to current record
        apply_validators/4,
        fetch_recs/4,
        fetch_recs_sort/4,
        %% ToDo: implement proper return of RowFun(), match conditions and joins
        fetch_recs_async/3,
        %% ToDo: implement proper return of RowFun(), match conditions and joins
        fetch_recs_async/4,
        filter_and_sort/4,
...
-export(
    [
        ...
        %% update (first) table and return updated keys
        update_tables/3
    ]
).
-export(
    [
        %% includes table ownership and readonly
        have_table_permission/3,
...
-export(
    [
        ...
        %% merge two data tables into a bigger one, presenting the differences side by side
        merge_diff/5,
        %% take (LeftType, LeftData, RightType, RightData) and produce data for a side-by-side view
        term_diff/5,
        %% take (LeftType, LeftData, RightType, RightData, Opts) and produce data for a side-by-side view
        term_diff/6
    ]
).
...
authenticate(_SKey, SessionId, Name, Credentials) -> imem_seco:authenticate(SessionId, Name, Credentials).

auth_start(_Skey, AppId, SessionId, Credential) -> imem_seco:auth_start(AppId, SessionId, Credential).

auth_add_cred(SKey, Credential) -> imem_seco:auth_add_cred(SKey, Credential).

auth_abort(SKey) -> imem_seco:auth_abort(SKey).

login(SKey) -> imem_seco:login(SKey).

change_credentials(SKey, OldCred, NewCred) -> imem_seco:change_credentials(SKey, OldCred, NewCred).

logout(SKey) -> imem_seco:logout(SKey).

clone_seco(SKey, Pid) -> imem_seco:clone_seco(SKey, Pid).

%% from imem_meta --- HELPER FUNCTIONS do not export!! --------

if_is_system_table(SKey, {_Node, Schema, Table}) -> if_is_system_table(SKey, {Schema, Table});
...
meta_field_list(_SKey) -> imem_meta:meta_field_list().

meta_field(SKey, Name) when is_atom(Name) -> meta_field(SKey, ?atom_to_binary(Name));
...
meta_field_info(_SKey, Name) -> imem_meta:meta_field_info(Name).
...
all_tables(SKey) -> all_selectable_tables(SKey, imem_meta:all_tables(), []).
...
is_readable_table(SKey, {_Node, Schema, Table}) -> is_readable_table(SKey, {Schema, Table});
...
tables_starting_with(SKey, Prefix) when is_atom(Prefix) -> tables_starting_with(SKey, atom_to_list(Prefix));
tables_starting_with(SKey, Prefix) when is_list(Prefix) -> atoms_starting_with(Prefix, all_tables(SKey)).
...
table_type(SKey, {_Node, Schema, Table}) -> table_type(SKey, {Schema, Table});
...
table_columns(SKey, {_Node, Schema, Table}) -> table_columns(SKey, {Schema, Table});
...
table_record_name(SKey, {_Node, Schema, Table}) -> table_record_name(SKey, {Schema, Table});
...
table_size(SKey, {_Node, Schema, Table}) -> table_size(SKey, {Schema, Table});
...
subscribe(SKey, {table, {_Node, Schema, Table}, Level}) -> subscribe(SKey, {table, {Schema, Table}, Level});
...
subscribe(SKey, EventCategory) -> ?SecurityException({"Unsupported event category", {EventCategory, SKey}}).

unsubscribe(_SKey, EventCategory) -> imem_meta:unsubscribe(EventCategory).
...
create_table(SKey, Table, RecordInfo, Opts) ->
    ...
    case Owner of
        false -> ?SecurityException({"Create table unauthorized", {Table, SKey}});
        Owner -> imem_meta:create_table(Table, RecordInfo, Opts, Owner)
    end.
    ...
    case Owner of
        ...
        Owner ->
            ...
            imem_meta:create_check_table(
                imem_meta:qualified_new_table_name(Table),
                lists:map(Conv, RecordInfo),
                Opts,
                Owner
            )
    end.
...
create_sys_conf(SKey, Path) ->
    #ddSeCo{accountId = AccountId} = seco_authorized(SKey),
    Owner =
        case if_is_system_table(SKey, Path) of
            true -> system;
            false ->
                case imem_seco:have_permission(SKey, [manage_user_tables, create_table]) of
                    true -> AccountId;
    ...
    case Owner of
        false -> ?SecurityException({"Create sys conf schema unauthorized", {Path, SKey}});
        Owner -> imem_meta:create_sys_conf(Path)
    end.
...
init_create_index(SKey, Table, IndexDefinition) ->
    case authorized_table_create_owner(SKey, Table) of
        false -> ?SecurityException({"Create index unauthorized", {Table, SKey}});
        _Owner ->
            case have_table_permission(SKey, Table, select) of
...
purge_table(SKey, Table) -> purge_table(SKey, Table, []).
...
drop_system_table(SKey, Table, Opts, _AccountId) ->
    case imem_seco:have_permission(SKey, manage_system_tables) of
        true -> imem_meta:drop_table(Table, Opts);
        false -> ?SecurityException({"Drop system table unauthorized", {Table, SKey}})
    end.

drop_system_table_index(SKey, Table, Index, _AccountId) ->
    case imem_seco:have_permission(SKey, manage_system_tables) of
        true -> imem_meta:drop_index(Table, Index);
        false -> ?SecurityException({"Drop system table index unauthorized", {Table, Index, SKey}})
    end.

drop_system_table_index(SKey, Table, _AccountId) ->
    case imem_seco:have_permission(SKey, manage_system_tables) of
        true -> imem_meta:drop_index(Table);
        false -> ?SecurityException({"Drop system table index unauthorized", {Table, SKey}})
    end.

purge_system_table(SKey, Table, Opts, _AccountId) ->
    case imem_seco:have_permission(SKey, manage_system_tables) of
        true -> imem_meta:purge_table(Table, Opts);
        false -> ?SecurityException({"Purge system table unauthorized", {Table, SKey}})
    end.
...
insert(SKey, Table, Row) -> insert(SKey, Table, Row, []).
...
update(SKey, Table, Row) -> update(SKey, Table, Row, []).
...
merge(SKey, Table, Row) -> merge(SKey, Table, Row, []).
...
remove(SKey, Table, Row) -> remove(SKey, Table, Row, []).
...
exec(SKey, Statement, BlockSize, Opts) -> imem_sql:exec(SKey, Statement, BlockSize, Opts, true).

fetch_recs(SKey, Pid, Sock, Timeout) -> imem_statement:fetch_recs(SKey, Pid, Sock, Timeout, true).

fetch_recs_sort(SKey, Pid, Sock, Timeout) -> imem_statement:fetch_recs_sort(SKey, Pid, Sock, Timeout, true).

fetch_recs_async(SKey, Pid, Sock) -> imem_statement:fetch_recs_async(SKey, Pid, Sock, true).

fetch_recs_async(SKey, Opts, Pid, Sock) -> imem_statement:fetch_recs_async(SKey, Pid, Sock, Opts, true).
...
fetch_start(SKey, Pid, dba_tables, MatchSpec, BlockSize, Opts) ->
    case imem_seco:have_permission(SKey, [manage_system_tables]) of
        true -> imem_meta:fetch_start(Pid, dba_tables, MatchSpec, BlockSize, Opts);
        false -> ?SecurityException({"Select unauthorized", {dba_tables, SKey}})
    end;
fetch_start(SKey, Pid, user_tables, MatchSpec, BlockSize, Opts) ->
    seco_authorized(SKey),
    %% ToDo: {select_filter_user(SKey, RList, []), true};
...
fetch_start(SKey, Pid, all_tables, MatchSpec, BlockSize, Opts) ->
    ...
    imem_meta:fetch_start(Pid, all_tables, MatchSpec, BlockSize, Opts);
fetch_start(SKey, Pid, Table, MatchSpec, BlockSize, Opts) ->
    ...
    case Table of
        {Node, Schema, T} ->
            % local schema select
            fetch_start_local(SKey, Pid, {Schema, T}, MatchSpec, BlockSize, Opts);
        {Schema, T} ->
            % local schema select
            fetch_start_local(SKey, Pid, {Schema, T}, MatchSpec, BlockSize, Opts);
        {Node, S, T} ->
            % ddSysConf / csv$ superuser select
            fetch_start_system(SKey, Pid, {S, T}, MatchSpec, BlockSize, Opts);
        {S, T} ->
            % ddSysConf / csv$ superuser select
            fetch_start_system(SKey, Pid, {S, T}, MatchSpec, BlockSize, Opts);
...
fetch_close(SKey, Pid) -> imem_statement:fetch_close(SKey, Pid, false).

update_prepare(SKey, Tables, ColMap, ChangeList) ->
    imem_statement:update_prepare(true, SKey, Tables, ColMap, ChangeList).

update_cursor_prepare(SKey, Pid, ChangeList) -> imem_statement:update_cursor_prepare(SKey, Pid, true, ChangeList).

update_cursor_execute(SKey, Pid, Lock) -> imem_statement:update_cursor_execute(SKey, Pid, true, Lock).
...
restore_table_as(SKey, {Table, NewTable}) -> restore_table_as(SKey, Table, NewTable).
...
select_filter_all(SKey, [#ddTable{qname = TableQN} = H | Tail], Acc0) ->
    Acc1 =
        case have_table_permission(SKey, TableQN, select) of
            true -> [H | Acc0];
            false -> Acc0
        end,
...
select_filter_user(SKey, [#ddTable{qname = TableQN} = H | Tail], Acc0) ->
    Acc1 =
        case have_table_ownership(SKey, TableQN) of
            true -> [H | Acc0];
            false -> Acc0
        end,
...
select(SKey, dba_tables, MatchSpec) ->
    case imem_seco:have_permission(SKey, [manage_system_tables]) of
        true -> imem_meta:select(ddTable, MatchSpec);
        false -> ?SecurityException({"Select unauthorized", {dba_tables, SKey}})
    end;
...
select(SKey, Table, MatchSpec, 0) -> select(SKey, Table, MatchSpec);
select(SKey, dba_tables, MatchSpec, Limit) ->
    case imem_seco:have_permission(SKey, [manage_system_tables]) of
        true -> imem_meta:select(ddTable, MatchSpec, Limit);
        false -> ?SecurityException({"Select unauthorized", {dba_tables, SKey}})
    end;
...
admin_apply(SKey, Module, Function, Params, Permissions) ->
    case imem_seco:have_permission(SKey, Permissions) of
        true -> apply(Module, Function, Params);
        false -> ?SecurityException({"Admin execute unauthorized", {Module, Function, Params, SKey}})
    end.
...
dal_apply(SKey, Module, Function, Params, Permissions) ->
    case imem_seco:have_permission(SKey, Permissions) of
        true -> apply(Module, Function, [imem_seco:account_id(SKey) | Params]);
        false -> ?SecurityException({"Dal execute unauthorized", {Module, Function, Params, SKey}})
    end.
...
merge_diff(SKey, Left, Right, Merged, Opts) ->
    case
    {
        have_table_permission(SKey, Left, read),
        have_table_permission(SKey, Right, read),
        have_table_permission(SKey, Merged, write)
    } of
                ...
                case Operation of
                    select ->
                        case lists:member(Table, ?DataTypes) of
                            false -> have_table_permission(SKey, Table, Operation, if_is_system_table(SKey, Table));
                            true -> true
                        end;
                    _ -> have_table_permission(SKey, Table, Operation, if_is_system_table(SKey, Table))
                end,
            set_permission_cache(SKey, Permission, Result),
            Result
    end.
...
seco_authorized(SKey) ->
    case imem_meta:read(ddSeCo@, SKey) of
        ...
        [#ddSeCo{pid = Pid}] -> ?SecurityViolation({"Not logged in", {SKey, Pid}});
        [] -> ?SecurityException({"Not logged in", SKey})
    end.

have_table_ownership(SKey, {_Node, Schema, Table}) -> have_table_ownership(SKey, {Schema, Table});
...
have_table_permission(SKey, {Schema, Table}, Operation, false) ->
    case imem_meta:read(ddTable, {Schema, Table}) of
        [#ddTable{qname = {Schema, Table}, readonly = true}] ->
            %% allow write for user table managers
