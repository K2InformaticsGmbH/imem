...
% API interface
-export([create_sys_conf/1, table_columns/1, table_type/1, table_record_name/1, fetch_start/5, update_tables/2]).
...
handle_call({add, Path, Table}, _From, State) ->
    try
        imem_meta:create_check_table(
            {ddSysConf, Table},
            {record_info(fields, ddSysConf), ?ddSysConf, #ddSysConf{}},
            [],
            system
        ),
    ...
    case filelib:is_dir(Path) of
        true ->
            [
                ...
                end || File <- filelib:wildcard("*.*", Path)
            ],
            ok;
        false ->
            ...
            case gen_server:call(?MODULE, {add, DirPath, File}) of
                {ok, {ddSysConf, _}} -> ok;
                {error, Reason} -> ?SystemExceptionNoLogging({"System Configuration setup failed", {Path, Reason}})
            end
    end.

-spec table_columns(atom()) -> [atom()].
table_columns(_Table) -> record_info(fields, ddSysConf).
...
fetch_start(Pid, Table, _MatchSpec, _BlockSize, _Opts) ->
    ...
    case filelib:is_file(File) of
        ...
        false -> ?SystemExceptionNoLogging({"File not found", File})
    end.
...
update_tables([[Table, Item, Old, New, Trigger, User, TrOpts] | UpdatePlan], Lock, Acc) ->
    if
        ...
        New#ddSysConf.item =/= content -> ?ClientErrorNoLogging({"Readonly field", New#ddSysConf.item});
        true -> ok
    end,
...
update_xt({_Table, _}, _Item, _Lock, {}, {}, _Trigger, _User, _TrOpts) -> ok;
        ...
        New ->
            case Lock of
                none ->
                    ...
                    {Item, New};
                _ -> ?ConcurrencyExceptionNoLogging({"Data is already created by someone else", {File, New}})
            end;
        Current ->
            case Lock of
                none ->
                    ...
                    {Item, New};
                _ -> ?ConcurrencyExceptionNoLogging({"Data is already created by someone else", {File, Current}})
            end
    end;
...
file_size_rec(FileInfo) -> #ddSysConf{item = size, itemStr = integer_to_binary(FileInfo#file_info.size)}.

file_time_rec(accessed, FileInfo) -> #ddSysConf{item = accessed, itemStr = ?FmtTime(FileInfo#file_info.atime)};
file_time_rec(modified, FileInfo) -> #ddSysConf{item = modified, itemStr = ?FmtTime(FileInfo#file_info.mtime)};
file_time_rec(created, FileInfo) -> #ddSysConf{item = created, itemStr = ?FmtTime(FileInfo#file_info.ctime)}.
...
check_content(Old, New) ->
    if
        (New#ddSysConf.itemStr =/= undefined) andalso (New#ddSysConf.itemBin =/= undefined) ->
            ?ClientErrorNoLogging(
                {
                    "Data provided in multiple formats",
    ...
    if
        (Old#ddSysConf.itemStr =/= undefined) andalso (New#ddSysConf.itemStr =:= undefined) ->
            ?ClientErrorNoLogging(
                {
                    "Input and Existing data format missmatch",
    ...
    if
        is_binary(New#ddSysConf.itemStr)
        andalso (byte_size(New#ddSysConf.itemStr) == 0)
        andalso is_binary(New#ddSysConf.itemBin)
        andalso (byte_size(New#ddSysConf.itemBin) == 0) ->
