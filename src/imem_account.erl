...
if_write(_SeKey, Table, Row) -> imem_meta:write(Table, Row).

if_read(_SeKey, Table, RowId) -> imem_meta:read(Table, RowId).

if_delete(_SeKey, Table, RowId) -> imem_meta:delete(Table, RowId).
...
if_select_account_by_name(_SeKey, <<"system">>) -> {if_read(_SeKey, ddAccount, system), true};
...
create(SeKey, #ddAccount{id = AccountId, name = Name} = Account) when is_binary(Name) ->
    case imem_seco:have_permission(SeKey, manage_accounts) of
        true ->
            case if_read(SeKey, ddAccount, AccountId) of
                [#ddAccount{}] -> ?ClientError({"Account already exists", AccountId});
                [] ->
                    case if_select_account_by_name(SeKey, Name) of
                        {[], true} ->
                            ...
                            try ok = imem_role:create(SeKey, AccountId) catch
                                _:Error ->
                                    %% simple transaction rollback
                                    delete(SeKey, Account),
                                    delete(SeKey, AccountDyn),
                                    ?SystemException(Error) end;
...
update_dyn(SeKey, #ddAccountDyn{id = AccountId} = AccountDyn, AccountDynNew) ->
    case imem_seco:have_permission(SeKey, manage_accounts) of
        true ->
            case if_read(SeKey, ddAccountDyn, AccountId) of
...
delete(SeKey, Name, []) -> delete(SeKey, Name);
delete(_SeKey, _Name, Opts) -> ?UnimplementedException({"Unimplemented account delete option", Opts}).

%% ToDo: [cascade]

delete(SeKey, Name) when is_binary(Name) -> delete(SeKey, get_by_name(SeKey, Name));
...
delete(SeKey, AccountId) ->
    case imem_seco:have_permission(SeKey, manage_accounts) of
        true -> case if_delete(SeKey, ddAccount, AccountId) of ok -> imem_role:delete(SeKey, AccountId) end;
        false -> ?SecurityException({"Delete account unauthorized", SeKey})
    end.

lock(SeKey, Name) when is_binary(Name) -> lock(SeKey, get_by_name(SeKey, Name));
...
unlock(SeKey, Name) when is_binary(Name) -> unlock(SeKey, get_by_name(SeKey, Name));
...
renew(SeKey, Name) when is_binary(Name) -> renew(SeKey, get_by_name(SeKey, Name));
...
expire(SeKey, Name) when is_binary(Name) -> expire(SeKey, get_by_name(SeKey, Name));
...
exists(SeKey, Name) when is_binary(Name) -> exists(SeKey, get_by_name(SeKey, Name));
exists(SeKey, #ddAccount{id = AccountId} = Account) ->
    %% exists unchanged
    case imem_seco:have_permission(SeKey, manage_accounts) of
        true ->
            case if_read(SeKey, ddAccount, AccountId) of
...
exists(SeKey, AccountId) ->
    %% exists, maybe in changed form
    case imem_seco:have_permission(SeKey, manage_accounts) of
        true ->
            case if_read(SeKey, ddAccount, AccountId) of
                [] -> false;
