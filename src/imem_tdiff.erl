...
%% not save to be called from SQL
...
-type options() :: [option()].
-type option() :: ignore_whitespace
                | ignore_casing
                | ignore_dquotes
                | {algorithm_tracer, no_tracer | algorithm_tracer()}.
-type algorithm_tracer() :: fun(
                              (
                                  {d, d()}
                                  | {dpath, dpath()}
                                  | {exhausted_kdiagonals, d()}
                                  | {final_edit_script, edit_script()}
                              )
                              ->
                              _
                          ).
...
is_eq(Diff) -> (remove_eq(Diff) == []).

%% Diff contains only {eq,_} terms

remove_eq(Diff) -> remove_eq(Diff, []).

%% Diff wirth {eq,_} terms removed
...
fdl([{eq, Ls} | T], X, Y) -> fdl(T, X + length(Ls), Y + length(Ls));
fdl([], _X, _Y) -> [].
...
format_lines(Indicator, Lines) -> lists:map(fun (Line) -> io_lib:format("~s~s", [Indicator, Line]) end, Lines).
...
diff(L, R, Opts) when is_binary(L) -> diff(binary_to_list(L), R, Opts);
diff(L, R, Opts) when is_binary(R) -> diff(L, binary_to_list(R), Opts);
diff(L, R, Opts) when is_list(L), is_list(R), is_list(Opts) ->
    {L1, R1} =
        case lists:member(ignore_casing, Opts) of
            true -> {norm_casing(L), norm_casing(R)};
            false -> {L, R}
        end,
    {L2, R2} =
        case lists:member(ignore_dquotes, Opts) of
            true -> {norm_dquotes(L1), norm_dquotes(R1)};
            false -> {L1, R1}
        end,
    {L3, R3} =
        case lists:member(ignore_whitespace, Opts) of
...
diff_reverse([{A, EL} | Script], Acc) -> diff_reverse(Script, [{A, lists:reverse(EL)} | Acc]).
...
norm_whitespace([I | Seq], Acc) -> norm_whitespace(Seq, [I | Acc]).
...
norm_dquotes([I | Seq], Acc) -> norm_dquotes(Seq, [I | Acc]).
...
norm_casing([I | Seq], Acc) -> norm_casing(Seq, [I | Acc]).
...
diff_raw(Sx, Sy, Opts) ->
    ...
    EditScript =
        case try_dpaths(0, DMax, [{0, 0, Sx, Sy, []}], Tracer) of
            no -> [{del, Sx}, {ins, Sy}];
...
try_dpaths(_, _DMax, _DPaths, _Tracer) -> no.
...
-spec patch_binaries(binary(), edit_script(Line)) -> [Line] when Line :: string().
patch_binaries(B, Diff) -> patch(split_bin_to_lines(B), Diff).
...
whitespace_1_test() -> [{eq, "A"}, {ins, " "}, {eq, "/"}, {ins, " "}, {eq, "B"}, {del, " "}] = diff("A/B ", "A / B").

whitespace_1r_test() -> [{eq, "A"}, {del, " "}, {eq, "/"}, {del, " "}, {eq, "B"}, {ins, " "}] = diff("A / B", "A/B ").

whitespace_1wb_test() -> [{eq, [{"A / B", "A/B "}]}] = diff(["A / B"], ["A/B "], [ignore_whitespace]).
...
whitespace_2_test() -> [{eq, "C("}, {ins, " "}, {eq, "D"}, {del, " "}] = diff("C(D ", "C( D").

whitespace_2r_test() -> [{eq, "C("}, {del, " "}, {eq, "D"}, {ins, " "}] = diff("C( D", "C(D ").

whitespace_2ro_test() -> [{eq, "C("}, {del, " "}, {eq, "D"}, {ins, " "}] = diff("C( D", "C(D ").

whitespace_3_test() -> [{eq, "E"}, {ins, " "}, {eq, "+"}, {del, " "}, {eq, "F"}, {del, " "}] = diff("E+ F ", "E +F").

whitespace_3r_test() -> [{eq, "E"}, {del, " "}, {eq, "+"}, {ins, " "}, {eq, "F"}, {ins, " "}] = diff("E +F", "E+ F ").

whitespace_3ro_test() -> [{eq, "E"}, {del, " "}, {eq, "+"}, {ins, " "}, {eq, "F"}, {ins, " "}] = diff("E +F", "E+ F ").

whitespace_4_test() -> [{eq, "E"}, {ins, "\t"}, {eq, "+"}, {del, " "}, {eq, "F"}, {del, " "}] = diff("E+ F ", "E\t+F").

whitespace_4r_test() -> [{eq, "E"}, {del, "\t"}, {eq, "+"}, {ins, " "}, {eq, "F"}, {ins, " "}] = diff("E\t+F", "E+ F ").
...
simple_diff_test() ->
    [{eq, "a"}, {del, "B"}, {ins, "X"}, {eq, "ccc"}, {del, "D"}, {ins, "Y"}, {eq, "e"}] = diff("aBcccDe", "aXcccYe").

case_diff_test() -> [{eq, "A "}, {del, "T"}, {ins, "t"}, {eq, "ext"}] = diff("A Text", "A text").

completely_mismatching_test() -> [{del, "aaa"}, {ins, "bbb"}] = diff("aaa", "bbb").

empty_inputs_produces_empty_diff_test() -> [] = diff("", "").

only_additions_test() ->
    [{ins, "aaa"}] = diff("", "aaa"),
    [{eq, "a"}, {ins, "b"}, {eq, "a"}, {ins, "b"}, {eq, "a"}, {ins, "b"}, {eq, "a"}] = diff("aaaa", "abababa").

only_deletions_test() ->
    [{del, "aaa"}] = diff("aaa", ""),
    [{eq, "a"}, {del, "b"}, {eq, "a"}, {del, "b"}, {eq, "a"}, {del, "b"}, {eq, "a"}] = diff("abababa", "aaaa").
