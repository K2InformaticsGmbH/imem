...
%    SyEx = 'SystemException',          %% cannot easily test that
    ...
    SeCoAdmin0=imem_seco:authenticate(adminSessionId, <<"admin">>, imem_seco:create_credentials(<<"change_on_install">>)),
    ...
    gen_server:cast(imem_seco, {'DOWN', simulated_reference, process, self(), simulated_exit}),
    timer:sleep(2000),
    ?assertEqual(0, imem_meta:table_size(ddSeCo)),
    io:format(user, "success ~p~n", [seco_table_size]), 
    ...
    SeCoAdmin1=imem_seco:authenticate(adminSessionId, <<"admin">>, imem_seco:create_credentials(<<"change_on_install">>)),
    ...
    ?assertException(throw,{SeEx,{"Password expired. Please change it", UserId}}, imem_seco:login(SeCo0)),
    ...
    ?assertException(throw, {ClEr,{"Account name already exists for",<<"test">>}}, imem_account:create(SeCo, Account0)),
    ...
    ?assertException(throw,{SeEx,{"Password expired. Please change it", AccountId}}, imem_seco:login(SeCo3)),
    io:format(user, "success ~p~n", [new_password]),
