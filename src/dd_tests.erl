...
%    SyEx = 'SystemException',          %% cannot easily test that
    ...
    ?assertEqual(1, imem_meta:table_size(ddAccount)),       %% pre_generated admin account
    ?assertEqual(1, imem_meta:table_size(ddRole)),          %% pre_generated admin role
    ?assertEqual(0, imem_meta:table_size(ddSeCo)),
    ?assertEqual(0, imem_meta:table_size(ddPerm)),
    ?assertEqual(0, imem_meta:table_size(ddQuota)),
    ?assertEqual(6, imem_meta:table_size(ddTable)),
    ...
    ?assertEqual(1, imem_seco:table_size(SeCoAdmin0, ddPerm)),    
    ?assertEqual(1, imem_seco:table_size(SeCoAdmin0, ddSeCo)),
    ?assertEqual(2, imem_seco:table_size(SeCoAdmin0, ddPerm)),    
    io:format(user, "success ~p~n", [seco_table_size]), 
    gen_server:cast(dd_seco, {'DOWN', simulated_reference, process, self(), simulated_exit}),
    timer:sleep(2000),
    ?assertEqual(0, imem_meta:table_size(ddSeCo)),
    io:format(user, "success ~p~n", [seco_table_size]), 
    ?assertException(throw,{'SecurityException',{"Not logged in", SeCoAdmin0}}, imem_seco:table_size(SeCoAdmin0, ddSeCo)),
    ?assertException(throw,{'SecurityException',{"Not logged in", SeCoAdmin0}}, dd_account:create(SeCoAdmin0, User)),
    ...
    ?assertEqual(1, imem_seco:table_size(SeCoAdmin1, ddSeCo)),
    io:format(user, "success ~p~n", [seco_table_size]), 
    AllTablesAdmin = imem_seco:all_tables(SeCoAdmin1),
    ...
    ?assertEqual(ok, dd_role:grant_permission(SeCoAdmin1, UserId, {ddQuota,select})),
    ...
    ?assertExit({SeVi,{"Not logged in",SeCo0}}, imem_seco:table_size(SeCo0, ddSeCo)),
    ...
    ?assertException(throw, {SeEx,{"Select unauthorized",SeCo1}}, imem_seco:table_size(SeCo1, ddSeCo)),
    ?assertException(throw, {SeEx,{"Not logged in",SeCo0}}, imem_seco:table_size(SeCo0, ddSeCo)),
    io:format(user, "success ~p~n", [table_access_rejected_after_logout]),
    AllTablesUser = imem_seco:all_tables(SeCo1),
    ...
    ?assertException(throw, {SeEx,{"Not logged in",SeCo1}}, imem_seco:table_size(SeCo1, ddSeCo)),
    ...
    ?assertExit({SeVi,{"Not logged in",SeCo2}}, dd_seco:have_permission(SeCo2, manage_bananas)), 
    ...
    io:format(user, "success ~p~n", [have_permission]),
    ?assertException(throw, {SeEx,{"Not logged in",SeCo1}}, dd_seco:have_permission(SeCo1, manage_accounts)), 
    io:format(user, "success ~p~n", [have_permission_rejected]),
