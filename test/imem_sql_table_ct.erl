...
test_with_or_without_sec(IsSec) ->
    ...
    Sql0 =
        "create table def (col1 varchar2(10) not null, col2 integer default 12, col3 list default fun() -> [/] end.);",
    ...
    ?assertMatch(
        {ok, _},
        imem_sql:exec(SKey, "create cluster table truncate_test (col1 integer, col2 string);", 0, imem, IsSec)
    ),
    ...
    ?assertEqual(ok, imem_sql:exec(SKey, "truncate table truncate_test;", 0, imem, IsSec)),
    ...
    ?assertException(
        throw,
        {ClEr, {"Unsupported option", {type, <<"someType">>}}},
        imem_sql:exec(SKey, Sql40, 0, imem, IsSec)
    ),
    Sql41 = "create imem_meta table skvhTEST();",
    ?assertException(
        throw,
        {ClEr, {"Invalid module name for table type", {type, imem_meta}}},
        imem_sql:exec(SKey, Sql41, 0, imem, IsSec)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Table does not exist", def}},
        imem_sql_table:if_call_mfa(IsSec, table_size, [SKey, def])
    ),
    ?assertException(
        throw,
        {ClEr, {"Table does not exist", def}},
        imem_sql:exec(SKey, "drop table def;", 0, imem, IsSec)
    ),
    ok.
