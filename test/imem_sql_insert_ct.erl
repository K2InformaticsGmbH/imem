...
test_with_or_without_sec(IsSec) ->
    ...
    ?assertEqual(
        [{def, "J", undefined, another_atom}],
        imem_sql:exec(SKey, Sql8, 0, [{params, [Pstring8, Patom8]}], IsSec)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Wrong data type for value, expecting type or default", {<<"a">>, string, []}}},
    ...
    ?assertException(
        throw,
        {ClEr, {"Unknown field or table name", <<"COL">>}},
        imem_sql:exec(SKey, Sql4, 0, [{schema, imem}], IsSec)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Unknown field or table name", <<"undefined">>}},
        imem_sql:exec(SKey, Sql4b, 0, [{schema, imem}], IsSec)
    ),
    ...
    ?assertEqual(
        [{def, <<"N">>, undefined, <<"not quite undefined">>}],
        imem_sql:exec(SKey, Sql4h, 0, [{schema, imem}], IsSec)
    ),
    ...
    ?assertException(
        ...
        imem_sql:exec(SKey, Sql6, 0, [{schema, imem}], IsSec)
    ),
    Sql6a = "insert into def (col1,col2) values ( 'O', sqrt(2)+1);",
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Sql6a: ~p~n", [Sql6a]),
    ?assertException(
        throw,
        {ClEr, {"Wrong data type for value, expecting type or default", _}},
        imem_sql:exec(SKey, Sql6a, 0, [{schema, imem}], IsSec)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Not null constraint violation", _}},
        imem_sql:exec(SKey, Sql22, 0, [{schema, imem}], IsSec)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Not null constraint violation", _}},
        imem_sql:exec(SKey, Sql23, 0, [{schema, imem}], IsSec)
    ),
