...
test(_Config) ->
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [admin_logged_out]),
    SeCoAdmin1 = ?imem_test_admin_login(),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [admin_re_login]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_authentication~n", [?MODULE]),

    SeCo0 = imem_seco:authenticate(someSessionId, UserName, UserCred),
    ?assertEqual(true, is_integer(SeCo0)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p ~p~n", [test_admin_authentication, SeCo0]),
    ?assertExit({SeVi, {"Not logged in", SeCo0}}, imem_sec:table_size(SeCo0, ddSeCo@)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [table_access_unauthorized]),
    ?assertException(throw, {SeEx, {?PasswordChangeNeeded, UserId}}, imem_seco:login(SeCo0)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [new_password]),
    SeCo1 = imem_seco:authenticate(someSessionId, UserName, UserCred),
    ?assertEqual(true, is_integer(SeCo1)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p ~p~n", [test_admin_authentication, SeCo1]),
    ...
    ?assertExit({SeVi, {"Not logged in", SeCo0}}, imem_sec:table_size(SeCo0, ddSeCo@)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [table_access_rejected_after_logout]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [all_tables_user]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [have_permission]),
    ?assertEqual(ok, imem_seco:logout(SeCo1)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [logout]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [test_admin_reauthentication]),
    ?assertExit({SeVi, {"Not logged in", SeCo2}}, imem_seco:have_permission(SeCo2, manage_bananas)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [have_permission_rejected]),
    ?assertEqual(SeCo2, imem_seco:login(SeCo2)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [login]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_manage_accounts~n", [?MODULE]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_create]),
    ?assertException(throw, {ClEr, {"Account already exists", AccountId}}, imem_account:create(SeCo, Account)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_create_already_exists]),
    ?assertException(throw, {ClEr, {"Account already exists", <<"test">>}}, imem_account:create(SeCo, Account0)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_create_name_already_exists]),
    ?assertEqual(Account, imem_account:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_get]),
    ?assertEqual(#ddRole{id = AccountId}, imem_role:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_get]),
    ?assertEqual(ok, imem_account:delete(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_delete]),
    ?assertEqual(ok, imem_account:delete(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_delete_even_no_exists]),
    ?assertException(throw, {ClEr, {"Account does not exist", AccountId}}, imem_account:delete(SeCo, Account)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_delete_no_exists]),
    ?assertEqual(false, imem_account:exists(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_no_exists]),
    ?assertException(throw, {ClEr, {"Account does not exist", AccountId}}, imem_account:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_get_no_exists]),
    ?assertException(throw, {ClEr, {"Role does not exist", AccountId}}, imem_role:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_get_no_exists]),
    ?assertEqual(ok, imem_account:create(SeCo, Account)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_create]),
    ?assertException(throw, {CoEx, {"Account is modified by someone else", AccountId}}, imem_account:delete(SeCo, Account1)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_delete_wrong_version]),
    ?assertEqual(ok, imem_account:delete(SeCo, AccountName)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_delete_with_check]),
    ?assertEqual(ok, imem_account:create(SeCo, Account)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_create]),
    ?assertEqual(true, imem_account:exists(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_exists]),
    ?assertEqual(Account, imem_account:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_get]),
    ?assertEqual(#ddRole{id = AccountId}, imem_role:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_get]),
    ?assertEqual(ok, imem_account:update(SeCo, Account, Account1)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [update_account]),
    ?assertEqual(Account1, imem_account:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_get_modified]),
    ?assertException(throw, {CoEx, {"Account is modified by someone else", AccountId}}, imem_account:update(SeCo, Account, Account2)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [update_account_reject]),
    ?assertEqual(Account1, imem_account:get(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_get_unchanged]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [has_permission]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [is_locked]),
    ?assertEqual(ok, imem_account:unlock(SeCo, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [unlock]),
    SeCo3 = imem_seco:authenticate(someSessionId, AccountName, AccountCredNew),
    ?assertEqual(true, is_integer(SeCo3)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [test_authentication]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [unauthorized_rejected]),


    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_manage_account_roles~n", [?MODULE]),

    ?assertEqual(true, imem_seco:has_role(SeCo, AccountId, AccountId)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_own_role]),
    ?assertEqual(false, imem_seco:has_role(SeCo, AccountId, some_unknown_role)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_some_unknown_role]),
    ?assertException(throw, {ClEr, {"Role does not exist", some_unknown_role}}, imem_role:grant_role(SeCo, AccountId, some_unknown_role)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_grant_reject]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_create_empty_role]),
    ?assertException(throw, {ClEr, {"Role already exists", table_creator}}, imem_role:create(SeCo, table_creator)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_create_existing_role]),
    ?assertEqual(false, imem_seco:has_role(SeCo, AccountId, table_creator)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_not_tc_role]),
    ?assertEqual(ok, imem_role:grant_role(SeCo, AccountId, table_creator)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_grant_tc_role]),
    ?assertEqual(true, imem_seco:has_role(SeCo, AccountId, table_creator)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_has_tc_role]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [account_has_has_not_roles]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [create_user_table_unauthorized]),
    ?assertEqual(ok, imem_role:grant_permission(SeCo, table_creator, create_table)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_re_grant_tc_role]),
    ?assertEqual(false, imem_seco:has_permission(SeCo, AccountId, manage_user_tables)),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_revoke_tc_role]),
    ?assertEqual(true, imem_sec:have_table_permission(SeCo4, user_table_123, select)),
    ?assertEqual(true, imem_sec:have_table_permission(SeCo4, user_table_123, insert)),
    ?assertEqual(true, imem_sec:have_table_permission(SeCo4, user_table_123, delete)),
    ?assertEqual(true, imem_sec:have_table_permission(SeCo4, user_table_123, update)),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_create_test_role]),
    ?assertEqual(true, imem_seco:has_permission(SeCo, test_role, perform_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_test_permission]),
    ?assertEqual(false, imem_seco:has_permission(SeCo, test_role, stupid_permission)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_stupid_permission]),
    ?assertEqual(false, imem_seco:has_role(SeCo, AccountId, test_role)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_test_role]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_grant_test_role]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_manage_account_role rejects~n", [?MODULE]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [manage_account_roles_rejects]),

    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_manage_account_permissions~n", [?MODULE]),

    ?assertEqual(ok, imem_role:grant_permission(SeCo, test_role, delete_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_grant_test_role_delete_tests]),
    ?assertEqual(ok, imem_role:grant_permission(SeCo, test_role, fake_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_grant_test_role_fake_tests]),
    ?assertEqual(true, imem_seco:has_permission(SeCo, AccountId, delete_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_delete_tests_permission]),
    ?assertEqual(true, imem_seco:has_permission(SeCo, AccountId, fake_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_fake_tests_permission]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_revoke_test_role_delete_tests]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_has_delete_tests_permission]),
    ?assertEqual(ok, imem_role:revoke_permission(SeCo, test_role, delete_tests)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [role_revoket_test_role_delete_tests]),

    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_manage_account_permission_rejects~n", [?MODULE]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [test_manage_account_permission_rejects]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [drop_seco_tables_reject]),
    % ?assertEqual(ok, imem_role:grant_permission(SeCo, UserId, manage_system_tables)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [grant_manage_system_tables]),
    % ?assertEqual(ok, imem_seco:drop_seco_tables(SeCo)),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [drop_seco_tables]),
    % ?assertEqual(ok, imem_meta:drop_meta_tables()),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":success ~p~n", [drop_meta_tables]),

    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":~p:test_imem_cleanup~n", [?MODULE]),
