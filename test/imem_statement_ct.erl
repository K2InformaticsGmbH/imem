...
test_with_or_without_sec_part1(IsSec) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_or_without_sec_part1/1 - Start(~p) ===>~n", [IsSec]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Sql1a:~n~s~n", [Sql1a]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Sql1b:~n~s~n", [Sql1b]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Sql1c:~n~s~n", [Sql1c]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":original table~n~p~n", [TT1Rows]),
    TT1RowsExpected =
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":changed table~n~p~n", [TT1aRows]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Meta table~n~p~n", [_Meta]),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":original table~n~p~n", [TableRows1]),
    ...
    %ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":change list~n~p~n", [ChangeList2]),
    ChangeList2 = [
...
test_with_sec_part1(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_sec_part1/2 - Start ===>~n", []),

    test_with_or_without_sec_part1(true).

test_without_sec_part1(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_without_sec_part1/1 - Start ===>~n", []),

    test_with_or_without_sec_part1(false).

test_with_or_without_sec_part2(IsSec) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_or_without_sec_part2/1 - Start(~p) ===>~n", [IsSec]),
    ...
    try
        ...
        ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":trying to insert one row before fetch complete~n", []),
        ?assertEqual(ok, insert_range(SKey, 1, def, imem, IsSec)),      % overwrite, still 10 rows in table, 5 fetched so far
        ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":completed insert one row before fetch complete~n", []),
        ...
        ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":12 tail rows received in single packets~n", []),
        ?assertEqual(ok, fetch_async(SKey, SR4, [], IsSec)),
    ...
    try
        ...
        ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":first read success (async)~n", []),
        ?assertEqual(ok, fetch_async(SKey, SR5, [], IsSec)),
        ...
        ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":second read success (async)~n", []),
...
test_with_sec_part2(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_sec_part2/2 - Start ===>~n", []),

    test_with_or_without_sec_part2(true).

test_without_sec_part2(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_without_sec_part2/1 - Start ===>~n", []),

    test_with_or_without_sec_part2(false).

test_with_or_without_sec_part3(IsSec) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_or_without_sec_part3/1 - Start(~p) ===>~n", [IsSec]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":original table~n~p~n", [_TableRows1]),
    [_Meta] = imem_statement:if_call_mfa(IsSec, read, [SKey, ddTable, {imem, def}]),
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Meta table~n~p~n", [_Meta]),
    ...
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Result10a ~p~n", [List10a]),
...
test_with_sec_part3(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_with_sec_part3/2 - Start ===>~n", []),

    test_with_or_without_sec_part3(true).

test_without_sec_part3(_Config) ->
    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":test_without_sec_part3/1 - Start ===>~n", []),
...
%                    [{StmtRef,[],Complete}];
            ...
            case lists:usort([element(1, SR) || SR <- Unchecked]) of
                [StmtRef] ->
                    ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Unchecked receive result :~n~p~n", [Unchecked]),
                    List = lists:flatten([element(1, element(2, T)) || T <- Unchecked]),
                    RT = imem_statement:result_tuples(List, RowFun),
                    if
                        length(RT) =< 10 ->
                            ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Received:~n~p~n", [RT]),
                            ok;
                        true ->
                            ct:pal(info, ?MAX_IMPORTANCE, ?MODULE_STRING ++ ":Received: ~p items:~n~p~n~p~n~p~n", [length(RT), hd(RT), '...', lists:last(RT)]),
                            ok
                    end,
                    RT;
                StmtRefs ->
