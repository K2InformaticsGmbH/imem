...
test_with_or_without_sec_part1(IsSec) ->
    ...
    ChangeList00 = [[1, upd, {{1, node()}, {fun_test, 12, 144}}, <<"13">>, <<"0">>]],
    ?CTPAL("ChangeList00"),
    ?assertEqual(ok, imem_statement:update_cursor_prepare(SKey, SR00, IsSec, ChangeList00)),
    ?assertEqual(
        [{1, {{1, node()}, {fun_test, 13, 169}}}],
        imem_statement:update_cursor_execute(SKey, SR00, IsSec, optimistic)
    ),
    ChangeList01 = [[2, ins, {{0, node()}, {}}, <<"15">>, <<"1">>]],
    ?CTPAL("ChangeList01"),
    ?assertEqual(ok, imem_statement:update_cursor_prepare(SKey, SR00, IsSec, ChangeList01)),
    ?assertEqual(
        [{2, {{1, node()}, {fun_test, 15, 225}}}],
        imem_statement:update_cursor_execute(SKey, SR00, IsSec, optimistic)
    ),
    ...
    try
        ...
        % was true, MNESIA in erlang 20 does not allow to preview the end
        List0 = receive_tuples(SR0, false),
    ...
    ChangeList2 = [[4, upd, {{1, node()}, {def, <<"12">>, 12}}, <<"112">>, <<"12">>]],
    ?CTPAL("ChangeList2"),
    ...
    ChangeList3 =
        [
            %% no operation on this line
            [1, nop, {{1, node()}, {def, <<"2">>, 2}}, <<"2">>, <<"2">>],
            %% insert {def,"99", undefined}
            [5, ins, {{0, node()}, {}}, <<"99">>, <<"undefined">>],
            %% delete {def,"5",5}
            [3, del, {{3, node()}, {def, <<"5">>, 5}}, <<"5">>, <<"5">>],
            %% nop update {def,"112",12}
            [4, upd, {{4, node()}, {def, <<"112">>, 12}}, <<"112">>, <<"12">>],
            %% update {def,"10",10} to {def,"10",110}
            [6, upd, {{6, node()}, {def, <<"10">>, 10}}, <<"10">>, <<"110">>]
        ],
    ExpectedRows3 =
        [
            %% no operation on this line
    ...
    try
        ...
        %% unchanged, all updates
        ?assertEqual(10, imem_meta:table_size(def)),
        ?CTPAL("List2b"),
        List2b = receive_tuples(SR2, tail),
        %% 10 updates, 5 filtered with TailFun()
        ?assertEqual(5, length(List2b)),
    ...
    try
        ...
        %% TODO: Should this come split into 2 + 1 rows ?
...
test_with_or_without_sec_part2(IsSec) ->
    ...
    % was true, MNESIA in Erlang 20 does not allow any more to detect the end
    Cube3a = receive_tuples(SR3a, false),
    ...
    % was true, MNESIA in Erlang 20 does not allow any more to detect the end
    Cube3b = receive_tuples(SR3b, false),
    ...
    % was true, MNESIA in Erlang 20 does not allow any more to detect the end
    Cube3c1 = receive_tuples(SR3c, false),
    %% TODO: streaming join evaluation needed to keep this down at 10
    ?assertEqual(20, length(Cube3c1)),
    ...
    % was true, cuanhed for MNESIA in Erlang 20
    ?assertEqual(1000, length(receive_tuples(SR3d, false))),
    ?assertEqual([], imem_statement:receive_raw()),
    ...
    %% test for stmt teardown while joining big result
    ?assertEqual(ok, imem_statement:close(SKey, SR3e)),
    ?assertEqual(10, imem_meta:table_size(def)),
    ...
    try
        ...
        % overwrite, still 10 rows in table, 5 fetched so far
        ...
        ?CTPAL("List4b"),
        % was true, changed for MNESIA in Erlang 20
        List4b = receive_tuples(SR4, false),
        % 10 rows now in table, 10 fetched so far
        ?assertEqual(5, length(List4b)),
        ?assertEqual(ok, fetch_async(SKey, SR4, [{tail_mode, true}], IsSec)),
        % forced cursor into tail mode now (needed with erlang 20)
        [{_, {[], true}}] = imem_statement:receive_raw(),
        % overwrite, still 10 rows now in table, 11 fetched so far
        ?assertEqual(ok, insert_range(SKey, 1, def, imem, IsSec)),
        ?CTPAL("List4c"),
        List4c = receive_tuples(SR4, tail),
        % 10 rows in table, 12 fetched so far
        ?assertEqual(1, length(List4c)),
        % 1 overwrite, still 10 rows in table, 12 fetched so far
        ?assertEqual(ok, insert_range(SKey, 1, def, imem, IsSec)),
        % 10 overwrites + 1 insert, 11 rows now in table, 12 fetched so far
    ...
    try
        ...
        D21 = List5b -- List5a,
        ?assert(
            (D12 == []) orelse (lists:usort([imem_meta:is_local_time_partitioned_table(N) || {N} <- D12]) == [true])
        ),
        ?assert(
            (D21 == []) orelse (lists:usort([imem_meta:is_local_time_partitioned_table(N) || {N} <- D21]) == [true])
        ),
        ...
        % actually not needed here, fetch_recs does it
        ?assertEqual(ok, imem_statement:fetch_close(SKey, SR5, IsSec)),
    ...
    try
        ...
        ?assertMatch(
            [{<<_ : 16, $., _ : 16, $., _ : 32, 32, _ : 16, $:, _ : 16, $:, _ : 16, $., _ : 48>>, <<"4">>}],
            List7b
        ),
        ...
        ?assertMatch(
            [{<<_ : 16, $., _ : 16, $., _ : 32, 32, _ : 16, $:, _ : 16, $:, _ : 16, $., _ : 48>>, <<"6">>}],
            List7b1
        ),
...
test_with_or_without_sec_part3(IsSec) ->
    ...
    try
        ...
        ?assertEqual(
            [{<<"11">>, <<"11">>}, {<<"10">>, <<"10">>}, {<<"3">>, <<"3">>}, {<<"2">>, <<"2">>}, {<<"1">>, <<"1">>}],
            result_tuples_sort(List8a, SR8#stmtResults.rowFun, SR8#stmtResults.sortFun)
        ),
        Result8a = imem_statement:filter_and_sort(SKey, SR8, {'and', []}, [{2, 2, <<"asc">>}], [], IsSec),
        ?CTPAL("Result8a"),
        {ok, Sql8b, SF8b} = Result8a,
        Sorted8b =
            [{<<"1">>, <<"1">>}, {<<"10">>, <<"10">>}, {<<"11">>, <<"11">>}, {<<"2">>, <<"2">>}, {<<"3">>, <<"3">>}],
        ...
        {ok, Sql8c, SF8c} =
            imem_statement:filter_and_sort(
                SKey,
                SR8,
                {'and', [{1, [<<"$in$">>, <<"1">>, <<"2">>, <<"3">>]}]},
                [{?MainIdx, 2, <<"asc">>}],
        ...
        Expected8e =
            "select col2, col1 c1 from def where (imem.def.col1 = '3' or imem.def.col2 = 3) and col1 < '4' order by col1 asc, col2 desc",
        ?CTPAL("Expected8e"),
    ...
    try
        ...
        ?CTPAL("Sql9b ~p", [Sql9b]),
        Expected9b =
            "select a.col1, b.col1, a.col2 from def a, def b where a.col1 = b.col1 order by b.col1 asc, a.col2 desc",
    ...
    ChangeList10 = [[1, upd, {{1, node()}, {def, <<"5">>, 5}, {def, <<"5">>, 5}}, <<"X">>, <<"Y">>]],
...
exec(SKey, Id, BS, IsSec, Sql) -> exec(SKey, Id, BS, IsSec, [], Sql).
...
receive_tuples(StmtResult, Complete) -> receive_tuples(StmtResult, Complete, 120, []).
...
receive_tuples(#stmtResults{stmtRefs = [StmtRef | _], rowFun = RowFun} = StmtResult, Complete, Timeout, Acc) ->
    ...
    after
        ...
        {_, Result} -> receive_tuples(StmtResult, Complete, Timeout, [Result | Acc])
    end.
