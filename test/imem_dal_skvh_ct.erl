...
-export([end_per_testcase/2, init_per_testcase/2, skvh_concurrency/1, skvh_operations/1, skvh_purge_history/1]).
...
skvh_concurrency(_Config) ->
    ...
    ?CTPAL("create_table"),
    [spawn(fun () -> Self ! {Ch, imem_dal_skvh:create_table(Ch, [], [], system)} end) || Ch <- ?Channels],
    ...
    ?CTPAL("insert"),
    [spawn(fun () -> Self ! {Ch, imem_dal_skvh:insert(system, Ch, TestKey, <<"0">>)} end) || Ch <- ?Channels],
    ...
    ?CTPAL("drop_table"),
    [spawn(fun () -> Self ! {Ch, imem_dal_skvh:drop_table(Ch)} end) || Ch <- ?Channels],
    _ = {timeout, 100, fun () -> ?assertEqual([ok], lists:usort([R || {_, R} <- receive_results(TabCount, [])])) end},
    ok.
...
skvh_operations(_Config) ->
    ...
    ?assertException(
        throw,
        {ClEr, {"Bad datatype, expected map", [a]}},
    ...
    ?assertEqual(
        #{chash => <<"24FBRP">>, ckey => test, cvalue => <<"{\"a\":\"a\"}">>},
        imem_dal_skvh:write(system, <<"noOptsChannel">>, test, <<"{\"a\":\"a\"}">>)
    ),
    ?assertEqual(
        #{chash => <<"24FBRP">>, ckey => test, cvalue => <<"{\"a\":\"a\"}">>},
        imem_dal_skvh:write(system, <<"noHistoryHChannel">>, test, <<"{\"a\":\"a\"}">>)
    ),
    ?assertEqual(
        [#{chash => <<"24FBRP">>, ckey => test, cvalue => <<"{\"a\":\"a\"}">>}],
        imem_dal_skvh:read(system, <<"noOptsChannel">>, [test])
    ),
    ?assertEqual(
        [#{chash => <<"24FBRP">>, ckey => test, cvalue => <<"{\"a\":\"a\"}">>}],
        imem_dal_skvh:read(system, <<"noHistoryHChannel">>, [test])
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Channel does not exist", <<"skvhTest">>}},
        imem_dal_skvh:read(system, ?Channel, <<"kvpair">>, K0)
    ),
    ...
    ?assertEqual(
        {ok, []},
        imem_dal_skvh:readGT(system, ?Channel, <<"khpair">>, <<"{<<\"0\">>,<<>>,<<>>}">>, <<"1000">>)
    ),
    ...
    ?assertEqual(
        {ok, [<<"1EXV0I">>, <<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:write(
            system,
            ?Channel,
            <<"[1,a]", 9, "123456", 10, "[1,b]", 9, "234567", 13, 10, "[1,c]", 9, "345678">>
        )
    ),
    ...
    ?assertEqual(
        {ok, [KVc, KVb, KVa]},
        imem_dal_skvh:read(system, ?Channel, <<"kvpair">>, <<"[1,c]", 13, 10, "[1,b]", 10, "[1,a]">>)
    ),
    ?assertEqual(
        {ok, [KVa, <<"[1,ab]", 9, "undefined">>, KVb, KVc]},
        imem_dal_skvh:read(system, ?Channel, <<"kvpair">>, <<"[1,a]", 13, 10, "[1,ab]", 13, 10, "[1,b]", 10, "[1,c]">>)
    ),
    ?CTPAL("test data"),
    ...
    ?assertEqual(
        {ok, [<<"1EXV0I">>, <<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:delete(system, ?Channel, <<"[1,a]", 10, "[1,b]", 13, 10, "[1,c]", 10>>)
    ),
    ?CTPAL("audit trail"),
    ...
    % windows wall clock may be 17ms behind
    timer:sleep(10),
    ...
    Ex4 =
        {
            'ClientError',
            {"Data conversion format error", {timestamp, "1900-01-01", {"Cannot handle dates before 1970"}}}
        },
    ?assertException(
        throw,
        Ex4,
        imem_dal_skvh:audit_readGT(system, ?Channel, <<"tkvuquadruple">>, <<"1900-01-01">>, 100)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {117, "Too many values, Limit exceeded", 1}},
        imem_dal_skvh:deleteGELT(system, ?Channel, <<"[1,ab]">>, <<"[1,d]">>, <<"1">>)
    ),
    ?assertEqual(
        {ok, [<<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:deleteGELT(system, ?Channel, <<"[1,ab]">>, <<"[1,d]">>, <<"2">>)
    ),
    ...
    ?assertEqual(
        {ok, [<<"24OMVH">>]},
        imem_dal_skvh:write(system, ?Channel, <<"[90074,[],\"ContentSizeMax\"]", 9, "297000">>)
    ),
    ?assertEqual(
        {ok, [<<"1W8TVA">>]},
        imem_dal_skvh:write(system, ?Channel, <<"[90074,[],<<\"MmscId\">>]", 9, "\"testMMSC\"">>)
    ),
    ?assertEqual(
        {ok, [<<"22D5ZL">>]},
        imem_dal_skvh:write(
            system,
            ?Channel,
            <<"[90074,\"MMS-DEL-90074\",\"TpDeliverUrl\"]", 9, "\"http://10.132.30.84:18888/deliver\"">>
        )
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {117, "Too many values, Limit exceeded", 1}},
        imem_dal_skvh:readGELT(system, ?Channel, <<"hash">>, <<"[]">>, <<"[1,d]">>, <<"1">>)
    ),
    ?assertEqual(
        {ok, [<<"1EXV0I">>, <<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"hash">>, <<"[]">>, <<"[1,d]">>, <<"3">>)
    ),
    ?assertEqual(
        {ok, [<<"1EXV0I">>, <<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"hash">>, <<"[1,a]">>, <<"[1,d]">>, <<"5">>)
    ),
    ?assertEqual(
        {ok, [<<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"hash">>, <<"[1,ab]">>, <<"[1,d]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"key">>, <<"[1,ab]">>, <<"[1,c]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"234567">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"value">>, <<"[1,ab]">>, <<"[1,c]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]", 9, "234567">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"kvpair">>, <<"[1,ab]">>, <<"[1,c]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]", 9, "BFFHP">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"khpair">>, <<"[1,ab]">>, <<"[1,c]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]", 9, "234567", 9, "BFFHP">>]},
        imem_dal_skvh:readGELT(system, ?Channel, <<"kvhtriple">>, <<"[1,ab]">>, <<"[1,c]">>, <<"2">>)
    ),
    ?assertEqual(ok, imem_meta:drop_table(skvhTestAudit_86400@_)),
    ?assertEqual(ok, imem_dal_skvh:create_check_channel(?Channel)),
    ?assertEqual(
        {ok, [<<"1EXV0I">>, <<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"hash">>, <<"[]">>, <<"1000">>)
    ),
    ?assertEqual(
        {ok, [<<"BFFHP">>, <<"ZCZ28">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"hash">>, <<"[1,a]">>, <<"1000">>)
    ),
    ?assertEqual({ok, [<<"BFFHP">>]}, imem_dal_skvh:readGT(system, ?Channel, <<"hash">>, <<"[1,a]">>, <<"1">>)),
    ?assertEqual(
        {ok, [<<"[1,b]">>, <<"[1,c]">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"key">>, <<"[1,a]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"234567">>, <<"345678">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"value">>, <<"[1,ab]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]", 9, "234567">>, <<"[1,c]", 9, "345678">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"kvpair">>, <<"[1,ab]">>, <<"2">>)
    ),
    ?assertEqual(
        {ok, [<<"[1,b]", 9, "BFFHP">>, <<"[1,c]", 9, "ZCZ28">>]},
        imem_dal_skvh:readGT(system, ?Channel, <<"khpair">>, <<"[1,ab]">>, <<"2">>)
    ),
    ?assertEqual(ok, imem_meta:truncate_table(skvhTest)),
    KVtab =
        <<
            "{<<\"52015\">>,<<>>,<<\"AaaEnabled\">>}\tfalse\n{<<\"52015\">>,<<\"SMS-SUB-52015\">>,<<\"AaaEnabled\">>}\tfalse"
        >>,
    ...
    Map6 =
        #{
            ...
            cvalue
            =>
            [#{<<"testKey">> => <<"b">>, <<"testNumber">> => 3}, #{<<"testKey">> => <<"a">>, <<"testNumber">> => 1}],
            chash => <<"1404CV">>
        },
    ...
    ?assertEqual(
        ...
        imem_dal_skvh:insert(
            system,
            <<"mapChannel">>,
            imem_datatype:term_to_binterm(maps:get(ckey, Map6)),
            maps:get(cvalue, Map6)
        )
    ),
    ...
    ?assertException(
        throw,
        {CoEx, {"Insert failed, key already exists in", _}},
        imem_dal_skvh:insert(system, ?Channel, maps:get(ckey, Map1), maps:get(cvalue, Map1))
    ),
    ?assertException(
        throw,
        {CoEx, {"Insert failed, key already exists in", _}},
        imem_dal_skvh:insert(system, ?Channel, [MapNotInserted, Map4#{chash := <<>>}])
    ),
    ...
    ?assertEqual(
        [Map1, Map2, Map3],
        imem_dal_skvh:read(system, ?Channel, [maps:get(ckey, Map1), maps:get(ckey, Map2), maps:get(ckey, Map3)])
    ),
    ...
    ?assertEqual(
        [maps:remove(chash, Map2Upd), maps:remove(chash, Map3Upd)],
        [maps:remove(chash, M) || M <- [Map2Done, Map3Done]]
    ),
    %% Concurrency exception
    ?assertException(
        throw,
        {CoEx, {"Data is modified by someone else", _}},
        imem_dal_skvh:update(system, ?Channel, Map1Upd)
    ),
    ?assertException(
        throw,
        {CoEx, {"Data is modified by someone else", _}},
        imem_dal_skvh:update(system, ?Channel, [Map2Upd, Map3Upd])
    ),
    %% Read tests
    ...
    ?assertException(
        throw,
        {ClEr, {117, "Too many values, Limit exceeded", 1}},
        imem_dal_skvh:readGELT(system, ?Channel, FirstKey, LastKey, 1)
    ),
    ?assertEqual(
        [Map1Done, Map2Done, Map3Done, Map4, Map5],
        imem_dal_skvh:readGELT(system, ?Channel, FirstKey, LastKey, 10)
    ),
    ?assertEqual([Map3Done, Map4, Map5], imem_dal_skvh:readGELT(system, ?Channel, maps:get(ckey, Map3), LastKey, 10)),
    ?assertEqual(
        [Map3Done, Map4],
        imem_dal_skvh:readGELT(system, ?Channel, maps:get(ckey, Map3), maps:get(ckey, Map5), 10)
    ),
    ...
    ?assertException(
        throw,
        {CoEx, {"Remove failed, key does not exist", _}},
        imem_dal_skvh:remove(system, ?Channel, Map1)
    ),
    ?assertException(
        throw,
        {CoEx, {"Data is modified by someone else", _}},
        imem_dal_skvh:remove(system, ?Channel, [Map2Upd, Map3])
    ),
    %% Remove in bulk
    ...
    AuditInsert1 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map1),
            ovalue => undefined,
            nvalue => maps:get(cvalue, Map1),
            cuser => system
        },
    AuditInsert2 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map2),
            ovalue => undefined,
            nvalue => maps:get(cvalue, Map2),
            cuser => system
        },
    AuditInsert3 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map3),
            ovalue => undefined,
            nvalue => maps:get(cvalue, Map3),
            cuser => system
        },
    ResultAuditInserts =
        [AuditRow#{time := {0, 0, 0}} || AuditRow <- imem_dal_skvh:audit_readGT(system, ?Channel, BeforeInsert, 3)],
    ?assertEqual([AuditInsert1, AuditInsert2, AuditInsert3], ResultAuditInserts),
    AuditUpdate1 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map1),
            ovalue => maps:get(cvalue, Map1),
            nvalue => maps:get(cvalue, Map1Upd),
            cuser => system
        },
    AuditUpdate2 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map2),
            ovalue => maps:get(cvalue, Map2),
            nvalue => maps:get(cvalue, Map2Upd),
            cuser => system
        },
    AuditUpdate3 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map3),
            ovalue => maps:get(cvalue, Map3),
            nvalue => maps:get(cvalue, Map3Upd),
            cuser => system
        },
    ResultAuditUpdates =
        [AuditRow#{time := {0, 0, 0}} || AuditRow <- imem_dal_skvh:audit_readGT(system, ?Channel, BeforeUpdate, 3)],
    ?assertEqual([AuditUpdate1, AuditUpdate2, AuditUpdate3], ResultAuditUpdates),
    AuditRemove1 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map1),
            ovalue => maps:get(cvalue, Map1Upd),
            nvalue => undefined,
            cuser => system
        },
    AuditRemove2 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map2),
            ovalue => maps:get(cvalue, Map2Upd),
            nvalue => undefined,
            cuser => system
        },
    AuditRemove3 =
        #{
            time => {0, 0, 0},
            ckey => maps:get(ckey, Map3),
            ovalue => maps:get(cvalue, Map3Upd),
            nvalue => undefined,
            cuser => system
        },
    ResultAuditRemoves =
        [AuditRow#{time := {0, 0, 0}} || AuditRow <- imem_dal_skvh:audit_readGT(system, ?Channel, BeforeRemove, 3)],
    ...
    CL1 =
        [
            #{time => {0, 0, 0}, ovalue => maps:get(cvalue, Map1Upd), nvalue => undefined, cuser => system},
    ...
    CL2 =
        [
            #{time => {0, 0, 0}, ovalue => maps:get(cvalue, Map2Upd), nvalue => undefined, cuser => system},
    ...
    CL3 =
        [
            #{time => {0, 0, 0}, ovalue => maps:get(cvalue, Map3Upd), nvalue => undefined, cuser => system},
    ...
    ?assertEqual(
        [maps:remove(chash, Map4Upd), maps:remove(chash, Map5Upd)],
        [maps:remove(chash, M) || M <- [Map4Done, Map5Done]]
    ),
    %% Read using a list of term keys.
    HistResult =
        hist_reset_time(
            imem_dal_skvh:hist_read(
                system,
                ?Channel,
                [maps:get(ckey, Map1), maps:get(ckey, Map2), maps:get(ckey, Map3)]
            )
        ),
    ...
    EncodedKeys =
        [
            imem_datatype:term_to_binterm(maps:get(ckey, Map1)),
...
skvh_purge_history(_Config) ->
    imem_config:put_config_hlk(
        ?CONFIG_TABLE,
        {imem, imem_dal_skvh, purgeHistDayThreshold},
        imem_dal_skvh,
        [],
        0,
        <<"zero">>
