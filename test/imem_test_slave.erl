...
-export([ start/1       % string(): slave node short name -> slave node full name atom()
        , stop/1        % string()|atom():slave node short of full name -> ok
        , name/1        % string(): slave node short name -> slave node full name atom()
        , is_running/1  % string()|atom():slave node short of full name -> true|false
        ]).
...
is_running(SlaveStr) when is_list(SlaveStr) ->
    is_running(name(SlaveStr));
is_running(Slave) when is_atom(Slave) ->
    lists:member(Slave,nodes()).
