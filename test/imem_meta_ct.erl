...
physical_table_names(_Config) ->
    ...
    [?assertEqual([Type], imem_meta:physical_table_names(atom_to_list(Type))) || Type <- ?DataTypes],
    ...
    ?assertEqual(
        [{node(), <<"csv$">>, <<"\"TestCsvFile.csv\"">>}],
        imem_meta:cluster_table_names(<<"csv$.\"TestCsvFile.csv\"">>)
    ),
    ...
    ?assertEqual(
        [{Slave, <<"csv$">>, <<"\"TestCsvFile.csv\"">>}],
        imem_meta:cluster_table_names("csv$.\"TestCsvFile.csv\"@" ++ ?TEST_SLAVE_IMEM_NODE_NAME)
    ),
    ...
    ?assertEqual(
        [{Slave, imem_meta:schema(), SlaveCacheName}],
        imem_meta:cluster_table_names("ddCache@" ++ ?TEST_SLAVE_IMEM_NODE_NAME)
    ),
    ...
    ?assertEqual(
        [{Slave, imem_meta:schema(), SlaveLogName}],
        imem_meta:cluster_table_names("ddLog_86400@" ++ ?TEST_SLAVE_IMEM_NODE_NAME)
    ),
    ?assertEqual(
        [{Slave, imem_meta:schema(), ddAccount}],
        imem_meta:cluster_table_names("ddAccount@" ++ ?TEST_SLAVE_IMEM_NODE_NAME)
    ),
    ?assertEqual(
        [{Slave, imem_meta:schema(), integer}],
        imem_meta:cluster_table_names("integer@" ++ ?TEST_SLAVE_IMEM_NODE_NAME)
    ),
...
meta_concurrency(_Config) ->
    ...
    [spawn(fun () -> Self ! {N, read_write_test(?TPTEST_1, Key, N)} end) || N <- lists:seq(1, 10)],
...
meta_operations(_Config) ->
    ...
    ?assertException(
        throw,
        {SyEx, {"Wrong table owner", {_, [system, admin]}}},
        imem_meta:create_check_table(
            ?LOG_TABLE,
            {record_info(fields, ddLog), ?ddLog, #ddLog{}},
    ...
    ?assertException(
        throw,
        {SyEx, {"Wrong table options", {_, _}}},
        imem_meta:create_check_table(
            ?LOG_TABLE,
            {record_info(fields, ddLog), ?ddLog, #ddLog{}},
            [{record_name, ddLog1}, {type, ordered_set}],
            system
        )
    ),
    ...
    ?assertEqual(
        ok,
        imem_meta:log_to_db(info, ?MODULE, test, [{test_3, value3}, {test_4, value4}], [stupid_error_message, 1])
    ),
    ?assertEqual(
        ok,
        imem_meta:log_to_db(info, ?MODULE, test, [{test_3, value3}, {test_4, value4}], {stupid_error_message, 2})
    ),
    ...
    Types1 =
        [
            %% key
            #ddColumn{name = a, type = string, len = 10},
            %% value 1
            #ddColumn{name = b1, type = binstr, len = 20},
            %% value 2
            #ddColumn{name = c1, type = string, len = 30}
        ],
    Types2 =
        [
            %% key
            ...
            %% value
            #ddColumn{name = b2, type = float, len = 8, prec = 3}
        ],
    BadTypes0 = [#ddColumn{name = a, type = integer, len = 10}],
    BadTypes1 = [#ddColumn{name = a, type = integer, len = 10}, #ddColumn{name = 'a:b', type = integer, len = 10}],
    BadTypes2 = [#ddColumn{name = a, type = integer, len = 10}, #ddColumn{name = current, type = integer, len = 10}],
    BadTypes3 = [#ddColumn{name = a, type = integer, len = 10}, #ddColumn{name = b, type = iinteger, len = 10}],
    BadNames1 = [#ddColumn{name = a, type = integer, len = 10}, #ddColumn{name = a, type = integer, len = 10}],
    ...
    ?assertException(
        throw,
        {'ClientError', {"Table does not exist", ?TPTEST_1IDX}},
        imem_meta:check_table(?TPTEST_1IDX)
    ),
    ?assertEqual(ok, imem_meta:create_index(?TPTEST_1, [])),
    ?assertException(
        throw,
        {'ClientError', {"Index already exists", {?TPTEST_1}}},
        imem_meta:create_index(?TPTEST_1, [])
    ),
    ...
    ?assertEqual(
        {?TPTEST_1, "meta", <<"täble"/utf8>>, "1"},
        imem_meta:insert(?TPTEST_1, {?TPTEST_1, "meta", <<"täble"/utf8>>, "1"})
    ),
    ...
    ?assertException(
        throw,
        {'ConcurrencyException', {"Data is modified by someone else", _}},
        imem_meta:update(
            ?TPTEST_1,
            {{?TPTEST_1, "meta", <<"tible"/utf8>>, "1"}, {?TPTEST_1, "meta", <<"tüble"/utf8>>, "1"}}
        )
    ),
    ?assertEqual(
        {?TPTEST_1, "meta", <<"tüble"/utf8>>, "1"},
        imem_meta:update(
            ?TPTEST_1,
            {{?TPTEST_1, "meta", <<"täble"/utf8>>, "1"}, {?TPTEST_1, "meta", <<"tüble"/utf8>>, "1"}}
        )
    ),
    ...
    ?assertException(
        throw,
        {'ConcurrencyException', {"Data is modified by someone else", _}},
        imem_meta:remove(?TPTEST_1, {?TPTEST_1, "meta", <<"tible"/utf8>>, "1"})
    ),
    ?assertEqual(
        {?TPTEST_1, "meta", <<"tüble"/utf8>>, "1"},
        imem_meta:remove(?TPTEST_1, {?TPTEST_1, "meta", <<"tüble"/utf8>>, "1"})
    ),
    ...
    ?assertEqual(
        {?TPTEST_1, "meta1", <<"tüble"/utf8>>, "2"},
        imem_meta:insert(?TPTEST_1, {?TPTEST_1, "meta1", <<"tüble"/utf8>>, "2"})
    ),
    ?assertEqual(2, length(imem_meta:read(?TPTEST_1))),
    ?assertEqual(2, length(imem_meta:read(?TPTEST_1IDX))),
    ?assertException(
        throw,
        {'ClientError', {"Unique index violation", {?TPTEST_1IDX, 2, <<"table">>, "meta"}}},
        imem_meta:insert(?TPTEST_1, {?TPTEST_1, "meta2", <<"table"/utf8>>, "2"})
    ),
    ?assertEqual(2, length(imem_meta:read(?TPTEST_1))),
    ?assertEqual(2, length(imem_meta:read(?TPTEST_1IDX))),
    ...
    JSON1 =
        <<"{" "\"a\":\"Value-a\"," "\"b\":\"Value-b\"," "\"c\":{" "\"a\":\"Value-ca\"," "\"b\":\"Value-cb\"" "}" "}">>,
    % {
    %     "a": "Value-a",
    %     "b": "Value-b",
    %     "c": {
    ...
    ?assertEqual(
        [#ddIndex{stu = {3, <<"value-b">>, "json1"}}, #ddIndex{stu = {3, <<"value-ca">>, "json1"}}],
    ...
    ?assertException(
        throw,
        {'ClientError', {"Index does not exist for", ?TPTEST_1, <<"non existent index">>}},
        imem_meta:drop_index(?TPTEST_1, <<"non existent index">>)
    ),
    ?assertException(
        throw,
        {'ClientError', {"Index does not exist for", ?TPTEST_1, 7}},
    ...
    ?assertEqual({'ClientError', {"Table does not exist", ?TPTEST_1IDX}}, imem_meta:drop_index(?TPTEST_1)),
    ?CTPAL("?TPTEST_1 ~p", [imem_meta:read(?TPTEST_1)]),
    Idx4Def =
        #ddIdxDef{
            id = 4,
            name = <<"integer index on b1">>,
            type = ivk,
            pl = [<<"c1">>],
            vnf = <<"fun imem_index:vnf_integer/1">>
        },
    ...
    Vnf5 =
        <<
            "fun(__X) -> case imem_index:vnf_integer(__X) of ['$not_a_value'] -> ['$not_a_value']; [__V] -> [2*__V] end end"
        >>,
    ...
    ?assertMatch(
        {ok, _},
        imem_meta:create_table(?TPTEST_3, {[a, ?nav], [datetime, term], {?TPTEST_3, ?nav, undefined}}, [])
    ),
    ?CTPAL("create_or_replace_trigger"),
    Trig =
        <<
            "fun(O,N,T,U,TO) -> imem_meta:log_to_db(debug,imem_meta,trigger,[{table,T},{old,O},{new,N},{user,U},{tropts,TO}],\"trigger\") end."
        >>,
    ?assertEqual(ok, imem_meta:create_or_replace_trigger(?TPTEST_3, Trig)),
    ?assertEqual(Trig, imem_meta:get_trigger(?TPTEST_3)),
    ?assertException(
        throw,
        {ClEr, {"No columns given in create table", bad_table_0}},
        imem_meta:create_table(bad_table_0, [], [])
    ),
    ?assertException(
        throw,
        {ClEr, {"No value column given in create table, add dummy value column", bad_table_0}},
        imem_meta:create_table(bad_table_0, BadTypes0, [])
    ),
    ?assertException(
        throw,
        {ClEr, {"Invalid character(s) in table name", 'bad_?table_1'}},
        imem_meta:create_table('bad_?table_1', BadTypes1, [])
    ),
    ?assertEqual({ok, {imem, select}}, imem_meta:create_table(select, BadTypes2, [])),
    ?assertException(
        throw,
        {ClEr, {"Invalid character(s) in column name", 'a:b'}},
        imem_meta:create_table(bad_table_1, BadTypes1, [])
    ),
    ...
    ?assertEqual(
        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, undefined},
        imem_meta:insert(?TPTEST_3, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, ?nav})
    ),
    ?assertEqual(1, imem_meta:table_size(?TPTEST_3)),
    %% trigger inserted one line
    ?assertEqual(LogCount3 + 1, imem_meta:table_size(?LOG_TABLE)),
    ?assertException(
        throw,
        {ClEr, {"Not null constraint violation", {?TPTEST_3, _}}},
        imem_meta:insert(?TPTEST_3, {?TPTEST_3, ?nav, undefined})
    ),
    %% error inserted one line
    ?assertEqual(LogCount3 + 2, imem_meta:table_size(?LOG_TABLE)),
    ?assertEqual(
        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, undefined},
        imem_meta:update(
            ?TPTEST_3,
            {{?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, undefined}, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, ?nav}}
        )
    ),
    ?assertEqual(1, imem_meta:table_size(?TPTEST_3)),
    %% trigger inserted one line
    ?assertEqual(LogCount3 + 3, imem_meta:table_size(?LOG_TABLE)),
    ?assertEqual(
        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 56}}, undefined},
        imem_meta:merge(?TPTEST_3, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 56}}, ?nav})
    ),
    ?assertEqual(2, imem_meta:table_size(?TPTEST_3)),
    %% trigger inserted one line
    ?assertEqual(LogCount3 + 4, imem_meta:table_size(?LOG_TABLE)),
    ?assertEqual(
        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 56}}, undefined},
        imem_meta:remove(?TPTEST_3, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 56}}, undefined})
    ),
    ?assertEqual(1, imem_meta:table_size(?TPTEST_3)),
    %% trigger inserted one line
    ?assertEqual(LogCount3 + 5, imem_meta:table_size(?LOG_TABLE)),
        ...
        fun
            () ->
                ...
                imem_meta:update(
                    ...
                    {
                        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 55}}, undefined},
                        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 56}}, "alternative"}
                    }
                ),
                %% return last result only
                imem_meta:insert(?TPTEST_3, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 57}}, ?nav})
        end,
    ?assertEqual(
        {?TPTEST_3, {{2000, 1, 1}, {12, 45, 57}}, undefined},
        imem_meta:return_atomic(imem_meta:transaction(Trans3))
    ),
    ?CTPAL("?TPTEST_3 after update ~p", [imem_meta:read(?TPTEST_3)]),
    ?assertEqual(2, imem_meta:table_size(?TPTEST_3)),
    %% no trigger, no more log
    ?assertEqual(LogCount3 + 5, imem_meta:table_size(?LOG_TABLE)),
    Keys4 = [{1, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 59}}, undefined}}],
    U = unknown,
    {_, _DefRec, TrigFun} = imem_meta:trigger_infos(?TPTEST_3),
    ?assertEqual(
        Keys4,
        imem_meta:update_tables(
            [[{imem, ?TPTEST_3, set}, 1, {}, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 59}}, undefined}, TrigFun, U, []]],
            optimistic
        )
    ),
    ?assertException(
        throw,
        {ClEr, {"Not null constraint violation", {1, {?TPTEST_3, _}}}},
        imem_meta:update_tables(
            [[{imem, ?TPTEST_3, set}, 1, {}, {?TPTEST_3, ?nav, undefined}, TrigFun, U, []]],
            optimistic
        )
    ),
    ?assertException(
        ...
        {ClEr, {"Not null constraint violation", {1, {?TPTEST_3, _}}}},
        imem_meta:update_tables(
            [[{imem, ?TPTEST_3, set}, 1, {}, {?TPTEST_3, {{2000, 1, 1}, {12, 45, 59}}, ?nav}, TrigFun, U, []]],
            optimistic
        )
    ),
...
meta_partitions(_Config) ->
    ...
    ?assertException(
        throw,
        {ClEr, {"Table to be purged does not exist", ?TPTEST_1000@}},
        imem_meta:purge_table(?TPTEST_1000@)
    ),
    ?assertException(
        throw,
        {UiEx, {"Purge not supported on this table type", not_existing_table}},
        imem_meta:purge_table(not_existing_table)
    ),
    ...
    % ?TIME_UID format
    LogRecP = LogRec#ddLog{logTime = {900000000, 0, node(), ?INTEGER_UID}},
    ...
    LogRecFF = LogRec#ddLog{logTime = {2900000000, 0}},
    ...
    ?assertEqual(3, length(imem_meta:physical_table_names(?TPTEST_999999999@_))),
    ...
    ?assertEqual(
        ...
        imem_meta:create_partitioned_table_sync(dummy_table_name, dummy_table_name)
    ),
    ...
    ?assertEqual(ok, imem_meta:dirty_write(?TPTEST_3@, LogRec3)),
    % one record in this partition now
    ?assertEqual(1, length(imem_meta:physical_table_names(?TPTEST_3@))),
    ct:sleep(4000),
    % one partition at least created by partition rolling
    ?assert(length(imem_meta:physical_table_names(?TPTEST_3@)) >= 2),
    % can write to second partition (maybe third)
    ?assertEqual(ok, imem_meta:dirty_write(?TPTEST_3@, LogRec3#ddLog{logTime = ?TIMESTAMP})),
    ct:sleep(4000),
...
meta_preparations(_Config) ->
    ?CTPAL("Start"),
    ?assertEqual(
        ["Schema", ".", "BaseName", "_", "01234", "@", "Node"],
        imem_meta:parse_table_name("Schema.BaseName_01234@Node")
    ),
    ...
    ?assertEqual(["", "", "BaseName", "", "", "", ""], imem_meta:parse_table_name("BaseName")),
    ?assertEqual(
        ["Schema", ".", "Name_Period", "", "", "@", "Node"],
        imem_meta:parse_table_name('Schema.Name_Period@Node')
    ),
    ?assertEqual(
        ["Schema", ".", "Name", "_", "12345", "@", "Node"],
        imem_meta:parse_table_name('Schema.Name_12345@Node')
    ),
