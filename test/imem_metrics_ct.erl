...
test_partition_size(_Config) ->
    ...
    ct:pal(
        ...
        ++
        [
            io_lib:format(" table : ~p size : ~p ", [T, imem_meta:table_size(T)])
            || T <- imem_meta:physical_table_names(PartitionTableAlias)
        ]
    ),
    ...
    ct:pal(
        info,
        ?MAX_IMPORTANCE,
        ?MODULE_STRING ": 2 partition size check with one partition after dropping 2nd partition~n",
        []
    ),
    ?assertEqual(#{size => 2}, imem_metrics:get_metric({partition_size, PartitionTableAlias, 0, 0})),
    ct:pal(
        info,
        ?MAX_IMPORTANCE,
        ?MODULE_STRING ": 2 partition size check with two partition after dropping 2nd partition~n",
        []
    ),
    ...
    ct:pal(
        info,
        ?MAX_IMPORTANCE,
        ?MODULE_STRING ": 6 partition size check with three partition after dropping 2nd partition~n",
        []
    ),
