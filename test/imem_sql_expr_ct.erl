...
test_with_or_without_sec(IsSec) ->
    ...
    ?assertEqual(
        <<"schema.table.field">>,
        imem_sql_expr:qname3_to_binstr(imem_sql_expr:binstr_to_qname3(<<"schema.table.field">>))
    ),
    ?assertEqual(true, is_atom(imem_meta:schema())),
    ...
    _Types1 =
        [
            %% key
            #ddColumn{name = a, type = char, len = 1},
            %% value 1
            #ddColumn{name = b1, type = char, len = 1},
            %% value 2
            #ddColumn{name = c1, type = char, len = 1}
        ],
    _Types2 =
        [
            %% key
    ...
    ?assertMatch(
        {ok, _},
        imem_sql:exec(anySKey, "create table meta_table_1 (a char, b1 char, c1 char);", 0, "imem", IsSec)
    ),
    ...
    ?assertMatch(
        {ok, _},
        imem_sql:exec(anySKey, "create table meta_table_3 (a char, b3 integer, c1 char);", 0, "imem", IsSec)
    ),
    ?assertEqual(0, if_call_mfa(IsSec, table_size, [anySKey, meta_table_1])),
    ...
    ?assertException(
        throw,
        {ClEr, {"Table does not exist", {imem, meta_table_x}}},
        imem_sql_expr:column_map_tables([Table1, TableX, Table3], [], [])
    ),
    ...
    %             , #bind{tag="A3", name= <<"c1">>}
    %             ],
    ColsE1 = [<<"imem.meta_table_1.a">>, <<"x">>, <<"c1">>],
    ?assertException(
        throw,
        {ClEr, {"Unknown field or table name", <<"x">>}},
        imem_sql_expr:column_map_columns(ColsE1, FullMap1)
    ),
    ...
    %             , #bind{tag="A3", name= <<"c1">>}
    %             ],
    ColsE2 = [<<"imem.meta_table_1.a">>, <<"meta_table_x.b1">>, <<"c1">>],
    ?assertException(
        throw,
        {ClEr, {"Unknown field or table name", <<"meta_table_x.b1">>}},
        imem_sql_expr:column_map_columns(ColsE2, FullMap1)
    ),
    ...
    %             , {as, <<"c1">>, <<"c1">>}
    %             ],
    ColsA = [{as, <<"imem.meta_table_1.a">>, <<"a">>}, {as, <<"meta_table_1.b1">>, <<"b1">>}, {as, <<"c1">>, <<"c1">>}],
    ?assertException(
        throw,
        {ClEr, {"Ambiguous field or table name", <<"a">>}},
        imem_sql_expr:column_map_columns([<<"a">>], FullMap13)
    ),
    ?CTPAL("success ~p", [columns_ambiguous_a]),
    ?assertException(
        throw,
        {ClEr, {"Ambiguous field or table name", <<"c1">>}},
        imem_sql_expr:column_map_columns(ColsA, FullMap13)
    ),
    ...
    ?assertException(
        throw,
        {ClEr, {"Unknown field or table name", <<"any.sysdate">>}},
        imem_sql_expr:column_map_columns([<<"alias1.a">>, <<"any.sysdate">>], AliasMap1)
    ),
    ...
    CC3 =
        {
            in,
            <<"c1">>,
            {list, [{'fun', <<"to_ipaddr">>, [<<"'3.1.2.3'">>]}, {'fun', <<"to_ipaddr">>, [<<"'3.3.2.1'">>]}]}
        },
    ?assertEqual(
        {'and', {'or', {'or', CA1, CB2}, CC3}, {wt}},
        imem_sql_expr:filter_spec_where({'or', [FA1, FB2, FC3]}, ColsFS, {wt})
    ),
    ?assertEqual(
        {'and', {'and', {'and', CA1, CB2}, CC3}, {wt}},
        imem_sql_expr:filter_spec_where({'and', [FA1, FB2, FC3]}, ColsFS, {wt})
    ),
    FB2a = {2, [<<"$in$">>, <<"22'2">>]},
