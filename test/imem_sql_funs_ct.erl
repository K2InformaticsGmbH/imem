...
test_with_or_without_sec(IsSec) ->
    ...
    ?assertEqual(<<"^.A.*S\\^\\$\\.\\[\\]\\|\\(\\)\\?\\*\\+\\-\\{\\}m.th.*$">>, imem_sql_funs:transform_like(<<"_A%S^$.[]|()?*+-{}m_th%">>, <<>>)),
    ...
    ?assertEqual(false, imem_sql_funs:re_match(RE1, "abc_123%@@")),     %% string is expanded using ~p before comparison        
    ?assertEqual(false, imem_sql_funs:re_match(RE1, "abc_123%%@@@")),   %% string is expanded using ~p before comparison     
    ...
    ?assertEqual(true, imem_sql_funs:re_match(RE4, 012)),
    ...
    ?assertEqual(true, imem_sql_funs:re_match(RE5, 12.50)),
